-- T-Ride open call MVP.
-- Adds OPEN so valid customer bookings can be published to online drivers.
-- Safe to re-run when a staging DB already has the enum value.
...
SET @sql = IF(
  @log_from_enum IS NOT NULL
    AND @log_from_enum NOT LIKE '%''OPEN''%',
