import 'package:flutter/material.dart';

class AppLocalizations {
  final String languageCode;

  AppLocalizations(this.languageCode);

  static const supportedLanguages = ['en', 'ko', 'zh', 'ja', 'th'];

  static const Map<String, String> languageNames = {
    'en': 'English',
    'ko': '한국어',
    'zh': '中文',
    'ja': '日本語',
    'th': 'ไทย',
  };

  static final Map<String, Map<String, String>> _translations = {
    'en': {
      'app_title': 'T-Rider',
      'app_subtitle': 'Thailand Airport Transfer',
      'landing_hero_eyebrow':
          'Professional airport & city transfers in Thailand',
      'landing_hero_title': 'A comfortable start in Thailand with T-Rider',
      'landing_hero_body':
          'Travel safely and smoothly from the airport to your destination with experienced drivers.',
      'landing_hero_cta': 'Book now',
      'landing_support_cta': 'Customer center',
      'landing_support_one_click_cta': 'One-click booking via Customer Center',
      'landing_hero_helper': 'Quick booking, no account required',
      'landing_hero_subtitle': 'Travel safely, smoothly, and with confidence.',
      'landing_highlight_fixed_price': 'Fixed Price',
      'landing_highlight_fixed_price_desc':
          'See your fare upfront with no hidden extras.',
      'landing_highlight_flight': 'Flight Tracking',
      'landing_highlight_flight_desc':
          'We track your flight and adjust pickup if it\'s delayed.',
      'landing_highlight_support': '24/7 Support',
      'landing_highlight_support_desc':
          'Get help anytime before, during, or after your ride.',
      'book_your_ride': 'Book Your Ride',
      'landing_services_title': 'Our Services',
      'landing_services_subtitle': 'Airport and city transfers across Thailand',
      'service_badge_popular': 'Most Popular',
      'landing_trust_title': 'Travel with confidence',
      'landing_trust_drivers_title': 'Experienced drivers',
      'landing_trust_drivers_desc':
          'Drivers familiar with Thai roads and airport transfers.',
      'landing_trust_confirmed_title': 'Confirmed booking details',
      'landing_trust_confirmed_desc':
          'Review pickup information and your trip schedule in advance.',
      'landing_trust_comfort_title': 'A comfortable journey',
      'landing_trust_comfort_desc':
          'Focus on your trip—not navigating unfamiliar transport.',
      'landing_trust_refund_title': 'Free Cancellation & Refund',
      'landing_trust_refund_desc':
          'You can cancel free of charge within 24 hours of booking confirmation, with no cancellation fee. If you find a lower confirmed quote for the same conditions (vehicle and schedule), contact us within 48 hours of booking and we\'ll refund the difference.',
      'landing_trust_driver_policy_title': 'Driver Quality Management',
      'landing_trust_driver_policy_desc':
          'If you have any concerns about your driver, you can report them through the booking details page or via KakaoTalk/phone. We will follow up within 24 hours of receiving your report. For serious issues such as safety threats, we will immediately suspend the driver from dispatch.',
      'landing_how_it_works_title': 'How booking works',
      'landing_how_step_1_title': 'Enter trip details',
      'landing_how_step_1_desc':
          'Choose your service, pickup time, and destination.',
      'landing_how_step_2_title': 'Choose vehicle & fare',
      'landing_how_step_2_desc':
          'See recommended vehicles and your fare before confirming.',
      'landing_how_step_3_title': 'Travel comfortably',
      'landing_how_step_3_desc':
          'Your driver takes you to your destination safely.',
      'landing_reassurance_body':
          'First time in Thailand? We guide you clearly from booking to arrival.',
      'landing_booking_lookup_title': 'Already booked?',
      'landing_booking_lookup_desc':
          'Check your schedule and trip details with your booking number.',
      'landing_booking_lookup_action': 'Find my booking',
      'landing_footer_note':
          'Professional airport and city transfers across Thailand.',
      'landing_login_title': 'Sign in',
      'landing_login_sheet_title': 'Sign in to T-Rider',
      'landing_login_prompt_cta': 'Sign in to earn mileage rewards',
      'landing_login_sheet_close': 'Close',
      'landing_login_support_prompt': 'Having trouble signing in?',
      'landing_login_support_link': 'Contact support',
      'landing_header_my_bookings_menu': 'My bookings',
      'landing_header_logout_menu': 'Sign out',
      'landing_header_logout_success': 'Signed out',
      'landing_logged_in_greeting': 'Signed in as {name}',
      'landing_my_bookings_button': 'My bookings',
      'landing_logout_button': 'Sign out',
      'auth_login_provider_hint':
          'To manage your mileage rewards, please always sign in with the same method (Google, Kakao, or LINE).',
      'landing_header_account_menu': 'My account',
      'account_page_title': 'My account',
      'account_menu_section_title': 'Account',
      'account_mileage_label': 'Mileage rewards',
      'account_mileage_balance': '{balance}P',
      'account_mileage_loading': 'Loading balance...',
      'account_mileage_load_failed': 'Unable to load balance',
      'account_mileage_retry': 'Retry',
      'account_my_bookings_menu': 'My bookings',
      'account_logout_menu': 'Sign out',
      'account_logout_cancel': 'Cancel',
      'account_logout_confirm_message': 'Sign out of your account?',
      'account_login_provider_google': 'Signed in with Google',
      'account_login_provider_kakao': 'Signed in with Kakao',
      'account_login_provider_line': 'Signed in with LINE',
      'account_sign_in_required': 'Please sign in to view your account.',
      'account_booking_counts_title': 'My bookings',
      'account_booking_count_waiting': 'Waiting',
      'account_booking_count_assigned': 'Assigned',
      'account_booking_count_in_progress': 'In progress',
      'account_booking_count_settlement_pending': 'Settlement',
      'account_booking_count_completed': 'Completed',
      'account_booking_count_review_pending': 'Review',
      'account_coupon_menu': 'Coupons',
      'account_coupons_title': 'Coupons',
      'account_coupons_available_section': 'Available coupons',
      'account_coupons_used_section': 'Used coupons',
      'account_coupons_empty_available': 'No available coupons',
      'account_coupons_empty_used': 'No used coupons yet',
      'account_coupons_load_failed': 'Unable to load coupons',
      'account_coupons_issued_at': 'Issued',
      'account_coupons_used_at': 'Used',
      'account_coupons_used_booking': 'Booking',
      'booking_coupon_section_title': 'Use coupon',
      'booking_coupon_none': 'Do not use a coupon',
      'booking_coupon_discount_label': 'Coupon discount',
      'booking_mileage_section_title': 'Use mileage',
      'booking_mileage_discount_label': 'Mileage discount',
      'booking_mileage_available': 'Available mileage: {balance} points',
      'booking_mileage_use_all': 'Use all',
      'admin_coupons_menu': 'Coupons',
      'admin_coupon_issue_title': 'Issue coupon',
      'admin_coupon_search_hint': 'Search by phone, email, or name',
      'admin_coupon_search_empty': 'No customers found',
      'admin_coupon_select_customer': 'Select customer',
      'admin_coupon_title_label': 'Coupon title',
      'admin_coupon_amount_label': 'Discount amount',
      'admin_coupon_issue_button': 'Issue coupon',
      'admin_coupon_issue_success': 'Coupon issued',
      'admin_coupon_issue_failed': 'Unable to issue coupon',
      'admin_coupon_recent_title': 'Recently issued coupons',
      'admin_coupon_recent_empty': 'No coupons issued yet',
      'admin_coupon_cancel': 'Cancel',
      'admin_coupon_cancel_success': 'Coupon cancelled',
      'admin_coupon_status_available': 'Available',
      'admin_coupon_status_used': 'Used',
      'admin_coupon_recent_customers_title': 'Recent customers',
      'admin_coupon_recent_customers_empty': 'No completed bookings yet',
      'admin_coupon_selected_customer': 'Selected customer',
      'admin_coupon_template_picker_title': 'Coupon template',
      'admin_coupon_template_manual': 'Manual entry',
      'admin_coupon_template_manage_title': 'Manage templates',
      'admin_coupon_template_create_button': 'Add template',
      'admin_coupon_template_create_success': 'Template created',
      'admin_coupon_template_create_failed': 'Unable to create template',
      'admin_coupon_template_active': 'Active',
      'admin_coupon_template_inactive': 'Inactive',
      'admin_home_banner_menu': 'Home banners',
      'admin_manual_booking_menu': 'Manual call registration',
      'admin_manual_booking_title': 'Register admin manual call',
      'admin_manual_booking_origin': 'Pickup location',
      'admin_manual_booking_destination': 'Drop-off location',
      'admin_manual_booking_pickup_datetime': 'Pickup date & time',
      'admin_manual_booking_pickup_datetime_hint': 'Select pickup date and time',
      'admin_manual_booking_pickup_timezone': 'Thailand local time (UTC+7)',
      'admin_manual_booking_special_items_text': 'Special luggage notes',
      'admin_manual_booking_special_items_hint': 'Describe oversized or unusual items',
      'admin_manual_booking_flight_pickup_date_required':
          'Select pickup date before flight lookup',
      'admin_manual_booking_flight_manual_fallback_hint':
          'If flight lookup fails, enter the flight number and set pickup time manually.',
      'admin_manual_booking_flight_pickup_conflict_warning':
          'Your pickup time and the flight scheduled arrival differ by {minutes} minutes.',
      'admin_manual_booking_flight_pickup_keep_manual': 'Keep my entered time',
      'admin_manual_booking_flight_pickup_use_flight': 'Use flight arrival time',
      'admin_manual_booking_flight_pickup_final_label': 'Final pickup request time',
      'admin_dispatch_driver_pickup_conflict':
          'Pickup time conflicts with another assigned job',
      'admin_manual_booking_vehicle_type': 'Vehicle class',
      'admin_manual_booking_passengers': 'Passengers',
      'admin_manual_booking_name_sign_hint':
          'Drivers will see a picket/name-sign badge on this call.',
      'admin_manual_booking_payout_amount': 'Driver payout amount (THB)',
      'admin_manual_booking_customer_charge_amount': 'Customer payment amount (THB)',
      'admin_manual_booking_customer_charge_hint':
          'Optional internal record of what the customer paid or will pay.',
      'admin_manual_booking_payment_method': 'Payment collection',
      'admin_manual_booking_payment_admin_collected': 'Admin already collected payment',
      'admin_manual_booking_payment_driver_collects': 'Driver collects on site',
      'admin_manual_booking_customer_section': 'Customer',
      'admin_manual_booking_guest_name': 'Guest name',
      'admin_manual_booking_guest_phone': 'Guest phone',
      'admin_manual_booking_guest_email': 'Guest email (optional)',
      'admin_manual_booking_clear_customer': 'Clear selected customer',
      'admin_manual_booking_memo': 'Notes (optional)',
      'admin_manual_booking_submit': 'Create open call',
      'admin_manual_booking_success': 'Open call created. Booking number:',
      'admin_manual_booking_view_detail': 'View booking',
      'admin_manual_booking_assign_driver': 'Assign driver',
      'admin_manual_booking_edit': 'Edit',
      'admin_manual_booking_edit_title': 'Edit admin manual call',
      'admin_manual_booking_edit_not_allowed': 'This call cannot be edited in its current status',
      'admin_manual_booking_cancel': 'Cancel call',
      'admin_manual_booking_cancel_confirm':
          'Cancel this admin manual call? Assigned drivers will be released.',
      'admin_manual_booking_cancel_success': 'Admin manual call cancelled',
      'admin_manual_booking_assign_success': 'Driver assigned',
      'admin_manual_booking_update_submit': 'Save changes',
      'admin_manual_booking_update_success': 'Admin manual call updated',
      'admin_manual_booking_failed': 'Unable to create manual call',
      'admin_manual_booking_validation_required': 'Please fill in all required fields',
      'admin_manual_booking_validation_customer':
          'Select a member or enter guest name and phone',
      'admin_manual_booking_preview_notice':
          'Driver preview: account confirmation notice will be shown',
      'admin_home_banner_title': 'Home banner management',
      'admin_home_banner_order_label': 'Display order',
      'admin_home_banner_order_hint': 'Leave blank to append at the end',
      'admin_home_banner_upload_button': 'Upload banner',
      'admin_home_banner_create_success': 'Banner uploaded',
      'admin_home_banner_create_failed': 'Unable to upload banner',
      'admin_home_banner_empty': 'No banners yet',
      'admin_home_banner_active': 'Active',
      'admin_home_banner_inactive': 'Inactive',
      'admin_home_banner_delete': 'Delete',
      'admin_home_banner_delete_success': 'Banner deleted',
      'admin_home_banner_move_up': 'Move up',
      'admin_home_banner_move_down': 'Move down',
      'account_feature_coming_soon': 'Coming soon',
      'account_recent_bookings_title': 'Recent trips',
      'account_recent_bookings_empty': 'No recent trips yet',
      'account_recent_bookings_load_failed': 'Unable to load recent trips',
      'account_my_reviews_menu': 'My reviews',
      'account_customer_support_menu': 'Customer support',
      'account_mileage_history_title': 'Mileage history',
      'account_mileage_history_empty': 'No mileage history yet',
      'account_mileage_history_load_failed': 'Unable to load mileage history',
      'account_my_reviews_title': 'My reviews',
      'account_my_reviews_empty': 'No reviews written yet',
      'account_my_reviews_load_failed': 'Unable to load reviews',
      'account_mileage_type_accrue': 'Earned',
      'account_mileage_type_reversal': 'Reversed',
      'landing_privacy_policy_link': 'Privacy policy',
      'privacy_policy_title': 'Privacy policy',
      'privacy_policy_load_failed':
          'Privacy policy could not be loaded. Please try again later.',
      'analytics_consent_title': 'Analytics cookies',
      'analytics_consent_body':
          'We use Google Analytics to measure visits and booking conversion. No name, phone, or email is sent. Marketing attribution is stored only if you allow analytics. You can change this anytime in Privacy settings.',
      'analytics_consent_allow': 'Allow analytics',
      'analytics_consent_deny': 'Decline',
      'analytics_consent_privacy': 'Privacy policy',
      'analytics_consent_settings': 'Analytics settings',
      'analytics_consent_settings_title': 'Analytics consent',
      'landing_language_label': 'Language',
      'support_title': 'Customer center',
      'support_contact_channels_title': 'Contact Customer Center',
      'support_contact_channels_hint':
          'Tap a messenger icon to start chatting with us',
      'support_wechat_qr_dialog_title': 'WeChat QR Code',
      'support_wechat_qr_dialog_hint':
          'Scan this QR code in WeChat to add us as a friend',
      'support_page_intro':
          'If booking is difficult or you need help, please contact customer center. You can also attach flight tickets, hotel vouchers, or reservation screenshots.',
      'support_inquiry_button': 'Contact us',
      'support_dialog_title': 'Customer center inquiry',
      'support_close_button': 'Close',
      'support_inquiry_title': 'Booking and inquiry via customer center',
      'support_inquiry_subtitle':
          'Leave your booking question, flight information, pickup place, and destination.',
      'support_default_guide':
          'Hello. This is T-Ride customer center. Please leave your booking inquiry, flight information, pickup place, and destination, and we will check and guide you.',
      'support_input_label': 'Inquiry message',
      'support_input_hint': 'Enter your reservation or travel question.',
      'support_attach_button': 'Attach image',
      'support_send_button': 'Send',
      'support_sending': 'Sending...',
      'support_receipt_number': 'Receipt number',
      'support_submit_failed':
          'Your inquiry could not be submitted. Please try again in a moment.',
      'support_attachment_help':
          'You can attach flight ticket images, hotel vouchers, or reservation screenshots.',
      'support_no_attachments': 'No attachments selected',
      'support_contact_help':
          'For agent inquiries, leave any contact information we can use to follow up. At least one contact method is recommended.',
      'support_contact_messenger_help':
          'If you leave your KakaoTalk ID or LINE ID, customer center can contact you there.',
      'support_contact_name': 'Name or nickname',
      'support_contact_phone': 'Phone number',
      'support_contact_kakao': 'KakaoTalk ID',
      'support_contact_line': 'LINE ID',
      'support_thread_loading': 'Loading your recent inquiry...',
      'support_thread_load_failed':
          'Could not load your recent inquiry. Please try again.',
      'support_auto_receipt':
          'Auto receipt: Your inquiry has been received. Customer center will check and guide you.',
      'support_faq_title': 'Frequently asked questions',
      'support_faq_placeholder':
          'Frequently asked questions will be updated by priority as customer inquiry data accumulates.',
      'admin_support_menu': 'Support inquiries',
      'admin_support_search': 'Search inquiry',
      'admin_support_all_statuses': 'All statuses',
      'admin_support_empty': 'No support inquiries',
      'admin_support_detail': 'Support inquiry detail',
      'admin_support_customer': 'Customer',
      'admin_support_customer_name': 'Name',
      'admin_support_customer_phone': 'Phone',
      'admin_support_customer_kakao': 'KakaoTalk ID',
      'admin_support_customer_line': 'LINE ID',
      'admin_support_messages': 'Messages',
      'admin_support_reply_title': 'Write reply',
      'admin_support_reply_label': 'Reply',
      'admin_support_reply_hint': 'Enter a reply to the customer.',
      'admin_support_send_reply': 'Send reply',
      'admin_support_reply_failed':
          'Could not send the reply. Please try again.',
      'admin_support_attachments': 'Attachments',
      'admin_support_no_attachments': 'No attachments',
      'admin_support_preview': 'Preview',
      'admin_support_download': 'Download',
      'admin_support_preview_failed': 'Could not preview the attachment.',
      'admin_support_download_failed': 'Could not download the attachment.',
      'admin_support_download_started': 'Download started.',
      'admin_support_status_new': 'New',
      'admin_support_status_in_progress': 'In progress',
      'admin_support_status_resolved': 'Resolved',
      'admin_support_status_closed': 'Closed',
      'trust_flight_delay': 'Flight Delay Monitoring',
      'trust_flight_delay_desc':
          'Pickup time updates automatically when your flight changes.',
      'trust_fixed_pricing': 'Fixed Pricing',
      'trust_fixed_pricing_desc': 'The price you book is the price you pay.',
      'trust_professional_drivers': 'Professional Drivers',
      'trust_professional_drivers_desc':
          'Licensed, vetted drivers for a safe, smooth trip.',
      'trust_multilingual': 'Multilingual Support',
      'trust_multilingual_desc':
          'Book in English, Korean, Chinese, Japanese, or Thai.',
      'airport_pickup': 'Airport Pickup',
      'airport_pickup_desc': 'From the airport straight to your door.',
      'airport_dropoff': 'Airport Dropoff',
      'airport_dropoff_desc': 'From your door to the airport, right on time.',
      'city_transfer': 'City Transfer',
      'city_transfer_desc': 'Comfortable transfers between cities.',
      'golf_transfer': 'Golf Transfer',
      'golf_transfer_desc': 'Plenty of room for your golf bags.',
      'select_service': 'Select Service Type',
      'passengers': 'Passengers',
      'adults': 'Adults (12+)',
      'children': 'Children',
      'luggage': 'Luggage',
      'small_carriers': '20" Carriers',
      'large_carriers': '24"+ Carriers',
      'golf_bags': 'Golf Bags',
      'special_luggage': 'Special Items',
      'name_sign': 'Name Sign Service (+100 THB)',
      'booking_prefer_female_driver': 'Prefer Female Driver',
      'booking_preference_disclaimer':
          "We'll do our best to accommodate this during driver assignment, but it cannot be guaranteed.",
      'next': 'Next',
      'back': 'Back',
      'confirm': 'Confirm Booking',
      'customer_review_booking': 'Review booking',
      'customer_confirm_booking': 'Confirm booking',
      'customer_booking_processing': 'Processing your booking',
      'customer_urgent_pickup_hint':
          'If your pickup time is within 2 hours, tap Urgent Request to continue.',
      'customer_urgent_request': 'Urgent request',
      'customer_urgent_confirm_title': 'Confirm urgent request',
      'customer_urgent_confirm_body':
          'Urgent requests may take longer to assign a driver. Once matched, we will share the driver\'s estimated arrival time first. Tap Confirm if you would like to proceed.',
      'customer_urgent_confirm_submit': 'Confirm',
      'customer_urgent_flow_title': 'Urgent request',
      'customer_urgent_searching_title': 'Finding a driver',
      'customer_urgent_searching_body':
          'We are searching for an available driver near you.',
      'customer_urgent_eta_title':
          'Your driver is expected to arrive in about {minutes} minutes',
      'customer_urgent_eta_body':
          'Please accept if this arrival time works for you, or reject to search for a faster option.',
      'customer_urgent_accept_eta': 'Accept',
      'customer_urgent_reject_eta': 'Reject',
      'customer_urgent_retry_title': 'Look for a faster driver?',
      'customer_urgent_retry_body':
          'We can keep searching for another driver with a shorter arrival time.',
      'customer_urgent_retry_accept': 'Keep searching',
      'customer_urgent_retry_cancel': 'Cancel booking',
      'customer_urgent_confirmed_title': 'Driver confirmed',
      'customer_urgent_confirmed_body':
          'Your driver has been assigned. Opening your booking details.',
      'customer_urgent_exhausted_title': 'No nearby drivers available',
      'customer_urgent_exhausted_body':
          'There are no waiting drivers nearby. Your booking will be cancelled.',
      'customer_urgent_go_home': 'Go to home',
      'origin': 'Origin',
      'destination': 'Destination',
      'airport': 'Airport',
      'selected_location': 'Selected location',
      'flight_number': 'Flight Number',
      'date': 'Date',
      'time': 'Time',
      'pickup_datetime': 'Pick-up date & time',
      'pickup_date': 'Pick-up date',
      'pickup_time': 'Pick-up time',
      'pickup_minimum_notice':
          'Please choose a pick-up time at least 2 hours from now. Times use Thailand local time.',
      'select_vehicle': 'Select Vehicle',
      'recommended': 'Recommended',
      'total': 'Total',
      'special_requests': 'Special Requests',
      'customer_info': 'Customer Information',
      'name': 'Name',
      'email': 'Email',
      'phone': 'Phone',
      'country': 'Country',
      'booking_complete': 'Booking Complete',
      'reservation_number': 'Reservation No.',
      'booking_number_label': 'Booking number',
      'booking_with_number': 'Booking {bookingNumber}',
      'booking_detail_view': 'View booking details',
      'notification_type_booking': 'Booking',
      'notification_type_pickup': 'Pickup',
      'notification_type_settlement': 'Settlement',
      'notification_type_system': 'System',
      'status': 'Status',
      'golf_region': 'Golf Region',
      'golf_course': 'Golf Course',
      'driver_included': 'Driver Included',
      'status_pending': 'Pending',
      'status_confirmed': 'Confirmed',
      'status_driver_assigned': 'Driver Assigned',
      'status_completed': 'Completed',
      'status_cancelled': 'Cancelled',
      'status_on_route': 'On the way',
      'status_driver_arrived': 'Driver arrived',
      'status_no_show': 'No show',
      'status_picked_up': 'Customer onboard',
      'status_customer_settlement_pending': 'Trip ended',
      'status_driver_settlement_pending': 'Trip ended · Settlement pending',
      'status_admin_settlement_pending': 'Trip ended · Settlement pending',
      'status_settlement_pending': 'Trip ended · Settlement pending',
      'search_place': 'Search place...',
      'no_results': 'No locations found',
      'infants': 'Infants',
      'multiple_vehicles_required': 'Multiple vehicles required',
      'base_price': 'Base price',
      'additional_charges': 'Additional charges',
      'pricing_summary': 'Price summary',
      'pricing_name_sign_with_picket': 'Name sign service (Picket)',
      'vehicle_not_available': 'Not available for this trip',
      'pricing_inquiry_required': 'Contact us for a quote',
      'complete_passengers_first': 'Complete passenger details first',
      'booking_preview_only': 'Preview only — booking is not created yet',
      'yes': 'Yes',
      'change_location': 'Change',
      'loading_place_details': 'Loading place details...',
      'airport_shortcuts': 'Airport shortcuts',
      'airport_shortcuts_origin': 'Airport shortcuts',
      'airport_shortcuts_destination': 'Airport shortcuts',
      'recent_non_airport_places': 'Recent places',
      'recent_locations': 'Recent locations',
      'admin_dashboard': 'Dashboard',
      'admin_reservations': 'Reservations',
      'admin_dispatch_new_bookings': 'New bookings',
      'admin_dispatch_existing_bookings': 'Existing bookings',
      'admin_dispatch_new_sort_hint': 'Newest request first',
      'admin_dispatch_existing_sort_hint': 'Latest pickup first',
      'admin_drivers': 'Drivers',
      'admin_driver_management_help':
          'Active drivers are listed by account status. Online/offline and live map are shown separately.',
      'admin_driver_filter_active_accounts': 'Active accounts',
      'admin_driver_filter_online_only': 'Online only',
      'admin_driver_filter_active_jobs': 'Active jobs',
      'admin_driver_filter_stale_locations': 'Stale locations',
      'admin_no_drivers_found': 'No drivers match the current filters',
      'admin_no_driver_locations': 'No live driver locations on the map',
      'admin_driver_online': 'Online',
      'admin_driver_offline': 'Offline',
      'admin_driver_active_jobs': 'Active jobs',
      'admin_driver_no_vehicle': 'No primary vehicle configured',
      'admin_driver_no_location': 'No location',
      'admin_driver_location_live': 'Live',
      'admin_driver_location_stale': 'Stale',
      'admin_driver_last_seen': 'Updated',
      'admin_pricing': 'Vehicle Pricing',
      'admin_golf': 'Golf Courses',
      'admin_airports': 'Airports',
      'admin_settlements': 'Settlements',
      'admin_notifications': 'Notifications',
      'admin_reviews': 'Reviews',
      'admin_flights': 'Flights',
      'admin_logout': 'Log out',
      'admin_placeholder_deferred':
          'Catalog management is deferred in MVP release.',
      'admin_translations': 'Translations',
      'admin_settings': 'Settings',
      'admin_settings_line_qr': 'Admin LINE QR',
      'admin_settings_description': 'Description (optional)',
      'admin_settings_upload_line_qr': 'Upload LINE QR',
      'admin_settings_bank': 'Driver settlement account',
      'admin_settings_bank_name': 'Bank name',
      'admin_settings_account_name': 'Account holder',
      'admin_settings_account_number': 'Account number',
      'admin_settings_upload_promptpay': 'Upload PromptPay QR',
      'admin_settings_save': 'Save settings',
      'admin_settings_saved': 'Settings saved',
      'admin_settings_contact_channels': 'Customer center contact channels',
      'admin_settings_contact_channels_save': 'Save contact channel settings',
      'admin_settings_contact_kakao_enabled': 'Enable KakaoTalk',
      'admin_settings_contact_line_enabled': 'Enable LINE',
      'admin_settings_contact_whatsapp_enabled': 'Enable WhatsApp',
      'admin_settings_contact_wechat_enabled': 'Enable WeChat',
      'admin_settings_contact_whatsapp_phone': 'WhatsApp phone number',
      'admin_settings_contact_wechat_account_id': 'WeChat account ID (optional)',
      'admin_settings_upload_wechat_qr': 'Upload WeChat QR',
      'admin_settings_guest_lookup_inquiry': 'Booking lookup / messenger guide',
      'admin_settings_guest_lookup_banner_enabled': 'Show inquiry banner on lookup results',
      'admin_settings_guest_lookup_banner_message': 'Banner message (optional)',
      'admin_settings_guest_lookup_kakao_url': 'KakaoTalk chat URL (optional)',
      'admin_settings_guest_lookup_line_url': 'LINE chat URL (optional)',
      'admin_settings_guest_lookup_save': 'Save messenger guide settings',
      'admin_settings_image_invalid': 'Please choose a PNG or JPG image file.',
      'admin_settings_image_too_large':
          'The image file is too large. Please choose a smaller file.',
      'admin_settings_image_upload_failed':
          'Could not upload the image. Please try again.',
      'admin_settlement_confirm_200': 'Confirm 200 THB received',
      'admin_settlement_confirm_question':
          'Confirm that 200 THB has been received from the driver?',
      'admin_settlement_section': 'Settlement confirmation',
      'admin_settlement_status': 'Settlement status',
      'admin_settlement_transfer_slip': 'Transfer slip',
      'admin_settlement_slip_submitted': 'Transfer slip submitted',
      'admin_settlement_slip_not_submitted': 'Not submitted',
      'admin_settlement_waiting_slip':
          'Payment can be confirmed after the driver uploads the transfer slip.',
      'admin_settlement_view_slip': 'View transfer slip',
      'admin_settlement_slip_load_failed': 'Unable to load the transfer slip.',
      'admin_detail_operations_summary': 'Operations summary',
      'admin_detail_current_status': 'Current status',
      'admin_detail_last_status_change': 'Last status change',
      'admin_detail_status_duration': 'Current status duration',
      'admin_detail_driver_assignment_status': 'Driver / assignment',
      'admin_detail_previous_driver': 'Previous driver',
      'admin_detail_driver_status': 'Driver status',
      'admin_detail_settlement_status': 'Settlement status',
      'admin_detail_receipt_status': 'Transfer slip status',
      'admin_detail_recommended_action': 'Recommended action',
      'admin_detail_settlement_required': 'Settlement required',
      'admin_detail_settlement_completed': 'Settlement completed',
      'admin_detail_receipt_missing': 'Transfer slip missing',
      'admin_detail_receipt_submitted': 'Transfer slip submitted',
      'admin_detail_receipt_rejected': 'Transfer slip rejected',
      'admin_detail_settlement_actionable': 'Approval availability',
      'admin_detail_settlement_not_actionable':
          'Settlement cannot be approved in the current booking state.',
      'admin_detail_settlement_state_mismatch':
          'The trip is completed, but the booking has not moved to settlement pending. Review the booking status and settlement records.',
      'admin_detail_rejection_reason': 'Rejection reason',
      'admin_detail_rejection_reason_missing':
          'No rejection reason was recorded.',
      'admin_detail_rejected_at': 'Rejected at',
      'admin_detail_receipt_submitted_at': 'Transfer slip submitted at',
      'admin_detail_receipt_uploaded_at': 'Transfer slip uploaded at',
      'admin_detail_receipt_file': 'Transfer slip file',
      'admin_detail_receipt_content_type': 'File type',
      'admin_detail_receipt_file_size': 'File size',
      'admin_detail_settlement_approved_at': 'Settlement approved at',
      'admin_detail_settlement_approval_mode': 'Approval mode',
      'admin_detail_settlement_manual_approval': 'Manual approval',
      'admin_detail_additional_memo': 'Additional memo',
      'admin_detail_actor_system': 'System',
      'admin_detail_actor_admin': 'Admin',
      'admin_detail_actor_driver': 'Driver',
      'admin_detail_actor_customer': 'Customer',
      'admin_detail_actor_unknown': 'Unknown',
      'admin_detail_settlement_load_failed':
          'Unable to load settlement information.',
      'admin_detail_retry_settlement': 'Retry settlement',
      'admin_detail_diag_pickup_overdue_stalled':
          'Pickup time has passed, but the trip has not progressed. Check the driver assignment and current driver status.',
      'admin_detail_diag_boarding_delay':
          'Boarding is delayed after the driver arrived. Check boarding status with the driver or customer.',
      'admin_detail_diag_long_trip':
          'The trip has been in progress longer than expected. Check driver location and trip status.',
      'admin_detail_diag_status_stale':
          'The booking status has not changed for a long time. Check the current driver and trip status.',
      'support_line_inquiry': 'LINE 1:1 Inquiry',
      'support_line_qr_missing': 'Admin LINE QR has not been registered yet.',
      'today_bookings': 'Today Bookings',
      'today_revenue': 'Today Revenue',
      'pending_count': 'Pending',
      'awaiting_driver': 'Awaiting Driver',
      'vehicle': 'Vehicle',
      'amount': 'Amount',
      'send': 'Send',
      'type_message': 'Type a message...',
      'other_airport': 'Other Thailand Airport',
      'booking_summary': 'Booking Summary',
      'service_type': 'Service Type',
      'thb': 'THB',
      'payment_method': 'Payment Method',
      'pay_driver_at_destination': 'Pay driver at destination',
      'customer_payment_method': 'Payment method',
      'customer_payment_pay_driver': 'Pay the driver at the destination',
      'customer_payment_cash': 'Cash payment',
      'customer_payment_qr': 'QR payment',
      'customer_payment_card': 'Card payment',
      'customer_payment_bank_transfer': 'Bank transfer',
      'customer_payment_unknown': 'Payment method unavailable',
      'customer_price_conditions':
          'Tolls, airport parking fees, and extra waiting charges may vary depending on the booking conditions.',
      'customer_luggage_guidance':
          'Add large suitcases and golf bags accurately so we can recommend a suitable vehicle. Capacity depends on the final vehicle assigned.',
      'customer_vehicle_guidance':
          'Vehicles smaller than the recommendation may be unavailable when passenger or luggage volume is high.',
      'customer_vehicle_not_suitable':
          'Not suitable for the selected passengers or luggage.',
      'customer_confirmation_title': 'Final booking check',
      'customer_confirmation_subtitle':
          'Please review the details before confirming your booking.',
      'customer_confirmation_edit': 'Edit',
      'customer_confirmation_trip': 'Trip details',
      'customer_confirmation_passengers': 'Passengers and luggage',
      'customer_confirmation_vehicle_payment': 'Vehicle and payment',
      'customer_confirmation_customer': 'Customer information',
      'customer_confirmation_required':
          'Please review the booking details before confirming.',
      'customer_next_action': 'What to do next',
      'customer_driver_assignment': 'Driver assignment',
      'customer_driver_pending': 'Driver assignment in progress',
      'customer_status_unknown_guidance':
          'Please check the latest booking status.',
      'customer_rate_driver': 'Rate your driver',
      'customer_review_thanks': 'Thank you for leaving a review.',
      'messenger_type': 'Messenger Type',
      'messenger_id': 'Messenger ID',
      'messenger_type_hint': 'LINE, WhatsApp, etc.',
      'customer_contact_sns_notice':
          'After booking, we will contact you via SNS in advance to share vehicle and driver details. If we cannot reach you, your booking may be cancelled. Please provide a contact number or SNS account that accepts friend requests.',
      'contact_connect_title': 'Connect on messenger',
      'contact_connect_booking_number': 'Booking number',
      'contact_connect_intro':
          'Your T-Rider booking is created. Connect on messenger so we can share driver and vehicle updates.',
      'contact_connect_choose_channel': 'Choose a messenger',
      'contact_connect_email_soon': 'Email (coming soon)',
      'contact_connect_after_launch':
          'After sending a message with your booking reference, tap the button below.',
      'contact_connect_confirm_sent': 'I sent the message',
      'contact_connect_waiting': 'Waiting for T-Rider to confirm your message',
      'contact_connect_waiting_hint':
          'This usually takes a few minutes. You can keep this page open.',
      'contact_connect_step_created': 'Booking created',
      'contact_connect_step_connecting': 'Connecting',
      'contact_connect_step_complete': 'Complete',
      'contact_connect_ref_copied': 'Booking reference copied',
      'contact_connect_launch_failed': 'Could not open messenger app',
      'contact_connect_unsafe_url': 'This messenger link is not allowed',
      'contact_connect_load_failed': 'Could not load contact connection status',
      'contact_connect_wechat_title': 'WeChat',
      'contact_connect_wechat_body':
          'Scan the QR code or copy the WeChat ID, then send your booking reference.',
      'contact_connect_wechat_id_copied': 'WeChat ID copied',
      'contact_connect_invalid_link': 'Invalid contact connect link',
      'additional_requests': 'Additional Requests',
      'country_code_hint': 'TH, US, KR…',
      'country_search_hint': 'Search country or enter a value',
      'boarding_qr_title': 'Boarding QR',
      'boarding_qr_hint':
          'Show this QR code to your driver when boarding. Once the driver scans it, your ride status will change to boarded.',
      'booking_trust_message':
          'Keep your booking number. You can check driver assignment and trip status on the booking lookup page.',
      'airport_meeting_semantics': 'BKK airport meeting guide, Gate {gate}',
      'airport_meeting_gate_3_title': 'Name sign meeting guide · Gate 3',
      'airport_meeting_gate_7_title': 'Vehicle pickup guide · Gate 7',
      'airport_meeting_gate_inside': 'Gate inside',
      'airport_meeting_gate_outside': 'Gate outside',
      'airport_meeting_no_picket_highlight':
          'After collecting your luggage, notify pickup before leaving the airport.',
      'airport_meeting_picket_highlight':
          'Please stay inside Gate 3 and look for the name sign.',
      'airport_meeting_no_picket_body_1':
          'Your driver needs time to move the vehicle to the Gate 7 curbside pickup area.',
      'airport_meeting_no_picket_body_2':
          'Check the vehicle number and driver before boarding.',
      'airport_meeting_picket_body_1':
          'After collecting luggage, move to the inside of Gate 3 and find the sign with your name.',
      'airport_meeting_picket_body_2':
          'Our staff will guide the driver and vehicle to the front of Gate 3.',
      'airport_meeting_step_luggage': 'Collect all luggage.',
      'airport_meeting_step_notify_driver':
          'Notify the driver that you are ready for pickup.',
      'airport_meeting_step_vehicle_number':
          'Check the vehicle number you receive.',
      'airport_meeting_step_gate_7': 'Move outside Arrival Gate 7.',
      'airport_meeting_step_board':
          'Confirm the vehicle and driver, then board.',
      'airport_meeting_step_gate_3': 'Move inside Arrival Gate 3.',
      'airport_meeting_step_find_picket': 'Find the name sign with your name.',
      'airport_meeting_step_wait_staff': 'Wait briefly with staff guidance.',
      'airport_meeting_step_board_after_staff':
          'After the vehicle arrival notice, go outside and board.',
      'airport_meeting_do_not_exit': 'Do not go outside before staff guidance.',
      'airport_meeting_vehicle_pending':
          'Driver and vehicle details are pending. They will appear after assignment.',
      'airport_meeting_vehicle_info': 'Driver and vehicle',
      'airport_meeting_driver': 'Driver',
      'airport_meeting_driver_phone': 'Driver phone',
      'airport_meeting_vehicle_type': 'Vehicle type',
      'airport_meeting_vehicle_color': 'Vehicle color',
      'airport_meeting_vehicle_plate': 'Plate number',
      'password': 'Password',
      'admin_dispatch_search': 'Search bookings',
      'admin_dispatch_all_statuses': 'All statuses',
      'admin_dispatch_assignment': 'Assignment',
      'admin_dispatch_all_assignments': 'All',
      'admin_dispatch_unassigned': 'Unassigned',
      'admin_dispatch_assigned': 'Assigned',
      'admin_dispatch_refresh': 'Refresh',
      'admin_dispatch_retry': 'Retry',
      'admin_dispatch_empty': 'No bookings found',
      'admin_dispatch_load_more': 'Load more',
      'admin_dispatch_login_title': 'Admin sign in',
      'admin_dispatch_login': 'Sign in',
      'admin_dispatch_assign_driver': 'Assign driver',
      'admin_dispatch_reassign_driver': 'Reassign driver',
      'admin_dispatch_unassign_driver': 'Unassign driver (reopen)',
      'admin_dispatch_unassign_driver_title':
          'Unassign driver and reopen booking',
      'admin_dispatch_unassign_driver_message':
          'The booking will return to OPEN and become available to other drivers.',
      'admin_dispatch_unassign_driver_warning':
          'This action cannot be undone. The assigned driver will be notified immediately.',
      'admin_dispatch_unassign_driver_reason': 'Reason for unassigning',
      'admin_dispatch_unassign_driver_confirm': 'Unassign and reopen',
      'admin_dispatch_unassign_success':
          'Driver unassigned. The booking is open for dispatch again.',
      'admin_dispatch_unassign_failed': 'Could not unassign the driver.',
      'admin_dispatch_unassign_error_invalid_status':
          'This booking cannot be unassigned in its current status. Refresh and try again.',
      'admin_dispatch_unassign_error_no_assignment':
          'This booking has no active driver assignment.',
      'admin_dispatch_unassign_error_validation':
          'Please enter a valid reason for unassigning the driver.',
      'admin_dispatch_assigned_driver': 'Assigned driver',
      'admin_dispatch_reassign_reason': 'Reassignment reason',
      'admin_dispatch_no_drivers': 'No drivers available',
      'admin_dispatch_loading_drivers': 'Loading drivers...',
      'admin_dispatch_loading_drivers_failed': 'Could not load drivers',
      'admin_dispatch_load_more_failed': 'Could not load more bookings',
      'admin_dispatch_next_action_recommend':
          'Review driver recommendations or assign manually.',
      'admin_dispatch_next_action_assign': 'Assign a driver to this booking.',
      'admin_dispatch_next_action_reassign':
          'Reassign if the current driver cannot complete the trip.',
      'admin_dispatch_next_action_none':
          'No dispatch actions available for the current status.',
      'admin_ops_search': 'Search booking, customer, phone, driver, or vehicle',
      'admin_ops_filters': 'Filters',
      'admin_ops_filter_apply': 'Apply',
      'admin_ops_filter_reset': 'Reset',
      'admin_ops_filter_date_from': 'Pickup from (YYYY-MM-DD)',
      'admin_ops_filter_date_to': 'Pickup to (YYYY-MM-DD)',
      'admin_ops_filter_date_from_invalid': 'Please choose a valid start date.',
      'admin_ops_filter_date_to_invalid': 'Please choose a valid end date.',
      'admin_ops_filter_date_range_invalid':
          'The end date cannot be earlier than the start date.',
      'admin_ops_filter_service_type': 'Service type code',
      'admin_ops_filter_origin': 'Origin area',
      'admin_ops_filter_destination': 'Destination area',
      'admin_ops_filter_settlement': 'Settlement status',
      'admin_ops_filter_low_rating': 'Low rating only',
      'admin_ops_filter_has_inquiry': 'Has inquiry',
      'admin_ops_view_needs_action': 'Needs action',
      'admin_ops_view_today': 'Today',
      'admin_ops_view_upcoming': 'Upcoming',
      'admin_ops_view_in_progress': 'In progress',
      'admin_ops_view_settlement': 'Settlement',
      'admin_ops_view_completed': 'Completed',
      'admin_ops_view_cancelled': 'Cancelled',
      'admin_ops_view_all': 'All',
      'admin_ops_summary_needs_action': 'Needs action',
      'admin_ops_summary_unassigned': 'Unassigned',
      'admin_ops_summary_today': 'Today',
      'admin_ops_summary_in_progress': 'In progress',
      'admin_ops_summary_settlement': 'Settlement',
      'admin_ops_summary_issues': 'Issues',
      'admin_ops_result_total': 'Total {total}',
      'admin_ops_unassigned_result_total': 'All unassigned {total}',
      'admin_ops_result_showing': 'Showing {count}',
      'admin_ops_result_filtered': 'current filters',
      'admin_ops_severity_urgent': 'Urgent',
      'admin_ops_severity_soon': 'Action soon',
      'admin_ops_severity_review': 'Review',
      'admin_ops_reason_low_rating': 'Low rating requires review',
      'admin_ops_reason_receipt_rejected': 'Receipt rejected',
      'admin_ops_reason_receipt_review': 'Receipt confirmation needed',
      'admin_ops_reason_receipt_missing': 'Receipt not submitted',
      'admin_ops_reason_pickup_overdue_unassigned':
          'Pickup overdue · unassigned',
      'admin_ops_reason_pickup_overdue_stalled': 'Pickup overdue · stalled',
      'admin_ops_reason_pickup_soon_unassigned': 'Pickup soon · unassigned',
      'admin_ops_reason_customer_inquiry': 'Customer inquiry pending',
      'admin_ops_reason_boarding_delay': 'Boarding delayed',
      'admin_ops_reason_long_trip': 'Long trip in progress',
      'admin_ops_reason_status_stale': 'No recent status change',
      'admin_ops_next_low_rating': 'Review the low customer rating.',
      'admin_ops_next_receipt_review':
          'A transfer slip was submitted. Confirm settlement.',
      'admin_ops_next_receipt_missing':
          'Ask the driver to submit the transfer slip.',
      'admin_ops_next_assign_driver': 'Assign a driver before pickup.',
      'admin_ops_next_customer_inquiry': 'Reply to the customer inquiry.',
      'admin_ops_next_boarding_delay': 'Check why boarding has not started.',
      'admin_ops_next_long_trip':
          'Check whether the trip is still in progress.',
      'admin_ops_next_confirm_settlement': 'Confirm the 200 THB settlement.',
      'admin_ops_cta_settlement_detail': 'Settlement detail',
      'admin_ops_cta_review_rating': 'Review rating',
      'admin_ops_cta_check_status': 'Check status',
      'admin_ops_cta_view_booking': 'View booking',
      'admin_ops_cta_review_act': 'Review & act',
      'admin_ops_more_reasons': '{count} more reasons',
      'admin_ops_select_booking': 'Select a booking to preview',
      'admin_ops_settlement_rejected': 'Receipt rejected',
      'admin_ops_settlement_submitted': 'Receipt submitted',
      'admin_ops_settlement_missing': 'Receipt missing',
      'admin_ops_next_action_label': 'Next action',
      'admin_ops_archive_show': 'Show hidden bookings',
      'admin_ops_archive_hide': 'Hide hidden bookings',
      'admin_ops_archive_bulk': 'Hide test bookings ({count})',
      'admin_ops_archive_confirm_title':
          'Hide the selected bookings as test data?',
      'admin_ops_archive_warning':
          'Some selected bookings already contain assignment, trip, or settlement history. Booking data and related records will not be deleted; they will only be hidden from the default admin list.',
      'admin_ops_archive_info':
          'Booking data and related records will not be deleted; they will only be hidden from the default admin list.',
      'admin_ops_archive_hidden_notice':
          'Hidden bookings are excluded from the default operations screen, driver/customer screens, and settlement queue.',
      'admin_ops_archive_success': 'Selected bookings were hidden.',
      'admin_ops_archive_restore_success': 'The hidden booking was restored.',
      'admin_ops_archive_failed': 'Failed to hide bookings.',
      'admin_ops_archive_restore_failed': 'Failed to restore the booking.',
      'admin_ops_archive_restore': 'Restore',
      'admin_ops_archive_badge': 'Archived/Test',
      'admin_ops_route_airport_pickup': 'Airport → Destination',
      'admin_ops_route_airport_dropoff': 'Origin → Airport',
      'admin_ops_route_city_transfer': 'Origin → Destination',
      'driver_nav_jobs': 'Jobs',
      'driver_nav_home': 'Home',
      'driver_nav_today': 'Today',
      'driver_nav_history': 'Trip history',
      'driver_nav_account': 'My account',
      'driver_qr_scan_menu': 'Scan QR',
      'driver_qr_scan_instruction':
          'Point the camera at the customer’s QR code.',
      'driver_qr_scan_no_active_job': 'No active trip to scan',
      'driver_qr_scan_invalid':
          'This QR code is invalid or no longer available.',
      'driver_qr_scan_success_boarding': 'Customer boarding confirmed.',
      'driver_qr_scan_success_dropoff': 'Ride completion confirmed.',
      'driver_nav_notifications': 'Notifications',
      'driver_nav_settlement': 'Settlement',
      'driver_nav_profile': 'Profile',
      'driver_refresh': 'Refresh',
      'driver_retry': 'Retry',
      'driver_jobs_error_title': 'Could not load jobs',
      'driver_jobs_empty_title': 'No jobs today',
      'driver_jobs_empty_message': 'You have no assigned pickups for today.',
      'driver_jobs_today_date': 'Today — {date}',
      'driver_jobs_group_active': 'Active / Current',
      'driver_jobs_group_upcoming': 'Upcoming',
      'driver_jobs_group_completed': 'Completed',
      'driver_jobs_tab_new': 'New calls',
      'driver_jobs_tab_mine': 'My jobs',
      'driver_jobs_tab_past': 'Past jobs',
      'driver_jobs_open_history': 'View all trip history',
      'driver_claim_call': 'Accept this job',
      'driver_claim_confirm_title': 'Accept this job?',
      'driver_claim_confirm_message':
          'Please check the time and locations before confirming.',
      'driver_claim_success': 'Job accepted.',
      'driver_claim_already_assigned':
          'Another driver accepted this job first.',
      'driver_open_calls_online_required':
          'You can receive calls only when you are online.',
      'driver_open_calls_empty': 'No available calls right now.',
      'driver_urgent_badge': 'Urgent',
      'driver_urgent_section_title': 'Urgent request calls',
      'driver_urgent_call_new_arrived': 'An urgent request call has arrived.',
      'driver_urgent_accept': 'Accept',
      'driver_urgent_already_locked':
          'Another driver has already accepted this call.',
      'driver_urgent_eta_dialog_title': 'Enter arrival ETA',
      'driver_urgent_eta_dialog_message':
          'After you enter your estimated arrival time, the booking will be confirmed once the customer accepts. Please enter your ETA.',
      'driver_urgent_eta_minutes_label': 'ETA (minutes)',
      'driver_urgent_eta_submit': 'Submit',
      'driver_urgent_eta_countdown': 'Time remaining: {time}',
      'driver_urgent_eta_invalid': 'Enter a valid number of minutes.',
      'driver_urgent_eta_success':
          'ETA submitted. Waiting for customer confirmation.',
      'driver_urgent_confirmed': 'Customer confirmed. The booking is assigned.',
      'driver_urgent_min_eta_hint':
          'A previous rejection requires ETA under {minutes} minutes.',
      'driver_urgent_awaiting_customer':
          'Waiting for customer confirmation ({time} remaining).',
      'driver_urgent_eta_lock_expired':
          'The time window to submit your ETA has expired.',
      'driver_urgent_eta_not_fast_enough':
          'Please enter a faster ETA than the previous rejection required.',
      'driver_urgent_round_ended':
          'The customer declined or this negotiation round ended.',
      'driver_urgent_cancelled': 'This urgent request call was cancelled.',
      'driver_settlement_block_title': 'Cannot receive new jobs yet',
      'driver_settlement_block_message':
          'Please pay the commission and upload your transfer slip. You can receive new jobs after admin review.',
      'driver_settlement_block_cta': 'Go to settlement',
      'driver_call_badge_admin_manual': 'Admin manual call',
      'driver_call_badge_no_commission': 'No commission',
      'driver_call_bank_account_confirm_notice':
          'Payment will be sent to the bank account registered at signup. Please double-check that your account details are correct.',
      'driver_home_new_calls_title': 'New jobs available',
      'driver_home_new_calls_message': '{count} new call(s) are waiting.',
      'driver_home_new_calls_cta': 'View new jobs',
      'driver_home_urgent_calls_title': 'Urgent request calls',
      'driver_home_urgent_calls_message':
          '{count} urgent request call(s) are waiting.',
      'driver_home_urgent_calls_cta': 'View urgent calls',
      'driver_call_customer': 'Call customer',
      'driver_account_notifications': 'Notifications',
      'driver_step_progress': 'Step {step} of {total}',
      'driver_step_assigned': 'Job confirmed',
      'driver_step_assigned_hint': 'Review the booking details before standby.',
      'driver_step_standby': 'Confirm standby',
      'driver_step_standby_hint':
          'Confirm you are ready before travel time (usually 1 hour before pickup).',
      'driver_step_depart_pickup': 'Head to pickup',
      'driver_step_depart_pickup_hint':
          'Open maps and start driving to the pickup point.',
      'driver_step_arrived_pickup': 'Arrived at pickup',
      'driver_step_arrived_pickup_hint':
          'Confirm after you reach the pickup point.',
      'driver_step_customer_boarded': 'Customer boarded',
      'driver_step_customer_boarded_hint':
          'Confirm after the customer is in the vehicle.',
      'driver_step_in_transit': 'Trip in progress',
      'driver_step_finish_trip': 'Finish trip',
      'driver_step_finish_trip_hint':
          'After finishing, you must pay commission and upload a slip before the next job.',
      'driver_step_settlement': 'Settlement required',
      'driver_step_settlement_hint':
          'Pay commission and upload your transfer slip.',
      'driver_today_greeting_generic': 'Hello',
      'driver_today_greeting_named': 'Hello, {name}',
      'driver_today_trip_count': 'You have {count} trips today',
      'driver_today_current_trip': 'Current trip',
      'driver_today_cta_continue': 'Continue trip',
      'driver_today_cta_settlement': 'View settlement',
      'driver_today_cta_settlement_submit': 'Settlement and receipt',
      'driver_today_cta_settlement_waiting': 'Waiting for confirmation',
      'driver_today_remaining_title': '{count} trips left today',
      'driver_today_completed_title': 'Completed today ({count})',
      'driver_today_empty_title': 'No scheduled trips.',
      'driver_today_empty_message':
          'You will be notified when a new trip is assigned.',
      'driver_quick_navigate': 'Navigate',
      'driver_quick_call': 'Call',
      'driver_status_guidance_assigned':
          'Assignment confirmed. Start heading to the pickup location.',
      'driver_status_guidance_on_route':
          'You are on the way to pickup. Tap arrived when you reach the pickup point.',
      'driver_status_guidance_arrived':
          'You have arrived at pickup. Confirm after the customer boards.',
      'driver_status_guidance_picked_up':
          'Customer onboard. End the trip after arriving at the destination.',
      'driver_status_guidance_settlement':
          'Trip ended. You cannot receive new calls until settlement is confirmed.',
      'driver_history_range_7d': 'Last 7 days',
      'driver_history_range_30d': 'Last 30 days',
      'driver_history_range_all': 'All',
      'driver_history_empty':
          'No completed trips or settlement records to show.',
      'driver_history_subtitle': 'View completed trips and settlement records.',
      'driver_history_pending_settlement':
          '{count} trip(s) need settlement action',
      'driver_account_settlement': 'Settlement',
      'driver_account_support': 'Customer support',
      'driver_account_profile': 'My profile',
      'driver_account_vehicle_title': 'My vehicle',
      'driver_account_vehicle_empty': 'No vehicle information registered yet.',
      'driver_account_vehicle_color': 'Color',
      'driver_account_vehicle_year': 'Year',
      'driver_account_vehicle_model': 'Model',
      'driver_profile_edit_title': 'Edit profile',
      'driver_profile_save': 'Save changes',
      'driver_profile_save_success': 'Profile updated.',
      'driver_profile_save_failed': 'Could not save profile. Please try again.',
      'driver_profile_readonly_email': 'Email (cannot be changed)',
      'driver_profile_upload_avatar': 'Change profile photo',
      'driver_profile_upload_vehicle_photo': 'Change vehicle photo',
      'driver_account_language': 'Language',
      'driver_account_notification_settings': 'Notification settings',
      'driver_account_terms': 'Terms of service',
      'driver_account_privacy': 'Privacy policy',
      'driver_account_terms_placeholder': 'Terms page coming soon',
      'driver_account_privacy_placeholder': 'Privacy page coming soon',
      'driver_logout_confirm_title': 'Log out?',
      'driver_logout_confirm_message': 'You will need to sign in again.',
      'driver_action_start_on_route': 'Head to pickup',
      'driver_action_accept_booking': 'Confirm ready for job',
      'driver_view_booking_detail': 'View booking',
      'driver_action_mark_arrived': 'Arrived at pickup',
      'driver_action_mark_picked_up': 'Customer boarded',
      'driver_action_end_trip': 'Finish trip',
      'driver_action_complete_trip': 'Finish trip',
      'driver_confirm_no': 'No',
      'driver_confirm_yes': 'Yes',
      'driver_confirm_start_on_route_title': 'Start heading to pickup?',
      'driver_confirm_start_on_route_message':
          'Start driving to the pickup location now.',
      'driver_confirm_start_on_route_yes': 'Start heading to pickup',
      'driver_confirm_accept_booking_title': 'Confirm this trip?',
      'driver_confirm_accept_booking_message':
          'You can confirm standby from one hour before pickup. After confirmation, start heading to pickup when ready.',
      'driver_confirm_accept_booking_yes': 'Confirm standby',
      'driver_accept_booking_processing': 'Confirming standby',
      'driver_success_accept_booking': 'Standby confirmed',
      'driver_release_assignment': 'Release assignment',
      'driver_release_assignment_title': 'Release this booking assignment?',
      'driver_release_assignment_message':
          'The customer booking stays active. Another driver will be assigned.',
      'driver_release_assignment_cancel': 'Keep assignment',
      'driver_release_assignment_confirm': 'Release assignment',
      'driver_release_assignment_success': 'Assignment released.',
      'driver_release_assignment_failed':
          'Could not release this assignment. Please try again.',
      'driver_release_assignment_blocked':
          'This assignment cannot be released in the current status.',
      'driver_release_assignment_emergency_hint':
          'Normal release is limited within 2 hours of pickup. You can release only for an emergency.',
      'driver_release_assignment_irreversible':
          'This cannot be undone. You will not keep this booking.',
      'driver_release_reason_label': 'Release reason',
      'driver_release_reason_detail_label': 'Details',
      'driver_release_reason_detail_required': 'Please enter another reason',
      'driver_release_reason_VEHICLE_BREAKDOWN': 'Vehicle breakdown',
      'driver_release_reason_ACCIDENT': 'Accident',
      'driver_release_reason_DRIVER_ILLNESS': 'Driver illness',
      'driver_release_reason_FAMILY_EMERGENCY': 'Family emergency',
      'driver_release_reason_SCHEDULE_CONFLICT': 'Schedule conflict',
      'driver_release_reason_LOCATION_TOO_FAR': 'Location too far',
      'driver_release_reason_OTHER': 'Other',
      'driver_assignment_ended_confirm': 'OK',
      'driver_booking_not_found': 'Booking information could not be found.',
      'driver_assignment_ended_generic_title': 'Assignment ended',
      'driver_assignment_ended_generic_message':
          'You can no longer continue this booking.',
      'driver_assignment_ended_generic_snackbar':
          'Assignment for {bookingNumber} was released.',
      'driver_assignment_ended_customer_title': 'Customer cancelled',
      'driver_assignment_ended_customer_message':
          'The customer cancelled the booking, so your assignment was released automatically.',
      'driver_assignment_ended_customer_snackbar':
          'Customer cancelled {bookingNumber}. Assignment released.',
      'driver_assignment_ended_admin_title': 'Admin cancelled',
      'driver_assignment_ended_admin_message':
          'An admin cancelled the booking, so your assignment was released.',
      'driver_assignment_ended_admin_snackbar':
          'Admin cancelled {bookingNumber}. Assignment released.',
      'driver_assignment_ended_released_title': 'Assignment released',
      'driver_assignment_ended_released_message':
          'Assignment release completed. This booking is open again for other drivers.',
      'driver_assignment_ended_released_snackbar':
          'You released {bookingNumber}. It is open for other drivers.',
      'driver_assignment_ended_reassigned_title': 'Reassigned',
      'driver_assignment_ended_reassigned_message':
          'This booking was assigned to another driver.',
      'driver_assignment_ended_reassigned_snackbar':
          '{bookingNumber} was assigned to another driver.',
      'driver_assignment_ended_completed_title': 'Trip completed',
      'driver_assignment_ended_completed_message':
          'This trip is already completed.',
      'driver_confirm_mark_arrived_title': 'Arrived at pickup?',
      'driver_confirm_mark_arrived_message':
          'Have you arrived at the pickup location?',
      'driver_confirm_mark_arrived_yes': 'Arrived',
      'driver_confirm_mark_picked_up_title': 'Has the customer boarded?',
      'driver_confirm_mark_picked_up_message':
          'After confirmation, the trip will start.',
      'driver_confirm_mark_picked_up_yes': 'Confirm boarding',
      'driver_confirm_end_trip_title':
          'Has the customer arrived at destination?',
      'driver_confirm_end_trip_message':
          'After ending the trip, it will move to settlement.',
      'driver_confirm_end_trip_yes': 'End trip',
      'driver_confirm_customer_boarded_processing':
          'Confirming customer boarding',
      'driver_end_trip_processing': 'Ending trip',
      'driver_trip_action_processing': 'Processing',
      'driver_customer_payment_amount': 'Amount to collect from customer',
      'driver_payment_method': 'Payment method',
      'driver_payment_method_pay_driver': 'Pay driver at destination',
      'driver_payment_method_bank_transfer': 'Bank transfer',
      'driver_payment_method_card': 'Card',
      'driver_payment_method_unknown': 'Payment method needs confirmation',
      'driver_payment_summary_title': 'Payment and income',
      'driver_customer_total_amount': 'Customer total',
      'driver_company_commission': 'Company commission',
      'driver_expected_income': 'Driver expected income',
      'driver_income_unavailable':
          'Income information is not available. Please contact operations.',
      'driver_commission_unavailable': 'Commission information needs review',
      'driver_receipt_status': 'Receipt status',
      'driver_settlement_status_not_due_yet': 'Not due yet',
      'driver_settlement_status_due': 'Payment due',
      'driver_settlement_status_overdue': 'Overdue',
      'driver_settlement_status_receipt_submitted': 'Receipt under review',
      'driver_settlement_status_rejected': 'Receipt rejected',
      'driver_settlement_status_completed': 'Settlement completed',
      'driver_settlement_status_waived': 'Commission waived',
      'driver_settlement_status_unknown': 'Status review needed',
      'driver_receipt_status_none': 'No receipt',
      'driver_receipt_status_submitted': 'Receipt submitted',
      'driver_receipt_status_rejected': 'Receipt rejected',
      'driver_receipt_status_approved': 'Receipt approved',
      'driver_receipt_status_unknown': 'Receipt status unknown',
      'driver_settlement_not_due_yet_message':
          'The company commission is not due yet. You can receive new calls.',
      'driver_settlement_due_message':
          'Please pay the company commission and upload the receipt.',
      'driver_settlement_due_blocked_message':
          'Please pay the company commission and upload the receipt. You cannot receive new calls until settlement is completed.',
      'driver_settlement_overdue_message':
          'The commission payment is overdue. Please pay the commission and upload the receipt.',
      'driver_settlement_receipt_review_message':
          'Your receipt is under review. You cannot receive new calls until admin approval.',
      'driver_settlement_rejected_message':
          'The receipt was rejected. Check the reason and submit it again.',
      'driver_settlement_completed_message': 'Settlement has been completed.',
      'driver_settlement_waived_message':
          'The company commission was waived for this trip. You can receive new calls.',
      'driver_new_calls_blocked_by_settlement':
          'The trip has ended, but settlement remains. Complete settlement before receiving new calls.',
      'driver_success_picked_up': 'Customer boarding confirmed',
      'driver_success_end_trip': 'Trip ended. Please review settlement.',
      'driver_trip_settlement_pending_info':
          'The trip has ended. You cannot receive new calls until settlement is confirmed.',
      'driver_trip_completed_info':
          'Settlement confirmed. This trip is complete.',
      'driver_standby_status': 'Standby',
      'driver_standby_confirmed': 'Confirmed',
      'driver_standby_airport_reference': 'Airport arrival reference',
      'driver_standby_departure_reference': 'Vehicle departure reference',
      'driver_standby_available_from': 'Standby available from',
      'driver_standby_too_early':
          'Standby is not available yet. You can confirm from one hour before the appointment time.',
      'driver_standby_reference_missing':
          'Appointment time was not found. Please contact admin.',
      'driver_standby_status_not_allowed':
          'This booking cannot be confirmed now. Please contact admin to check the booking status.',
      'driver_open_google_maps': 'Open in Google Maps',
      'driver_google_maps_failed': 'Could not open Google Maps.',
      'driver_location_unavailable': 'Location information unavailable',
      'driver_name_sign_required':
          'Name-board service requested. Prepare the placard before meeting the customer.',
      'driver_settlement_popup_title': 'Settlement required',
      'driver_settlement_popup_message':
          'The trip is ended. Pay the company commission, then upload the transfer slip from settlement detail.',
      'driver_action_scan_boarding': 'Scan boarding QR',
      'driver_action_scan_dropoff': 'Scan dropoff QR',
      'driver_job_assigned_hint': 'Head to pickup when ready',
      'driver_status_on_route': 'On route',
      'driver_status_arrived': 'Arrived at pickup',
      'driver_status_picked_up': 'Passenger on board',
      'driver_status_no_show': 'No show',
      'driver_success_boarding': 'Passenger boarded',
      'driver_success_completed': 'Trip completed',
      'driver_success_on_route': 'Trip started',
      'driver_success_arrived': 'Marked arrived',
      'driver_call_failed': 'Could not start call',
      'driver_detail_error': 'Could not load booking',
      'driver_section_trip': 'Trip',
      'driver_pickup_time': 'Pickup time',
      'driver_message_customer': 'Message customer',
      'driver_estimated_arrival': 'Estimated arrival',
      'driver_next_action': 'Next action',
      'driver_section_settlement': 'Settlement',
      'driver_settlement_loading_failed':
          'We could not load the settlement information. Please try again.',
      'driver_rejection_reason': 'Rejection reason',
      'driver_view_settlement': 'View settlement',
      'driver_qr_boarding_title': 'Boarding QR',
      'driver_qr_dropoff_title': 'Dropoff QR',
      'driver_qr_boarding_help':
          'Scan the customer boarding QR shown in their app.',
      'driver_qr_dropoff_help': 'Scan the customer dropoff QR from their app.',
      'driver_qr_boarding_manual_label': 'Boarding QR token',
      'driver_qr_dropoff_manual_label': 'Dropoff QR token',
      'driver_qr_manual_hint': 'Enter code manually — no camera required',
      'driver_qr_manual_entry': 'Enter code manually',
      'driver_qr_use_manual': 'Enter code manually',
      'driver_qr_use_camera': 'Use camera',
      'driver_qr_submit': 'Submit',
      'admin_qr_management_title': 'QR management',
      'admin_qr_management_help':
          'Reissue one-time QR tokens for local testing. Tokens are shown once only.',
      'admin_qr_management_disabled_help':
          'QR reissue is not available in this environment.',
      'admin_dev_qr_reissue_boarding': 'Reissue boarding QR',
      'admin_dev_qr_reissue_dropoff': 'Reissue dropoff QR',
      'driver_notification_no_target': 'This notification has no linked job',
      'driver_mark_all_read': 'Mark all read',
      'driver_notifications_empty': 'No notifications',
      'driver_rating_error': 'Could not load rating',
      'driver_no_ratings': 'No ratings yet',
      'driver_rating_average': 'average',
      'driver_rating_count': 'reviews',
      'driver_logout': 'Log out',
      'driver_pwa_install_title': 'Install T-Ride Driver',
      'driver_pwa_install_body':
          'Install T-Ride on your home screen for easier access to jobs and driver features.',
      'driver_pwa_install_now': 'Install now',
      'driver_pwa_install_preparing': 'Preparing installation...',
      'driver_pwa_install_later': 'Later',
      'driver_pwa_install_manual':
          'If the install window does not appear, open the Chrome menu and choose “Install app” or “Add to Home screen”.',
      'driver_pwa_install_ios_steps':
          'On iPhone or iPad, open Safari, tap Share, choose “Add to Home Screen”, then tap Add.',
      'driver_pwa_install_open_in_chrome':
          'This browser cannot install the app automatically. Open this page in Chrome, then install T-Ride from the Chrome menu.',
      'driver_pwa_install_success': 'T-Ride has been installed.',
      'driver_pwa_install_dismissed':
          'Installation was cancelled. You can install it next time.',
      'driver_pwa_install_unavailable':
          'Automatic installation cannot start now. Use the browser menu to install the app.',
      'driver_pwa_install_error':
          'Could not start installation. Please use the browser menu.',
      'driver_online': 'Online',
      'driver_offline': 'Offline',
      'driver_session_ready': 'Ready for assignments',
      'driver_session_offline': 'You are offline',
      'driver_status_online_ready': 'Online · Ready for assignments',
      'driver_status_offline_unavailable':
          'Offline · Cannot receive assignments',
      'driver_status_active_busy': 'On trip · Cannot go offline',
      'driver_active_job_in_progress': 'Active job in progress',
      'driver_active_job_stay_online':
          'Active job in progress — stay online until finished.',
      'driver_call_status_ready': 'Ready for calls',
      'driver_call_status_offline': 'Calls paused',
      'driver_call_status_active_trip': 'On trip',
      'driver_call_status_active_trip_helper': 'Cannot receive new calls',
      'driver_call_status_settlement_required': 'Settlement required',
      'driver_call_status_customer_issue_review': 'Customer issue review',
      'driver_call_status_account_under_review': 'Account under review',
      'driver_call_status_account_restricted': 'Account restricted',
      'driver_call_status_driver_approval_pending': 'Driver approval pending',
      'driver_call_status_vehicle_review_required': 'Vehicle review required',
      'driver_call_status_unknown': 'Status review needed',
      'driver_call_status_detail_title': 'Call status',
      'driver_call_status_detail_ready': 'You can receive new booking calls.',
      'driver_call_status_detail_offline':
          'Go online when you are ready to receive new calls.',
      'driver_call_status_detail_active_trip':
          'Finish the current trip before receiving another call.',
      'driver_call_status_detail_settlement_required':
          'Settlement confirmation is required before receiving a new call.',
      'driver_call_status_detail_customer_issue_review':
          'The operations team is reviewing a customer issue.',
      'driver_call_status_detail_account_under_review':
          'Your driver account is under review by operations.',
      'driver_call_status_detail_account_restricted':
          'Your driver account is restricted. Please contact operations.',
      'driver_call_status_detail_driver_approval_pending':
          'Driver approval is not complete yet.',
      'driver_call_status_detail_vehicle_review_required':
          'A valid active vehicle is required before receiving calls.',
      'driver_call_status_detail_unknown':
          'Please refresh or contact operations if this continues.',
      'driver_notification_new': 'New',
      'driver_notification_mark_read': 'Mark read',
      'driver_qr_camera_unavailable': 'Camera unavailable',
      'driver_qr_camera_fallback': 'Use manual entry below.',
      'driver_live_location_interval_hint':
          'Updates every {seconds}s while enabled. Turn off when not driving.',
      'driver_location_status_title': 'Location sharing',
      'driver_location_sharing': 'Sharing location',
      'driver_location_starting': 'Starting location sharing',
      'driver_location_stopped': 'Location sharing stopped',
      'driver_location_temporarily_unavailable':
          'Location sharing was temporarily interrupted. We will retry automatically.',
      'driver_location_permission_required': 'Location permission is required',
      'driver_location_permission_explanation':
          'Location is shared only while this trip is active.',
      'driver_location_permission_denied':
          'Location permission was not allowed. The trip can continue, but customers may not see your live location.',
      'driver_location_service_disabled':
          'Device location services are turned off.',
      'driver_location_retry': 'Try location sharing again',
      'driver_location_open_settings': 'Open settings',
      'driver_location_auto_start_guidance':
          'Start driving to the pickup location and location sharing will start automatically.',
      'driver_location_not_shared_warning':
          'Location is not being shared. You can continue the trip.',
      'driver_location_error': 'Location sharing failed. Please try again.',
      'driver_location_checking_status': 'Checking driver status',
      'driver_location_checking_status_detail':
          'We will start location sharing after confirming you are online.',
      'driver_location_status_load_failed':
          'Could not check driver online status',
      'driver_location_status_load_failed_detail':
          'The trip can continue. Try location sharing again after the status check succeeds.',
      'driver_settlement_receipt_upload': 'Transfer slip upload',
      'driver_settlement_receipt_formats': 'JPG, PNG, or PDF up to 10 MB',
      'driver_settlement_due': 'Due: {date}',
      'ui_load_failed': 'Could not load data. Please try again.',
      'ui_action_failed': 'Action failed. Please try again.',
      'guest_lookup_title': 'Find my booking',
      'guest_lookup_loading': 'Loading saved booking...',
      'guest_lookup_subtitle':
          'Enter your booking number and phone to view trip details.',
      'guest_lookup_contact_subtitle':
          'Enter the name and phone number used when booking to see a summary.',
      'guest_lookup_mode_booking_number': 'By booking number',
      'guest_lookup_mode_contact_name': 'By name',
      'guest_lookup_name': 'Name on booking',
      'guest_lookup_invalid_name': 'Enter the name used for the booking',
      'guest_lookup_contact_results_title': 'Matching bookings',
      'guest_lookup_contact_detail_guidance':
          'For full trip details, please look up again using your booking number.',
      'guest_lookup_contact_back_to_list': 'Back to list',
      'guest_lookup_pickup_period_morning': 'Morning',
      'guest_lookup_pickup_period_afternoon': 'Afternoon',
      'guest_lookup_pickup_period_evening': 'Evening',
      'guest_lookup_passengers': 'Passengers',
      'guest_lookup_luggage': 'Luggage',
      'guest_lookup_booking_number': 'Booking number',
      'guest_lookup_phone': 'Phone number',
      'guest_lookup_invalid_number': 'Enter a valid booking number',
      'guest_lookup_invalid_phone': 'Enter the booking phone number',
      'guest_lookup_find': 'Find booking',
      'guest_lookup_not_found':
          'Booking not found. Please check your booking number and phone.',
      'guest_lookup_load_error': 'Unable to load booking. Please try again.',
      'guest_lookup_trip_details': 'Trip details',
      'guest_lookup_pickup': 'Pickup',
      'guest_lookup_service': 'Service',
      'guest_lookup_from': 'From',
      'guest_lookup_to': 'To',
      'guest_lookup_driver': 'Driver',
      'guest_lookup_driver_phone': 'Driver phone',
      'guest_lookup_driver_status': 'Driver status',
      'guest_lookup_vehicle': 'Vehicle',
      'guest_lookup_vehicle_photo_load_failed': 'Unable to load vehicle photo.',
      'guest_lookup_payment': 'Payment',
      'guest_lookup_total': 'Total',
      'guest_lookup_another': 'Look up another booking',
      'guest_lookup_inquiry_channels_hint': 'Contact us on KakaoTalk or LINE',
      'guest_lookup_inquiry_channel_kakao': 'KakaoTalk',
      'guest_lookup_inquiry_channel_line': 'LINE',
      'guest_lookup_refresh': 'Refresh',
      'guest_lookup_refresh_needs_phone':
          'Enter your phone on the lookup form to refresh this booking.',
      'booking_complete_track_cta': 'Track my booking',
      'auth_social_login_title':
          'Create an account to make your next booking easier',
      'auth_social_login_description':
          'View booking history and auto-fill your details on future trips.',
      'auth_google_continue': 'Continue with Google',
      'auth_kakao_continue': 'Continue with Kakao',
      'auth_kakao_callback_error': 'Kakao sign-in failed. Please try again.',
      'auth_kakao_callback_already_processed':
          'This sign-in request was already handled. Please try again if needed.',
      'auth_kakao_callback_slow_loading':
          'This is taking longer than expected. Please refresh the page.',
      'auth_kakao_callback_refresh': 'Refresh page',
      'profile_completion_title': 'Complete your profile',
      'profile_completion_body':
          'Please enter your name and phone number to continue using T-Rider.',
      'profile_completion_submit': 'Save and continue',
      'profile_completion_name_required': 'Name is required.',
      'profile_completion_phone_required':
          'Enter a valid phone number (at least 5 characters).',
      'profile_completion_phone_duplicate':
          'This phone number is already used by another account.',
      'auth_line_continue': 'Continue with LINE',
      'auth_line_callback_error': 'LINE sign-in failed. Please try again.',
      'auth_line_callback_state_mismatch':
          'LINE sign-in could not be verified. Please try again.',
      'auth_line_callback_already_processed':
          'This sign-in request was already handled. Please try again if needed.',
      'auth_line_callback_slow_loading':
          'This is taking longer than expected. Please refresh the page.',
      'auth_line_callback_refresh': 'Refresh page',
      'auth_social_login_later': 'Maybe later',
      'auth_social_login_connected': '{name}, your account is connected.',
      'my_bookings_title': 'My bookings',
      'my_bookings_empty':
          'No linked bookings yet. Complete a booking and sign in to see it here.',
      'my_bookings_view_button': 'View my bookings',
      'my_bookings_driver_summary': 'Driver: {name}',
      'my_bookings_load_error':
          'Unable to load your bookings. Please try again.',
      'booking_cancel_section': 'Cancel booking',
      'booking_cancel_action': 'Cancel booking',
      'booking_cancel_policy_hint':
          'You can cancel until 2 hours before pickup.',
      'booking_cancel_confirm_title': 'Cancel this booking?',
      'booking_cancel_confirm_action': 'Confirm cancellation',
      'booking_cancel_confirm_irreversible': 'Cancellation cannot be undone.',
      'booking_cancel_success': 'Your booking has been cancelled.',
      'booking_cancel_failed':
          'Unable to cancel this booking. Please try again.',
      'booking_cancel_blocked_within_two_hours':
          'Bookings cannot be cancelled within 2 hours of the scheduled pickup time.',
      'booking_cancel_blocked_trip_started':
          'Trips that have already started cannot be cancelled.',
      'booking_cancel_blocked_already_cancelled':
          'This booking is already cancelled.',
      'booking_cancel_blocked_completed':
          'Completed bookings cannot be cancelled.',
      'booking_cancel_blocked_no_show': 'No-show bookings cannot be cancelled.',
      'booking_cancel_blocked_invalid_pickup':
          'This booking cannot be cancelled because the pickup time is invalid.',
      'booking_cancel_blocked_locked':
          'This booking is locked and cannot be cancelled.',
      'booking_cancel_driver_assigned': 'Driver assigned: {name}',
      'booking_cancel_driver_assigned_generic': 'Driver assigned',
      'guest_status_guidance_pending':
          'Your booking is received. We will confirm it shortly.',
      'guest_status_guidance_confirmed':
          'Your booking is confirmed. A driver will be assigned before pickup.',
      'guest_status_guidance_reassignment':
          'Your previous driver became unavailable. We are assigning a new driver. Your schedule and fare stay the same.',
      'status_reassignment_in_progress': 'Reassigning driver',
      'admin_ops_severity_critical': 'Critical',
      'admin_ops_reason_critical_reassignment': 'Critical reassignment',
      'admin_ops_reason_urgent_reassignment': 'Urgent reassignment',
      'admin_ops_reason_driver_released_reassignment':
          'Driver released · reassignment',
      'admin_ops_reason_critical_unassigned': 'Critical · unassigned',
      'admin_ops_reason_urgent_unassigned': 'Urgent · unassigned',
      'guest_status_guidance_driver_assigned':
          'A driver has been assigned and will head to your pickup point soon.',
      'guest_status_guidance_on_route':
          'Your driver is on the way to the pickup location.',
      'guest_status_guidance_driver_arrived':
          'Your driver has arrived at the pickup point.',
      'guest_status_guidance_picked_up':
          'You are on the way to your destination.',
      'guest_status_guidance_settlement_pending':
          'Your trip has ended. Thank you for riding with us.',
      'guest_status_guidance_completed':
          'Your trip is complete. Please rate your driver.',
      'guest_status_guidance_cancelled':
          'This booking was cancelled. Please contact customer support for details.',
      'guest_status_guidance_no_show': 'This booking was marked as no-show.',
      'guest_lookup_issue_dropoff_qr': 'Issue dropoff QR',
      'loading_recommendation': 'Loading recommendation...',
      'calculating_price': 'Calculating price...',
      'ui_cancel': 'Cancel',
      'ui_save': 'Save',
      'ui_confirm_action': 'Confirm',
      'ui_active': 'Active',
      'ui_inactive': 'Inactive',
      'ui_all': 'All',
      'ui_yes': 'Yes',
      'ui_no': 'No',
      'admin_dashboard_loading': 'Loading dashboard...',
      'admin_dashboard_refresh': 'Refresh',
      'admin_dashboard_operations': 'Operations for {date}',
      'admin_dashboard_subtitle': '{timezone} · Last updated {updatedAt}',
      'admin_dashboard_needs_attention': 'Needs attention',
      'admin_dashboard_needs_attention_sub':
          'Prioritize unassigned bookings, active trips, and settlement issues.',
      'admin_dashboard_unassigned': 'Unassigned',
      'admin_dashboard_active_trips': 'Active trips',
      'admin_dashboard_overdue_settlements': 'Overdue settlements',
      'admin_dashboard_operational_focus': 'Operational focus',
      'admin_dashboard_operational_focus_sub':
          'Tap a row to open the relevant queue.',
      'admin_dashboard_bookings_needing_assignment':
          'Bookings needing assignment',
      'admin_dashboard_today_bookings_section': 'Today\'s bookings',
      'admin_dashboard_today_bookings_sub': 'Tap a card to open dispatch.',
      'admin_dashboard_today_bookings': 'Today bookings',
      'admin_dashboard_assigned': 'Assigned',
      'admin_dashboard_on_route': 'On route',
      'admin_dashboard_arrived': 'Arrived',
      'admin_dashboard_completed': 'Completed',
      'admin_dashboard_cancelled': 'Cancelled',
      'admin_dashboard_fleet_finance': 'Fleet & finance',
      'admin_dashboard_online_drivers': 'Online drivers',
      'admin_dashboard_pending_settlements': 'Pending settlements',
      'admin_dashboard_today_revenue': 'Today revenue',
      'admin_dashboard_completed_revenue_today':
          'Completed-trip revenue today: {amount}',
      'admin_dashboard_mixed_currencies': 'Mixed currencies',
      'admin_pricing_manager': 'Pricing Manager',
      'admin_pricing_tab_routes': 'Routes',
      'admin_pricing_tab_prices': 'Vehicle Prices',
      'admin_pricing_tab_policies': 'Charge Policies',
      'admin_pricing_tab_simulator': 'Simulator',
      'admin_pricing_kpi_active_routes': 'Active routes',
      'admin_pricing_kpi_active_prices': 'Active vehicle prices',
      'admin_pricing_kpi_active_policies': 'Active charge policies',
      'admin_pricing_kpi_current_prices': 'Current prices',
      'admin_pricing_kpi_expiring_soon': 'Expiring soon',
      'admin_pricing_no_routes': 'No routes found',
      'admin_pricing_no_prices': 'No vehicle prices found',
      'admin_pricing_no_policies': 'No charge policies found',
      'admin_pricing_create_route': 'Create route',
      'admin_pricing_create_price': 'Create price',
      'admin_pricing_create_policy': 'Create policy',
      'admin_pricing_filter_service_type': 'Service type',
      'admin_pricing_filter_origin': 'Origin',
      'admin_pricing_filter_destination': 'Destination',
      'admin_pricing_filter_active': 'Active',
      'admin_pricing_filter_search': 'Search',
      'admin_pricing_filter_route': 'Route',
      'admin_pricing_filter_all_routes': 'All routes',
      'admin_pricing_filter_status': 'Status',
      'admin_pricing_status_current': 'Current',
      'admin_pricing_status_future': 'Future',
      'admin_pricing_status_expired': 'Expired',
      'admin_pricing_status_inactive': 'Inactive',
      'admin_pricing_dialog_create_route': 'Create route',
      'admin_pricing_dialog_edit_route': 'Edit route',
      'admin_pricing_dialog_create_price': 'Create price',
      'admin_pricing_dialog_edit_price': 'Edit price',
      'admin_pricing_dialog_create_policy': 'Create charge policy',
      'admin_pricing_dialog_edit_policy': 'Edit charge policy',
      'admin_pricing_dialog_copy_route': 'Copy route #{id}',
      'admin_pricing_sim_calculate': 'Calculate',
      'admin_pricing_sim_name_sign': 'Name sign',
      'admin_pricing_sim_waiting': 'Waiting',
      'admin_pricing_sim_parking': 'Parking',
      'admin_pricing_sim_toll': 'Toll',
      'admin_pricing_sim_total': 'Total: {amount} {currency}',
      'admin_flight_sync_title': 'Automatic flight sync',
      'admin_flight_running': 'Running',
      'admin_flight_run_sync': 'Run sync cycle',
      'admin_flight_worker_enabled': 'Worker: Enabled',
      'admin_flight_worker_disabled': 'Worker: Disabled',
      'admin_flight_provider_configured': 'Provider: Configured',
      'admin_flight_provider_not_configured': 'Provider: Not configured',
      'admin_flight_last_completed': 'Last completed: {value}',
      'admin_flight_date': 'Date',
      'admin_flight_clear_date': 'Clear date',
      'admin_flight_flight_number': 'Flight number',
      'admin_flight_booking_number': 'Booking number',
      'admin_flight_delayed_only': 'Delayed only',
      'admin_flight_empty': 'No airport pickup flights found',
      'admin_flight_route': 'Route',
      'admin_flight_pickup': 'Pickup',
      'admin_flight_sync_line': 'Sync: {status} · Last: {last}',
      'admin_flight_sync_error': 'Sync error: {message}',
      'admin_flight_delay_minutes': 'Delay {minutes}m',
      'admin_flight_provider_missing': 'Flight provider is not configured',
      'admin_flight_sync_unavailable':
          'Automatic flight sync status unavailable',
      'booking_trip_completed': 'Trip completed',
      'booking_dropoff_qr_title': 'Ride completion QR',
      'booking_dropoff_qr_hint':
          'Show this QR code to your driver again when getting off. Once the driver scans it, your ride will be marked as completed.',
      'booking_dropoff_qr_unavailable':
          'Dropoff QR is available after pickup and before trip completion.',
      'booking_qr_load_failed':
          'We couldn’t load the QR code. Please check the booking status or contact admin.',
      'booking_refresh_dropoff_qr': 'Refresh dropoff QR',
      'booking_issue_new_dropoff_qr': 'Issue new dropoff QR',
      'booking_review_title': 'Rate your trip',
      'booking_review_thanks': 'Thank you for your review',
      'booking_review_rating': 'Rating: {rating} / 5',
      'booking_review_comment': 'Comment (optional)',
      'booking_review_submitting': 'Submitting...',
      'booking_review_submit': 'Submit review',
      'booking_review_load_error': 'Could not load review',
      'review_card_title': 'How was your ride?',
      'review_card_subtitle': 'Please select a rating.',
      'review_star_label': '{value} stars',
      'review_rating_desc_1': 'Very dissatisfied',
      'review_rating_desc_2': 'Dissatisfied',
      'review_rating_desc_3': 'Average',
      'review_rating_desc_4': 'Satisfied',
      'review_rating_desc_5': 'Very satisfied',
      'review_comment_label': 'Leave a comment (optional)',
      'review_comment_hint': 'Do not include personal information.',
      'review_submit_button': 'Submit rating',
      'review_success_title': 'Thank you for your feedback.',
      'review_success_body': 'Your feedback helps us improve our service.',
      'review_tag_FRIENDLY': 'Friendly',
      'review_tag_SAFE_DRIVING': 'Safe driving',
      'review_tag_CLEAN_VEHICLE': 'Clean vehicle',
      'review_tag_ON_TIME': 'On time',
      'review_tag_GOOD_COMMUNICATION': 'Good communication',
      'review_tag_UNSAFE_DRIVING': 'Unsafe driving',
      'review_tag_LATE_ARRIVAL': 'Late arrival',
      'review_tag_VEHICLE_NOT_CLEAN': 'Vehicle was not clean',
      'review_tag_UNFRIENDLY_SERVICE': 'Unfriendly service',
      'review_tag_ROUTE_ISSUE': 'Route issue',
      'review_tag_OTHER_ISSUE': 'Other issue',
      'admin_booking_review_section': 'Customer review',
      'admin_detail_more_reasons': '+{count} more reasons',
      'admin_detail_customer': 'Customer information',
      'admin_contact_status': 'Contact status',
      'admin_contact_channel': 'Contact channel',
      'admin_contact_confirm_requested_hint':
          'The customer confirmed they sent the booking number on messenger. Verify the same booking number in your messenger, then confirm the connection.',
      'admin_contact_verify_button': 'Confirm connection',
      'admin_contact_verify_success': 'Contact connection verified',
      'admin_detail_trip': 'Trip information',
      'admin_detail_driver_vehicle': 'Driver and vehicle',
      'admin_detail_pricing_settlement': 'Fare and settlement',
      'admin_detail_available': 'Available',
      'admin_detail_status_history': 'Status history',
      'admin_detail_technical': 'Technical information',
      'admin_detail_booking_id': 'Internal booking ID',
      'admin_detail_raw_status': 'Raw booking status',
      'admin_detail_service_code': 'Service type code',
      'admin_detail_updated': 'Updated at',
      'admin_detail_created': 'Created at',
      'admin_pickup_requested_at': 'Requested pickup time',
      'admin_detail_country': 'Country',
      'admin_detail_passengers': 'Passengers (adult / child / infant)',
      'admin_detail_luggage': 'Luggage',
      'admin_detail_special_requests': 'Special requests',
      'admin_customer_preference_summary_prefix': 'Customer preferences:',
      'admin_customer_preference_badge': 'Preference',
      'admin_detail_flight': 'Flight',
      'admin_detail_recommend_drivers': 'Recommend drivers',
      'admin_notes_title': 'Internal notes',
      'admin_notes_admin_only': 'Visible to administrators only.',
      'admin_notes_hint': 'Add an operational note',
      'admin_notes_add': 'Add note',
      'admin_notes_empty': 'No internal notes yet.',
      'admin_notes_append_only':
          'Existing notes cannot be edited. Add a new note for corrections.',
      'admin_notes_remaining': '{count} characters remaining',
      'admin_notes_load_failed': 'Could not load internal notes.',
      'admin_notes_add_failed': 'Could not add the internal note.',
      'admin_booking_low_rating_badge': 'Low rating requires review',
      'admin_booking_review_rating': 'Rating',
      'admin_booking_review_tags': 'Tags',
      'admin_booking_review_comment': 'Comment',
      'admin_booking_review_comment_empty': 'No written review was provided.',
      'admin_booking_review_created_at': 'Submitted at',
      'booking_notification_updates': 'Updates',
      'booking_notification_enabling': 'Enabling...',
      'booking_notification_enable': 'Enable notifications',
      'booking_notification_empty': 'No updates yet',
      'booking_notification_default_title': 'Notification',
      'booking_notification_load_error': 'Could not load notifications',
      'booking_notification_enabled': 'Notifications enabled',
      'booking_notification_permission_denied':
          'Notification permission was denied',
      'booking_notification_unsupported':
          'Push notifications are not supported in this browser',
      'booking_notification_config_missing':
          'Push notifications are not configured for this environment',
      'booking_notification_register_failed':
          'Notification registration failed',
      'guest_lookup_trip_completed_qr_hidden':
          'Trip completed. Active QR codes are hidden.',
      'guest_lookup_dropoff_qr_token': 'Dropoff QR token: {token}',
      'guest_lookup_boarding_qr_security':
          'Boarding QR was issued when the booking was created. '
          'For security, it cannot be recovered from lookup.',
      'track_driver_title': 'Track driver',
      'track_driver_unavailable': 'Driver location is not available yet.',
      'track_driver_stale': 'Location may be stale',
      'track_driver_live': 'Location is live',
      'track_driver_updated': 'Updated {time}',
      'track_driver_load_error': 'Could not load driver location',
      'customer_driver_location_title': 'Driver location',
      'customer_driver_location_waiting':
          'Your driver has been assigned. You can see live location after the driver starts moving to pickup.',
      'customer_driver_location_live': 'Driver location sharing',
      'customer_driver_location_updated_label': 'Last update',
      'customer_driver_location_updated_now': 'Updated just now',
      'customer_driver_location_updated_minutes': 'Updated {minutes} min ago',
      'customer_driver_location_temporarily_unavailable':
          'Driver location is temporarily unavailable. Please check again shortly.',
      'customer_driver_location_stale':
          'Driver location has not updated recently.',
      'customer_driver_location_ended': 'Live location sharing has ended.',
      'customer_driver_location_open_map': 'Open in map',
      'customer_driver_location_recenter': 'Move to driver location',
      'customer_driver_on_route':
          'Your driver is on the way to the pickup location.',
      'customer_driver_arrived':
          'Your driver has arrived at the pickup location. Please check the driver and vehicle information.',
      'customer_trip_in_progress': 'Your trip is in progress.',
      'customer_driver_location_load_failed':
          'Could not check driver location. Please try again shortly.',
      'customer_driver_location_retry': 'Try again',
      'wizard_step_progress': 'Step {current} of {total}',
      'pickup_date_past': 'Pickup date cannot be in the past',
      'pickup_time_minimum': 'Pickup time must be later than the current time',
      'pickup_time_am': 'AM',
      'pickup_time_pm': 'PM',
      'pickup_time_select': 'Select pick-up time',
      'pickup_time_enter_manually': 'Enter time manually',
      'pickup_time_invalid': 'Enter a valid time, e.g. 09:30 AM',
      'pickup_time_apply_manual': 'Apply',
      'field_required': '(Required)',
      'name_sign_info_before_price':
          'When you choose the name sign service, an additional fee of ',
      'name_sign_info_after_price':
          ' will apply and the pick-up gate moves to ',
      'name_sign_info_after_gate': '.',
      'name_sign_service_price_highlight': '100 THB',
      'name_sign_service_gate_highlight': 'Gate 3',
      'name_sign_text_label': 'Name to display on the name sign',
      'name_sign_text_hint': 'e.g. JOHN SMITH',
      'wizard_required_name_sign_text':
          'Please enter the name to display on the name sign.',
      'wizard_customer_email_invalid': 'Please enter a valid email address.',
      'pickup_datetime_required': 'Pickup date and time are required',
      'booking_location_reselect_required':
          'Location coordinates are unavailable. Please select the place again.',
      'ui_generic_error': 'Something went wrong. Please try again.',
      'ui_retry': 'Retry',
      'flight_number_hint': 'TG401, 7C2203',
      'flight_number_invalid':
          'Check the flight number. Examples: TG401, 7C2203',
      'flight_lookup_search': 'Look up',
      'flight_lookup_not_found':
          'No flight was found for that number. Please double-check the flight number.',
      'flight_lookup_invalid_date':
          'The pickup date is not valid for flight lookup. You can still continue booking with the flight number entered.',
      'flight_lookup_provider_unavailable':
          'Flight lookup is temporarily unavailable. You can still continue booking with the flight number entered.',
      'flight_lookup_departure': 'Departure',
      'flight_lookup_arrival': 'Arrival',
      'flight_lookup_estimated_arrival': 'Estimated arrival',
      'flight_lookup_confirm': 'This is my flight',
      'booking_number_copied': 'Reservation number copied',
      'booking_number_copy_failed': 'Could not copy reservation number',
      'booking_number_copy': 'Copy reservation number',
      'booking_number_save_notice':
          'Please save your reservation number — you will need it to look up your booking.',
      'wizard_edit_section': 'Edit',
      'wizard_section_incomplete':
          'Please complete this section before continuing.',
      'wizard_completion_progress':
          '{completed} of {total} required fields complete',
      'wizard_required_service': 'Please select a service type.',
      'wizard_step_route_title': 'Route',
      'wizard_step_schedule_title': 'Date & time',
      'wizard_step_vehicle_title': 'Vehicle',
      'wizard_step_customer_title': 'Passenger info',
      'wizard_step_review_title': 'Review booking',
      'wizard_step_indicator': '{current}/{total} {title}',
      'wizard_cta_route': 'Select date and time',
      'wizard_cta_schedule': 'Select vehicle',
      'wizard_cta_vehicle_unselected': 'Select a vehicle',
      'wizard_cta_vehicle_selected': 'Enter passenger info · {price}',
      'wizard_cta_customer': 'Review booking · {price}',
      'wizard_cta_confirm': 'Confirm booking · {price}',
      'wizard_same_place_error': 'Origin and destination cannot be the same.',
      'wizard_swap_route': 'Swap locations',
      'wizard_trust_toll_included': 'Highway tolls included',
      'wizard_trust_no_airport_parking': 'No airport parking fees',
      'wizard_trust_no_night_surcharge':
          'No late-night or early-morning surcharge',
      'wizard_trust_flight_delay_wait':
          'Free waiting if your flight is delayed',
      'wizard_places_searching': 'Searching places…',
      'wizard_places_no_results_hint':
          'No places matched. Try a different search term or pick a location on the map.',
      'wizard_summary_expand': 'Show booking summary',
      'wizard_summary_collapse': 'Hide booking summary',
      'wizard_required_origin': 'Please select an origin.',
      'wizard_required_destination': 'Please select a destination.',
      'wizard_required_passengers': 'Please enter passenger details.',
      'wizard_required_vehicle': 'Please select a vehicle.',
      'wizard_required_customer': 'Please enter your information.',
      'wizard_required_customer_name': 'Please enter your name.',
      'wizard_required_customer_phone': 'Please enter your phone number.',
      'select_on_map': 'Select on map',
      'map_picker_title': 'Select location on map',
      'map_picker_hint': 'Tap the map to place the marker.',
      'map_picker_confirm': 'Use this location',
      'current_location': 'My location',
      'locating_current_position': 'Locating...',
      'location_permission_required':
          'Location permission is required to use your current position.',
      'location_permission_denied': 'Location permission was denied.',
      'location_permission_permanently_denied':
          'Allow location access in your browser or device settings.',
      'location_service_disabled': 'Device location services are turned off.',
      'location_unavailable': 'Your current location is unavailable.',
      'location_timeout': 'Location request timed out. Please try again.',
      'location_requires_https':
          'Location is unavailable in this environment. Please try again from an HTTPS address.',
      'low_location_accuracy':
          'Location accuracy is low. Adjust the marker on the map.',
      'wizard_vehicle_prerequisites':
          'Complete service, origin, destination, pick-up time, and passengers first.',
      'wizard_pricing_after_vehicle':
          'Price is calculated after you select a vehicle.',
      'guest_lookup_booking_number_hint': 'TX202607010001',
    },
    'ko': {
      'app_title': 'T-Rider',
      'app_subtitle': '태국 공항 이동 서비스',
      'airport_pickup': '공항 픽업',
      'airport_pickup_desc': '공항 도착 후 숙소까지 편안하게 모셔다드립니다',
      'airport_dropoff': '공항 샌딩',
      'airport_dropoff_desc': '숙소에서 공항까지 여유롭게 출발하세요',
      'city_transfer': '도시간 이동',
      'city_transfer_desc': '파타야, 방콕 등 도시 간 이동을 편안하게',
      'golf_transfer': '골프 이동',
      'golf_transfer_desc': '골프백까지 넉넉하게 싣고 골프장까지 모셔다드립니다',
      'select_service': '서비스 유형 선택',
      'passengers': '인원',
      'adults': '성인 (12세 이상)',
      'children': '어린이',
      'luggage': '수하물',
      'small_carriers': '20인치 캐리어',
      'large_carriers': '24인치 이상 캐리어',
      'golf_bags': '골프백',
      'special_luggage': '특수 수하물',
      'name_sign': '피켓 서비스 (+100 THB)',
      'booking_prefer_female_driver': '여성 기사 선호',
      'booking_preference_disclaimer': '배정 시 최대한 반영하며, 100% 보장되지 않습니다',
      'next': '다음',
      'back': '뒤로',
      'confirm': '예약 확정',
      'customer_review_booking': '예약 내용 확인하기',
      'customer_confirm_booking': '예약 확정하기',
      'customer_booking_processing': '예약을 처리하고 있습니다',
      'customer_urgent_pickup_hint': '픽업 요청시간이 2시간 이내인 경우 긴급요청을 클릭하셔서 진행 가능합니다',
      'customer_urgent_request': '긴급요청',
      'customer_urgent_confirm_title': '긴급 요청 확인',
      'customer_urgent_confirm_body':
          '긴급 요청의 경우 배차에 시간이 다소 소요될 수 있습니다. 배차가 완료되면 기사님의 도착시간을 먼저 안내 해드리오니 배차를 원하시면 확정 버튼을 눌러주시기 바랍니다.',
      'customer_urgent_confirm_submit': '확정',
      'customer_urgent_flow_title': '긴급 요청',
      'customer_urgent_searching_title': '기사를 찾고 있습니다',
      'customer_urgent_searching_body': '주변에서 배차 가능한 기사를 찾고 있습니다.',
      'customer_urgent_eta_title': '기사님이 약 {minutes}분 후 도착 예정입니다',
      'customer_urgent_eta_body': '도착 시간이 괜찮으시면 수락을, 더 빠른 기사를 원하시면 거절을 눌러주세요.',
      'customer_urgent_accept_eta': '수락',
      'customer_urgent_reject_eta': '거절',
      'customer_urgent_retry_title': '더 빠른 기사를 찾아볼까요?',
      'customer_urgent_retry_body': '다른 기사를 계속 찾아볼 수 있습니다.',
      'customer_urgent_retry_accept': '수락',
      'customer_urgent_retry_cancel': '취소',
      'customer_urgent_confirmed_title': '배차가 확정되었습니다',
      'customer_urgent_confirmed_body': '기사님이 배정되었습니다. 예약 상세 화면으로 이동합니다.',
      'customer_urgent_exhausted_title': '대기 중인 기사가 없습니다',
      'customer_urgent_exhausted_body': '가까운 곳에 대기중인 기사가 없습니다. 예약을 취소하겠습니다',
      'customer_urgent_go_home': '홈으로',
      'origin': '출발지',
      'destination': '도착지',
      'airport': '공항',
      'selected_location': '선택한 장소',
      'flight_number': '항공편 번호',
      'date': '날짜',
      'time': '시간',
      'pickup_datetime': '픽업 날짜·시간',
      'pickup_date': '픽업 날짜',
      'pickup_time': '픽업 시간',
      'pickup_minimum_notice': '현재 시각 기준 최소 2시간 이후의 태국 현지 시간을 선택해 주세요.',
      'select_vehicle': '차량 선택',
      'recommended': '추천',
      'total': '총 금액',
      'special_requests': '추가 요청사항',
      'customer_info': '고객 정보',
      'name': '이름',
      'email': '이메일',
      'phone': '전화번호',
      'country': '국가',
      'booking_complete': '예약 완료',
      'reservation_number': '예약번호',
      'booking_number_label': '예약번호',
      'booking_with_number': '예약 {bookingNumber}',
      'booking_detail_view': '예약 상세 보기',
      'notification_type_booking': '예약',
      'notification_type_pickup': '픽업',
      'notification_type_settlement': '정산',
      'notification_type_system': '시스템',
      'status': '상태',
      'golf_region': '골프 지역',
      'golf_course': '골프장',
      'driver_included': '기사 포함',
      'status_pending': '대기',
      'status_confirmed': '확정',
      'status_driver_assigned': '기사 배정',
      'status_completed': '완료',
      'status_cancelled': '취소',
      'status_on_route': '픽업 이동 중',
      'status_driver_arrived': '픽업지 도착',
      'status_no_show': '노쇼',
      'status_picked_up': '고객 탑승 · 운행 중',
      'status_customer_settlement_pending': '운행 종료',
      'status_driver_settlement_pending': '운행 종료 · 정산 대기',
      'status_admin_settlement_pending': '운행 종료 · 정산 대기',
      'status_settlement_pending': '운행 종료 · 정산 대기',
      'search_place': '장소 검색...',
      'no_results': '검색 결과 없음',
      'infants': '유아',
      'multiple_vehicles_required': '복수 차량 필요',
      'base_price': '기본 요금',
      'additional_charges': '추가 요금',
      'pricing_summary': '요금 요약',
      'pricing_name_sign_with_picket': '피켓 서비스',
      'vehicle_not_available': '이 운행에 이용 불가',
      'pricing_inquiry_required': '문의',
      'complete_passengers_first': '먼저 승객 정보를 입력하세요',
      'yes': '예',
      'change_location': '변경',
      'loading_place_details': '장소 정보 불러오는 중...',
      'airport_shortcuts': '공항 바로가기',
      'airport_shortcuts_origin': '공항 바로가기',
      'airport_shortcuts_destination': '공항 바로가기',
      'recent_non_airport_places': '최근 지역',
      'recent_locations': '최근 장소',
      'landing_hero_eyebrow': '태국 전문 공항·도시 이동 서비스',
      'landing_hero_title': '태국에서 만나는 편안한 출발 T-Rider',
      'landing_hero_body': '숙련된 기사와 함께 공항에서 목적지까지 안전하고 편안하게 이동하세요.',
      'landing_hero_cta': '예약하기',
      'landing_support_cta': '고객센터',
      'landing_support_one_click_cta': '고객센터를 통한 원클릭 예약',
      'landing_hero_helper': '회원가입 없이 간편 예약',
      'landing_hero_subtitle': '즐겁고 안전한 여행을 약속드립니다.',
      'landing_highlight_fixed_price': '정액 요금',
      'landing_highlight_fixed_price_desc': '예약 시 확인한 요금 그대로, 추가 비용 걱정 없이',
      'landing_highlight_flight': '항공편 추적',
      'landing_highlight_flight_desc': '항공 지연 시 픽업 시간을 자동으로 조정합니다',
      'landing_highlight_support': '24시간 지원',
      'landing_highlight_support_desc': '예약 전후 언제든 도움을 받을 수 있습니다',
      'book_your_ride': '예약하기',
      'landing_services_title': '서비스',
      'landing_services_subtitle': '공항·도시 이동 서비스',
      'service_badge_popular': '가장 많이 이용',
      'landing_trust_title': '안심하고 이용하세요',
      'landing_trust_drivers_title': '숙련된 기사',
      'landing_trust_drivers_desc': '태국 도로와 공항 이동에 익숙한 기사가 함께합니다.',
      'landing_trust_confirmed_title': '예약 사전 확인',
      'landing_trust_confirmed_desc': '픽업 정보와 이동 일정을 미리 확인합니다.',
      'landing_trust_comfort_title': '편안한 여정',
      'landing_trust_comfort_desc': '복잡한 이동 걱정 없이 여행에 집중하세요.',
      'landing_trust_refund_title': '안심 취소·환불',
      'landing_trust_refund_desc':
          '예약 확정 후 24시간 이내 위약금 없이 무료 취소 가능합니다. 동일 조건으로 더 저렴한 확정 견적을 발견하시면 예약 후 48시간 이내 문의 시 차액을 환불해드립니다.',
      'landing_trust_driver_policy_title': '기사 품질 관리',
      'landing_trust_driver_policy_desc':
          '기사 관련 불편사항은 예약 상세 화면 또는 카카오톡/전화로 신고 가능하며, 접수 후 24시간 이내 확인 연락드립니다. 안전 위협 등 중대 사안은 즉시 배차를 정지합니다.',
      'landing_how_it_works_title': '이용 방법',
      'landing_how_step_1_title': '이동 정보 입력',
      'landing_how_step_1_desc': '서비스, 픽업 시간, 목적지를 선택하세요.',
      'landing_how_step_2_title': '차량과 요금 확인',
      'landing_how_step_2_desc': '추천 차량과 요금을 확인한 뒤 예약을 완료하세요.',
      'landing_how_step_3_title': '기사와 편안하게 이동',
      'landing_how_step_3_desc': '기사가 목적지까지 안전하게 모셔다 드립니다.',
      'landing_reassurance_body':
          '처음 방문하는 태국이어도 걱정하지 마세요. 예약부터 도착까지 필요한 정보를 차분하게 안내해드립니다.',
      'landing_booking_lookup_title': '이미 예약하셨나요?',
      'landing_booking_lookup_desc': '예약번호로 일정과 기사 정보를 확인하세요.',
      'landing_booking_lookup_action': '예약 조회',
      'landing_footer_note': '태국 전역 공항·도시 이동 전문 서비스',
      'landing_login_title': '로그인',
      'landing_login_sheet_title': 'T-Rider 로그인',
      'landing_login_prompt_cta': '로그인하고 마일리지 적립받기',
      'landing_login_sheet_close': '닫기',
      'landing_login_support_prompt': '로그인에 문제가 있으신가요?',
      'landing_login_support_link': '고객센터 문의',
      'landing_header_my_bookings_menu': '내 예약 보기',
      'landing_header_logout_menu': '로그아웃',
      'landing_header_logout_success': '로그아웃되었습니다',
      'landing_logged_in_greeting': '{name}님, 로그인되었습니다',
      'landing_my_bookings_button': '내 예약 보기',
      'landing_logout_button': '로그아웃',
      'auth_login_provider_hint':
          '마일리지 적립 내역 관리를 위해, 항상 같은 로그인 방법(구글/카카오/LINE 중 하나)을 이용해주세요',
      'landing_header_account_menu': '내 정보',
      'account_page_title': '내 정보',
      'account_menu_section_title': '계정',
      'account_mileage_label': '적립 마일리지',
      'account_mileage_balance': '{balance}P',
      'account_mileage_loading': '잔액 불러오는 중...',
      'account_mileage_load_failed': '불러오기 실패',
      'account_mileage_retry': '다시 시도',
      'account_my_bookings_menu': '내 예약',
      'account_logout_menu': '로그아웃',
      'account_logout_cancel': '취소',
      'account_logout_confirm_message': '로그아웃하시겠습니까?',
      'account_login_provider_google': 'Google로 로그인',
      'account_login_provider_kakao': '카카오로 로그인',
      'account_login_provider_line': 'LINE으로 로그인',
      'account_sign_in_required': '내 정보를 보려면 로그인해 주세요.',
      'account_booking_counts_title': '예약 내역',
      'account_booking_count_waiting': '예약대기',
      'account_booking_count_assigned': '배차완료',
      'account_booking_count_in_progress': '이용중',
      'account_booking_count_settlement_pending': '정산대기',
      'account_booking_count_completed': '이용완료',
      'account_booking_count_review_pending': '리뷰대기',
      'account_coupon_menu': '쿠폰',
      'account_coupons_title': '쿠폰',
      'account_coupons_available_section': '사용 가능한 쿠폰',
      'account_coupons_used_section': '사용한 쿠폰',
      'account_coupons_empty_available': '사용 가능한 쿠폰이 없습니다',
      'account_coupons_empty_used': '사용한 쿠폰이 없습니다',
      'account_coupons_load_failed': '쿠폰을 불러오지 못했습니다',
      'account_coupons_issued_at': '발급일',
      'account_coupons_used_at': '사용일',
      'account_coupons_used_booking': '예약번호',
      'booking_coupon_section_title': '쿠폰 사용',
      'booking_coupon_none': '쿠폰 사용 안 함',
      'booking_coupon_discount_label': '쿠폰 할인',
      'booking_mileage_section_title': '마일리지 사용',
      'booking_mileage_discount_label': '마일리지 할인',
      'booking_mileage_available': '보유 마일리지: {balance}점',
      'booking_mileage_use_all': '전액 사용',
      'admin_coupons_menu': '쿠폰 지급',
      'admin_coupon_issue_title': '쿠폰 지급',
      'admin_coupon_search_hint': '전화번호, 이메일, 이름으로 검색',
      'admin_coupon_search_empty': '검색 결과가 없습니다',
      'admin_coupon_select_customer': '고객 선택',
      'admin_coupon_title_label': '쿠폰 제목',
      'admin_coupon_amount_label': '할인 금액',
      'admin_coupon_issue_button': '쿠폰 지급',
      'admin_coupon_issue_success': '쿠폰이 지급되었습니다',
      'admin_coupon_issue_failed': '쿠폰 지급에 실패했습니다',
      'admin_coupon_recent_title': '최근 지급 쿠폰',
      'admin_coupon_recent_empty': '지급된 쿠폰이 없습니다',
      'admin_coupon_cancel': '취소',
      'admin_coupon_cancel_success': '쿠폰이 취소되었습니다',
      'admin_coupon_status_available': '사용 가능',
      'admin_coupon_status_used': '사용됨',
      'admin_coupon_recent_customers_title': '최근 이용 고객',
      'admin_coupon_recent_customers_empty': '완료된 예약이 없습니다',
      'admin_coupon_selected_customer': '선택된 고객',
      'admin_coupon_template_picker_title': '쿠폰 템플릿',
      'admin_coupon_template_manual': '수동 입력',
      'admin_coupon_template_manage_title': '템플릿 관리',
      'admin_coupon_template_create_button': '템플릿 추가',
      'admin_coupon_template_create_success': '템플릿이 등록되었습니다',
      'admin_coupon_template_create_failed': '템플릿 등록에 실패했습니다',
      'admin_coupon_template_active': '활성',
      'admin_coupon_template_inactive': '비활성',
      'admin_home_banner_menu': '홈 배너 관리',
      'admin_manual_booking_menu': '관리자 콜 등록',
      'admin_manual_booking_title': '관리자 수동 콜 등록',
      'admin_manual_booking_origin': '출발지',
      'admin_manual_booking_destination': '도착지',
      'admin_manual_booking_pickup_datetime': '픽업 일시',
      'admin_manual_booking_pickup_datetime_hint': '픽업 날짜와 시간을 선택하세요',
      'admin_manual_booking_pickup_timezone': '태국 현지 시간 (UTC+7)',
      'admin_manual_booking_special_items_text': '특수 수하물 설명',
      'admin_manual_booking_special_items_hint': '대형·특수 수하물을 입력하세요',
      'admin_manual_booking_flight_pickup_date_required':
          '항공편 조회 전 픽업 날짜를 선택하세요',
      'admin_manual_booking_flight_manual_fallback_hint':
          '항공편 조회가 실패해도 항공편 번호와 픽업 시간을 직접 입력해 진행할 수 있습니다.',
      'admin_manual_booking_flight_pickup_conflict_warning':
          '입력하신 픽업 시간과 항공편 도착 예정 시간이 {minutes}분 차이납니다.',
      'admin_manual_booking_flight_pickup_keep_manual': '직접 입력한 시간 사용',
      'admin_manual_booking_flight_pickup_use_flight': '항공편 시간 기준으로 변경',
      'admin_manual_booking_flight_pickup_final_label': '최종 픽업 요청시간',
      'admin_dispatch_driver_pickup_conflict':
          '다른 배정 콜과 픽업 시간이 충돌합니다',
      'admin_manual_booking_vehicle_type': '차량 등급',
      'admin_manual_booking_passengers': '탑승 인원',
      'admin_manual_booking_name_sign_hint':
          '기사 앱에서 피켓(네임사인) 필요 콜로 표시됩니다.',
      'admin_manual_booking_payout_amount': '기사 지급 금액 (THB)',
      'admin_manual_booking_customer_charge_amount': '고객 결제 금액 (THB)',
      'admin_manual_booking_customer_charge_hint':
          '관리자 기록용 선택 항목입니다. 고객이 실제로 낸/낼 금액을 입력하세요.',
      'admin_manual_booking_payment_method': '결제 방식',
      'admin_manual_booking_payment_admin_collected': '관리자가 이미 결제 받음',
      'admin_manual_booking_payment_driver_collects': '현장에서 기사가 직접 수금',
      'admin_manual_booking_customer_section': '고객 정보',
      'admin_manual_booking_guest_name': '게스트 이름',
      'admin_manual_booking_guest_phone': '게스트 전화번호',
      'admin_manual_booking_guest_email': '게스트 이메일 (선택)',
      'admin_manual_booking_clear_customer': '선택 고객 해제',
      'admin_manual_booking_memo': '메모 (선택)',
      'admin_manual_booking_submit': '오픈콜 생성',
      'admin_manual_booking_success': '오픈콜이 생성되었습니다. 예약번호:',
      'admin_manual_booking_view_detail': '예약 상세',
      'admin_manual_booking_assign_driver': '기사 배정',
      'admin_manual_booking_edit': '수정',
      'admin_manual_booking_edit_title': '관리자 콜 수정',
      'admin_manual_booking_edit_not_allowed': '현재 상태에서는 수정할 수 없습니다',
      'admin_manual_booking_cancel': '콜 취소',
      'admin_manual_booking_cancel_confirm':
          '관리자 등록 콜을 취소할까요? 배정된 기사는 해제됩니다.',
      'admin_manual_booking_cancel_success': '관리자 콜이 취소되었습니다',
      'admin_manual_booking_assign_success': '기사가 배정되었습니다',
      'admin_manual_booking_update_submit': '변경 저장',
      'admin_manual_booking_update_success': '관리자 콜이 수정되었습니다',
      'admin_manual_booking_failed': '관리자 콜 생성에 실패했습니다',
      'admin_manual_booking_validation_required': '필수 항목을 모두 입력해주세요',
      'admin_manual_booking_validation_customer':
          '회원을 선택하거나 게스트 이름과 전화번호를 입력해주세요',
      'admin_manual_booking_preview_notice':
          '기사 화면 미리보기: 계좌 확인 안내 문구가 표시됩니다',
      'driver_call_badge_admin_manual': '관리자 등록 콜',
      'driver_call_badge_no_commission': '커미션 없음',
      'driver_call_bank_account_confirm_notice':
          '회원가입시 등록된 계좌로 지급될 예정이오니 계좌가 맞는지 한번 더 확인해주세요',
      'admin_home_banner_title': '홈 배너 관리',
      'admin_home_banner_order_label': '표시 순서',
      'admin_home_banner_order_hint': '비워두면 맨 뒤에 추가됩니다',
      'admin_home_banner_upload_button': '배너 업로드',
      'admin_home_banner_create_success': '배너가 업로드되었습니다',
      'admin_home_banner_create_failed': '배너 업로드에 실패했습니다',
      'admin_home_banner_empty': '등록된 배너가 없습니다',
      'admin_home_banner_active': '활성',
      'admin_home_banner_inactive': '비활성',
      'admin_home_banner_delete': '삭제',
      'admin_home_banner_delete_success': '배너가 삭제되었습니다',
      'admin_home_banner_move_up': '위로',
      'admin_home_banner_move_down': '아래로',
      'account_feature_coming_soon': '준비 중입니다',
      'account_recent_bookings_title': '최근 이용 내역',
      'account_recent_bookings_empty': '최근 이용 내역이 없습니다',
      'account_recent_bookings_load_failed': '최근 이용 내역을 불러오지 못했습니다',
      'account_my_reviews_menu': '내가 쓴 리뷰',
      'account_customer_support_menu': '고객센터/문의',
      'account_mileage_history_title': '마일리지 적립 내역',
      'account_mileage_history_empty': '적립 내역이 없습니다',
      'account_mileage_history_load_failed': '적립 내역을 불러오지 못했습니다',
      'account_my_reviews_title': '내가 쓴 리뷰',
      'account_my_reviews_empty': '작성한 리뷰가 없습니다',
      'account_my_reviews_load_failed': '리뷰 목록을 불러오지 못했습니다',
      'account_mileage_type_accrue': '적립',
      'account_mileage_type_reversal': '회수',
      'landing_privacy_policy_link': '개인정보처리방침',
      'privacy_policy_title': '개인정보처리방침',
      'privacy_policy_load_failed': '개인정보처리방침을 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.',
      'analytics_consent_title': '쿠키 사용 안내',
      'analytics_consent_body': '서비스 개선을 위해 분석 쿠키를 사용합니다. 거부해도 예약할 수 있습니다.',
      'analytics_consent_allow': '쿠키 허용',
      'analytics_consent_deny': '거부',
      'analytics_consent_privacy': '개인정보처리방침',
      'analytics_consent_settings': '분석 설정',
      'analytics_consent_settings_title': '분석 동의 설정',
      'landing_language_label': '언어',
      'support_title': '고객센터',
      'support_contact_channels_title': '고객센터 연결',
      'support_contact_channels_hint':
          '원하시는 메신저 아이콘을 눌러 바로 상담을 시작하세요',
      'support_wechat_qr_dialog_title': '위챗 QR 코드',
      'support_wechat_qr_dialog_hint':
          '위챗 앱에서 QR 코드를 스캔해 친구로 추가해 주세요',
      'support_page_intro':
          '예약이 어렵거나 문의가 필요하신 경우 고객센터로 문의해 주세요. 항공권, 호텔 바우처, 예약 캡처 이미지도 첨부할 수 있습니다.',
      'support_inquiry_button': '문의하기',
      'support_dialog_title': '고객센터 문의',
      'support_close_button': '닫기',
      'support_inquiry_title': '고객센터를 통한 예약 및 문의',
      'support_inquiry_subtitle': '예약 문의, 항공편 정보, 픽업 장소, 목적지를 남겨주세요.',
      'support_default_guide':
          '안녕하세요. T-Ride 고객센터입니다. 예약 문의, 항공편 정보, 픽업 장소, 목적지를 남겨주시면 확인 후 안내드리겠습니다.',
      'support_input_label': '문의 내용',
      'support_input_hint': '예약 또는 이동 문의를 입력해 주세요.',
      'support_attach_button': '이미지 첨부',
      'support_send_button': '보내기',
      'support_sending': '전송 중...',
      'support_receipt_number': '접수번호',
      'support_submit_failed': '문의 접수에 실패했습니다. 잠시 후 다시 시도해 주세요.',
      'support_attachment_help': '항공권 이미지, 호텔 바우처, 예약 화면 캡처 등을 첨부할 수 있습니다.',
      'support_no_attachments': '첨부된 파일 없음',
      'support_contact_help':
          '상담원 문의의 경우 답변 확인을 위해 연락 가능한 정보를 남겨주세요. 최소 1개 연락수단 입력을 권장합니다.',
      'support_contact_messenger_help':
          '카카오톡 ID 또는 LINE ID를 남겨주시면 고객센터에서 연락드릴 수 있습니다.',
      'support_contact_name': '이름 또는 닉네임',
      'support_contact_phone': '휴대폰 번호',
      'support_contact_kakao': '카카오톡 ID',
      'support_contact_line': 'LINE ID',
      'support_thread_loading': '최근 문의 내역을 불러오는 중입니다...',
      'support_thread_load_failed': '최근 문의 내역을 불러오지 못했습니다. 다시 시도해 주세요.',
      'support_auto_receipt': '자동 접수 안내: 문의가 접수되었습니다. 고객센터에서 확인 후 안내드리겠습니다.',
      'support_faq_title': '자주 하는 질문',
      'support_faq_placeholder':
          '자주 하는 질문은 고객 문의 데이터가 축적되면 우선순위에 따라 업데이트될 예정입니다.',
      'admin_support_menu': '고객센터 문의',
      'admin_support_search': '문의 검색',
      'admin_support_all_statuses': '전체 상태',
      'admin_support_empty': '고객센터 문의가 없습니다',
      'admin_support_detail': '고객센터 문의 상세',
      'admin_support_customer': '고객 정보',
      'admin_support_customer_name': '고객명',
      'admin_support_customer_phone': '연락처',
      'admin_support_customer_kakao': '카카오톡 ID',
      'admin_support_customer_line': 'LINE ID',
      'admin_support_messages': '메시지',
      'admin_support_reply_title': '답변 작성',
      'admin_support_reply_label': '답변',
      'admin_support_reply_hint': '고객에게 보낼 답변을 입력해 주세요.',
      'admin_support_send_reply': '답변 보내기',
      'admin_support_reply_failed': '답변 전송에 실패했습니다. 다시 시도해 주세요.',
      'admin_support_attachments': '첨부',
      'admin_support_no_attachments': '첨부 파일 없음',
      'admin_support_preview': '미리보기',
      'admin_support_download': '다운로드',
      'admin_support_preview_failed': '첨부 파일을 미리 볼 수 없습니다.',
      'admin_support_download_failed': '첨부 파일을 다운로드하지 못했습니다.',
      'admin_support_download_started': '다운로드를 시작했습니다.',
      'admin_support_status_new': '신규',
      'admin_support_status_in_progress': '처리중',
      'admin_support_status_resolved': '해결',
      'admin_support_status_closed': '종료',
      'trust_flight_delay': '항공 지연 모니터링',
      'trust_flight_delay_desc': '항공 지연 발생 시 픽업 시간을 자동으로 맞춰드립니다',
      'trust_fixed_pricing': '정액 요금',
      'trust_fixed_pricing_desc': '예약 시 안내된 요금이 최종 요금입니다',
      'trust_professional_drivers': '전문 기사',
      'trust_professional_drivers_desc': '검증된 전문 기사가 안전하게 모셔드립니다',
      'trust_multilingual': '다국어 지원',
      'trust_multilingual_desc': '한국어, 영어, 중국어, 일본어, 태국어로 이용하세요',
      'payment_method': '결제 방법',
      'pay_driver_at_destination': '목적지에서 기사에게 직접 결제',
      'customer_payment_method': '결제 방법',
      'customer_payment_pay_driver': '목적지 도착 후 기사에게 현장 결제',
      'customer_payment_cash': '현금 결제',
      'customer_payment_qr': 'QR 결제',
      'customer_payment_card': '카드 결제',
      'customer_payment_bank_transfer': '계좌이체',
      'customer_payment_unknown': '결제 방법 확인 필요',
      'customer_price_conditions':
          '통행료, 공항 주차비 및 추가 대기요금은 예약 조건에 따라 달라질 수 있습니다.',
      'customer_luggage_guidance':
          '대형 캐리어와 골프백 수량을 정확히 입력해 주세요. 최종 수용 가능 여부는 배정 차량에 따라 달라질 수 있습니다.',
      'customer_vehicle_guidance': '승객 또는 수하물이 많으면 추천 차량보다 작은 차량은 선택할 수 없습니다.',
      'customer_vehicle_not_suitable': '선택한 승객 또는 수하물 조건에 적합하지 않습니다.',
      'customer_confirmation_title': '예약 최종 확인',
      'customer_confirmation_subtitle': '예약을 확정하기 전에 내용을 확인해 주세요.',
      'customer_confirmation_edit': '수정',
      'customer_confirmation_trip': '여정 정보',
      'customer_confirmation_passengers': '승객 및 수하물',
      'customer_confirmation_vehicle_payment': '차량 및 결제',
      'customer_confirmation_customer': '고객 정보',
      'customer_confirmation_required': '예약을 확정하기 전에 내용을 확인해 주세요.',
      'customer_next_action': '다음 안내',
      'customer_driver_assignment': '기사 배정',
      'customer_driver_pending': '기사 배정 중',
      'customer_status_unknown_guidance': '최신 예약 상태를 확인해 주세요.',
      'customer_rate_driver': '기사 평가하기',
      'customer_review_thanks': '평가를 남겨주셔서 감사합니다.',
      'messenger_type': '메신저 종류',
      'messenger_id': '메신저 ID',
      'messenger_type_hint': 'LINE, WhatsApp 등',
      'customer_contact_sns_notice':
          '예약 완료후 차량과 기사 등의 정보 안내를 위해 SNS로 미리 연락드립니다. 연락이 안되는 경우 예약이 취소될 수 있으니 필히 SNS로 친구추가가 가능한 연락처나 SNS 계정 정보를 기재해 주시기 바랍니다.',
      'contact_connect_title': '메신저 연결',
      'contact_connect_booking_number': '예약번호',
      'contact_connect_intro':
          'T-Rider 예약이 생성되었습니다. 기사·차량 안내를 받으려면 메신저로 연결해 주세요.',
      'contact_connect_choose_channel': '메신저 선택',
      'contact_connect_email_soon': '이메일 (준비 중)',
      'contact_connect_after_launch': '예약번호가 포함된 메시지를 보낸 뒤 아래 버튼을 눌러 주세요.',
      'contact_connect_confirm_sent': '메시지를 보냈습니다',
      'contact_connect_waiting': 'T-Rider에서 메시지 확인 중입니다',
      'contact_connect_waiting_hint': '보통 몇 분 정도 걸립니다. 이 화면을 유지해 주세요.',
      'contact_connect_step_created': '예약 생성',
      'contact_connect_step_connecting': '연결 중',
      'contact_connect_step_complete': '완료',
      'contact_connect_ref_copied': '예약 참조 문구가 복사되었습니다',
      'contact_connect_launch_failed': '메신저 앱을 열 수 없습니다',
      'contact_connect_unsafe_url': '허용되지 않은 메신저 링크입니다',
      'contact_connect_load_failed': '연결 상태를 불러오지 못했습니다',
      'contact_connect_wechat_title': 'WeChat',
      'contact_connect_wechat_body':
          'QR 코드를 스캔하거나 WeChat ID를 복사한 뒤 예약 참조 문구를 보내 주세요.',
      'contact_connect_wechat_id_copied': 'WeChat ID가 복사되었습니다',
      'contact_connect_invalid_link': '연결 링크가 올바르지 않습니다',
      'additional_requests': '추가 요청',
      'country_code_hint': 'TH, US, KR…',
      'country_search_hint': '국가 검색 또는 직접 입력',
      'boarding_qr_title': '탑승 QR',
      'boarding_qr_hint':
          '탑승 시 QR 코드를 기사님에게 보여주세요. 기사님이 QR 코드를 스캔하면 탑승 완료로 상태가 변경됩니다.',
      'booking_trust_message':
          '예약번호를 보관해 주세요. 기사 배정과 운행 상태는 예약 조회 화면에서 확인할 수 있습니다.',
      'airport_meeting_semantics': 'BKK 공항 미팅 안내, {gate}번 게이트',
      'airport_meeting_gate_3_title': '네임사인 미팅 안내 · 3번 게이트',
      'airport_meeting_gate_7_title': '차량 픽업 안내 · 7번 게이트',
      'airport_meeting_gate_inside': '게이트 안쪽',
      'airport_meeting_gate_outside': '게이트 밖',
      'airport_meeting_no_picket_highlight': '수하물을 찾은 직후, 먼저 픽업 알림을 보내주세요.',
      'airport_meeting_picket_highlight': '공항 밖으로 나가지 말고 3번 게이트 안쪽에서 기다려 주세요.',
      'airport_meeting_no_picket_body_1':
          '기사가 7번 게이트 차량 픽업 구역으로 이동하는 데 시간이 필요합니다.',
      'airport_meeting_no_picket_body_2': '전달받은 차량번호와 기사를 확인한 뒤 탑승해 주세요.',
      'airport_meeting_picket_body_1':
          '수하물을 찾은 뒤 3번 게이트 안쪽으로 이동해 고객님의 이름이 적힌 네임사인을 찾아주세요.',
      'airport_meeting_picket_body_2': '직원이 기사와 차량을 3번 게이트 앞으로 안내합니다.',
      'airport_meeting_step_luggage': '수하물을 모두 찾습니다.',
      'airport_meeting_step_notify_driver': '기사에게 픽업 알림을 보냅니다.',
      'airport_meeting_step_vehicle_number': '전달받은 차량번호를 확인합니다.',
      'airport_meeting_step_gate_7': '입국층 7번 게이트 밖으로 이동합니다.',
      'airport_meeting_step_board': '차량과 기사를 확인한 후 탑승합니다.',
      'airport_meeting_step_gate_3': '3번 게이트 안쪽으로 이동합니다.',
      'airport_meeting_step_find_picket': '고객님의 이름이 적힌 네임사인을 찾습니다.',
      'airport_meeting_step_wait_staff': '직원의 안내를 받으며 잠시 기다립니다.',
      'airport_meeting_step_board_after_staff': '차량 도착 안내를 받은 뒤 밖으로 나가 탑승합니다.',
      'airport_meeting_do_not_exit': '직원 안내 전에는 공항 밖으로 나가지 마세요.',
      'airport_meeting_vehicle_pending':
          '기사와 차량 정보는 아직 배정 대기 중입니다. 배정 후 이곳에 표시됩니다.',
      'airport_meeting_vehicle_info': '기사 및 차량 정보',
      'airport_meeting_driver': '기사',
      'airport_meeting_driver_phone': '기사 전화',
      'airport_meeting_vehicle_type': '차량 종류',
      'airport_meeting_vehicle_color': '차량 색상',
      'airport_meeting_vehicle_plate': '차량번호',
      'wizard_step_progress': '단계 {current} / {total}',
      'pickup_date_past': '픽업 날짜는 과거일 수 없습니다',
      'pickup_time_minimum': '픽업 시간은 현재 시각 이후여야 합니다',
      'pickup_time_am': '오전',
      'pickup_time_pm': '오후',
      'pickup_time_select': '픽업 시간 선택',
      'pickup_time_enter_manually': '시간 직접 입력',
      'pickup_time_invalid': '올바른 시간을 입력해 주세요. 예: 09:30 AM',
      'pickup_time_apply_manual': '적용',
      'field_required': '(필수)',
      'name_sign_info_before_price': '피켓 서비스를 선택하시면 이용 요금이 ',
      'name_sign_info_after_price': ' 추가되며, 픽업 게이트 위치가 ',
      'name_sign_info_after_gate': '로 변경됩니다.',
      'name_sign_service_price_highlight': '100바트',
      'name_sign_service_gate_highlight': '3번 게이트',
      'name_sign_text_label': '피켓에 표시할 이름',
      'name_sign_text_hint': '예: KIM MINSU',
      'wizard_required_name_sign_text': '피켓에 표시할 이름을 입력해 주세요.',
      'wizard_customer_email_invalid': '올바른 이메일 주소를 입력해 주세요.',
      'pickup_datetime_required': '픽업 날짜와 시간을 입력해 주세요',
      'booking_location_reselect_required':
          '장소 좌표를 확인할 수 없습니다. 출발지/도착지를 다시 선택해 주세요.',
      'ui_generic_error': '문제가 발생했습니다. 다시 시도해 주세요.',
      'ui_retry': '다시 시도',
      'flight_number_hint': 'TG401, 7C2203',
      'flight_number_invalid': '항공편 번호를 확인해 주세요. 예: TG401, 7C2203',
      'flight_lookup_search': '조회',
      'flight_lookup_not_found': '해당 편명의 항공편을 찾을 수 없습니다. 편명을 다시 확인해 주세요.',
      'flight_lookup_invalid_date':
          '픽업 날짜로 항공편 조회를 할 수 없습니다. 편명은 그대로 입력하셔도 예약 진행에는 문제없습니다.',
      'flight_lookup_provider_unavailable':
          '항공편 조회 서비스가 일시적으로 원활하지 않습니다. 편명은 그대로 입력하셔도 예약 진행에는 문제없습니다.',
      'flight_lookup_departure': '출발',
      'flight_lookup_arrival': '도착',
      'flight_lookup_estimated_arrival': '예상 도착',
      'flight_lookup_confirm': '이 항공편이 맞습니다',
      'booking_number_copied': '예약번호가 복사되었습니다',
      'booking_number_copy_failed': '예약번호를 복사하지 못했습니다',
      'booking_number_copy': '예약번호 복사',
      'booking_number_save_notice': '예약번호는 예약 조회 시 필요하오니 반드시 저장해두시기 바랍니다.',
      'wizard_edit_section': '수정',
      'wizard_section_incomplete': '계속하기 전에 이 단계를 완료해 주세요.',
      'wizard_completion_progress': '필수 입력 {completed}/{total} 완료',
      'wizard_required_service': '서비스 유형을 선택해 주세요.',
      'wizard_step_route_title': '이동 경로',
      'wizard_step_schedule_title': '날짜·시간',
      'wizard_step_vehicle_title': '차량 선택',
      'wizard_step_customer_title': '승객 정보',
      'wizard_step_review_title': '예약 확인',
      'wizard_step_indicator': '{current}/{total} {title}',
      'wizard_cta_route': '날짜와 시간 선택',
      'wizard_cta_schedule': '차량 선택',
      'wizard_cta_vehicle_unselected': '차량을 선택해 주세요',
      'wizard_cta_vehicle_selected': '승객 정보 입력 · {price}',
      'wizard_cta_customer': '예약 내용 확인 · {price}',
      'wizard_cta_confirm': '예약 확정 · {price}',
      'wizard_same_place_error': '출발지와 도착지가 같을 수 없습니다.',
      'wizard_swap_route': '출발지/도착지 교환',
      'wizard_trust_toll_included': '고속도로 통행료 포함',
      'wizard_trust_no_airport_parking': '공항 주차비 없음',
      'wizard_trust_no_night_surcharge': '심야·새벽 추가요금 없음',
      'wizard_trust_flight_delay_wait': '항공편 지연 시 무료 대기',
      'wizard_places_searching': '장소 검색 중…',
      'wizard_places_no_results_hint':
          '검색 결과가 없습니다. 다른 검색어를 입력하거나 지도에서 선택해 주세요.',
      'wizard_summary_expand': '예약 요약 펼치기',
      'wizard_summary_collapse': '예약 요약 접기',
      'wizard_required_origin': '출발지를 선택해 주세요.',
      'wizard_required_destination': '도착지를 선택해 주세요.',
      'wizard_required_passengers': '승객 정보를 입력해 주세요.',
      'wizard_required_vehicle': '차량을 선택해 주세요.',
      'wizard_required_customer': '고객님의 정보를 입력해 주세요.',
      'wizard_required_customer_name': '고객님의 이름을 입력해 주세요.',
      'wizard_required_customer_phone': '전화번호를 입력해 주세요.',
      'select_on_map': '맵에서 선택',
      'map_picker_title': '지도에서 위치 선택',
      'map_picker_hint': '지도를 눌러 위치를 지정해 주세요.',
      'map_picker_confirm': '이 위치 선택',
      'current_location': '내 위치',
      'locating_current_position': '현재 위치를 확인하고 있습니다.',
      'location_permission_required': '현재 위치를 사용하려면 위치 권한이 필요합니다.',
      'location_permission_denied': '위치 권한이 거부되었습니다.',
      'location_permission_permanently_denied':
          '브라우저 또는 기기 설정에서 위치 권한을 허용해 주세요.',
      'location_service_disabled': '기기의 위치 서비스가 꺼져 있습니다.',
      'location_unavailable': '현재 위치를 확인할 수 없습니다.',
      'location_timeout': '위치 확인 시간이 초과되었습니다.',
      'location_requires_https':
          '현재 접속 환경에서는 위치 기능을 사용할 수 없습니다. HTTPS 주소에서 다시 시도해 주세요.',
      'low_location_accuracy': '위치 정확도가 낮습니다. 지도에서 정확한 위치를 조정해 주세요.',
      'wizard_vehicle_prerequisites':
          '서비스, 출발지, 도착지, 픽업 시간, 승객 정보를 먼저 입력해 주세요.',
      'wizard_pricing_after_vehicle': '차량 선택 후 요금이 계산됩니다.',
      'guest_lookup_booking_number_hint': 'TX202607010001',
      'admin_dashboard': '대시보드',
      'admin_reservations': '예약 관리',
      'admin_dispatch_new_bookings': '신규 예약',
      'admin_dispatch_existing_bookings': '기존 예약',
      'admin_dispatch_new_sort_hint': '최근 접수순',
      'admin_dispatch_existing_sort_hint': '최근 예약일순',
      'admin_dispatch_unassign_driver': '기사 배정 해제(재오픈)',
      'admin_dispatch_unassign_driver_title': '기사 배정 해제 및 예약 재오픈',
      'admin_dispatch_unassign_driver_message':
          '예약이 OPEN 상태로 돌아가 다른 기사에게 다시 노출됩니다.',
      'admin_dispatch_unassign_driver_warning':
          '이 작업은 되돌릴 수 없으며, 배정된 기사에게 즉시 알림이 전송됩니다.',
      'admin_dispatch_unassign_driver_reason': '배정 해제 사유',
      'admin_dispatch_unassign_driver_confirm': '배정 해제 및 재오픈',
      'admin_dispatch_unassign_success': '기사 배정이 해제되었습니다. 예약이 다시 배차 대기 상태입니다.',
      'admin_dispatch_unassign_failed': '기사 배정을 해제하지 못했습니다.',
      'admin_dispatch_unassign_error_invalid_status':
          '현재 상태에서는 배정을 해제할 수 없습니다. 새로고침 후 다시 시도해 주세요.',
      'admin_dispatch_unassign_error_no_assignment': '활성 기사 배정이 없습니다.',
      'admin_dispatch_unassign_error_validation': '배정 해제 사유를 올바르게 입력해 주세요.',
      'admin_ops_search': '예약번호, 고객, 전화번호, 기사, 차량번호 검색',
      'admin_ops_filters': '필터',
      'admin_ops_filter_apply': '적용',
      'admin_ops_filter_reset': '초기화',
      'admin_ops_filter_date_from': '예약일 시작 (YYYY-MM-DD)',
      'admin_ops_filter_date_to': '예약일 종료 (YYYY-MM-DD)',
      'admin_ops_filter_date_from_invalid': '올바른 시작 날짜를 선택해 주세요.',
      'admin_ops_filter_date_to_invalid': '올바른 종료 날짜를 선택해 주세요.',
      'admin_ops_filter_date_range_invalid': '종료 날짜는 시작 날짜보다 빠를 수 없습니다.',
      'admin_ops_filter_service_type': '서비스 유형',
      'admin_ops_filter_origin': '출발 지역',
      'admin_ops_filter_destination': '도착 지역',
      'admin_ops_filter_settlement': '정산 상태',
      'admin_ops_filter_low_rating': '저평점만',
      'admin_ops_filter_has_inquiry': '문의 있음',
      'admin_ops_view_needs_action': '처리 필요',
      'admin_ops_view_today': '오늘',
      'admin_ops_view_upcoming': '예정',
      'admin_ops_view_in_progress': '운행 중',
      'admin_ops_view_settlement': '정산 대기',
      'admin_ops_view_completed': '완료',
      'admin_ops_view_cancelled': '취소',
      'admin_ops_view_all': '전체',
      'admin_ops_summary_needs_action': '처리 필요',
      'admin_ops_summary_unassigned': '기사 미배정',
      'admin_ops_summary_today': '오늘 예약',
      'admin_ops_summary_in_progress': '운행 중',
      'admin_ops_summary_settlement': '정산 확인',
      'admin_ops_summary_issues': '문제 확인',
      'admin_ops_result_total': '총 {total}건',
      'admin_ops_unassigned_result_total': '전체 미배정 {total}건',
      'admin_ops_result_showing': '현재 {count}건 표시',
      'admin_ops_result_filtered': '현재 조건 결과',
      'admin_ops_severity_urgent': '긴급',
      'admin_ops_severity_soon': '곧 처리',
      'admin_ops_severity_review': '확인 필요',
      'admin_ops_reason_low_rating': '낮은 평가 확인 필요',
      'admin_ops_reason_receipt_rejected': '송금증 반려',
      'admin_ops_reason_receipt_review': '송금증 확인 필요',
      'admin_ops_reason_receipt_missing': '송금증 미제출',
      'admin_ops_reason_pickup_overdue_unassigned': '출발시간 경과 · 기사 미배정',
      'admin_ops_reason_pickup_overdue_stalled': '출발시간 경과 · 진행 지연',
      'admin_ops_reason_pickup_soon_unassigned': '출발 임박 · 기사 미배정',
      'admin_ops_reason_customer_inquiry': '고객 문의 확인 필요',
      'admin_ops_reason_boarding_delay': '탑승 지연',
      'admin_ops_reason_long_trip': '장시간 운행',
      'admin_ops_reason_status_stale': '상태 변경 없음',
      'admin_ops_next_low_rating': '낮은 고객 평가를 확인해 주세요.',
      'admin_ops_next_receipt_review': '송금증이 제출되었습니다. 입금 여부를 확인해 주세요.',
      'admin_ops_next_receipt_missing': '기사에게 송금증 제출을 안내해 주세요.',
      'admin_ops_next_assign_driver': '출발 전 기사를 배정해 주세요.',
      'admin_ops_next_customer_inquiry': '고객 문의에 답변해 주세요.',
      'admin_ops_next_boarding_delay': '탑승이 지연되고 있습니다. 상황을 확인해 주세요.',
      'admin_ops_next_long_trip': '운행이 오래 지속되고 있습니다. 확인해 주세요.',
      'admin_ops_next_confirm_settlement': '200 THB 정산을 확인해 주세요.',
      'admin_ops_cta_settlement_detail': '정산 상세',
      'admin_ops_cta_review_rating': '평가 확인',
      'admin_ops_cta_check_status': '상태 확인',
      'admin_ops_cta_view_booking': '예약 보기',
      'admin_ops_cta_review_act': '처리하기',
      'admin_ops_more_reasons': '추가 사유 {count}개',
      'admin_ops_select_booking': '미리볼 예약을 선택하세요',
      'admin_ops_settlement_rejected': '송금증 반려',
      'admin_ops_settlement_submitted': '송금증 제출',
      'admin_ops_settlement_missing': '송금증 미제출',
      'admin_ops_next_action_label': '다음 작업',
      'admin_ops_archive_show': '숨긴 예약 보기',
      'admin_ops_archive_hide': '숨긴 예약 감추기',
      'admin_ops_archive_bulk': '테스트 예약 숨기기 ({count})',
      'admin_ops_archive_confirm_title': '선택한 예약을 테스트 데이터로 숨기시겠습니까?',
      'admin_ops_archive_warning':
          '이미 배정, 운행 또는 정산 기록이 있는 예약이 포함되어 있습니다. 예약과 연결 데이터는 삭제되지 않고 관리자 기본 목록에서만 숨겨집니다.',
      'admin_ops_archive_info': '예약과 연결 데이터는 삭제되지 않고 관리자 기본 목록에서만 숨겨집니다.',
      'admin_ops_archive_hidden_notice':
          '숨긴 예약은 기본 운영 화면, 기사/고객 화면, 정산 대기 목록에서 제외됩니다.',
      'admin_ops_archive_success': '선택한 예약을 숨겼습니다.',
      'admin_ops_archive_restore_success': '숨긴 예약을 복원했습니다.',
      'admin_ops_archive_failed': '예약 숨기기에 실패했습니다.',
      'admin_ops_archive_restore_failed': '예약 복원에 실패했습니다.',
      'admin_ops_archive_restore': '복원',
      'admin_ops_archive_badge': '숨김/Test',
      'admin_ops_route_airport_pickup': '공항 → 목적지',
      'admin_ops_route_airport_dropoff': '출발지 → 공항',
      'admin_ops_route_city_transfer': '출발지 → 목적지',
      'admin_drivers': '기사 관리',
      'admin_driver_management_help':
          '활성 기사는 계정 상태 기준으로 표시됩니다. 온라인/오프라인과 지도 위치는 별도로 표시됩니다.',
      'admin_driver_filter_active_accounts': '활성 계정',
      'admin_driver_filter_online_only': '온라인만',
      'admin_driver_filter_active_jobs': '진행 중 운행',
      'admin_driver_filter_stale_locations': '오래된 위치',
      'admin_no_drivers_found': '현재 필터에 맞는 기사가 없습니다',
      'admin_no_driver_locations': '지도에 표시할 실시간 위치가 없습니다',
      'admin_driver_online': '온라인',
      'admin_driver_offline': '오프라인',
      'admin_driver_active_jobs': '진행 중 운행',
      'admin_driver_no_vehicle': '주 차량 미설정',
      'admin_driver_no_location': '위치 없음',
      'admin_driver_location_live': '실시간',
      'admin_driver_location_stale': '오래됨',
      'admin_driver_last_seen': '업데이트',
      'admin_pricing': '차량 요금',
      'admin_golf': '골프장',
      'admin_airports': '공항',
      'admin_notifications': '알림',
      'admin_reviews': '리뷰',
      'admin_flights': '항공편',
      'admin_settlements': '정산',
      'admin_logout': '로그아웃',
      'admin_placeholder_deferred': '카탈로그 관리는 MVP 이후 제공 예정입니다.',
      'admin_translations': '번역',
      'admin_settings': '설정',
      'admin_settings_line_qr': '관리자 LINE QR',
      'admin_settings_description': '설명/안내 문구 (선택)',
      'admin_settings_upload_line_qr': 'LINE QR 업로드',
      'admin_settings_bank': '기사 정산 계좌 정보',
      'admin_settings_bank_name': '은행명',
      'admin_settings_account_name': '계좌명',
      'admin_settings_account_number': '계좌번호',
      'admin_settings_upload_promptpay': 'PromptPay QR 업로드',
      'admin_settings_save': '설정 저장',
      'admin_settings_saved': '설정을 저장했습니다.',
      'admin_settings_contact_channels': '고객센터 연결 채널',
      'admin_settings_contact_channels_save': '고객센터 연결 설정 저장',
      'admin_settings_contact_kakao_enabled': '카카오톡 사용',
      'admin_settings_contact_line_enabled': '라인 사용',
      'admin_settings_contact_whatsapp_enabled': '왓츠앱 사용',
      'admin_settings_contact_wechat_enabled': '위챗 사용',
      'admin_settings_contact_whatsapp_phone': '왓츠앱 전화번호',
      'admin_settings_contact_wechat_account_id': '위챗 계정 ID (선택)',
      'admin_settings_upload_wechat_qr': '위챗 QR 업로드',
      'admin_settings_guest_lookup_inquiry': '예약조회 / 메신저 안내',
      'admin_settings_guest_lookup_banner_enabled': '예약조회 결과에 안내 배너 표시',
      'admin_settings_guest_lookup_banner_message': '안내 문구 (선택)',
      'admin_settings_guest_lookup_kakao_url': '카카오톡 채팅 URL (선택)',
      'admin_settings_guest_lookup_line_url': 'LINE 채팅 URL (선택)',
      'admin_settings_guest_lookup_save': '메신저 안내 설정 저장',
      'admin_settings_image_invalid': 'PNG 또는 JPG 이미지 파일을 선택해 주세요.',
      'admin_settings_image_too_large': '이미지 용량이 너무 큽니다. 더 작은 파일을 선택해 주세요.',
      'admin_settings_image_upload_failed':
          '이미지를 업로드하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      'admin_settlement_confirm_200': '200바트 입금 확인',
      'admin_settlement_confirm_question': '기사 송금 200바트를 확인 처리하시겠습니까?',
      'admin_settlement_section': '정산 확인',
      'admin_settlement_status': '정산 상태',
      'admin_settlement_transfer_slip': '송금증',
      'admin_settlement_slip_submitted': '송금증 제출 완료',
      'admin_settlement_slip_not_submitted': '미제출',
      'admin_settlement_waiting_slip': '기사가 송금증을 제출하면 입금 확인을 진행할 수 있습니다.',
      'admin_settlement_view_slip': '송금증 보기',
      'admin_settlement_slip_load_failed': '송금증을 불러올 수 없습니다.',
      'admin_detail_operations_summary': '운영 상태 요약',
      'admin_detail_current_status': '현재 상태',
      'admin_detail_last_status_change': '마지막 상태 변경',
      'admin_detail_status_duration': '현재 상태 지속 시간',
      'admin_detail_driver_assignment_status': '기사/배정 상태',
      'admin_detail_previous_driver': '이전 기사',
      'admin_detail_driver_status': '기사 상태',
      'admin_detail_settlement_status': '정산 상태',
      'admin_detail_receipt_status': '송금증 상태',
      'admin_detail_recommended_action': '다음 권장 조치',
      'admin_detail_settlement_required': '정산 필요',
      'admin_detail_settlement_completed': '정산 완료',
      'admin_detail_receipt_missing': '송금증 미제출',
      'admin_detail_receipt_submitted': '송금증 제출됨',
      'admin_detail_receipt_rejected': '송금증 반려됨',
      'admin_detail_settlement_actionable': '승인 가능 여부',
      'admin_detail_settlement_not_actionable': '현재 예약 상태에서는 정산을 승인할 수 없습니다.',
      'admin_detail_settlement_state_mismatch':
          '운행은 완료됐지만 정산 대기 상태로 전환되지 않았습니다. 예약 상태와 정산 기록을 확인하세요.',
      'admin_detail_rejection_reason': '반려 사유',
      'admin_detail_rejection_reason_missing': '반려 사유가 등록되지 않았습니다.',
      'admin_detail_rejected_at': '반려 시각',
      'admin_detail_receipt_submitted_at': '송금증 제출 시각',
      'admin_detail_receipt_uploaded_at': '송금증 업로드 시각',
      'admin_detail_receipt_file': '송금증 파일',
      'admin_detail_receipt_content_type': '파일 형식',
      'admin_detail_receipt_file_size': '파일 크기',
      'admin_detail_settlement_approved_at': '정산 승인 시각',
      'admin_detail_settlement_approval_mode': '승인 방식',
      'admin_detail_settlement_manual_approval': '수동 승인',
      'admin_detail_additional_memo': '추가 메모',
      'admin_detail_actor_system': '시스템',
      'admin_detail_actor_admin': '관리자',
      'admin_detail_actor_driver': '기사',
      'admin_detail_actor_customer': '고객',
      'admin_detail_actor_unknown': '알 수 없음',
      'admin_detail_settlement_load_failed': '정산 정보를 불러오지 못했습니다.',
      'admin_detail_retry_settlement': '정산 다시 불러오기',
      'admin_detail_diag_pickup_overdue_stalled':
          '예정 픽업 시간이 지났지만 운행 상태가 진행되지 않았습니다. 기사 배정과 현재 기사 상태를 확인하세요.',
      'admin_detail_diag_boarding_delay':
          '기사가 도착한 뒤 고객 탑승이 지연되고 있습니다. 기사 또는 고객에게 탑승 상태를 확인하세요.',
      'admin_detail_diag_long_trip':
          '고객 탑승 이후 운행 시간이 기준보다 길어지고 있습니다. 기사 위치와 운행 상태를 확인하세요.',
      'admin_detail_diag_status_stale':
          '예약 상태가 오랫동안 변경되지 않았습니다. 현재 기사 및 운행 상태를 확인하세요.',
      'support_line_inquiry': 'LINE 1:1 문의',
      'support_line_qr_missing': '관리자 LINE QR이 아직 등록되지 않았습니다.',
      'today_bookings': '오늘 예약',
      'today_revenue': '오늘 매출',
      'pending_count': '대기 중',
      'awaiting_driver': '기사 배정 대기',
      'vehicle': '차량',
      'amount': '금액',
      'send': '전송',
      'type_message': '메시지 입력...',
      'other_airport': '기타 태국 공항',
      'booking_summary': '예약 확인',
      'service_type': '서비스 유형',
      'thb': 'THB',
      'driver_pwa_install_title': 'T-Ride 기사 앱 설치',
      'driver_pwa_install_body':
          '신규 콜 확인과 편리한 이용을 위해 T-Ride를 휴대폰 홈 화면에 설치해 주세요.',
      'driver_pwa_install_now': '지금 설치',
      'driver_pwa_install_preparing': '설치 준비 중...',
      'driver_pwa_install_later': '나중에',
      'driver_pwa_install_manual':
          'Chrome 오른쪽 위 메뉴를 누른 뒤 ‘앱 설치’ 또는 ‘홈 화면에 추가’를 선택하세요.',
      'driver_pwa_install_ios_steps':
          'iPhone 또는 iPad에서는 Safari 공유 버튼을 누르고 “홈 화면에 추가”를 선택한 뒤 “추가”를 누르세요.',
      'driver_pwa_install_open_in_chrome':
          '현재 브라우저에서는 자동 설치를 시작할 수 없습니다. Chrome에서 이 페이지를 연 뒤 Chrome 메뉴에서 T-Ride를 설치해 주세요.',
      'driver_pwa_install_success': 'T-Ride 설치가 완료되었습니다.',
      'driver_pwa_install_dismissed': '설치가 취소되었습니다. 다음에 다시 설치할 수 있습니다.',
      'driver_pwa_install_unavailable':
          '지금은 자동 설치를 시작할 수 없습니다. 브라우저 메뉴에서 앱 설치를 선택해 주세요.',
      'driver_pwa_install_error': '설치를 시작할 수 없습니다. 브라우저 메뉴에서 앱 설치를 선택해 주세요.',
      'driver_online': '온라인',
      'driver_offline': '오프라인',
      'driver_session_ready': '배차 대기 중',
      'driver_session_offline': '오프라인 상태입니다',
      'driver_status_online_ready': '온라인 · 배차 대기중',
      'driver_status_offline_unavailable': '오프라인 · 배차를 받을 수 없음',
      'driver_status_active_busy': '운행 중 · 오프라인 전환 불가',
      'driver_active_job_in_progress': '진행 중인 운행',
      'driver_active_job_stay_online': '운행 진행 중 — 완료 전까지 온라인을 유지하세요.',
      'driver_call_status_ready': '콜 수신 가능',
      'driver_call_status_offline': '콜 수신 일시 중지',
      'driver_call_status_active_trip': '운행 중',
      'driver_call_status_active_trip_helper': '새 콜 수신 불가',
      'driver_call_status_settlement_required': '정산 확인 필요',
      'driver_call_status_customer_issue_review': '고객 이슈 확인 중',
      'driver_call_status_account_under_review': '계정 검토 중',
      'driver_call_status_account_restricted': '계정 제한됨',
      'driver_call_status_driver_approval_pending': '기사 승인 대기',
      'driver_call_status_vehicle_review_required': '차량 확인 필요',
      'driver_call_status_unknown': '상태 확인 필요',
      'driver_call_status_detail_title': '콜 수신 상태',
      'driver_call_status_detail_ready': '새 예약 콜을 받을 수 있습니다.',
      'driver_call_status_detail_offline': '콜을 받으려면 온라인으로 전환해 주세요.',
      'driver_call_status_detail_active_trip': '현재 운행을 마친 뒤 새 콜을 받을 수 있습니다.',
      'driver_call_status_detail_settlement_required':
          '이전 운행의 정산 확인이 완료되어야 새 콜을 받을 수 있습니다.',
      'driver_call_status_detail_customer_issue_review':
          '운영팀이 고객 이슈를 확인하고 있습니다.',
      'driver_call_status_detail_account_under_review':
          '운영팀이 기사 계정을 검토하고 있습니다.',
      'driver_call_status_detail_account_restricted':
          '기사 계정이 제한되어 있습니다. 운영팀에 문의해 주세요.',
      'driver_call_status_detail_driver_approval_pending':
          '기사 승인이 아직 완료되지 않았습니다.',
      'driver_call_status_detail_vehicle_review_required':
          '콜을 받으려면 유효한 활성 차량이 필요합니다.',
      'driver_call_status_detail_unknown': '계속 표시되면 새로고침하거나 운영팀에 문의해 주세요.',
      'driver_notification_new': '새 알림',
      'driver_notification_mark_read': '읽음 처리',
      'driver_qr_camera_unavailable': '카메라를 사용할 수 없습니다',
      'driver_qr_camera_fallback': '아래에서 코드를 직접 입력하세요.',
      'driver_live_location_interval_hint':
          '켜 두면 {seconds}초마다 전송됩니다. 운행 중이 아닐 때는 끄세요.',
      'driver_settlement_receipt_upload': '송금증 업로드',
      'driver_settlement_receipt_formats': 'JPG, PNG, PDF (최대 10MB)',
      'driver_settlement_due': '마감: {date}',
      'ui_load_failed': '데이터를 불러오지 못했습니다. 다시 시도해 주세요.',
      'ui_action_failed': '작업에 실패했습니다. 다시 시도해 주세요.',
      'guest_lookup_title': '예약 조회',
      'guest_lookup_loading': '저장된 예약 불러오는 중...',
      'guest_lookup_subtitle': '예약번호와 전화번호를 입력해 여행 정보를 확인하세요.',
      'guest_lookup_contact_subtitle': '예약 시 입력한 이름과 전화번호로 요약 정보를 확인하세요.',
      'guest_lookup_mode_booking_number': '예약번호로 조회',
      'guest_lookup_mode_contact_name': '이름으로 조회',
      'guest_lookup_name': '예약자 이름',
      'guest_lookup_invalid_name': '예약자 이름을 입력해 주세요',
      'guest_lookup_contact_results_title': '일치하는 예약',
      'guest_lookup_contact_detail_guidance':
          '정확한 정보가 필요하면 예약번호로 조회해주세요.',
      'guest_lookup_contact_back_to_list': '목록으로',
      'guest_lookup_pickup_period_morning': '오전',
      'guest_lookup_pickup_period_afternoon': '오후',
      'guest_lookup_pickup_period_evening': '저녁',
      'guest_lookup_passengers': '탑승 인원',
      'guest_lookup_luggage': '수하물',
      'guest_lookup_booking_number': '예약번호',
      'guest_lookup_phone': '전화번호',
      'guest_lookup_invalid_number': '올바른 예약번호를 입력해 주세요',
      'guest_lookup_invalid_phone': '예약 전화번호를 입력해 주세요',
      'guest_lookup_find': '예약 조회',
      'guest_lookup_not_found': '예약을 찾을 수 없습니다. 예약번호와 전화번호를 확인해 주세요.',
      'guest_lookup_load_error': '예약을 불러오지 못했습니다. 다시 시도해 주세요.',
      'guest_lookup_trip_details': '여행 정보',
      'guest_lookup_pickup': '픽업',
      'guest_lookup_service': '서비스',
      'guest_lookup_from': '출발',
      'guest_lookup_to': '도착',
      'guest_lookup_driver': '기사',
      'guest_lookup_driver_phone': '기사 전화',
      'guest_lookup_driver_status': '기사 배정 상태',
      'guest_lookup_vehicle': '차량',
      'guest_lookup_vehicle_photo_load_failed': '차량 사진을 불러오지 못했습니다.',
      'guest_lookup_payment': '결제',
      'guest_lookup_total': '총 금액',
      'guest_lookup_another': '다른 예약 조회',
      'guest_lookup_inquiry_channels_hint': '카카오톡 또는 라인으로 문의하기',
      'guest_lookup_inquiry_channel_kakao': '카카오톡',
      'guest_lookup_inquiry_channel_line': '라인',
      'guest_lookup_refresh': '새로고침',
      'guest_lookup_refresh_needs_phone': '새로고침하려면 조회 화면에서 전화번호를 입력해 주세요.',
      'booking_complete_track_cta': '예약 상태 확인',
      'auth_social_login_title': '계정을 만들면 다음 예약이 더 편해집니다',
      'auth_social_login_description': '예약 내역 조회, 정보 자동입력이 가능해집니다',
      'auth_google_continue': 'Google로 계속하기',
      'auth_kakao_continue': '카카오로 계속하기',
      'auth_kakao_callback_error': '카카오 로그인에 실패했습니다. 다시 시도해 주세요.',
      'auth_kakao_callback_already_processed':
          '이미 처리된 로그인 요청입니다. 필요하면 다시 시도해 주세요.',
      'auth_kakao_callback_slow_loading': '처리 시간이 오래 걸리고 있어요. 새로고침해 주세요.',
      'auth_kakao_callback_refresh': '새로고침',
      'profile_completion_title': '프로필 완성',
      'profile_completion_body':
          'T-Rider 이용을 위해 이름과 전화번호를 입력해 주세요.',
      'profile_completion_submit': '저장하고 계속',
      'profile_completion_name_required': '이름을 입력해 주세요.',
      'profile_completion_phone_required':
          '올바른 전화번호를 입력해 주세요(5자 이상).',
      'profile_completion_phone_duplicate':
          '이미 다른 계정에서 사용 중인 전화번호입니다.',
      'auth_line_continue': 'LINE으로 계속하기',
      'auth_line_callback_error': 'LINE 로그인에 실패했습니다. 다시 시도해 주세요.',
      'auth_line_callback_state_mismatch': 'LINE 로그인을 확인할 수 없습니다. 다시 시도해 주세요.',
      'auth_line_callback_already_processed':
          '이미 처리된 로그인 요청입니다. 필요하면 다시 시도해 주세요.',
      'auth_line_callback_slow_loading': '처리 시간이 오래 걸리고 있어요. 새로고침해 주세요.',
      'auth_line_callback_refresh': '새로고침',
      'auth_social_login_later': '나중에 하기',
      'auth_social_login_connected': '{name}님, 연결되었습니다',
      'my_bookings_title': '내 예약',
      'my_bookings_empty': '연결된 예약이 없습니다. 예약 후 로그인하면 여기에서 확인할 수 있습니다.',
      'my_bookings_view_button': '내 예약 보기',
      'my_bookings_driver_summary': '기사: {name}',
      'my_bookings_load_error': '예약 목록을 불러오지 못했습니다. 다시 시도해 주세요.',
      'booking_cancel_section': '예약 취소',
      'booking_cancel_action': '예약 취소',
      'booking_cancel_policy_hint': '픽업 2시간 전까지 취소할 수 있습니다.',
      'booking_cancel_confirm_title': '예약을 취소할까요?',
      'booking_cancel_confirm_action': '취소 확정',
      'booking_cancel_confirm_irreversible': '취소 후에는 되돌릴 수 없습니다.',
      'booking_cancel_success': '예약이 취소되었습니다.',
      'booking_cancel_failed': '예약을 취소하지 못했습니다. 다시 시도해 주세요.',
      'booking_cancel_blocked_within_two_hours':
          '픽업 예정 시각 2시간 이내에는 예약을 취소할 수 없습니다.',
      'booking_cancel_blocked_trip_started': '운행이 시작된 예약은 취소할 수 없습니다.',
      'booking_cancel_blocked_already_cancelled': '이미 취소된 예약입니다.',
      'booking_cancel_blocked_completed': '완료된 예약은 취소할 수 없습니다.',
      'booking_cancel_blocked_no_show': '노쇼 처리된 예약은 취소할 수 없습니다.',
      'booking_cancel_blocked_invalid_pickup': '픽업 시각이 올바르지 않아 예약을 취소할 수 없습니다.',
      'booking_cancel_blocked_locked': '이 예약은 잠겨 있어 취소할 수 없습니다.',
      'booking_cancel_driver_assigned': '기사 배정됨: {name}',
      'booking_cancel_driver_assigned_generic': '기사 배정됨',
      'guest_status_guidance_pending': '예약이 접수되었습니다. 곧 확인 처리됩니다.',
      'guest_status_guidance_confirmed': '예약이 확정되었습니다. 픽업 전에 기사가 배정됩니다.',
      'guest_status_guidance_reassignment':
          '기존 기사의 사정으로 새 기사를 배정하고 있습니다. 예약 일정과 요금은 변경되지 않습니다.',
      'status_reassignment_in_progress': '기사 재배정 중',
      'admin_ops_severity_critical': '최긴급',
      'admin_ops_reason_critical_reassignment': '긴급 재배차',
      'admin_ops_reason_urgent_reassignment': '주의 재배차',
      'admin_ops_reason_driver_released_reassignment': '기사 반납 · 재배차',
      'admin_ops_reason_critical_unassigned': '최긴급 · 미배차',
      'admin_ops_reason_urgent_unassigned': '주의 · 미배차',
      'driver_release_assignment': '배정 반납',
      'driver_release_assignment_title': '이 예약의 배정을 반납하시겠습니까?',
      'driver_release_assignment_message': '고객 예약은 유지되며 다른 기사를 다시 배정합니다.',
      'driver_release_assignment_cancel': '유지',
      'driver_release_assignment_confirm': '배정 반납',
      'driver_release_assignment_success': '배정이 해제되었습니다.',
      'driver_release_assignment_failed': '배정 반납에 실패했습니다. 다시 시도해 주세요.',
      'driver_release_assignment_blocked': '현재 상태에서는 배정을 반납할 수 없습니다.',
      'driver_release_assignment_emergency_hint':
          '픽업까지 2시간 이내에는 일반 배정 반납이 제한됩니다. 긴급 상황인 경우에만 배정을 반납할 수 있습니다.',
      'driver_release_assignment_irreversible': '반납 후에는 되돌릴 수 없습니다.',
      'driver_release_reason_label': '반납 사유',
      'driver_release_reason_detail_label': '상세 설명',
      'driver_release_reason_detail_required': '기타 사유를 입력하세요.',
      'driver_release_reason_VEHICLE_BREAKDOWN': '차량 고장',
      'driver_release_reason_ACCIDENT': '사고',
      'driver_release_reason_DRIVER_ILLNESS': '질병',
      'driver_release_reason_FAMILY_EMERGENCY': '가족 긴급 상황',
      'driver_release_reason_SCHEDULE_CONFLICT': '일정 충돌',
      'driver_release_reason_LOCATION_TOO_FAR': '거리 문제',
      'driver_release_reason_OTHER': '기타',
      'driver_assignment_ended_confirm': '확인',
      'driver_booking_not_found': '예약 정보를 찾을 수 없습니다.',
      'driver_assignment_ended_generic_title': '배정이 해제되었습니다',
      'driver_assignment_ended_generic_message': '더 이상 이 예약을 진행할 수 없습니다.',
      'driver_assignment_ended_generic_snackbar':
          '예약 {bookingNumber} 배정이 해제되었습니다.',
      'driver_assignment_ended_customer_title': '고객이 예약을 취소했습니다',
      'driver_assignment_ended_customer_message':
          '고객이 예약을 취소하여 배정이 자동 해제되었습니다.',
      'driver_assignment_ended_customer_snackbar':
          '고객 취소로 예약 {bookingNumber} 배정이 해제되었습니다.',
      'driver_assignment_ended_admin_title': '관리자가 예약을 취소했습니다',
      'driver_assignment_ended_admin_message': '관리자가 예약을 취소하여 배정이 해제되었습니다.',
      'driver_assignment_ended_admin_snackbar':
          '관리자 취소로 예약 {bookingNumber} 배정이 해제되었습니다.',
      'driver_assignment_ended_released_title': '배정 반납이 완료되었습니다',
      'driver_assignment_ended_released_message':
          '배정 반납이 완료되었습니다. 해당 예약은 다른 기사에게 다시 공개됩니다.',
      'driver_assignment_ended_released_snackbar':
          '예약 {bookingNumber} 배정 반납이 완료되었습니다.',
      'driver_assignment_ended_reassigned_title': '다른 기사에게 배정되었습니다',
      'driver_assignment_ended_reassigned_message': '이 예약은 다른 기사에게 배정되었습니다.',
      'driver_assignment_ended_reassigned_snackbar':
          '예약 {bookingNumber}이(가) 다른 기사에게 배정되었습니다.',
      'driver_assignment_ended_completed_title': '운행이 완료되었습니다',
      'driver_assignment_ended_completed_message': '이 예약은 이미 완료되었습니다.',
      'guest_status_guidance_driver_assigned': '기사가 배정되었습니다. 곧 픽업지로 이동합니다.',
      'guest_status_guidance_on_route': '기사가 픽업지로 이동 중입니다.',
      'guest_status_guidance_driver_arrived': '기사가 픽업지에 도착했습니다.',
      'guest_status_guidance_picked_up': '현재 목적지로 이동 중입니다.',
      'guest_status_guidance_settlement_pending': '운행이 종료되었습니다. 이용해 주셔서 감사합니다.',
      'guest_status_guidance_completed': '운행이 완료되었습니다. 기사 평가를 남겨 주세요.',
      'guest_status_guidance_cancelled': '예약이 취소되었습니다. 자세한 내용은 고객센터에 문의해 주세요.',
      'guest_status_guidance_no_show': '이 예약은 노쇼로 처리되었습니다.',
      'guest_lookup_issue_dropoff_qr': '하차 QR 발급',
      'loading_recommendation': '추천 불러오는 중...',
      'calculating_price': '요금 계산 중...',
      'ui_cancel': '취소',
      'ui_save': '저장',
      'ui_confirm_action': '확인',
      'ui_active': '활성',
      'ui_inactive': '비활성',
      'ui_all': '전체',
      'ui_yes': '예',
      'ui_no': '아니오',
      'admin_dashboard_loading': '대시보드 불러오는 중...',
      'admin_dashboard_refresh': '새로고침',
      'admin_dashboard_operations': '{date} 운영 현황',
      'admin_dashboard_subtitle': '{timezone} · 최종 업데이트 {updatedAt}',
      'admin_dashboard_needs_attention': '주의 필요',
      'admin_dashboard_needs_attention_sub': '미배차, 진행 중 운행, 정산 이슈를 우선 처리하세요.',
      'admin_dashboard_unassigned': '미배차',
      'admin_dashboard_active_trips': '진행 중 운행',
      'admin_dashboard_overdue_settlements': '연체 정산',
      'admin_dashboard_operational_focus': '운영 포커스',
      'admin_dashboard_operational_focus_sub': '행을 눌러 해당 큐를 여세요.',
      'admin_dashboard_bookings_needing_assignment': '배차 필요 예약',
      'admin_dashboard_today_bookings_section': '오늘 예약',
      'admin_dashboard_today_bookings_sub': '카드를 눌러 배차 화면을 여세요.',
      'admin_dashboard_today_bookings': '오늘 예약',
      'admin_dashboard_assigned': '배차됨',
      'admin_dashboard_on_route': '운행 중',
      'admin_dashboard_arrived': '도착',
      'admin_dashboard_completed': '완료',
      'admin_dashboard_cancelled': '취소',
      'admin_dashboard_fleet_finance': '차량·재무',
      'admin_dashboard_online_drivers': '온라인 기사',
      'admin_dashboard_pending_settlements': '대기 정산',
      'admin_dashboard_today_revenue': '오늘 매출',
      'admin_dashboard_completed_revenue_today': '오늘 완료 운행 매출: {amount}',
      'admin_dashboard_mixed_currencies': '복합 통화',
      'admin_pricing_manager': '요금 관리',
      'admin_pricing_tab_routes': '노선',
      'admin_pricing_tab_prices': '차량 요금',
      'admin_pricing_tab_policies': '부가 요금',
      'admin_pricing_tab_simulator': '시뮬레이터',
      'admin_pricing_kpi_active_routes': '활성 노선',
      'admin_pricing_kpi_active_prices': '활성 차량 요금',
      'admin_pricing_kpi_active_policies': '활성 부가 요금',
      'admin_pricing_kpi_current_prices': '현재 요금',
      'admin_pricing_kpi_expiring_soon': '만료 임박',
      'admin_pricing_no_routes': '노선 없음',
      'admin_pricing_no_prices': '차량 요금 없음',
      'admin_pricing_no_policies': '부가 요금 없음',
      'admin_pricing_create_route': '노선 생성',
      'admin_pricing_create_price': '요금 생성',
      'admin_pricing_create_policy': '정책 생성',
      'admin_pricing_filter_service_type': '서비스 유형',
      'admin_pricing_filter_origin': '출발',
      'admin_pricing_filter_destination': '도착',
      'admin_pricing_filter_active': '활성',
      'admin_pricing_filter_search': '검색',
      'admin_pricing_filter_route': '노선',
      'admin_pricing_filter_all_routes': '전체 노선',
      'admin_pricing_filter_status': '상태',
      'admin_pricing_status_current': '현재',
      'admin_pricing_status_future': '예정',
      'admin_pricing_status_expired': '만료',
      'admin_pricing_status_inactive': '비활성',
      'admin_pricing_dialog_create_route': '노선 생성',
      'admin_pricing_dialog_edit_route': '노선 수정',
      'admin_pricing_dialog_create_price': '요금 생성',
      'admin_pricing_dialog_edit_price': '요금 수정',
      'admin_pricing_dialog_create_policy': '부가 요금 생성',
      'admin_pricing_dialog_edit_policy': '부가 요금 수정',
      'admin_pricing_dialog_copy_route': '노선 #{id} 복사',
      'admin_pricing_sim_calculate': '계산',
      'admin_pricing_sim_name_sign': '피켓',
      'admin_pricing_sim_waiting': '대기',
      'admin_pricing_sim_parking': '주차',
      'admin_pricing_sim_toll': '통행료',
      'admin_pricing_sim_total': '합계: {amount} {currency}',
      'admin_flight_sync_title': '자동 항공편 동기화',
      'admin_flight_running': '실행 중',
      'admin_flight_run_sync': '동기화 실행',
      'admin_flight_worker_enabled': 'Worker: Enabled',
      'admin_flight_worker_disabled': 'Worker: Disabled',
      'admin_flight_provider_configured': 'Provider: Configured',
      'admin_flight_provider_not_configured': 'Provider: Not configured',
      'admin_flight_last_completed': '마지막 완료: {value}',
      'admin_flight_date': '날짜',
      'admin_flight_clear_date': '날짜 지우기',
      'admin_flight_flight_number': '항공편 번호',
      'admin_flight_booking_number': '예약번호',
      'admin_flight_delayed_only': '지연만',
      'admin_flight_empty': '공항 픽업 항공편 없음',
      'admin_flight_route': '노선',
      'admin_flight_pickup': '픽업',
      'admin_flight_sync_line': 'Sync: {status} · Last: {last}',
      'admin_flight_sync_error': 'Sync error: {message}',
      'admin_flight_delay_minutes': 'Delay {minutes}m',
      'admin_flight_provider_missing': '항공편 제공자가 설정되지 않았습니다',
      'admin_flight_sync_unavailable': '자동 항공편 동기화 상태를 불러올 수 없습니다',
      'booking_trip_completed': '운행 완료',
      'booking_dropoff_qr_title': '운행 완료 QR',
      'booking_dropoff_qr_hint':
          '하차 시 QR 코드를 기사님에게 한 번 더 보여주세요. 기사님이 QR 코드를 스캔하면 운행 종료로 전환됩니다.',
      'booking_dropoff_qr_unavailable': '하차 QR은 승차 후·운행 완료 전에 이용할 수 있습니다.',
      'booking_qr_load_failed':
          'QR 코드를 불러올 수 없습니다. 예약 상태를 확인하거나 관리자에게 문의해 주세요.',
      'booking_refresh_dropoff_qr': '하차 QR 새로고침',
      'booking_issue_new_dropoff_qr': '새 하차 QR 발급',
      'booking_review_title': '운행 평가',
      'booking_review_thanks': '리뷰 감사합니다',
      'booking_review_rating': '평점: {rating} / 5',
      'booking_review_comment': '코멘트 (선택)',
      'booking_review_submitting': '제출 중...',
      'booking_review_submit': '리뷰 제출',
      'booking_review_load_error': '리뷰를 불러오지 못했습니다',
      'review_card_title': '기사님의 운행은 어떠셨나요?',
      'review_card_subtitle': '별점을 선택해 주세요.',
      'review_star_label': '{value}점',
      'review_rating_desc_1': '매우 불만족',
      'review_rating_desc_2': '불만족',
      'review_rating_desc_3': '보통',
      'review_rating_desc_4': '만족',
      'review_rating_desc_5': '매우 만족',
      'review_comment_label': '의견을 남겨주세요 (선택)',
      'review_comment_hint': '개인정보는 입력하지 마세요.',
      'review_submit_button': '평가 완료',
      'review_success_title': '평가해 주셔서 감사합니다.',
      'review_success_body': '더 나은 서비스를 만드는 데 도움이 됩니다.',
      'review_tag_FRIENDLY': '친절했어요',
      'review_tag_SAFE_DRIVING': '안전하게 운전했어요',
      'review_tag_CLEAN_VEHICLE': '차량이 깨끗했어요',
      'review_tag_ON_TIME': '시간을 잘 지켰어요',
      'review_tag_GOOD_COMMUNICATION': '의사소통이 좋았어요',
      'review_tag_UNSAFE_DRIVING': '운전이 불안했어요',
      'review_tag_LATE_ARRIVAL': '시간이 늦었어요',
      'review_tag_VEHICLE_NOT_CLEAN': '차량이 깨끗하지 않았어요',
      'review_tag_UNFRIENDLY_SERVICE': '기사 응대가 불친절했어요',
      'review_tag_ROUTE_ISSUE': '경로에 문제가 있었어요',
      'review_tag_OTHER_ISSUE': '기타 문제가 있었어요',
      'admin_booking_review_section': '고객 평가',
      'admin_detail_more_reasons': '추가 처리 사유 {count}건',
      'admin_detail_customer': '고객 정보',
      'admin_contact_status': '연락 상태',
      'admin_contact_channel': '연락 채널',
      'admin_contact_confirm_requested_hint':
          '고객이 메신저로 예약번호를 보냈다고 확인했습니다. 운영 메신저에서 동일 예약번호를 확인한 뒤 연결을 확인해 주세요.',
      'admin_contact_verify_button': '연결 확인',
      'admin_contact_verify_success': '연락 연결이 확인되었습니다',
      'admin_detail_trip': '운행 정보',
      'admin_detail_driver_vehicle': '기사 및 차량',
      'admin_detail_pricing_settlement': '요금 및 정산',
      'admin_detail_available': '이용 가능',
      'admin_detail_status_history': '상태 변경 기록',
      'admin_detail_technical': '기술 정보',
      'admin_detail_booking_id': '내부 예약 ID',
      'admin_detail_raw_status': '원본 예약 상태',
      'admin_detail_service_code': '서비스 유형 코드',
      'admin_detail_updated': '수정 시간',
      'admin_detail_created': '예약시간',
      'admin_pickup_requested_at': '출발 요청시간',
      'admin_detail_country': '국가',
      'admin_detail_passengers': '승객 수 (성인 / 아동 / 유아)',
      'admin_detail_luggage': '수하물 수',
      'admin_detail_special_requests': '특별 요청',
      'admin_customer_preference_summary_prefix': '고객 선호:',
      'admin_customer_preference_badge': '선호',
      'admin_detail_flight': '항공편',
      'admin_detail_recommend_drivers': '기사 추천',
      'admin_notes_title': '내부 메모',
      'admin_notes_admin_only': '관리자만 볼 수 있습니다.',
      'admin_notes_hint': '운영 메모를 입력하세요',
      'admin_notes_add': '메모 추가',
      'admin_notes_empty': '등록된 내부 메모가 없습니다.',
      'admin_notes_append_only': '기존 메모는 수정되지 않습니다. 정정 내용은 새 메모로 추가해 주세요.',
      'admin_notes_remaining': '{count}자 남음',
      'admin_notes_load_failed': '내부 메모를 불러오지 못했습니다.',
      'admin_notes_add_failed': '내부 메모를 추가하지 못했습니다.',
      'admin_booking_low_rating_badge': '낮은 평가 확인 필요',
      'admin_booking_review_rating': '별점',
      'admin_booking_review_tags': '태그',
      'admin_booking_review_comment': '후기',
      'booking_notification_updates': '업데이트',
      'booking_notification_enabling': '활성화 중...',
      'booking_notification_enable': '알림 켜기',
      'booking_notification_empty': '업데이트 없음',
      'booking_notification_default_title': '알림',
      'booking_notification_load_error': '알림을 불러오지 못했습니다',
      'booking_notification_enabled': '알림이 활성화되었습니다',
      'booking_notification_permission_denied': '알림 권한이 거부되었습니다',
      'booking_notification_unsupported': '이 브라우저에서는 푸시 알림을 지원하지 않습니다',
      'booking_notification_config_missing': '이 환경에서는 푸시 알림이 설정되지 않았습니다',
      'booking_notification_register_failed': '알림 등록에 실패했습니다',
      'guest_lookup_trip_completed_qr_hidden': '운행이 완료되어 QR 코드가 숨겨집니다.',
      'guest_lookup_dropoff_qr_token': '하차 QR 토큰: {token}',
      'guest_lookup_boarding_qr_security':
          '탑승 QR은 예약 생성 시 발급되었습니다. 보안상 조회 화면에서 복구할 수 없습니다.',
      'track_driver_title': '기사 추적',
      'track_driver_unavailable': '기사 위치를 아직 확인할 수 없습니다.',
      'track_driver_stale': '위치 정보가 오래되었을 수 있습니다',
      'track_driver_live': '실시간 위치',
      'track_driver_updated': '업데이트 {time}',
      'track_driver_load_error': '기사 위치를 불러오지 못했습니다',
      'customer_driver_location_title': '기사 위치',
      'customer_driver_location_waiting':
          '기사님이 배정되었습니다. 기사님이 픽업 장소로 이동을 시작하면 실시간 위치를 확인할 수 있습니다.',
      'customer_driver_location_live': '기사 위치 공유 중',
      'customer_driver_location_updated_label': '마지막 업데이트',
      'customer_driver_location_updated_now': '방금 업데이트됨',
      'customer_driver_location_updated_minutes': '{minutes}분 전 업데이트',
      'customer_driver_location_temporarily_unavailable':
          '기사 위치를 일시적으로 확인할 수 없습니다. 잠시 후 다시 확인해 주세요.',
      'customer_driver_location_stale': '기사 위치가 최근 업데이트되지 않았습니다.',
      'customer_driver_location_ended': '실시간 위치 공유가 종료되었습니다.',
      'customer_driver_location_open_map': '지도에서 보기',
      'customer_driver_location_recenter': '기사 위치로 이동',
      'customer_driver_on_route': '기사님이 픽업 장소로 이동 중입니다.',
      'customer_driver_arrived': '기사님이 픽업 장소에 도착했습니다. 기사와 차량 정보를 확인해 주세요.',
      'customer_trip_in_progress': '운행 중입니다.',
      'customer_driver_location_load_failed':
          '기사 위치를 확인할 수 없습니다. 잠시 후 다시 시도해 주세요.',
      'customer_driver_location_retry': '다시 시도',
    },
    'zh': {
      'app_title': 'T-Rider',
      'app_subtitle': '泰国机场接送服务',
      'airport_pickup': '机场接机',
      'airport_pickup_desc': '从机场直接送到您的住处',
      'airport_dropoff': '机场送机',
      'airport_dropoff_desc': '从住处准时送达机场',
      'city_transfer': '市内接送',
      'city_transfer_desc': '城市间轻松往返',
      'golf_transfer': '高尔夫接送',
      'golf_transfer_desc': '高尔夫球包空间充足',
      'select_service': '选择服务类型',
      'passengers': '乘客',
      'adults': '成人 (12岁以上)',
      'children': '儿童',
      'luggage': '行李',
      'small_carriers': '20寸行李箱',
      'large_carriers': '24寸以上行李箱',
      'golf_bags': '高尔夫球包',
      'special_luggage': '特殊行李',
      'name_sign': '举牌服务 (+100 THB)',
      'booking_prefer_female_driver': '偏好女性司机',
      'booking_preference_disclaimer': '我们将在安排司机时尽量满足此需求，但无法100%保证。',
      'next': '下一步',
      'back': '返回',
      'confirm': '确认预订',
      'origin': '出发地',
      'destination': '目的地',
      'airport': '机场',
      'flight_number': '航班号',
      'date': '日期',
      'time': '时间',
      'pickup_datetime': '接送日期和时间',
      'admin_pickup_requested_at': '请求出发时间',
      'pickup_date': '接送日期',
      'pickup_time': '接送时间',
      'pickup_minimum_notice': '请选择至少 2 小时后的泰国当地时间。',
      'select_vehicle': '选择车辆',
      'recommended': '推荐',
      'total': '总计',
      'special_requests': '特殊要求',
      'customer_info': '客户信息',
      'name': '姓名',
      'email': '邮箱',
      'phone': '电话',
      'country': '国家',
      'booking_complete': '预订完成',
      'reservation_number': '预订号',
      'booking_number_label': '预订号',
      'booking_with_number': '预订 {bookingNumber}',
      'booking_detail_view': '查看预订详情',
      'notification_type_booking': '预订',
      'notification_type_pickup': '接送',
      'notification_type_settlement': '结算',
      'notification_type_system': '系统',
      'status': '状态',
      'golf_region': '高尔夫区域',
      'golf_course': '高尔夫球场',
      'driver_included': '含司机',
      'status_pending': '待处理',
      'status_confirmed': '已确认',
      'status_driver_assigned': '已分配司机',
      'status_completed': '已完成',
      'status_cancelled': '已取消',
      'status_on_route': '前往接客',
      'status_driver_arrived': '司机已到达',
      'status_no_show': '未出现',
      'status_picked_up': '乘客已上车',
      'status_customer_settlement_pending': '行程已结束',
      'search_place': '搜索地点...',
      'no_results': '未找到地点',
      'infants': '婴儿',
      'multiple_vehicles_required': '需要多辆车',
      'base_price': '基础价格',
      'additional_charges': '附加费用',
      'pricing_summary': '价格摘要',
      'pricing_name_sign_with_picket': '接机举牌服务',
      'vehicle_not_available': '此行程不可用',
      'pricing_inquiry_required': '请咨询',
      'complete_passengers_first': '请先填写乘客信息',
      'yes': '是',
      'change_location': '更改',
      'loading_place_details': '正在加载地点详情...',
      'airport_shortcuts': '机场快捷方式',
      'airport_shortcuts_origin': '机场快捷方式',
      'airport_shortcuts_destination': '机场快捷方式',
      'recent_non_airport_places': '最近地点',
      'recent_locations': '最近地点',
      'landing_hero_eyebrow': '泰国专业机场与城市接送',
      'landing_hero_title': '在泰国，和 T-Rider 一起舒适出发',
      'landing_hero_body': '经验丰富的司机，从机场到目的地，安全顺畅。',
      'landing_hero_cta': '立即预订',
      'landing_support_cta': '客服中心',
      'landing_support_one_click_cta': '通过客服中心一键预约',
      'landing_hero_helper': '无需注册，快速预约',
      'landing_hero_subtitle': '安心、顺畅，值得信赖。',
      'landing_highlight_fixed_price': '固定价格',
      'landing_highlight_fixed_price_desc': '预订时即见最终价格，无隐藏费用',
      'landing_highlight_flight': '航班追踪',
      'landing_highlight_flight_desc': '追踪您的航班，延误时自动调整接车时间',
      'landing_highlight_support': '24/7 支持',
      'landing_highlight_support_desc': '行程前后随时获得帮助',
      'book_your_ride': '立即预订',
      'landing_services_title': '我们的服务',
      'landing_services_subtitle': '机场与城市接送',
      'service_badge_popular': '最受欢迎',
      'landing_trust_title': '放心出行',
      'landing_trust_drivers_title': '经验丰富的司机',
      'landing_trust_drivers_desc': '熟悉泰国道路与机场接送路线。',
      'landing_trust_confirmed_title': '预约信息确认',
      'landing_trust_confirmed_desc': '提前确认接车信息与行程安排。',
      'landing_trust_comfort_title': '舒适旅程',
      'landing_trust_comfort_desc': '无需为交通操心，专注享受旅行。',
      'landing_trust_refund_title': '安心取消·退款',
      'landing_trust_refund_desc':
          '预约确认后24小时内可免费取消，无需支付违约金。若您在相同条件(车型·行程)下发现更优惠的确定报价，可在预约后48小时内联系我们，我们将为您退还差价。',
      'landing_trust_driver_policy_title': '司机服务质量管理',
      'landing_trust_driver_policy_desc':
          '如对司机有任何不满，可通过预约详情页面或KakaoTalk/电话进行投诉。收到投诉后我们将在24小时内与您联系确认。对于安全威胁等严重情况，我们将立即暂停该司机的派单。',
      'landing_how_it_works_title': '预约流程',
      'landing_how_step_1_title': '填写行程信息',
      'landing_how_step_1_desc': '选择服务、接车时间与目的地。',
      'landing_how_step_2_title': '确认车辆与费用',
      'landing_how_step_2_desc': '查看推荐车辆与价格后再确认预约。',
      'landing_how_step_3_title': '舒适抵达',
      'landing_how_step_3_desc': '司机安全送您至目的地。',
      'landing_reassurance_body': '第一次来泰国？从预约到抵达，我们会清楚指引每一步。',
      'landing_booking_lookup_title': '已经预约了？',
      'landing_booking_lookup_desc': '使用预约号查看行程与司机信息。',
      'landing_booking_lookup_action': '查询预约',
      'landing_footer_note': '泰国全境专业机场与城市接送',
      'landing_login_title': '登录',
      'landing_login_sheet_title': '登录 T-Rider',
      'landing_login_prompt_cta': '登录赚取里程积分',
      'landing_login_sheet_close': '关闭',
      'landing_login_support_prompt': '登录遇到问题？',
      'landing_login_support_link': '联系客服',
      'landing_header_my_bookings_menu': '查看我的预约',
      'landing_header_logout_menu': '退出登录',
      'landing_header_logout_success': '已退出登录',
      'landing_logged_in_greeting': '已登录：{name}',
      'landing_my_bookings_button': '我的预约',
      'landing_logout_button': '退出登录',
      'auth_login_provider_hint': '为管理里程积分，请始终使用同一种登录方式（Google、Kakao 或 LINE）。',
      'landing_header_account_menu': '我的账户',
      'account_page_title': '我的账户',
      'account_menu_section_title': '账户',
      'account_mileage_label': '里程积分',
      'account_mileage_balance': '{balance}P',
      'account_mileage_loading': '正在加载余额...',
      'account_mileage_load_failed': '加载失败',
      'account_mileage_retry': '重试',
      'account_my_bookings_menu': '我的预约',
      'account_logout_menu': '退出登录',
      'account_logout_cancel': '取消',
      'account_logout_confirm_message': '确定要退出登录吗？',
      'account_login_provider_google': 'Google 登录',
      'account_login_provider_kakao': 'Kakao 登录',
      'account_login_provider_line': 'LINE 登录',
      'account_sign_in_required': '请先登录以查看账户信息。',
      'account_booking_counts_title': '预约记录',
      'account_booking_count_waiting': '等待中',
      'account_booking_count_assigned': '已派车',
      'account_booking_count_in_progress': '进行中',
      'account_booking_count_settlement_pending': '待结算',
      'account_booking_count_completed': '已完成',
      'account_booking_count_review_pending': '待评价',
      'account_coupon_menu': '优惠券',
      'account_coupons_title': '优惠券',
      'account_coupons_available_section': '可用优惠券',
      'account_coupons_used_section': '已使用优惠券',
      'account_coupons_empty_available': '暂无可用优惠券',
      'account_coupons_empty_used': '暂无已使用优惠券',
      'account_coupons_load_failed': '无法加载优惠券',
      'account_coupons_issued_at': '发放日期',
      'account_coupons_used_at': '使用日期',
      'account_coupons_used_booking': '预约编号',
      'booking_coupon_section_title': '使用优惠券',
      'booking_coupon_none': '不使用优惠券',
      'booking_coupon_discount_label': '优惠券折扣',
      'booking_mileage_section_title': '使用里程',
      'booking_mileage_discount_label': '里程抵扣',
      'booking_mileage_available': '可用里程：{balance}点',
      'booking_mileage_use_all': '全部使用',
      'admin_coupons_menu': '优惠券发放',
      'admin_coupon_issue_title': '发放优惠券',
      'admin_coupon_search_hint': '按电话、邮箱或姓名搜索',
      'admin_coupon_search_empty': '未找到客户',
      'admin_coupon_select_customer': '选择客户',
      'admin_coupon_title_label': '优惠券标题',
      'admin_coupon_amount_label': '折扣金额',
      'admin_coupon_issue_button': '发放优惠券',
      'admin_coupon_issue_success': '优惠券已发放',
      'admin_coupon_issue_failed': '发放优惠券失败',
      'admin_coupon_recent_title': '最近发放的优惠券',
      'admin_coupon_recent_empty': '暂无已发放优惠券',
      'admin_coupon_cancel': '取消',
      'admin_coupon_cancel_success': '优惠券已取消',
      'admin_coupon_status_available': '可用',
      'admin_coupon_status_used': '已使用',
      'admin_coupon_recent_customers_title': '最近客户',
      'admin_coupon_recent_customers_empty': '暂无已完成预约',
      'admin_coupon_selected_customer': '已选客户',
      'admin_coupon_template_picker_title': '优惠券模板',
      'admin_coupon_template_manual': '手动输入',
      'admin_coupon_template_manage_title': '模板管理',
      'admin_coupon_template_create_button': '添加模板',
      'admin_coupon_template_create_success': '模板已创建',
      'admin_coupon_template_create_failed': '创建模板失败',
      'admin_coupon_template_active': '启用',
      'admin_coupon_template_inactive': '停用',
      'admin_home_banner_menu': '首页横幅',
      'admin_manual_booking_menu': '管理员手动登记',
      'admin_manual_booking_title': '管理员手动登记叫车',
      'admin_manual_booking_origin': '上车地点',
      'admin_manual_booking_destination': '下车地点',
      'admin_manual_booking_pickup_datetime': '接客日期与时间',
      'admin_manual_booking_pickup_datetime_hint': '请选择接客日期和时间',
      'admin_manual_booking_pickup_timezone': '泰国当地时间 (UTC+7)',
      'admin_manual_booking_special_items_text': '特殊行李说明',
      'admin_manual_booking_special_items_hint': '请描述超大或特殊行李',
      'admin_manual_booking_flight_pickup_date_required': '航班查询前请先选择接客日期',
      'admin_manual_booking_flight_manual_fallback_hint':
          '若航班查询失败，请手动输入航班号并设置接客时间。',
      'admin_manual_booking_flight_pickup_conflict_warning':
          '您输入的接送时间与航班预计到达时间相差 {minutes} 分钟。',
      'admin_manual_booking_flight_pickup_keep_manual': '使用我输入的时间',
      'admin_manual_booking_flight_pickup_use_flight': '按航班到达时间更改',
      'admin_manual_booking_flight_pickup_final_label': '最终接送请求时间',
      'admin_dispatch_driver_pickup_conflict': '接客时间与该司机其他已分配订单冲突',
      'admin_manual_booking_vehicle_type': '车型等级',
      'admin_manual_booking_passengers': '乘客人数',
      'admin_manual_booking_name_sign_hint': '司机端将显示需要举牌服务的标识。',
      'admin_manual_booking_payout_amount': '司机支付金额 (THB)',
      'admin_manual_booking_customer_charge_amount': '客户支付金额 (THB)',
      'admin_manual_booking_customer_charge_hint': '可选，仅供管理员记录客户实际支付金额。',
      'admin_manual_booking_payment_method': '收款方式',
      'admin_manual_booking_payment_admin_collected': '管理员已收款',
      'admin_manual_booking_payment_driver_collects': '司机现场收款',
      'admin_manual_booking_customer_section': '客户信息',
      'admin_manual_booking_guest_name': '访客姓名',
      'admin_manual_booking_guest_phone': '访客电话',
      'admin_manual_booking_guest_email': '访客邮箱（可选）',
      'admin_manual_booking_clear_customer': '清除所选客户',
      'admin_manual_booking_memo': '备注（可选）',
      'admin_manual_booking_submit': '创建开放叫车',
      'admin_manual_booking_success': '开放叫车已创建。预约号：',
      'admin_manual_booking_view_detail': '查看预约',
      'admin_manual_booking_assign_driver': '分配司机',
      'admin_manual_booking_edit': '编辑',
      'admin_manual_booking_edit_title': '编辑管理员手动叫车',
      'admin_manual_booking_edit_not_allowed': '当前状态无法编辑',
      'admin_manual_booking_cancel': '取消叫车',
      'admin_manual_booking_cancel_confirm': '确定取消此管理员手动叫车？已分配的司机将被解除。',
      'admin_manual_booking_cancel_success': '管理员手动叫车已取消',
      'admin_manual_booking_assign_success': '司机已分配',
      'admin_manual_booking_update_submit': '保存更改',
      'admin_manual_booking_update_success': '管理员手动叫车已更新',
      'admin_manual_booking_failed': '无法创建管理员手动叫车',
      'admin_manual_booking_validation_required': '请填写所有必填项',
      'admin_manual_booking_validation_customer': '请选择会员或输入访客姓名和电话',
      'admin_manual_booking_preview_notice': '司机端预览：将显示银行账户确认提示',
      'driver_call_badge_admin_manual': '管理员登记',
      'driver_call_badge_no_commission': '无佣金',
      'driver_call_bank_account_confirm_notice':
          '款项将汇入您注册时登记的银行账户，请再次确认账户信息是否正确。',
      'admin_home_banner_title': '首页横幅管理',
      'admin_home_banner_order_label': '显示顺序',
      'admin_home_banner_order_hint': '留空则追加到末尾',
      'admin_home_banner_upload_button': '上传横幅',
      'admin_home_banner_create_success': '横幅已上传',
      'admin_home_banner_create_failed': '上传横幅失败',
      'admin_home_banner_empty': '暂无横幅',
      'admin_home_banner_active': '启用',
      'admin_home_banner_inactive': '停用',
      'admin_home_banner_delete': '删除',
      'admin_home_banner_delete_success': '横幅已删除',
      'admin_home_banner_move_up': '上移',
      'admin_home_banner_move_down': '下移',
      'account_feature_coming_soon': '即将推出',
      'account_recent_bookings_title': '最近行程',
      'account_recent_bookings_empty': '暂无最近行程',
      'account_recent_bookings_load_failed': '无法加载最近行程',
      'account_my_reviews_menu': '我的评价',
      'account_customer_support_menu': '客服/咨询',
      'account_mileage_history_title': '里程明细',
      'account_mileage_history_empty': '暂无里程明细',
      'account_mileage_history_load_failed': '无法加载里程明细',
      'account_my_reviews_title': '我的评价',
      'account_my_reviews_empty': '暂无评价',
      'account_my_reviews_load_failed': '无法加载评价',
      'account_mileage_type_accrue': '获得',
      'account_mileage_type_reversal': '退回',
      'landing_privacy_policy_link': '隐私政策',
      'privacy_policy_title': '隐私政策',
      'privacy_policy_load_failed': '无法加载隐私政策。请稍后重试。',
      'analytics_consent_title': '分析 Cookie',
      'analytics_consent_body':
          '我们使用 Google Analytics 衡量访问和预约转化，不会发送姓名、电话或电子邮件。您可在隐私设置中随时更改。',
      'analytics_consent_allow': '允许分析',
      'analytics_consent_deny': '拒绝',
      'analytics_consent_privacy': '隐私政策',
      'analytics_consent_settings': '分析设置',
      'analytics_consent_settings_title': '分析同意设置',
      'landing_language_label': '语言',
      'support_title': '客服中心',
      'support_contact_channels_title': '联系客服中心',
      'support_contact_channels_hint': '点击您常用的 messenger 图标即可开始咨询',
      'support_wechat_qr_dialog_title': '微信二维码',
      'support_wechat_qr_dialog_hint': '请在微信中扫描此二维码添加好友',
      'support_page_intro': '如果预约有困难或需要咨询，请联系客户中心。也可以附加机票、酒店凭证或预约截图。',
      'support_inquiry_button': '咨询',
      'support_dialog_title': '客服中心咨询',
      'support_close_button': '关闭',
      'support_inquiry_title': '通过客服中心预约和咨询',
      'support_inquiry_subtitle': '请留下预约问题、航班信息、接送地点和目的地。',
      'support_default_guide':
          '您好，这里是 T-Ride 客服中心。请留下预约咨询、航班信息、接送地点和目的地，我们确认后会为您说明。',
      'support_input_label': '咨询内容',
      'support_input_hint': '请输入预约或行程咨询。',
      'support_attach_button': '附加图片',
      'support_send_button': '发送',
      'support_sending': '发送中...',
      'support_receipt_number': '受理编号',
      'support_submit_failed': '咨询提交失败。请稍后再试。',
      'support_attachment_help': '可以附加机票图片、酒店凭证或预约画面截图等。',
      'support_no_attachments': '尚未选择附件',
      'support_auto_receipt': '自动接收通知：您的咨询已收到。客服中心确认后会为您说明。',
      'support_faq_title': '常见问题',
      'support_faq_placeholder': '常见问题将在客户咨询数据积累后按优先级更新。',
      'trust_flight_delay': '航班延误监控',
      'trust_flight_delay_desc': '航班变动时自动更新接车时间',
      'trust_fixed_pricing': '固定定价',
      'trust_fixed_pricing_desc': '预订价格即为最终价格',
      'trust_professional_drivers': '专业司机',
      'trust_professional_drivers_desc': '持证专业司机，安全顺畅的行程',
      'trust_multilingual': '多语言支持',
      'trust_multilingual_desc': '支持英语、韩语、中文、日语和泰语预订',
      'payment_method': '付款方式',
      'pay_driver_at_destination': '到达目的地后向司机付款',
      'customer_review_booking': '检查预约',
      'customer_confirm_booking': '确认预约',
      'customer_booking_processing': '正在处理预约',
      'customer_urgent_pickup_hint': '若接送时间在2小时以内，请点击“紧急请求”继续。',
      'customer_urgent_request': '紧急请求',
      'customer_urgent_confirm_title': '确认紧急请求',
      'customer_urgent_confirm_body':
          '紧急请求可能需要更长时间才能安排司机。匹配成功后，我们会先告知预计到达时间。如需继续，请点击确认。',
      'customer_urgent_confirm_submit': '确认',
      'customer_urgent_flow_title': '紧急请求',
      'customer_urgent_searching_title': '正在寻找司机',
      'customer_urgent_searching_body': '我们正在为您寻找附近可用的司机。',
      'customer_urgent_eta_title': '司机预计约 {minutes} 分钟后到达',
      'customer_urgent_eta_body': '若到达时间合适请点击接受，若希望更快到达请点击拒绝。',
      'customer_urgent_accept_eta': '接受',
      'customer_urgent_reject_eta': '拒绝',
      'customer_urgent_retry_title': '要继续寻找更快的司机吗？',
      'customer_urgent_retry_body': '我们可以继续为您寻找到达时间更短的司机。',
      'customer_urgent_retry_accept': '继续寻找',
      'customer_urgent_retry_cancel': '取消预约',
      'customer_urgent_confirmed_title': '司机已确认',
      'customer_urgent_confirmed_body': '司机已分配。正在打开预约详情页面。',
      'customer_urgent_exhausted_title': '附近暂无可用司机',
      'customer_urgent_exhausted_body': '附近没有待命的司机。您的预约将被取消。',
      'customer_urgent_go_home': '返回首页',
      'selected_location': '已选位置',
      'customer_payment_method': '付款方式',
      'customer_payment_pay_driver': '到达目的地后向司机付款',
      'customer_payment_cash': '现金付款',
      'customer_payment_qr': 'QR 付款',
      'customer_payment_card': '银行卡付款',
      'customer_payment_bank_transfer': '银行转账',
      'customer_payment_unknown': '付款方式不可用',
      'customer_price_conditions': '过路费、机场停车费和额外等待费可能根据预约条件而有所不同。',
      'customer_luggage_guidance': '请准确填写大件行李和高尔夫球包数量，以便推荐合适车辆。实际容量取决于最终分配车辆。',
      'customer_vehicle_guidance': '乘客或行李较多时，可能无法选择小于推荐车型的车辆。',
      'customer_vehicle_not_suitable': '不适合所选乘客或行李条件。',
      'customer_confirmation_title': '最终预约确认',
      'customer_confirmation_subtitle': '确认预约前，请检查以下内容。',
      'customer_confirmation_edit': '编辑',
      'customer_confirmation_trip': '行程信息',
      'customer_confirmation_passengers': '乘客和行李',
      'customer_confirmation_vehicle_payment': '车辆和付款',
      'customer_confirmation_customer': '客户信息',
      'customer_confirmation_required': '确认预约前，请先检查预约内容。',
      'customer_next_action': '下一步',
      'customer_driver_assignment': '司机分配',
      'customer_driver_pending': '正在分配司机',
      'customer_status_unknown_guidance': '请查看最新预约状态。',
      'customer_rate_driver': '评价司机',
      'customer_review_thanks': '感谢您的评价。',
      'messenger_type': '通讯软件类型',
      'messenger_id': '通讯软件 ID',
      'messenger_type_hint': 'LINE、WhatsApp 等',
      'customer_contact_sns_notice':
          '预约完成后，我们会通过 SNS 提前联系您，告知车辆和司机等信息。如无法联系，预约可能会被取消，请务必填写可通过 SNS 添加好友的联系方式或 SNS 账号。',
      'contact_connect_title': '连接 Messenger',
      'contact_connect_booking_number': '预订号',
      'contact_connect_intro': '您的 T-Rider 预订已创建。请通过 Messenger 连接，以便接收司机和车辆更新。',
      'contact_connect_choose_channel': '选择 Messenger',
      'contact_connect_email_soon': '电子邮件（即将推出）',
      'contact_connect_after_launch': '发送包含预订参考号的消息后，请点击下方按钮。',
      'contact_connect_confirm_sent': '我已发送消息',
      'contact_connect_waiting': 'T-Rider 正在确认您的消息',
      'contact_connect_waiting_hint': '通常需要几分钟。请保持此页面打开。',
      'contact_connect_step_created': '预订已创建',
      'contact_connect_step_connecting': '连接中',
      'contact_connect_step_complete': '完成',
      'contact_connect_ref_copied': '预订参考号已复制',
      'contact_connect_launch_failed': '无法打开 Messenger 应用',
      'contact_connect_unsafe_url': '不允许使用该 Messenger 链接',
      'contact_connect_load_failed': '无法加载连接状态',
      'contact_connect_wechat_title': 'WeChat',
      'contact_connect_wechat_body': '扫描二维码或复制 WeChat ID，然后发送预订参考号。',
      'contact_connect_wechat_id_copied': 'WeChat ID 已复制',
      'contact_connect_invalid_link': '联系连接链接无效',
      'additional_requests': '附加要求',
      'country_code_hint': 'TH、US、KR…',
      'country_search_hint': '搜索国家或直接输入',
      'boarding_qr_title': '登车 QR',
      'boarding_qr_hint': '上车时请向司机出示此 QR 码。司机扫码后，行程状态将变更为已上车。',
      'booking_trust_message': '请保存预约编号。您可以在预约查询页面确认司机分配和行程状态。',
      'airport_meeting_semantics': 'BKK 机场接机指南，{gate} 号门',
      'airport_meeting_gate_3_title': '举牌接机指南 · 3 号门',
      'airport_meeting_gate_7_title': '车辆接机指南 · 7 号门',
      'airport_meeting_gate_inside': '门内',
      'airport_meeting_gate_outside': '门外',
      'airport_meeting_no_picket_highlight': '取到行李后，请先通知司机准备接您。',
      'airport_meeting_picket_highlight': '请不要走出机场，在 3 号门内等候并寻找姓名牌。',
      'airport_meeting_no_picket_body_1': '司机需要时间把车辆开到 7 号门外的上车区。',
      'airport_meeting_no_picket_body_2': '上车前请确认车牌号和司机。',
      'airport_meeting_picket_body_1': '取到行李后，请前往 3 号门内，寻找写有您姓名的接机牌。',
      'airport_meeting_picket_body_2': '工作人员会引导司机和车辆到 3 号门前。',
      'airport_meeting_step_luggage': '取齐所有行李。',
      'airport_meeting_step_notify_driver': '通知司机您已准备好上车。',
      'airport_meeting_step_vehicle_number': '确认收到的车牌号。',
      'airport_meeting_step_gate_7': '前往到达层 7 号门外。',
      'airport_meeting_step_board': '确认车辆和司机后上车。',
      'airport_meeting_step_gate_3': '前往到达层 3 号门内。',
      'airport_meeting_step_find_picket': '寻找写有您姓名的接机牌。',
      'airport_meeting_step_wait_staff': '按照工作人员指引稍候。',
      'airport_meeting_step_board_after_staff': '收到车辆到达提示后，再到门外上车。',
      'airport_meeting_do_not_exit': '在工作人员指引前，请不要走出机场。',
      'airport_meeting_vehicle_pending': '司机和车辆信息尚未分配，分配后会显示在这里。',
      'airport_meeting_vehicle_info': '司机和车辆信息',
      'airport_meeting_driver': '司机',
      'airport_meeting_driver_phone': '司机电话',
      'airport_meeting_vehicle_type': '车型',
      'airport_meeting_vehicle_color': '车辆颜色',
      'airport_meeting_vehicle_plate': '车牌号',
      'ui_load_failed': '无法加载数据，请重试。',
      'ui_action_failed': '操作失败，请重试。',
      'ui_generic_error': '出现问题，请重试。',
      'admin_ops_filter_date_from': '接送开始日期 (YYYY-MM-DD)',
      'admin_ops_filter_date_to': '接送结束日期 (YYYY-MM-DD)',
      'admin_ops_filter_date_from_invalid': '请选择有效的开始日期。',
      'admin_ops_filter_date_to_invalid': '请选择有效的结束日期。',
      'admin_ops_filter_date_range_invalid': '结束日期不能早于开始日期。',
      'ui_retry': '重试',
      'loading_recommendation': '正在加载推荐...',
      'calculating_price': '正在计算价格...',
      'guest_lookup_title': '查找我的预订',
      'guest_lookup_loading': '正在加载已保存的预订...',
      'guest_lookup_subtitle': '输入预订号和电话以查看行程详情。',
      'guest_lookup_booking_number': '预订号',
      'guest_lookup_phone': '电话号码',
      'guest_lookup_invalid_number': '请输入有效的预订号',
      'guest_lookup_invalid_phone': '请输入预订电话号码',
      'guest_lookup_find': '查找预订',
      'guest_lookup_not_found': '未找到预订，请检查预订号和电话。',
      'guest_lookup_load_error': '无法加载预订，请重试。',
      'guest_lookup_trip_details': '行程详情',
      'guest_lookup_pickup': '接送',
      'guest_lookup_service': '服务',
      'guest_lookup_from': '出发',
      'guest_lookup_to': '到达',
      'guest_lookup_driver': '司机',
      'guest_lookup_driver_phone': '司机电话',
      'guest_lookup_driver_status': '司机状态',
      'guest_lookup_vehicle': '车辆',
      'guest_lookup_vehicle_photo_load_failed': '无法加载车辆照片。',
      'guest_lookup_payment': '付款',
      'guest_lookup_total': '总计',
      'guest_lookup_another': '查找其他预订',
      'guest_lookup_refresh': '刷新',
      'guest_lookup_refresh_needs_phone': '请在查询表单中输入电话号码以刷新此预订。',
      'booking_complete_track_cta': '查看我的预订',
      'auth_social_login_title':
          'Create an account to make your next booking easier',
      'auth_social_login_description':
          'View booking history and auto-fill your details on future trips.',
      'auth_google_continue': 'Continue with Google',
      'auth_kakao_continue': '使用 Kakao 继续',
      'auth_kakao_callback_error': 'Kakao 登录失败，请重试。',
      'auth_kakao_callback_already_processed': '该登录请求已处理。如需请重新尝试。',
      'auth_kakao_callback_slow_loading': '处理时间较长，请刷新页面。',
      'auth_kakao_callback_refresh': '刷新页面',
      'profile_completion_title': '完善个人资料',
      'profile_completion_body': '请输入姓名和电话号码以继续使用 T-Rider。',
      'profile_completion_submit': '保存并继续',
      'profile_completion_name_required': '请输入姓名。',
      'profile_completion_phone_required': '请输入有效的电话号码（至少5位）。',
      'profile_completion_phone_duplicate': '该电话号码已被其他账户使用。',
      'auth_line_continue': '使用 LINE 继续',
      'auth_line_callback_error': 'LINE 登录失败，请重试。',
      'auth_line_callback_state_mismatch': '无法验证 LINE 登录，请重试。',
      'auth_line_callback_already_processed': '该登录请求已处理。如需请重新尝试。',
      'auth_line_callback_slow_loading': '处理时间较长，请刷新页面。',
      'auth_line_callback_refresh': '刷新页面',
      'auth_social_login_later': 'Maybe later',
      'auth_social_login_connected': '{name}, your account is connected.',
      'my_bookings_title': '我的预订',
      'my_bookings_empty': '暂无已关联的预订。完成预订并登录后可在此查看。',
      'my_bookings_view_button': '查看我的预订',
      'my_bookings_driver_summary': '司机：{name}',
      'my_bookings_load_error': '无法加载预订列表，请重试。',
      'booking_cancel_section': '取消预订',
      'booking_cancel_action': '取消预订',
      'booking_cancel_policy_hint': '可在上车前 2 小时取消。',
      'booking_cancel_confirm_title': '要取消此预订吗？',
      'booking_cancel_confirm_action': '确认取消',
      'booking_cancel_confirm_irreversible': '取消后无法恢复。',
      'booking_cancel_success': '您的预订已取消。',
      'booking_cancel_failed': '无法取消此预订，请重试。',
      'booking_cancel_blocked_within_two_hours': '距预定上车时间不足 2 小时时无法取消预订。',
      'booking_cancel_blocked_trip_started': '行程已开始的预订无法取消。',
      'booking_cancel_blocked_already_cancelled': '此预订已取消。',
      'booking_cancel_blocked_completed': '已完成的预订无法取消。',
      'booking_cancel_blocked_no_show': '标记为未到场的预订无法取消。',
      'booking_cancel_blocked_invalid_pickup': '上车时间无效，无法取消此预订。',
      'booking_cancel_blocked_locked': '此预订已锁定，无法取消。',
      'booking_cancel_driver_assigned': '已分配司机：{name}',
      'booking_cancel_driver_assigned_generic': '已分配司机',
      'guest_status_guidance_pending': '您的预订已收到，我们将尽快确认。',
      'guest_status_guidance_confirmed': '您的预订已确认，接客前将分配司机。',
      'guest_status_guidance_reassignment': '原定司机因故无法接送，正在为您重新安排司机。行程时间与费用不变。',
      'status_reassignment_in_progress': '正在重新安排司机',
      'admin_ops_severity_critical': '紧急',
      'admin_ops_reason_critical_reassignment': '紧急再派车',
      'admin_ops_reason_urgent_reassignment': '加急再派车',
      'admin_ops_reason_driver_released_reassignment': '司机退回 · 再派车',
      'admin_ops_reason_critical_unassigned': '紧急 · 未派车',
      'admin_ops_reason_urgent_unassigned': '加急 · 未派车',
      'driver_release_assignment': '退回派单',
      'driver_release_assignment_title': '要退回此预订的派单吗？',
      'driver_release_assignment_message': '客户预订将保留，并重新安排其他司机。',
      'driver_release_assignment_cancel': '保留派单',
      'driver_release_assignment_confirm': '退回派单',
      'driver_release_assignment_success': '派单已解除。',
      'driver_release_assignment_failed': '退回派单失败，请重试。',
      'driver_release_assignment_blocked': '当前状态无法退回派单。',
      'driver_release_assignment_emergency_hint':
          '距接客不足 2 小时时，普通退回派单受限制。仅紧急情况可退回。',
      'driver_release_assignment_irreversible': '退回后无法撤销。',
      'driver_release_reason_label': '退回原因',
      'driver_release_reason_detail_label': '详细说明',
      'driver_release_reason_detail_required': '请输入其他原因',
      'driver_release_reason_VEHICLE_BREAKDOWN': '车辆故障',
      'driver_release_reason_ACCIDENT': '事故',
      'driver_release_reason_DRIVER_ILLNESS': '司机身体不适',
      'driver_release_reason_FAMILY_EMERGENCY': '家庭紧急情况',
      'driver_release_reason_SCHEDULE_CONFLICT': '行程冲突',
      'driver_release_reason_LOCATION_TOO_FAR': '距离过远',
      'driver_release_reason_OTHER': '其他',
      'driver_assignment_ended_confirm': '确定',
      'driver_booking_not_found': '找不到预订信息。',
      'driver_assignment_ended_generic_title': '派单已结束',
      'driver_assignment_ended_generic_message': '无法继续进行此预订。',
      'driver_assignment_ended_generic_snackbar': '预订 {bookingNumber} 的派单已解除。',
      'driver_assignment_ended_customer_title': '客户已取消预订',
      'driver_assignment_ended_customer_message': '客户取消了预订，派单已自动解除。',
      'driver_assignment_ended_customer_snackbar':
          '客户取消了预订 {bookingNumber}，派单已解除。',
      'driver_assignment_ended_admin_title': '管理员已取消预订',
      'driver_assignment_ended_admin_message': '管理员取消了预订，派单已解除。',
      'driver_assignment_ended_admin_snackbar':
          '管理员取消了预订 {bookingNumber}，派单已解除。',
      'driver_assignment_ended_released_title': '已退回派单',
      'driver_assignment_ended_released_message': '派单退回已完成。该预订将重新开放给其他司机。',
      'driver_assignment_ended_released_snackbar':
          '您已退回预订 {bookingNumber}，已重新开放。',
      'driver_assignment_ended_reassigned_title': '已改派其他司机',
      'driver_assignment_ended_reassigned_message': '此预订已分配给其他司机。',
      'driver_assignment_ended_reassigned_snackbar':
          '预订 {bookingNumber} 已分配给其他司机。',
      'driver_assignment_ended_completed_title': '行程已完成',
      'driver_assignment_ended_completed_message': '此行程已完成。',
      'guest_status_guidance_driver_assigned': '司机已分配，即将前往接客地点。',
      'guest_status_guidance_on_route': '司机正在前往接客地点。',
      'guest_status_guidance_driver_arrived': '司机已到达接客地点。',
      'guest_status_guidance_picked_up': '行程正在进行中。',
      'guest_status_guidance_settlement_pending': '行程已结束，感谢您的乘坐。',
      'guest_status_guidance_completed': '行程已完成，请为司机评分。',
      'guest_status_guidance_cancelled': '此预订已取消。如需更多信息，请联系客服中心。',
      'guest_status_guidance_no_show': '此预订已标记为未出现。',
      'guest_lookup_issue_dropoff_qr': '签发下车 QR',
      'guest_lookup_trip_completed_qr_hidden': '行程已完成，QR 代码已隐藏。',
      'guest_lookup_dropoff_qr_token': '下车 QR 令牌：{token}',
      'guest_lookup_boarding_qr_security': '登车 QR 在创建预订时签发，出于安全原因无法在此恢复。',
      'guest_lookup_booking_number_hint': 'TX202607010001',
      'booking_trip_completed': '行程已完成',
      'booking_dropoff_qr_title': '行程完成 QR',
      'booking_dropoff_qr_hint': '下车时请再次向司机出示此 QR 码。司机扫码后，行程将标记为已完成。',
      'booking_dropoff_qr_unavailable': '下车 QR 在上车后、行程完成前可用。',
      'booking_qr_load_failed': '无法加载 QR 码。请检查预订状态或联系管理员。',
      'booking_refresh_dropoff_qr': '刷新下车 QR',
      'booking_issue_new_dropoff_qr': '签发新下车 QR',
      'booking_review_title': '评价行程',
      'booking_review_thanks': '感谢您的评价',
      'booking_review_rating': '评分：{rating} / 5',
      'booking_review_comment': '评论（可选）',
      'booking_review_submitting': '提交中...',
      'booking_review_submit': '提交评价',
      'booking_review_load_error': '无法加载评价',
      'review_card_title': '本次乘车体验如何？',
      'review_card_subtitle': '请选择评分',
      'review_star_label': '{value} 星',
      'review_rating_desc_1': '非常不满意',
      'review_rating_desc_2': '不满意',
      'review_rating_desc_3': '一般',
      'review_rating_desc_4': '满意',
      'review_rating_desc_5': '非常满意',
      'review_comment_label': '请留下意见（选填）',
      'review_comment_hint': '请勿填写个人信息。',
      'review_submit_button': '提交评分',
      'review_success_title': '感谢您的评价',
      'review_success_body': '您的反馈将帮助我们改进服务',
      'review_tag_FRIENDLY': '服务友好',
      'review_tag_SAFE_DRIVING': '驾驶安全',
      'review_tag_CLEAN_VEHICLE': '车辆干净',
      'review_tag_ON_TIME': '准时到达',
      'review_tag_GOOD_COMMUNICATION': '沟通顺畅',
      'review_tag_UNSAFE_DRIVING': '驾驶不安全',
      'review_tag_LATE_ARRIVAL': '到达较晚',
      'review_tag_VEHICLE_NOT_CLEAN': '车辆不干净',
      'review_tag_UNFRIENDLY_SERVICE': '服务态度不好',
      'review_tag_ROUTE_ISSUE': '路线有问题',
      'review_tag_OTHER_ISSUE': '其他问题',
      'booking_notification_updates': '更新',
      'booking_notification_enabling': '启用中...',
      'booking_notification_enable': '启用通知',
      'booking_notification_empty': '暂无更新',
      'booking_notification_default_title': '通知',
      'booking_notification_load_error': '无法加载通知',
      'booking_notification_enabled': '通知已启用',
      'booking_notification_permission_denied': '通知权限被拒绝',
      'booking_notification_unsupported': '此浏览器不支持推送通知',
      'booking_notification_config_missing': '此环境未配置推送通知',
      'booking_notification_register_failed': '通知注册失败',
      'track_driver_title': '追踪司机',
      'track_driver_unavailable': '司机位置暂不可用。',
      'track_driver_stale': '位置可能已过期',
      'track_driver_live': '实时位置',
      'track_driver_updated': '更新 {time}',
      'track_driver_load_error': '无法加载司机位置',
      'customer_driver_location_title': '司机位置',
      'customer_driver_location_waiting': '司机已分配。司机开始前往接送地点后，您可以查看实时位置。',
      'customer_driver_location_live': '正在共享司机位置',
      'customer_driver_location_updated_label': '最后更新',
      'customer_driver_location_updated_now': '刚刚更新',
      'customer_driver_location_updated_minutes': '{minutes} 分钟前更新',
      'customer_driver_location_temporarily_unavailable': '暂时无法查看司机位置。请稍后再试。',
      'customer_driver_location_stale': '司机位置最近没有更新。',
      'customer_driver_location_ended': '实时位置共享已结束。',
      'customer_driver_location_open_map': '在地图中打开',
      'customer_driver_location_recenter': '移动到司机位置',
      'customer_driver_on_route': '司机正在前往接送地点。',
      'customer_driver_arrived': '司机已到达接送地点。请确认司机和车辆信息。',
      'customer_trip_in_progress': '行程进行中。',
      'customer_driver_location_load_failed': '无法查看司机位置。请稍后重试。',
      'customer_driver_location_retry': '重试',
      'wizard_step_progress': '第 {current} 步，共 {total} 步',
      'pickup_date_past': '接送日期不能是过去',
      'pickup_time_minimum': '接送时间须晚于当前时间',
      'pickup_time_am': '上午',
      'pickup_time_pm': '下午',
      'pickup_time_select': '选择接送时间',
      'pickup_time_enter_manually': '手动输入时间',
      'pickup_time_invalid': '请输入有效时间，例如 09:30 AM',
      'pickup_time_apply_manual': '应用',
      'field_required': '（必填）',
      'name_sign_info_before_price': '选择举牌服务后，将额外收取 ',
      'name_sign_info_after_price': '，接送门位置将变更为 ',
      'name_sign_info_after_gate': '。',
      'name_sign_service_price_highlight': '100 泰铢',
      'name_sign_service_gate_highlight': '3 号门',
      'name_sign_text_label': '举牌上显示的姓名',
      'name_sign_text_hint': '例如：WANG WEI',
      'wizard_required_name_sign_text': '请输入举牌上显示的姓名。',
      'wizard_customer_email_invalid': '请输入有效的电子邮箱地址。',
      'pickup_datetime_required': '请填写接送日期和时间',
      'booking_location_reselect_required': '无法获取地点坐标，请重新选择出发地/目的地。',
      'flight_number_hint': 'TG401, 7C2203',
      'flight_number_invalid': '请检查航班号。例如：TG401, 7C2203',
      'flight_lookup_search': '查询',
      'flight_lookup_not_found': '未找到该航班号。请再次确认航班号。',
      'flight_lookup_invalid_date': '当前接送日期无法查询航班。您仍可继续预订并保留已输入的航班号。',
      'flight_lookup_provider_unavailable': '航班查询服务暂时不可用。您仍可继续预订并保留已输入的航班号。',
      'flight_lookup_departure': '出发',
      'flight_lookup_arrival': '到达',
      'flight_lookup_estimated_arrival': '预计到达',
      'flight_lookup_confirm': '这是我的航班',
      'booking_number_copied': '预订号已复制',
      'booking_number_copy_failed': '无法复制预订号',
      'booking_number_copy': '复制预订号',
      'booking_number_save_notice': '请保存您的预订号，查询预订时需要使用。',
      'wizard_edit_section': '修改',
      'wizard_section_incomplete': '请先完成此步骤后再继续。',
      'wizard_completion_progress': '必填项 {completed}/{total} 已完成',
      'wizard_required_service': '请选择服务类型。',
      'wizard_step_route_title': '路线',
      'wizard_step_schedule_title': '日期与时间',
      'wizard_step_vehicle_title': '车辆选择',
      'wizard_step_customer_title': '乘客信息',
      'wizard_step_review_title': '确认预约',
      'wizard_step_indicator': '{current}/{total} {title}',
      'wizard_cta_route': '选择日期和时间',
      'wizard_cta_schedule': '选择车辆',
      'wizard_cta_vehicle_unselected': '请选择车辆',
      'wizard_cta_vehicle_selected': '填写乘客信息 · {price}',
      'wizard_cta_customer': '确认预约内容 · {price}',
      'wizard_cta_confirm': '确认预约 · {price}',
      'wizard_same_place_error': '出发地和目的地不能相同。',
      'wizard_swap_route': '交换出发地/目的地',
      'wizard_trust_toll_included': '含高速公路通行费',
      'wizard_trust_no_airport_parking': '无机场停车费',
      'wizard_trust_no_night_surcharge': '无深夜/凌晨附加费',
      'wizard_trust_flight_delay_wait': '航班延误时免费等待',
      'wizard_places_searching': '正在搜索地点…',
      'wizard_places_no_results_hint': '未找到地点。请尝试其他关键词或在地图上选择。',
      'wizard_summary_expand': '展开预约摘要',
      'wizard_summary_collapse': '收起预约摘要',
      'wizard_required_origin': '请选择出发地。',
      'wizard_required_destination': '请选择目的地。',
      'wizard_required_passengers': '请输入乘客信息。',
      'wizard_required_vehicle': '请选择车辆。',
      'wizard_required_customer': '请输入您的信息。',
      'wizard_required_customer_name': '请输入您的姓名。',
      'wizard_required_customer_phone': '请输入电话号码。',
      'select_on_map': '在地图上选择',
      'map_picker_title': '在地图上选择位置',
      'map_picker_hint': '点击地图放置标记。',
      'map_picker_confirm': '使用此位置',
      'current_location': '我的位置',
      'locating_current_position': '正在获取当前位置。',
      'location_permission_required': '使用当前位置需要位置权限。',
      'location_permission_denied': '位置权限已被拒绝。',
      'location_permission_permanently_denied': '请在浏览器或设备设置中允许位置权限。',
      'location_service_disabled': '设备的位置服务已关闭。',
      'location_unavailable': '无法获取当前位置。',
      'location_timeout': '获取位置超时，请重试。',
      'location_requires_https': '当前环境无法使用位置功能，请通过 HTTPS 地址重试。',
      'low_location_accuracy': '位置精度较低，请在地图上调整准确位置。',
      'wizard_vehicle_prerequisites': '请先填写服务、出发地、目的地、接送时间和乘客信息。',
      'wizard_pricing_after_vehicle': '选择车辆后将计算价格。',
      'admin_dispatch_retry': '重试',
      'admin_dashboard': '仪表板',
      'admin_reservations': '预订管理',
      'admin_drivers': '司机管理',
      'admin_pricing': '车辆定价',
      'admin_golf': '高尔夫球场',
      'admin_airports': '机场',
      'admin_notifications': '通知',
      'admin_translations': '翻译',
      'admin_settings': '设置',
      'today_bookings': '今日预订',
      'today_revenue': '今日收入',
      'pending_count': '待处理',
      'awaiting_driver': '等待司机',
      'vehicle': '车辆',
      'amount': '金额',
      'send': '发送',
      'type_message': '输入消息...',
      'other_airport': '其他泰国机场',
      'booking_summary': '预订摘要',
      'service_type': '服务类型',
      'thb': 'THB',
    },
    'ja': {
      'app_title': 'T-Rider',
      'app_subtitle': 'タイ空港送迎サービス',
      'airport_pickup': '空港ピックアップ',
      'airport_pickup_desc': '空港到着後、宿泊先まで快適にお送りします',
      'airport_dropoff': '空港送迎',
      'airport_dropoff_desc': '宿泊先から空港まで余裕を持ってご出発いただけます',
      'city_transfer': '市内送迎',
      'city_transfer_desc': '都市間の移動も快適に',
      'golf_transfer': 'ゴルフ送迎',
      'golf_transfer_desc': 'ゴルフバッグもゆったり収納できます',
      'select_service': 'サービスを選択',
      'passengers': '人数',
      'adults': '大人 (12歳以上)',
      'children': '子供',
      'luggage': '荷物',
      'small_carriers': '20インチキャリー',
      'large_carriers': '24インチ以上キャリー',
      'golf_bags': 'ゴルフバッグ',
      'special_luggage': '特殊荷物',
      'name_sign': '名前看板サービス (+100 THB)',
      'booking_prefer_female_driver': '女性ドライバー希望',
      'booking_preference_disclaimer': '配車の際できる限り反映いたしますが、100%保証するものではありません。',
      'next': '次へ',
      'back': '戻る',
      'confirm': '予約確定',
      'origin': '出発地',
      'destination': '目的地',
      'airport': '空港',
      'flight_number': '便名',
      'date': '日付',
      'time': '時間',
      'pickup_datetime': 'ピックアップ日時',
      'admin_pickup_requested_at': '出発リクエスト時間',
      'pickup_date': 'ピックアップ日',
      'pickup_time': 'ピックアップ時間',
      'pickup_minimum_notice': '現在時刻から2時間以上後のタイ現地時間を選択してください。',
      'select_vehicle': '車両選択',
      'recommended': 'おすすめ',
      'total': '合計',
      'special_requests': '特別リクエスト',
      'customer_info': 'お客様情報',
      'name': '名前',
      'email': 'メール',
      'phone': '電話',
      'country': '国',
      'booking_complete': '予約完了',
      'reservation_number': '予約番号',
      'booking_number_label': '予約番号',
      'booking_with_number': '予約 {bookingNumber}',
      'booking_detail_view': '予約詳細を見る',
      'notification_type_booking': '予約',
      'notification_type_pickup': 'ピックアップ',
      'notification_type_settlement': '精算',
      'notification_type_system': 'システム',
      'status': 'ステータス',
      'golf_region': 'ゴルフエリア',
      'golf_course': 'ゴルフ場',
      'driver_included': 'ドライバー込み',
      'status_pending': '保留中',
      'status_confirmed': '確認済み',
      'status_driver_assigned': 'ドライバー割当',
      'status_completed': '完了',
      'status_cancelled': 'キャンセル',
      'status_on_route': '迎えに向かっています',
      'status_driver_arrived': '迎え場所に到着',
      'status_no_show': 'ノーショー',
      'status_picked_up': 'お客様乗車済み',
      'status_customer_settlement_pending': '運行終了',
      'search_place': '場所を検索...',
      'no_results': '場所が見つかりません',
      'infants': '乳幼児',
      'multiple_vehicles_required': '複数台必要',
      'base_price': '基本料金',
      'additional_charges': '追加料金',
      'pricing_summary': '料金概要',
      'pricing_name_sign_with_picket': 'ネームボードサービス',
      'vehicle_not_available': 'この行程では利用不可',
      'pricing_inquiry_required': 'お問い合わせ',
      'complete_passengers_first': '先に乗客情報を入力してください',
      'yes': 'はい',
      'change_location': '変更',
      'loading_place_details': '場所の詳細を読み込み中...',
      'airport_shortcuts': '空港ショートカット',
      'airport_shortcuts_origin': '空港ショートカット',
      'airport_shortcuts_destination': '空港ショートカット',
      'recent_non_airport_places': '最近の場所',
      'recent_locations': '最近の場所',
      'landing_hero_eyebrow': 'タイのプロ空港・市内送迎',
      'landing_hero_title': 'タイで始まる快適な旅 T-Rider',
      'landing_hero_body': '経験豊富なドライバーが、空港から目的地まで安全・快適にお送りします。',
      'landing_hero_cta': '予約する',
      'landing_support_cta': 'カスタマーセンター',
      'landing_support_one_click_cta': 'カスタマーセンターでワンクリック予約',
      'landing_hero_helper': '会員登録不要、かんたん予約',
      'landing_hero_subtitle': '安全でスムーズな旅をお約束します。',
      'landing_highlight_fixed_price': '定額料金',
      'landing_highlight_fixed_price_desc': '予約時の料金がそのまま最終料金、追加料金なし',
      'landing_highlight_flight': 'フライト追跡',
      'landing_highlight_flight_desc': 'フライト遅延時もピックアップ時間を自動調整',
      'landing_highlight_support': '24時間サポート',
      'landing_highlight_support_desc': '予約前後いつでもサポートを受けられます',
      'book_your_ride': '予約する',
      'landing_services_title': 'サービス',
      'landing_services_subtitle': '空港・都市間送迎',
      'service_badge_popular': '人気No.1',
      'landing_trust_title': '安心してご利用ください',
      'landing_trust_drivers_title': '経験豊富なドライバー',
      'landing_trust_drivers_desc': 'タイの道路と空港送迎に精通したドライバーが担当します。',
      'landing_trust_confirmed_title': '事前確認の予約',
      'landing_trust_confirmed_desc': 'ピックアップ情報と行程を事前に確認できます。',
      'landing_trust_comfort_title': '快適な移動',
      'landing_trust_comfort_desc': '移動の心配なく、旅をお楽しみください。',
      'landing_trust_refund_title': '安心キャンセル・返金',
      'landing_trust_refund_desc':
          'ご予約確定後24時間以内であれば、違約金なしで無料キャンセルが可能です。また、同条件(車種・日程)でより安い確定見積もりを見つけられた場合、ご予約後48時間以内にお問い合わせいただければ差額を返金いたします。',
      'landing_trust_driver_policy_title': 'ドライバー品質管理',
      'landing_trust_driver_policy_desc':
          'ドライバーに関するご不満は、予約詳細画面またはカカオトーク/電話でご報告いただけます。ご報告後24時間以内にご連絡いたします。安全上の脅威など重大な事案については、直ちに配車を停止いたします。',
      'landing_how_it_works_title': 'ご利用の流れ',
      'landing_how_step_1_title': '移動情報を入力',
      'landing_how_step_1_desc': 'サービス、ピックアップ時間、目的地を選びます。',
      'landing_how_step_2_title': '車両と料金を確認',
      'landing_how_step_2_desc': 'おすすめ車両と料金を確認してから予約します。',
      'landing_how_step_3_title': '快適に移動',
      'landing_how_step_3_desc': 'ドライバーが安全に目的地までお送りします。',
      'landing_reassurance_body': 'タイが初めてでもご安心ください。予約から到着まで、必要な情報を丁寧にお伝えします。',
      'landing_booking_lookup_title': 'すでに予約済みですか？',
      'landing_booking_lookup_desc': '予約番号で行程とドライバー情報を確認できます。',
      'landing_booking_lookup_action': '予約を検索',
      'landing_footer_note': 'タイ全土のプロ空港・市内送迎サービス',
      'landing_login_title': 'ログイン',
      'landing_login_sheet_title': 'T-Rider にログイン',
      'landing_login_prompt_cta': 'ログインしてマイルを貯める',
      'landing_login_sheet_close': '閉じる',
      'landing_login_support_prompt': 'ログインでお困りですか？',
      'landing_login_support_link': 'カスタマーサポート',
      'landing_header_my_bookings_menu': '予約一覧を見る',
      'landing_header_logout_menu': 'ログアウト',
      'landing_header_logout_success': 'ログアウトしました',
      'landing_logged_in_greeting': '{name} さん、ログイン済みです',
      'landing_my_bookings_button': '予約一覧',
      'landing_logout_button': 'ログアウト',
      'auth_login_provider_hint':
          'マイルの管理のため、常に同じログイン方法（Google、Kakao、LINEのいずれか）をご利用ください。',
      'landing_header_account_menu': 'マイページ',
      'account_page_title': 'マイページ',
      'account_menu_section_title': 'アカウント',
      'account_mileage_label': 'マイル',
      'account_mileage_balance': '{balance}P',
      'account_mileage_loading': '残高を読み込み中...',
      'account_mileage_load_failed': '読み込みに失敗しました',
      'account_mileage_retry': '再試行',
      'account_my_bookings_menu': '予約一覧',
      'account_logout_menu': 'ログアウト',
      'account_logout_cancel': 'キャンセル',
      'account_logout_confirm_message': 'ログアウトしますか？',
      'account_login_provider_google': 'Googleでログイン',
      'account_login_provider_kakao': 'Kakaoでログイン',
      'account_login_provider_line': 'LINEでログイン',
      'account_sign_in_required': 'マイページを表示するにはログインしてください。',
      'account_booking_counts_title': '予約履歴',
      'account_booking_count_waiting': '予約待ち',
      'account_booking_count_assigned': '配車完了',
      'account_booking_count_in_progress': '利用中',
      'account_booking_count_settlement_pending': '精算待ち',
      'account_booking_count_completed': '利用完了',
      'account_booking_count_review_pending': 'レビュー待ち',
      'account_coupon_menu': 'クーポン',
      'account_coupons_title': 'クーポン',
      'account_coupons_available_section': '利用可能なクーポン',
      'account_coupons_used_section': '使用済みクーポン',
      'account_coupons_empty_available': '利用可能なクーポンはありません',
      'account_coupons_empty_used': '使用済みクーポンはありません',
      'account_coupons_load_failed': 'クーポンを読み込めませんでした',
      'account_coupons_issued_at': '発行日',
      'account_coupons_used_at': '使用日',
      'account_coupons_used_booking': '予約番号',
      'booking_coupon_section_title': 'クーポン利用',
      'booking_coupon_none': 'クーポンを使わない',
      'booking_coupon_discount_label': 'クーポン割引',
      'booking_mileage_section_title': 'マイル利用',
      'booking_mileage_discount_label': 'マイル割引',
      'booking_mileage_available': '保有マイル：{balance}ポイント',
      'booking_mileage_use_all': '全額利用',
      'admin_coupons_menu': 'クーポン発行',
      'admin_coupon_issue_title': 'クーポン発行',
      'admin_coupon_search_hint': '電話番号、メール、名前で検索',
      'admin_coupon_search_empty': '該当する顧客がいません',
      'admin_coupon_select_customer': '顧客を選択',
      'admin_coupon_title_label': 'クーポン名',
      'admin_coupon_amount_label': '割引額',
      'admin_coupon_issue_button': 'クーポン発行',
      'admin_coupon_issue_success': 'クーポンを発行しました',
      'admin_coupon_issue_failed': 'クーポンの発行に失敗しました',
      'admin_coupon_recent_title': '最近発行したクーポン',
      'admin_coupon_recent_empty': '発行済みクーポンはありません',
      'admin_coupon_cancel': '取消',
      'admin_coupon_cancel_success': 'クーポンを取消しました',
      'admin_coupon_status_available': '利用可能',
      'admin_coupon_status_used': '使用済み',
      'admin_coupon_recent_customers_title': '最近利用したお客様',
      'admin_coupon_recent_customers_empty': '完了した予約がありません',
      'admin_coupon_selected_customer': '選択中のお客様',
      'admin_coupon_template_picker_title': 'クーポンテンプレート',
      'admin_coupon_template_manual': '手動入力',
      'admin_coupon_template_manage_title': 'テンプレート管理',
      'admin_coupon_template_create_button': 'テンプレート追加',
      'admin_coupon_template_create_success': 'テンプレートを作成しました',
      'admin_coupon_template_create_failed': 'テンプレート作成に失敗しました',
      'admin_coupon_template_active': '有効',
      'admin_coupon_template_inactive': '無効',
      'admin_home_banner_menu': 'ホームバナー管理',
      'admin_manual_booking_menu': '管理者手動コール登録',
      'admin_manual_booking_title': '管理者手動コール登録',
      'admin_manual_booking_origin': '出発地',
      'admin_manual_booking_destination': '目的地',
      'admin_manual_booking_pickup_datetime': 'ピックアップ日時',
      'admin_manual_booking_pickup_datetime_hint': 'ピックアップ日時を選択してください',
      'admin_manual_booking_pickup_timezone': 'タイ現地時間 (UTC+7)',
      'admin_manual_booking_special_items_text': '特殊荷物メモ',
      'admin_manual_booking_special_items_hint': '大型・特殊荷物を記入してください',
      'admin_manual_booking_flight_pickup_date_required':
          'フライト検索の前にピックアップ日を選択してください',
      'admin_manual_booking_flight_manual_fallback_hint':
          'フライト検索に失敗した場合は、便名を入力しピックアップ時間を手動で設定してください。',
      'admin_manual_booking_flight_pickup_conflict_warning':
          '入力したピックアップ時間とフライト到着予定時間が {minutes} 分異なります。',
      'admin_manual_booking_flight_pickup_keep_manual': '入力した時間を使用',
      'admin_manual_booking_flight_pickup_use_flight': 'フライト到着時間に変更',
      'admin_manual_booking_flight_pickup_final_label': '最終ピックアップ希望時間',
      'admin_dispatch_driver_pickup_conflict':
          'ピックアップ時間がこのドライバーの他の割当と重なります',
      'admin_manual_booking_vehicle_type': '車両クラス',
      'admin_manual_booking_passengers': '乗客人数',
      'admin_manual_booking_name_sign_hint':
          'ドライバーアプリでネームボード（ピケット）必要として表示されます。',
      'admin_manual_booking_payout_amount': 'ドライバー支払額 (THB)',
      'admin_manual_booking_customer_charge_amount': '顧客支払額 (THB)',
      'admin_manual_booking_customer_charge_hint': '任意。管理者記録用の顧客実支払額です。',
      'admin_manual_booking_payment_method': '支払い回収方法',
      'admin_manual_booking_payment_admin_collected': '管理者が既に受領済み',
      'admin_manual_booking_payment_driver_collects': '現地でドライバーが直接回収',
      'admin_manual_booking_customer_section': '顧客情報',
      'admin_manual_booking_guest_name': 'ゲスト名',
      'admin_manual_booking_guest_phone': 'ゲスト電話番号',
      'admin_manual_booking_guest_email': 'ゲストメール（任意）',
      'admin_manual_booking_clear_customer': '選択顧客を解除',
      'admin_manual_booking_memo': 'メモ（任意）',
      'admin_manual_booking_submit': 'オープンコール作成',
      'admin_manual_booking_success': 'オープンコールを作成しました。予約番号：',
      'admin_manual_booking_view_detail': '予約詳細',
      'admin_manual_booking_assign_driver': 'ドライバー割当',
      'admin_manual_booking_edit': '編集',
      'admin_manual_booking_edit_title': '管理者手動コール編集',
      'admin_manual_booking_edit_not_allowed': '現在の状態では編集できません',
      'admin_manual_booking_cancel': 'コール取消',
      'admin_manual_booking_cancel_confirm':
          'この管理者手動コールを取消しますか？割当ドライバーは解除されます。',
      'admin_manual_booking_cancel_success': '管理者手動コールを取消しました',
      'admin_manual_booking_assign_success': 'ドライバーを割当しました',
      'admin_manual_booking_update_submit': '変更を保存',
      'admin_manual_booking_update_success': '管理者手動コールを更新しました',
      'admin_manual_booking_failed': '管理者手動コールの作成に失敗しました',
      'admin_manual_booking_validation_required': '必須項目を入力してください',
      'admin_manual_booking_validation_customer': '会員を選択するか、ゲスト名と電話番号を入力してください',
      'admin_manual_booking_preview_notice': 'ドライバー画面プレビュー：口座確認案内が表示されます',
      'driver_call_badge_admin_manual': '管理者登録コール',
      'driver_call_badge_no_commission': 'コミッションなし',
      'driver_call_bank_account_confirm_notice':
          '会員登録時に登録した口座へ支払われます。口座情報が正しいか再度ご確認ください。',
      'admin_home_banner_title': 'ホームバナー管理',
      'admin_home_banner_order_label': '表示順',
      'admin_home_banner_order_hint': '空欄の場合は末尾に追加',
      'admin_home_banner_upload_button': 'バナーをアップロード',
      'admin_home_banner_create_success': 'バナーをアップロードしました',
      'admin_home_banner_create_failed': 'バナーのアップロードに失敗しました',
      'admin_home_banner_empty': 'バナーがありません',
      'admin_home_banner_active': '有効',
      'admin_home_banner_inactive': '無効',
      'admin_home_banner_delete': '削除',
      'admin_home_banner_delete_success': 'バナーを削除しました',
      'admin_home_banner_move_up': '上へ',
      'admin_home_banner_move_down': '下へ',
      'account_feature_coming_soon': '準備中です',
      'account_recent_bookings_title': '最近の利用履歴',
      'account_recent_bookings_empty': '最近の利用履歴はありません',
      'account_recent_bookings_load_failed': '最近の利用履歴を読み込めませんでした',
      'account_my_reviews_menu': '投稿したレビュー',
      'account_customer_support_menu': 'カスタマーセンター',
      'account_mileage_history_title': 'マイル履歴',
      'account_mileage_history_empty': 'マイル履歴はありません',
      'account_mileage_history_load_failed': 'マイル履歴を読み込めませんでした',
      'account_my_reviews_title': '投稿したレビュー',
      'account_my_reviews_empty': '投稿したレビューはありません',
      'account_my_reviews_load_failed': 'レビューを読み込めませんでした',
      'account_mileage_type_accrue': '獲得',
      'account_mileage_type_reversal': '取消',
      'landing_privacy_policy_link': 'プライバシーポリシー',
      'privacy_policy_title': 'プライバシーポリシー',
      'privacy_policy_load_failed': 'プライバシーポリシーを読み込めませんでした。しばらくしてから再度お試しください。',
      'analytics_consent_title': '分析 Cookie',
      'analytics_consent_body':
          'Google Analytics で訪問と予約コンバージョンを測定します。氏名・電話・メールは送信しません。プライバシー設定でいつでも変更できます。',
      'analytics_consent_allow': '分析を許可',
      'analytics_consent_deny': '拒否',
      'analytics_consent_privacy': 'プライバシーポリシー',
      'analytics_consent_settings': '分析設定',
      'analytics_consent_settings_title': '分析同意設定',
      'landing_language_label': '言語',
      'support_title': 'カスタマーセンター',
      'support_contact_channels_title': 'カスタマーセンター連絡',
      'support_contact_channels_hint':
          'ご希望のメッセンジャーアイコンをタップしてすぐに相談を始めてください',
      'support_wechat_qr_dialog_title': 'WeChat QRコード',
      'support_wechat_qr_dialog_hint':
          'WeChatアプリでQRコードをスキャンして友だち追加してください',
      'support_page_intro':
          '予約が難しい場合やお問い合わせが必要な場合は、カスタマーセンターへご連絡ください。航空券、ホテルバウチャー、予約画面の画像も添付できます。',
      'support_inquiry_button': '問い合わせる',
      'support_dialog_title': 'カスタマーセンターお問い合わせ',
      'support_close_button': '閉じる',
      'support_inquiry_title': 'カスタマーセンターでの予約・お問い合わせ',
      'support_inquiry_subtitle': '予約相談、フライト情報、ピックアップ場所、目的地をお送りください。',
      'support_default_guide':
          'こんにちは。T-Ride カスタマーセンターです。予約相談、フライト情報、ピックアップ場所、目的地をお送りいただければ、確認後にご案内します。',
      'support_input_label': 'お問い合わせ内容',
      'support_input_hint': '予約または移動に関するお問い合わせを入力してください。',
      'support_attach_button': '画像を添付',
      'support_send_button': '送信',
      'support_sending': '送信中...',
      'support_receipt_number': '受付番号',
      'support_submit_failed': 'お問い合わせを送信できませんでした。しばらくしてからもう一度お試しください。',
      'support_attachment_help': '航空券画像、ホテルバウチャー、予約画面のスクリーンショットなどを添付できます。',
      'support_no_attachments': '添付ファイルはありません',
      'support_auto_receipt': '自動受付通知：お問い合わせを受け付けました。カスタマーセンターで確認後にご案内します。',
      'support_faq_title': 'よくある質問',
      'support_faq_placeholder': 'よくある質問は、お問い合わせデータが蓄積され次第、優先度に応じて更新されます。',
      'trust_flight_delay': 'フライト遅延モニタリング',
      'trust_flight_delay_desc': 'フライト変更時にピックアップ時間を自動更新',
      'trust_fixed_pricing': '定額料金',
      'trust_fixed_pricing_desc': '予約時の料金がそのまま最終料金です',
      'trust_professional_drivers': 'プロドライバー',
      'trust_professional_drivers_desc': '認定されたプロドライバーが安全にお送りします',
      'trust_multilingual': '多言語サポート',
      'trust_multilingual_desc': '英語・韓国語・中国語・日本語・タイ語で予約できます',
      'payment_method': '支払方法',
      'pay_driver_at_destination': '到着地でドライバーに直接支払い',
      'customer_review_booking': '予約内容を確認',
      'customer_confirm_booking': '予約を確定',
      'customer_booking_processing': '予約を処理しています',
      'customer_urgent_pickup_hint': 'ピックアップ時間が2時間以内の場合は、緊急リクエストをタップして進めてください。',
      'customer_urgent_request': '緊急リクエスト',
      'customer_urgent_confirm_title': '緊急リクエストの確認',
      'customer_urgent_confirm_body':
          '緊急リクエストは配車に時間がかかる場合があります。配車が完了すると、まずドライバーの到着予定時刻をお知らせします。配車を希望される場合は確定を押してください。',
      'customer_urgent_confirm_submit': '確定',
      'customer_urgent_flow_title': '緊急リクエスト',
      'customer_urgent_searching_title': 'ドライバーを探しています',
      'customer_urgent_searching_body': '近くで対応可能なドライバーを探しています。',
      'customer_urgent_eta_title': 'ドライバーは約{minutes}分後に到着予定です',
      'customer_urgent_eta_body': '到着時間で問題なければ承認、より早いドライバーを希望する場合は拒否を押してください。',
      'customer_urgent_accept_eta': '承認',
      'customer_urgent_reject_eta': '拒否',
      'customer_urgent_retry_title': 'より早いドライバーを探しますか？',
      'customer_urgent_retry_body': '到着時間が短い別のドライバーを引き続き探せます。',
      'customer_urgent_retry_accept': '探す',
      'customer_urgent_retry_cancel': '予約をキャンセル',
      'customer_urgent_confirmed_title': '配車が確定しました',
      'customer_urgent_confirmed_body': 'ドライバーが割り当てられました。予約詳細画面を開いています。',
      'customer_urgent_exhausted_title': '近くに待機中のドライバーがいません',
      'customer_urgent_exhausted_body': '近くに待機中のドライバーがいません。予約をキャンセルします。',
      'customer_urgent_go_home': 'ホームへ',
      'selected_location': '選択した場所',
      'customer_payment_method': '支払方法',
      'customer_payment_pay_driver': '目的地到着後にドライバーへ直接支払い',
      'customer_payment_cash': '現金払い',
      'customer_payment_qr': 'QR決済',
      'customer_payment_card': 'カード決済',
      'customer_payment_bank_transfer': '銀行振込',
      'customer_payment_unknown': '支払方法を確認できません',
      'customer_price_conditions': '通行料、空港駐車料金、追加待機料金は予約条件により変動する場合があります。',
      'customer_luggage_guidance':
          '大型スーツケースやゴルフバッグの数を正確に入力してください。実際の積載可否は割り当て車両により異なります。',
      'customer_vehicle_guidance': '乗客または荷物が多い場合、推奨より小さい車両は選択できないことがあります。',
      'customer_vehicle_not_suitable': '選択した乗客または荷物条件には適していません。',
      'customer_confirmation_title': '予約の最終確認',
      'customer_confirmation_subtitle': '予約を確定する前に内容をご確認ください。',
      'customer_confirmation_edit': '編集',
      'customer_confirmation_trip': '行程情報',
      'customer_confirmation_passengers': '乗客・荷物',
      'customer_confirmation_vehicle_payment': '車両・支払い',
      'customer_confirmation_customer': 'お客様情報',
      'customer_confirmation_required': '予約を確定する前に内容をご確認ください。',
      'customer_next_action': '次の案内',
      'customer_driver_assignment': 'ドライバー割当',
      'customer_driver_pending': 'ドライバー割当中',
      'customer_status_unknown_guidance': '最新の予約状況をご確認ください。',
      'customer_rate_driver': 'ドライバーを評価',
      'customer_review_thanks': 'レビューありがとうございます。',
      'messenger_type': 'メッセンジャー種類',
      'messenger_id': 'メッセンジャー ID',
      'messenger_type_hint': 'LINE、WhatsApp など',
      'customer_contact_sns_notice':
          '予約完了後、車両やドライバーなどの情報をお知らせするため、事前にSNSでご連絡します。連絡が取れない場合、予約がキャンセルされることがあります。SNSで友だち追加が可能な連絡先またはSNSアカウントを必ずご記入ください。',
      'contact_connect_title': 'Messenger 連携',
      'contact_connect_booking_number': '予約番号',
      'contact_connect_intro':
          'T-Rider の予約が作成されました。ドライバーと車両の案内を受けるため Messenger で連携してください。',
      'contact_connect_choose_channel': 'Messenger を選択',
      'contact_connect_email_soon': 'メール（準備中）',
      'contact_connect_after_launch': '予約参照番号を含むメッセージを送信した後、下のボタンを押してください。',
      'contact_connect_confirm_sent': 'メッセージを送信しました',
      'contact_connect_waiting': 'T-Rider がメッセージを確認しています',
      'contact_connect_waiting_hint': '通常数分かかります。この画面を開いたままお待ちください。',
      'contact_connect_step_created': '予約作成',
      'contact_connect_step_connecting': '連携中',
      'contact_connect_step_complete': '完了',
      'contact_connect_ref_copied': '予約参照番号をコピーしました',
      'contact_connect_launch_failed': 'Messenger アプリを開けませんでした',
      'contact_connect_unsafe_url': 'このメッセンジャーリンクは許可されていません',
      'contact_connect_load_failed': '連携状態を読み込めませんでした',
      'contact_connect_wechat_title': 'WeChat',
      'contact_connect_wechat_body':
          'QR コードをスキャンするか WeChat ID をコピーし、予約参照番号を送信してください。',
      'contact_connect_wechat_id_copied': 'WeChat ID をコピーしました',
      'contact_connect_invalid_link': '連携リンクが無効です',
      'additional_requests': '追加リクエスト',
      'country_code_hint': 'TH、US、KR…',
      'country_search_hint': '国を検索または直接入力',
      'boarding_qr_title': '乗車 QR',
      'boarding_qr_hint': '乗車時にこの QR コードをドライバーへ提示してください。スキャン後、乗車済みの状態に変わります。',
      'booking_trust_message': '予約番号を保管してください。ドライバーの割り当てと運行状況は予約照会画面で確認できます。',
      'airport_meeting_semantics': 'BKK空港ミーティング案内、ゲート{gate}',
      'airport_meeting_gate_3_title': 'ネームサイン集合案内 · 3番ゲート',
      'airport_meeting_gate_7_title': '車両ピックアップ案内 · 7番ゲート',
      'airport_meeting_gate_inside': 'ゲート内側',
      'airport_meeting_gate_outside': 'ゲート外側',
      'airport_meeting_no_picket_highlight':
          '荷物を受け取ったら、空港を出る前にピックアップ準備を知らせてください。',
      'airport_meeting_picket_highlight': '空港の外へ出ず、3番ゲート内側でネームサインを探してください。',
      'airport_meeting_no_picket_body_1': 'ドライバーが7番ゲート外の乗車エリアへ車を移動する時間が必要です。',
      'airport_meeting_no_picket_body_2': '乗車前に車両番号とドライバーを確認してください。',
      'airport_meeting_picket_body_1':
          '荷物を受け取った後、3番ゲート内側へ移動し、お名前のネームサインを探してください。',
      'airport_meeting_picket_body_2': 'スタッフがドライバーと車両を3番ゲート前へ案内します。',
      'airport_meeting_step_luggage': '荷物をすべて受け取ります。',
      'airport_meeting_step_notify_driver': 'ピックアップ準備ができたことをドライバーへ知らせます。',
      'airport_meeting_step_vehicle_number': '受け取った車両番号を確認します。',
      'airport_meeting_step_gate_7': '到着階7番ゲートの外へ移動します。',
      'airport_meeting_step_board': '車両とドライバーを確認して乗車します。',
      'airport_meeting_step_gate_3': '到着階3番ゲートの内側へ移動します。',
      'airport_meeting_step_find_picket': 'お名前が書かれたネームサインを探します。',
      'airport_meeting_step_wait_staff': 'スタッフの案内に従って少しお待ちください。',
      'airport_meeting_step_board_after_staff': '車両到着の案内後、外へ出て乗車します。',
      'airport_meeting_do_not_exit': 'スタッフの案内前に空港の外へ出ないでください。',
      'airport_meeting_vehicle_pending': 'ドライバーと車両情報は未割当です。割当後ここに表示されます。',
      'airport_meeting_vehicle_info': 'ドライバーと車両',
      'airport_meeting_driver': 'ドライバー',
      'airport_meeting_driver_phone': 'ドライバー電話',
      'airport_meeting_vehicle_type': '車種',
      'airport_meeting_vehicle_color': '車両色',
      'airport_meeting_vehicle_plate': '車両番号',
      'ui_load_failed': 'データを読み込めませんでした。もう一度お試しください。',
      'ui_action_failed': '操作に失敗しました。もう一度お試しください。',
      'ui_generic_error': '問題が発生しました。もう一度お試しください。',
      'admin_ops_filter_date_from': '送迎開始日 (YYYY-MM-DD)',
      'admin_ops_filter_date_to': '送迎終了日 (YYYY-MM-DD)',
      'admin_ops_filter_date_from_invalid': '有効な開始日を選択してください。',
      'admin_ops_filter_date_to_invalid': '有効な終了日を選択してください。',
      'admin_ops_filter_date_range_invalid': '終了日は開始日より前にできません。',
      'ui_retry': '再試行',
      'loading_recommendation': 'おすすめを読み込み中...',
      'calculating_price': '料金を計算中...',
      'guest_lookup_title': '予約を検索',
      'guest_lookup_loading': '保存済み予約を読み込み中...',
      'guest_lookup_subtitle': '予約番号と電話番号を入力して行程を確認してください。',
      'guest_lookup_booking_number': '予約番号',
      'guest_lookup_phone': '電話番号',
      'guest_lookup_invalid_number': '有効な予約番号を入力してください',
      'guest_lookup_invalid_phone': '予約の電話番号を入力してください',
      'guest_lookup_find': '予約を検索',
      'guest_lookup_not_found': '予約が見つかりません。予約番号と電話番号を確認してください。',
      'guest_lookup_load_error': '予約を読み込めませんでした。もう一度お試しください。',
      'guest_lookup_trip_details': '行程詳細',
      'guest_lookup_pickup': 'ピックアップ',
      'guest_lookup_service': 'サービス',
      'guest_lookup_from': '出発',
      'guest_lookup_to': '到着',
      'guest_lookup_driver': 'ドライバー',
      'guest_lookup_driver_phone': 'ドライバー電話',
      'guest_lookup_driver_status': 'ドライバー状況',
      'guest_lookup_vehicle': '車両',
      'guest_lookup_vehicle_photo_load_failed': '車両写真を読み込めませんでした。',
      'guest_lookup_payment': '支払い',
      'guest_lookup_total': '合計',
      'guest_lookup_another': '別の予約を検索',
      'guest_lookup_refresh': '更新',
      'guest_lookup_refresh_needs_phone': '更新するには検索フォームで電話番号を入力してください。',
      'booking_complete_track_cta': '予約状況を確認',
      'auth_social_login_title':
          'Create an account to make your next booking easier',
      'auth_social_login_description':
          'View booking history and auto-fill your details on future trips.',
      'auth_google_continue': 'Continue with Google',
      'auth_kakao_continue': 'Kakaoで続ける',
      'auth_kakao_callback_error': 'Kakaoログインに失敗しました。もう一度お試しください。',
      'auth_kakao_callback_already_processed':
          'このログイン要求はすでに処理されています。必要なら再度お試しください。',
      'auth_kakao_callback_slow_loading': '処理に時間がかかっています。ページを更新してください。',
      'auth_kakao_callback_refresh': 'ページを更新',
      'profile_completion_title': 'プロフィールを完成',
      'profile_completion_body':
          'T-Riderをご利用いただくために、お名前と電話番号を入力してください。',
      'profile_completion_submit': '保存して続行',
      'profile_completion_name_required': '名前を入力してください。',
      'profile_completion_phone_required':
          '有効な電話番号を入力してください（5文字以上）。',
      'profile_completion_phone_duplicate':
          'この電話番号は別のアカウントですでに使用されています。',
      'auth_line_continue': 'LINEで続ける',
      'auth_line_callback_error': 'LINEログインに失敗しました。もう一度お試しください。',
      'auth_line_callback_state_mismatch': 'LINEログインを確認できませんでした。もう一度お試しください。',
      'auth_line_callback_already_processed':
          'このログイン要求はすでに処理されています。必要なら再度お試しください。',
      'auth_line_callback_slow_loading': '処理に時間がかかっています。ページを更新してください。',
      'auth_line_callback_refresh': 'ページを更新',
      'auth_social_login_later': 'Maybe later',
      'auth_social_login_connected': '{name}, your account is connected.',
      'my_bookings_title': '予約一覧',
      'my_bookings_empty': '連携された予約はまだありません。予約後にログインするとここに表示されます。',
      'my_bookings_view_button': '予約一覧を見る',
      'my_bookings_driver_summary': 'ドライバー: {name}',
      'my_bookings_load_error': '予約一覧を読み込めませんでした。もう一度お試しください。',
      'booking_cancel_section': '予約キャンセル',
      'booking_cancel_action': '予約をキャンセル',
      'booking_cancel_policy_hint': 'ピックアップの2時間前までキャンセルできます。',
      'booking_cancel_confirm_title': 'この予約をキャンセルしますか？',
      'booking_cancel_confirm_action': 'キャンセルを確定',
      'booking_cancel_confirm_irreversible': 'キャンセル後は元に戻せません。',
      'booking_cancel_success': '予約をキャンセルしました。',
      'booking_cancel_failed': '予約をキャンセルできませんでした。もう一度お試しください。',
      'booking_cancel_blocked_within_two_hours':
          '予定ピックアップ時刻の2時間以内は予約をキャンセルできません。',
      'booking_cancel_blocked_trip_started': '運行が開始された予約はキャンセルできません。',
      'booking_cancel_blocked_already_cancelled': 'この予約はすでにキャンセル済みです。',
      'booking_cancel_blocked_completed': '完了した予約はキャンセルできません。',
      'booking_cancel_blocked_no_show': 'ノーショー扱いの予約はキャンセルできません。',
      'booking_cancel_blocked_invalid_pickup':
          'ピックアップ時刻が無効なため、この予約はキャンセルできません。',
      'booking_cancel_blocked_locked': 'この予約はロックされているためキャンセルできません。',
      'booking_cancel_driver_assigned': 'ドライバー割当済み: {name}',
      'booking_cancel_driver_assigned_generic': 'ドライバー割当済み',
      'guest_status_guidance_pending': '予約を受け付けました。まもなく確認します。',
      'guest_status_guidance_confirmed': '予約が確定しました。迎え前にドライバーが割り当てられます。',
      'guest_status_guidance_reassignment':
          '担当ドライバーの都合により、新しいドライバーを手配しています。日時と料金は変わりません。',
      'status_reassignment_in_progress': 'ドライバー再手配中',
      'admin_ops_severity_critical': '最優先',
      'admin_ops_reason_critical_reassignment': '緊急再配車',
      'admin_ops_reason_urgent_reassignment': '至急再配車',
      'admin_ops_reason_driver_released_reassignment': 'ドライバー返却 · 再配車',
      'admin_ops_reason_critical_unassigned': '最優先 · 未配車',
      'admin_ops_reason_urgent_unassigned': '至急 · 未配車',
      'driver_release_assignment': '配車を返却',
      'driver_release_assignment_title': 'この予約の配車を返却しますか？',
      'driver_release_assignment_message': 'お客様の予約は維持され、別のドライバーを再手配します。',
      'driver_release_assignment_cancel': '配車を維持',
      'driver_release_assignment_confirm': '配車を返却',
      'driver_release_assignment_success': '配車を解除しました。',
      'driver_release_assignment_failed': '配車の返却に失敗しました。もう一度お試しください。',
      'driver_release_assignment_blocked': '現在の状態では配車を返却できません。',
      'driver_release_assignment_emergency_hint':
          'ピックアップまで2時間以内は通常の配車返却が制限されます。緊急時のみ返却できます。',
      'driver_release_assignment_irreversible': '返却後は取り消せません。',
      'driver_release_reason_label': '返却理由',
      'driver_release_reason_detail_label': '詳細',
      'driver_release_reason_detail_required': 'その他の理由を入力してください',
      'driver_release_reason_VEHICLE_BREAKDOWN': '車両トラブル',
      'driver_release_reason_ACCIDENT': '事故',
      'driver_release_reason_DRIVER_ILLNESS': '体調不良',
      'driver_release_reason_FAMILY_EMERGENCY': '家族の緊急事態',
      'driver_release_reason_SCHEDULE_CONFLICT': 'スケジュールの都合',
      'driver_release_reason_LOCATION_TOO_FAR': '距離が遠い',
      'driver_release_reason_OTHER': 'その他',
      'driver_assignment_ended_confirm': '確認',
      'driver_booking_not_found': '予約情報が見つかりません。',
      'driver_assignment_ended_generic_title': '配車が終了しました',
      'driver_assignment_ended_generic_message': 'この予約はこれ以上進められません。',
      'driver_assignment_ended_generic_snackbar':
          '予約 {bookingNumber} の配車が解除されました。',
      'driver_assignment_ended_customer_title': 'お客様が予約をキャンセルしました',
      'driver_assignment_ended_customer_message':
          'お客様が予約をキャンセルしたため、配車が自動的に解除されました。',
      'driver_assignment_ended_customer_snackbar':
          'お客様キャンセルにより予約 {bookingNumber} の配車が解除されました。',
      'driver_assignment_ended_admin_title': '管理者が予約をキャンセルしました',
      'driver_assignment_ended_admin_message': '管理者が予約をキャンセルしたため、配車が解除されました。',
      'driver_assignment_ended_admin_snackbar':
          '管理者キャンセルにより予約 {bookingNumber} の配車が解除されました。',
      'driver_assignment_ended_released_title': '配車の返却が完了しました',
      'driver_assignment_ended_released_message':
          '配車の返却が完了しました。この予約は他のドライバーに再公開されます。',
      'driver_assignment_ended_released_snackbar':
          '予約 {bookingNumber} の配車返却が完了しました。',
      'driver_assignment_ended_reassigned_title': '他のドライバーに割り当てられました',
      'driver_assignment_ended_reassigned_message': 'この予約は他のドライバーに割り当てられました。',
      'driver_assignment_ended_reassigned_snackbar':
          '予約 {bookingNumber} は他のドライバーに割り当てられました。',
      'driver_assignment_ended_completed_title': '運行が完了しました',
      'driver_assignment_ended_completed_message': 'この予約はすでに完了しています。',
      'guest_status_guidance_driver_assigned':
          'ドライバーが割り当てられました。まもなく迎え場所へ向かいます。',
      'guest_status_guidance_on_route': 'ドライバーが迎え場所へ向かっています。',
      'guest_status_guidance_driver_arrived': 'ドライバーが迎え場所に到着しました。',
      'guest_status_guidance_picked_up': '運行中です。',
      'guest_status_guidance_settlement_pending': '運行が終了しました。ご利用ありがとうございました。',
      'guest_status_guidance_completed': '運行が完了しました。ドライバーを評価してください。',
      'guest_status_guidance_cancelled':
          'この予約はキャンセルされました。詳細はカスタマーセンターへお問い合わせください。',
      'guest_status_guidance_no_show': 'この予約はノーショーとして処理されました。',
      'guest_lookup_issue_dropoff_qr': '降車 QR を発行',
      'guest_lookup_trip_completed_qr_hidden': '行程完了。QR コードは非表示です。',
      'guest_lookup_dropoff_qr_token': '降車 QR トークン: {token}',
      'guest_lookup_boarding_qr_security':
          '乗車 QR は予約作成時に発行され、セキュリティ上ここでは復元できません。',
      'guest_lookup_booking_number_hint': 'TX202607010001',
      'booking_trip_completed': '行程完了',
      'booking_dropoff_qr_title': '運行完了 QR',
      'booking_dropoff_qr_hint':
          '降車時にこの QR コードをもう一度ドライバーへ提示してください。スキャン後、運行完了になります。',
      'booking_dropoff_qr_unavailable': '降車 QR は乗車後〜行程完了前に利用できます。',
      'booking_qr_load_failed': 'QR コードを読み込めませんでした。予約状況を確認するか、管理者にお問い合わせください。',
      'booking_refresh_dropoff_qr': '降車 QR を更新',
      'booking_issue_new_dropoff_qr': '新しい降車 QR を発行',
      'booking_review_title': '行程を評価',
      'booking_review_thanks': 'レビューありがとうございます',
      'booking_review_rating': '評価: {rating} / 5',
      'booking_review_comment': 'コメント（任意）',
      'booking_review_submitting': '送信中...',
      'booking_review_submit': 'レビューを送信',
      'booking_review_load_error': 'レビューを読み込めませんでした',
      'review_card_title': '今回の乗車はいかがでしたか？',
      'review_card_subtitle': '評価を選択してください',
      'review_star_label': '星{value}つ',
      'review_rating_desc_1': '非常に不満',
      'review_rating_desc_2': '不満',
      'review_rating_desc_3': '普通',
      'review_rating_desc_4': '満足',
      'review_rating_desc_5': '非常に満足',
      'review_comment_label': 'ご意見をお聞かせください（任意）',
      'review_comment_hint': '個人情報は入力しないでください。',
      'review_submit_button': '評価を送信',
      'review_success_title': 'ご評価ありがとうございます',
      'review_success_body': 'サービス改善の参考にさせていただきます',
      'review_tag_FRIENDLY': '親切でした',
      'review_tag_SAFE_DRIVING': '安全運転でした',
      'review_tag_CLEAN_VEHICLE': '車内が清潔でした',
      'review_tag_ON_TIME': '時間どおりでした',
      'review_tag_GOOD_COMMUNICATION': 'コミュニケーションが良好でした',
      'review_tag_UNSAFE_DRIVING': '運転が不安でした',
      'review_tag_LATE_ARRIVAL': '到着が遅れました',
      'review_tag_VEHICLE_NOT_CLEAN': '車内が清潔ではありませんでした',
      'review_tag_UNFRIENDLY_SERVICE': '対応が不親切でした',
      'review_tag_ROUTE_ISSUE': 'ルートに問題がありました',
      'review_tag_OTHER_ISSUE': 'その他の問題',
      'booking_notification_updates': '更新',
      'booking_notification_enabling': '有効化中...',
      'booking_notification_enable': '通知を有効化',
      'booking_notification_empty': '更新はありません',
      'booking_notification_default_title': '通知',
      'booking_notification_load_error': '通知を読み込めませんでした',
      'booking_notification_enabled': '通知が有効になりました',
      'booking_notification_permission_denied': '通知の権限が拒否されました',
      'booking_notification_unsupported': 'このブラウザはプッシュ通知に対応していません',
      'booking_notification_config_missing': 'この環境ではプッシュ通知が設定されていません',
      'booking_notification_register_failed': '通知登録に失敗しました',
      'track_driver_title': 'ドライバーを追跡',
      'track_driver_unavailable': 'ドライバーの位置はまだ利用できません。',
      'track_driver_stale': '位置情報が古い可能性があります',
      'track_driver_live': 'リアルタイム位置',
      'track_driver_updated': '更新 {time}',
      'track_driver_load_error': 'ドライバーの位置を読み込めませんでした',
      'customer_driver_location_title': 'ドライバー位置',
      'customer_driver_location_waiting':
          'ドライバーが割り当てられました。ドライバーが迎車場所へ移動を開始すると、リアルタイム位置を確認できます。',
      'customer_driver_location_live': 'ドライバー位置を共有中',
      'customer_driver_location_updated_label': '最終更新',
      'customer_driver_location_updated_now': 'たった今更新',
      'customer_driver_location_updated_minutes': '{minutes} 分前に更新',
      'customer_driver_location_temporarily_unavailable':
          'ドライバー位置を一時的に確認できません。しばらくしてからもう一度お試しください。',
      'customer_driver_location_stale': 'ドライバー位置が最近更新されていません。',
      'customer_driver_location_ended': 'リアルタイム位置共有は終了しました。',
      'customer_driver_location_open_map': '地図で開く',
      'customer_driver_location_recenter': 'ドライバー位置へ移動',
      'customer_driver_on_route': 'ドライバーは迎車場所へ向かっています。',
      'customer_driver_arrived': 'ドライバーが迎車場所に到着しました。ドライバーと車両情報をご確認ください。',
      'customer_trip_in_progress': '移動中です。',
      'customer_driver_location_load_failed':
          'ドライバー位置を確認できません。しばらくしてからもう一度お試しください。',
      'customer_driver_location_retry': '再試行',
      'wizard_step_progress': 'ステップ {current} / {total}',
      'pickup_date_past': 'ピックアップ日は過去にできません',
      'pickup_time_minimum': 'ピックアップ時間は現在より後である必要があります',
      'pickup_time_am': '午前',
      'pickup_time_pm': '午後',
      'pickup_time_select': 'ピックアップ時間を選択',
      'pickup_time_enter_manually': '時間を直接入力',
      'pickup_time_invalid': '有効な時間を入力してください（例: 09:30 AM）',
      'pickup_time_apply_manual': '適用',
      'field_required': '（必須）',
      'name_sign_info_before_price': 'ネームサインサービスを選択すると、追加料金 ',
      'name_sign_info_after_price': ' が発生し、ピックアップゲートは ',
      'name_sign_info_after_gate': ' に変更されます。',
      'name_sign_service_price_highlight': '100バーツ',
      'name_sign_service_gate_highlight': '3番ゲート',
      'name_sign_text_label': 'ネームボードに表示する名前',
      'name_sign_text_hint': '例: TANAKA YUKI',
      'wizard_required_name_sign_text': 'ネームボードに表示する名前を入力してください。',
      'wizard_customer_email_invalid': '有効なメールアドレスを入力してください。',
      'pickup_datetime_required': 'ピックアップ日時を入力してください',
      'booking_location_reselect_required': '場所の座標を取得できません。出発地/目的地を再度選択してください。',
      'flight_number_hint': 'TG401, 7C2203',
      'flight_number_invalid': '便名を確認してください。例: TG401, 7C2203',
      'flight_lookup_search': '検索',
      'flight_lookup_not_found': '該当する便名のフライトが見つかりません。便名を再度ご確認ください。',
      'flight_lookup_invalid_date':
          'このピックアップ日では便情報を照会できません。便名を入力したまま予約を続行できます。',
      'flight_lookup_provider_unavailable':
          '便情報の照会サービスが一時的に利用できません。便名を入力したまま予約を続行できます。',
      'flight_lookup_departure': '出発',
      'flight_lookup_arrival': '到着',
      'flight_lookup_estimated_arrival': '到着予定',
      'flight_lookup_confirm': 'この便で間違いありません',
      'booking_number_copied': '予約番号をコピーしました',
      'booking_number_copy_failed': '予約番号をコピーできませんでした',
      'booking_number_copy': '予約番号をコピー',
      'booking_number_save_notice': '予約番号は予約照会に必要です。必ず保存してください。',
      'wizard_edit_section': '編集',
      'wizard_section_incomplete': '続行する前にこのステップを完了してください。',
      'wizard_completion_progress': '必須入力 {completed}/{total} 完了',
      'wizard_required_service': 'サービス種別を選択してください。',
      'wizard_step_route_title': 'ルート',
      'wizard_step_schedule_title': '日時',
      'wizard_step_vehicle_title': '車両選択',
      'wizard_step_customer_title': '乗客情報',
      'wizard_step_review_title': '予約確認',
      'wizard_step_indicator': '{current}/{total} {title}',
      'wizard_cta_route': '日時を選択',
      'wizard_cta_schedule': '車両を選択',
      'wizard_cta_vehicle_unselected': '車両を選択してください',
      'wizard_cta_vehicle_selected': '乗客情報入力 · {price}',
      'wizard_cta_customer': '予約内容確認 · {price}',
      'wizard_cta_confirm': '予約確定 · {price}',
      'wizard_same_place_error': '出発地と目的地を同じにすることはできません。',
      'wizard_swap_route': '出発地/目的地を入れ替え',
      'wizard_trust_toll_included': '高速道路通行料込み',
      'wizard_trust_no_airport_parking': '空港駐車料金なし',
      'wizard_trust_no_night_surcharge': '深夜・早朝追加料金なし',
      'wizard_trust_flight_delay_wait': 'フライト遅延時は無料待機',
      'wizard_places_searching': '場所を検索中…',
      'wizard_places_no_results_hint': '該当する場所がありません。別のキーワードを試すか、地図から選択してください。',
      'wizard_summary_expand': '予約概要を表示',
      'wizard_summary_collapse': '予約概要を非表示',
      'wizard_required_origin': '出発地を選択してください。',
      'wizard_required_destination': '目的地を選択してください。',
      'wizard_required_passengers': '乗客情報を入力してください。',
      'wizard_required_vehicle': '車両を選択してください。',
      'wizard_required_customer': 'お客様の情報を入力してください。',
      'wizard_required_customer_name': 'お名前を入力してください。',
      'wizard_required_customer_phone': '電話番号を入力してください。',
      'select_on_map': '地図から選択',
      'map_picker_title': '地図で場所を選択',
      'map_picker_hint': '地図をタップして位置を指定してください。',
      'map_picker_confirm': 'この場所を使用',
      'current_location': '現在地',
      'locating_current_position': '現在地を確認しています。',
      'location_permission_required': '現在地を使用するには位置情報の権限が必要です。',
      'location_permission_denied': '位置情報の権限が拒否されました。',
      'location_permission_permanently_denied': 'ブラウザまたは端末の設定で位置情報を許可してください。',
      'location_service_disabled': '端末の位置情報サービスがオフです。',
      'location_unavailable': '現在地を取得できません。',
      'location_timeout': '位置情報の取得がタイムアウトしました。もう一度お試しください。',
      'location_requires_https': '現在の接続環境では位置機能を使用できません。HTTPS アドレスから再試行してください。',
      'low_location_accuracy': '位置の精度が低いため、地図上で正確な位置を調整してください。',
      'wizard_vehicle_prerequisites': 'サービス、出発地、目的地、ピックアップ時間、乗客情報を先に入力してください。',
      'wizard_pricing_after_vehicle': '車両選択後に料金が計算されます。',
      'admin_dispatch_retry': '再試行',
      'admin_dashboard': 'ダッシュボード',
      'admin_reservations': '予約管理',
      'admin_drivers': 'ドライバー管理',
      'admin_pricing': '車両料金',
      'admin_golf': 'ゴルフ場',
      'admin_airports': '空港',
      'admin_notifications': '通知',
      'admin_translations': '翻訳',
      'admin_settings': '設定',
      'today_bookings': '本日の予約',
      'today_revenue': '本日の売上',
      'pending_count': '保留中',
      'awaiting_driver': 'ドライバー待ち',
      'vehicle': '車両',
      'amount': '金額',
      'send': '送信',
      'type_message': 'メッセージを入力...',
      'other_airport': 'その他のタイ空港',
      'booking_summary': '予約確認',
      'service_type': 'サービス種別',
      'thb': 'THB',
    },
    'th': {
      'app_title': 'T-Rider',
      'app_subtitle': 'บริการรับส่งสนามบินไทย',
      'airport_pickup': 'รับที่สนามบิน',
      'airport_pickup_desc': 'รับจากสนามบินไปยังที่พักของคุณอย่างสบาย',
      'airport_dropoff': 'ส่งสนามบิน',
      'airport_dropoff_desc': 'จากที่พักไปสนามบินตรงเวลา',
      'city_transfer': 'รับส่งในเมือง',
      'city_transfer_desc': 'เดินทางระหว่างเมืองอย่างสบาย',
      'golf_transfer': 'รับส่งสนามกอล์ฟ',
      'golf_transfer_desc': 'พื้นที่กว้างขวางสำหรับกระเป๋ากอล์ฟ',
      'select_service': 'เลือกประเภทบริการ',
      'passengers': 'ผู้โดยสาร',
      'adults': 'ผู้ใหญ่ (12+)',
      'children': 'เด็ก',
      'luggage': 'สัมภาระ',
      'small_carriers': 'กระเป๋า 20 นิ้ว',
      'large_carriers': 'กระเป๋า 24 นิ้ว+',
      'golf_bags': 'กระเป๋ากอล์ฟ',
      'special_luggage': 'สัมภาระพิเศษ',
      'name_sign': 'บริการป้ายชื่อ (+100 THB)',
      'booking_prefer_female_driver': 'ต้องการคนขับหญิง',
      'booking_preference_disclaimer':
          'เราจะพยายามจัดสรรตามที่ท่านต้องการให้มากที่สุด แต่ไม่สามารถรับประกันได้ 100%',
      'next': 'ถัดไป',
      'back': 'กลับ',
      'confirm': 'ยืนยันการจอง',
      'customer_review_booking': 'ตรวจสอบการจอง',
      'customer_confirm_booking': 'ยืนยันการจอง',
      'customer_booking_processing': 'กำลังดำเนินการจอง',
      'customer_urgent_pickup_hint':
          'หากเวลารับอยู่ภายใน 2 ชั่วโมง ให้แตะ คำขอด่วน เพื่อดำเนินการต่อ',
      'customer_urgent_request': 'คำขอด่วน',
      'customer_urgent_confirm_title': 'ยืนยันคำขอด่วน',
      'customer_urgent_confirm_body':
          'คำขอด่วนอาจใช้เวลาจัดรถนานขึ้น เมื่อจัดรถแล้วเราจะแจ้งเวลาถึงโดยประมาณของคนขับก่อน หากต้องการดำเนินการต่อ กรุณาแตะ ยืนยัน',
      'customer_urgent_confirm_submit': 'ยืนยัน',
      'customer_urgent_flow_title': 'คำขอด่วน',
      'customer_urgent_searching_title': 'กำลังค้นหาคนขับ',
      'customer_urgent_searching_body':
          'เรากำลังค้นหาคนขับที่พร้อมให้บริการใกล้คุณ',
      'customer_urgent_eta_title': 'คนขับคาดว่าจะถึงในอีกประมาณ {minutes} นาที',
      'customer_urgent_eta_body':
          'หากเวลานี้เหมาะสม กรุณากด ยอมรับ หากต้องการคนขับที่ถึงเร็วกว่า กรุณากด ปฏิเสธ',
      'customer_urgent_accept_eta': 'ยอมรับ',
      'customer_urgent_reject_eta': 'ปฏิเสธ',
      'customer_urgent_retry_title': 'ต้องการหาคนขับที่เร็วกว่านี้ไหม?',
      'customer_urgent_retry_body':
          'เราสามารถค้นหาคนขับคนอื่นที่มีเวลาถึงสั้นกว่าได้',
      'customer_urgent_retry_accept': 'ค้นหาต่อ',
      'customer_urgent_retry_cancel': 'ยกเลิกการจอง',
      'customer_urgent_confirmed_title': 'ยืนยันการจัดรถแล้ว',
      'customer_urgent_confirmed_body':
          'มีการมอบหมายคนขับแล้ว กำลังเปิดหน้ารายละเอียดการจอง',
      'customer_urgent_exhausted_title': 'ไม่มีคนขับใกล้เคียง',
      'customer_urgent_exhausted_body':
          'ไม่มีคนขับที่รอรับงานใกล้เคียง เราจะยกเลิกการจองของคุณ',
      'customer_urgent_go_home': 'กลับหน้าแรก',
      'origin': 'จุดเริ่มต้น',
      'destination': 'ปลายทาง',
      'airport': 'สนามบิน',
      'selected_location': 'ตำแหน่งที่เลือก',
      'flight_number': 'หมายเลขเที่ยวบิน',
      'date': 'วันที่',
      'time': 'เวลา',
      'pickup_datetime': 'วันที่และเวลารับ',
      'pickup_date': 'วันที่รับ',
      'pickup_time': 'เวลารับ',
      'pickup_minimum_notice':
          'กรุณาเลือกเวลารับอย่างน้อย 2 ชั่วโมงจากตอนนี้ตามเวลาไทย',
      'select_vehicle': 'เลือกรถ',
      'recommended': 'แนะนำ',
      'total': 'รวม',
      'special_requests': 'คำขอพิเศษ',
      'customer_info': 'ข้อมูลลูกค้า',
      'name': 'ชื่อ',
      'email': 'อีเมล',
      'phone': 'โทรศัพท์',
      'country': 'ประเทศ',
      'booking_complete': 'จองสำเร็จ',
      'reservation_number': 'หมายเลขจอง',
      'booking_number_label': 'หมายเลขการจอง',
      'booking_with_number': 'การจอง {bookingNumber}',
      'booking_detail_view': 'ดูรายละเอียดการจอง',
      'notification_type_booking': 'การจอง',
      'notification_type_pickup': 'รับรถ',
      'notification_type_settlement': 'ยอดชำระ',
      'notification_type_system': 'ระบบ',
      'status': 'สถานะ',
      'golf_region': 'พื้นที่กอล์ฟ',
      'golf_course': 'สนามกอล์ฟ',
      'driver_included': 'รวมคนขับ',
      'status_pending': 'รอดำเนินการ',
      'status_confirmed': 'ยืนยันแล้ว',
      'status_driver_assigned': 'มอบหมายคนขับ',
      'status_completed': 'เสร็จสิ้น',
      'status_cancelled': 'ยกเลิก',
      'status_on_route': 'กำลังไปรับ',
      'status_driver_arrived': 'ถึงจุดรับแล้ว',
      'status_no_show': 'ไม่มาตามนัด',
      'status_picked_up': 'ลูกค้าขึ้นรถแล้ว · กำลังเดินทาง',
      'status_customer_settlement_pending': 'การเดินทางสิ้นสุดแล้ว',
      'status_driver_settlement_pending': 'จบการเดินทาง · รอชำระบัญชี',
      'status_admin_settlement_pending': 'จบการเดินทาง · รอตรวจสอบการชำระเงิน',
      'status_settlement_pending': 'จบการเดินทาง · รอชำระบัญชี',
      'search_place': 'ค้นหาสถานที่...',
      'no_results': 'ไม่พบสถานที่',
      'infants': 'ทารก',
      'multiple_vehicles_required': 'ต้องใช้รถหลายคัน',
      'base_price': 'ราคาพื้นฐาน',
      'additional_charges': 'ค่าใช้จ่ายเพิ่มเติม',
      'pricing_summary': 'สรุปราคา',
      'pricing_name_sign_with_picket': 'บริการป้ายรับผู้โดยสาร',
      'vehicle_not_available': 'ไม่พร้อมให้บริการสำหรับทริปนี้',
      'pricing_inquiry_required': 'สอบถาม',
      'complete_passengers_first': 'กรอกข้อมูลผู้โดยสารก่อน',
      'yes': 'ใช่',
      'change_location': 'เปลี่ยน',
      'loading_place_details': 'กำลังโหลดรายละเอียดสถานที่...',
      'airport_shortcuts': 'ทางลัดสนามบิน',
      'airport_shortcuts_origin': 'ทางลัดสนามบิน',
      'airport_shortcuts_destination': 'ทางลัดสนามบิน',
      'recent_non_airport_places': 'สถานที่ล่าสุด',
      'recent_locations': 'สถานที่ล่าสุด',
      'landing_hero_eyebrow': 'บริการรับส่งสนามบินและในเมืองมืออาชีพ',
      'landing_hero_title': 'เริ่มต้นการเดินทางในไทยอย่างสบายใจกับ T-Rider',
      'landing_hero_body':
          'คนขับมืออาชีพพาคุณจากสนามบินถึงที่พักอย่างปลอดภัยและสบาย',
      'landing_hero_cta': 'จองเลย',
      'landing_support_cta': 'ศูนย์บริการลูกค้า',
      'landing_support_one_click_cta': 'จองง่ายผ่านศูนย์บริการลูกค้า',
      'landing_hero_helper': 'จองง่าย ไม่ต้องสมัครสมาชิก',
      'landing_hero_subtitle': 'เดินทางอย่างปลอดภัย ราบรื่น และมั่นใจ',
      'landing_highlight_fixed_price': 'ราคาคงที่',
      'landing_highlight_fixed_price_desc':
          'เห็นราคาชัดเจนตั้งแต่จอง ไม่มีค่าใช้จ่ายแอบแฝง',
      'landing_highlight_flight': 'ติดตามเที่ยวบิน',
      'landing_highlight_flight_desc':
          'ติดตามเที่ยวบินและปรับเวลารับอัตโนมัติเมื่อล่าช้า',
      'landing_highlight_support': 'สนับสนุน 24/7',
      'landing_highlight_support_desc':
          'พร้อมช่วยเหลือก่อนและระหว่างการเดินทาง',
      'book_your_ride': 'จองเลย',
      'landing_services_title': 'บริการของเรา',
      'landing_services_subtitle': 'รับส่งสนามบินและระหว่างเมือง',
      'service_badge_popular': 'ยอดนิยม',
      'landing_trust_title': 'เดินทางอย่างมั่นใจ',
      'landing_trust_drivers_title': 'คนขับมืออาชีพ',
      'landing_trust_drivers_desc': 'คนขับที่คุ้นเคยกับถนนไทยและเส้นทางสนามบิน',
      'landing_trust_confirmed_title': 'ยืนยันการจองล่วงหน้า',
      'landing_trust_confirmed_desc':
          'ตรวจสอบข้อมูลรับและตารางเดินทางก่อนถึงวัน',
      'landing_trust_comfort_title': 'การเดินทางที่สบาย',
      'landing_trust_comfort_desc':
          'ไม่ต้องกังวลเรื่องการเดินทาง โฟกัสกับทริปของคุณ',
      'landing_trust_refund_title': 'ยกเลิก·คืนเงินอย่างอุ่นใจ',
      'landing_trust_refund_desc':
          'สามารถยกเลิกการจองได้ฟรีภายใน 24 ชั่วโมงหลังจากยืนยันการจอง โดยไม่มีค่าธรรมเนียม หากท่านพบราคาที่ถูกกว่าสำหรับเงื่อนไขเดียวกัน (ประเภทรถ·กำหนดการ) กรุณาติดต่อเราภายใน 48 ชั่วโมงหลังการจอง เราจะคืนส่วนต่างให้ท่าน',
      'landing_trust_driver_policy_title': 'การจัดการคุณภาพคนขับ',
      'landing_trust_driver_policy_desc':
          'หากท่านมีข้อกังวลเกี่ยวกับคนขับ สามารถแจ้งได้ผ่านหน้ารายละเอียดการจองหรือทาง KakaoTalk/โทรศัพท์ เราจะติดต่อกลับภายใน 24 ชั่วโมงหลังจากได้รับแจ้ง สำหรับกรณีร้ายแรง เช่น การคุกคามด้านความปลอดภัย เราจะระงับการจัดสรรคนขับทันที',
      'landing_how_it_works_title': 'วิธีการจอง',
      'landing_how_step_1_title': 'กรอกข้อมูลการเดินทาง',
      'landing_how_step_1_desc': 'เลือกบริการ เวลารับ และจุดหมาย',
      'landing_how_step_2_title': 'เลือกรถและดูค่าโดยสาร',
      'landing_how_step_2_desc': 'ดูรถที่แนะนำและราคาก่อนยืนยันการจอง',
      'landing_how_step_3_title': 'เดินทางอย่างสบาย',
      'landing_how_step_3_desc': 'คนขับพาคุณถึงจุดหมายอย่างปลอดภัย',
      'landing_reassurance_body':
          'มาไทยครั้งแรก? เราจะแนะนำอย่างชัดเจนตั้งแต่จองจนถึงวันเดินทาง',
      'landing_booking_lookup_title': 'จองแล้วหรือยัง?',
      'landing_booking_lookup_desc': 'ตรวจสอบตารางและข้อมูลทริปด้วยหมายเลขจอง',
      'landing_booking_lookup_action': 'ค้นหาการจอง',
      'landing_footer_note': 'บริการรับส่งสนามบินและในเมืองทั่วประเทศไทย',
      'landing_login_title': 'เข้าสู่ระบบ',
      'landing_login_sheet_title': 'เข้าสู่ระบบ T-Rider',
      'landing_login_prompt_cta': 'เข้าสู่ระบบเพื่อสะสมไมล์',
      'landing_login_sheet_close': 'ปิด',
      'landing_login_support_prompt': 'มีปัญหาในการเข้าสู่ระบบ?',
      'landing_login_support_link': 'ติดต่อฝ่ายสนับสนุน',
      'landing_header_my_bookings_menu': 'ดูการจองของฉัน',
      'landing_header_logout_menu': 'ออกจากระบบ',
      'landing_header_logout_success': 'ออกจากระบบแล้ว',
      'landing_logged_in_greeting': 'เข้าสู่ระบบแล้ว: {name}',
      'landing_my_bookings_button': 'การจองของฉัน',
      'landing_logout_button': 'ออกจากระบบ',
      'auth_login_provider_hint':
          'เพื่อจัดการไมล์สะสม โปรดเข้าสู่ระบบด้วยวิธีเดิมเสมอ (Google, Kakao หรือ LINE)',
      'landing_header_account_menu': 'บัญชีของฉัน',
      'account_page_title': 'บัญชีของฉัน',
      'account_menu_section_title': 'บัญชี',
      'account_mileage_label': 'ไมล์สะสม',
      'account_mileage_balance': '{balance}P',
      'account_mileage_loading': 'กำลังโหลดยอดคงเหลือ...',
      'account_mileage_load_failed': 'โหลดไม่สำเร็จ',
      'account_mileage_retry': 'ลองอีกครั้ง',
      'account_my_bookings_menu': 'การจองของฉัน',
      'account_logout_menu': 'ออกจากระบบ',
      'account_logout_cancel': 'ยกเลิก',
      'account_logout_confirm_message': 'ต้องการออกจากระบบหรือไม่?',
      'account_login_provider_google': 'เข้าสู่ระบบด้วย Google',
      'account_login_provider_kakao': 'เข้าสู่ระบบด้วย Kakao',
      'account_login_provider_line': 'เข้าสู่ระบบด้วย LINE',
      'account_sign_in_required': 'กรุณาเข้าสู่ระบบเพื่อดูบัญชีของคุณ',
      'account_booking_counts_title': 'ประวัติการจอง',
      'account_booking_count_waiting': 'รอจอง',
      'account_booking_count_assigned': 'จัดรถแล้ว',
      'account_booking_count_in_progress': 'กำลังใช้งาน',
      'account_booking_count_settlement_pending': 'รอชำระ',
      'account_booking_count_completed': 'เสร็จสิ้น',
      'account_booking_count_review_pending': 'รอรีวิว',
      'account_coupon_menu': 'คูปอง',
      'account_coupons_title': 'คูปอง',
      'account_coupons_available_section': 'คูปองที่ใช้ได้',
      'account_coupons_used_section': 'คูปองที่ใช้แล้ว',
      'account_coupons_empty_available': 'ไม่มีคูปองที่ใช้ได้',
      'account_coupons_empty_used': 'ยังไม่มีคูปองที่ใช้แล้ว',
      'account_coupons_load_failed': 'โหลดคูปองไม่สำเร็จ',
      'account_coupons_issued_at': 'วันที่ออก',
      'account_coupons_used_at': 'วันที่ใช้',
      'account_coupons_used_booking': 'หมายเลขการจอง',
      'booking_coupon_section_title': 'ใช้คูปอง',
      'booking_coupon_none': 'ไม่ใช้คูปอง',
      'booking_coupon_discount_label': 'ส่วนลดคูปอง',
      'booking_mileage_section_title': 'ใช้ไมล์',
      'booking_mileage_discount_label': 'ส่วนลดไมล์',
      'booking_mileage_available': 'ไมล์คงเหลือ: {balance} คะแนน',
      'booking_mileage_use_all': 'ใช้ทั้งหมด',
      'admin_coupons_menu': 'ออกคูปอง',
      'admin_coupon_issue_title': 'ออกคูปอง',
      'admin_coupon_search_hint': 'ค้นหาด้วยเบอร์โทร อีเมล หรือชื่อ',
      'admin_coupon_search_empty': 'ไม่พบลูกค้า',
      'admin_coupon_select_customer': 'เลือกลูกค้า',
      'admin_coupon_title_label': 'ชื่อคูปอง',
      'admin_coupon_amount_label': 'จำนวนส่วนลด',
      'admin_coupon_issue_button': 'ออกคูปョン',
      'admin_coupon_issue_success': 'ออกคูปองแล้ว',
      'admin_coupon_issue_failed': 'ออกคูปองไม่สำเร็จ',
      'admin_coupon_recent_title': 'คูปองที่ออกล่าสุด',
      'admin_coupon_recent_empty': 'ยังไม่มีคูปองที่ออก',
      'admin_coupon_cancel': 'ยกเลิก',
      'admin_coupon_cancel_success': 'ยกเลิกคูปองแล้ว',
      'admin_coupon_status_available': 'ใช้ได้',
      'admin_coupon_status_used': 'ใช้แล้ว',
      'admin_coupon_recent_customers_title': 'ลูกค้าที่ใช้บริการล่าสุด',
      'admin_coupon_recent_customers_empty': 'ยังไม่มีการจองที่เสร็จสิ้น',
      'admin_coupon_selected_customer': 'ลูกค้าที่เลือก',
      'admin_coupon_template_picker_title': 'เทมเพลตคูปอง',
      'admin_coupon_template_manual': 'กรอกเอง',
      'admin_coupon_template_manage_title': 'จัดการเทมเพลต',
      'admin_coupon_template_create_button': 'เพิ่มเทมเพลต',
      'admin_coupon_template_create_success': 'สร้างเทมเพลตแล้ว',
      'admin_coupon_template_create_failed': 'สร้างเทมเพลตไม่สำเร็จ',
      'admin_coupon_template_active': 'เปิดใช้งาน',
      'admin_coupon_template_inactive': 'ปิดใช้งาน',
      'admin_home_banner_menu': 'จัดการแบนเนอร์หน้าแรก',
      'admin_manual_booking_menu': 'ลงทะเบียนคอลด้วยตนเอง',
      'admin_manual_booking_title': 'ลงทะเบียนคอลโดยแอดมิน',
      'admin_manual_booking_origin': 'จุดรับ',
      'admin_manual_booking_destination': 'จุดส่ง',
      'admin_manual_booking_pickup_datetime': 'วันและเวลารับ',
      'admin_manual_booking_pickup_datetime_hint': 'เลือกวันและเวลารับ',
      'admin_manual_booking_pickup_timezone': 'เวลาท้องถิ่นประเทศไทย (UTC+7)',
      'admin_manual_booking_special_items_text': 'หมายเหตุสัมภาระพิเศษ',
      'admin_manual_booking_special_items_hint': 'อธิบายสัมภาระขนาดใหญ่หรือพิเศษ',
      'admin_manual_booking_flight_pickup_date_required':
          'เลือกวันรับก่อนค้นหาเที่ยวบิน',
      'admin_manual_booking_flight_manual_fallback_hint':
          'หากค้นหาเที่ยวบินไม่ได้ ให้กรอกหมายเลขเที่ยวบินและตั้งเวลารับด้วยตนเอง',
      'admin_manual_booking_flight_pickup_conflict_warning':
          'เวลารับที่คุณกรอกกับเวลาถึงตามกำหนดของเที่ยวบินต่างกัน {minutes} นาที',
      'admin_manual_booking_flight_pickup_keep_manual': 'ใช้เวลาที่กรอกเอง',
      'admin_manual_booking_flight_pickup_use_flight': 'เปลี่ยนตามเวลาเที่ยวบิน',
      'admin_manual_booking_flight_pickup_final_label': 'เวลารับสุดท้ายที่ขอ',
      'admin_dispatch_driver_pickup_conflict':
          'เวลารับขัดแย้งกับงานที่มอบหมายอื่นของคนขับคนนี้',
      'admin_manual_booking_vehicle_type': 'ประเภทรถ',
      'admin_manual_booking_passengers': 'จำนวนผู้โดยสาร',
      'admin_manual_booking_name_sign_hint':
          'คนขับจะเห็นป้ายว่าต้องถือป้ายชื่อสำหรับงานนี้',
      'admin_manual_booking_payout_amount': 'ยอดจ่ายคนขับ (THB)',
      'admin_manual_booking_customer_charge_amount': 'ยอดที่ลูกค้าจ่าย (THB)',
      'admin_manual_booking_customer_charge_hint':
          'ไม่บังคับ ใช้บันทึกภายในว่าลูกค้าจ่าย/จะจ่ายเท่าใด',
      'admin_manual_booking_payment_method': 'วิธีรับเงิน',
      'admin_manual_booking_payment_admin_collected': 'แอดมินรับเงินแล้ว',
      'admin_manual_booking_payment_driver_collects': 'คนขับเก็บเงินหน้างาน',
      'admin_manual_booking_customer_section': 'ข้อมูลลูกค้า',
      'admin_manual_booking_guest_name': 'ชื่อผู้ใช้ทั่วไป',
      'admin_manual_booking_guest_phone': 'เบอร์โทรผู้ใช้ทั่วไป',
      'admin_manual_booking_guest_email': 'อีเมลผู้ใช้ทั่วไป (ไม่บังคับ)',
      'admin_manual_booking_clear_customer': 'ล้างลูกค้าที่เลือก',
      'admin_manual_booking_memo': 'หมายเหตุ (ไม่บังคับ)',
      'admin_manual_booking_submit': 'สร้างงานเปิด',
      'admin_manual_booking_success': 'สร้างงานเปิดแล้ว หมายเลขจอง:',
      'admin_manual_booking_view_detail': 'ดูรายละเอียดจอง',
      'admin_manual_booking_assign_driver': 'มอบหมายคนขับ',
      'admin_manual_booking_edit': 'แก้ไข',
      'admin_manual_booking_edit_title': 'แก้ไขคอลที่ลงทะเบียนด้วยตนเอง',
      'admin_manual_booking_edit_not_allowed': 'ไม่สามารถแก้ไขในสถานะปัจจุบัน',
      'admin_manual_booking_cancel': 'ยกเลิกคอล',
      'admin_manual_booking_cancel_confirm':
          'ยกเลิกคอลที่ลงทะเบียนนี้หรือไม่? คนขับที่มอบหมายจะถูกปลด',
      'admin_manual_booking_cancel_success': 'ยกเลิกคอลที่ลงทะเบียนแล้ว',
      'admin_manual_booking_assign_success': 'มอบหมายคนขับแล้ว',
      'admin_manual_booking_update_submit': 'บันทึกการเปลี่ยนแปลง',
      'admin_manual_booking_update_success': 'อัปเดตคอลที่ลงทะเบียนแล้ว',
      'admin_manual_booking_failed': 'ไม่สามารถสร้างคอลด้วยตนเองได้',
      'admin_manual_booking_validation_required': 'กรุณากรอกข้อมูลที่จำเป็นให้ครบ',
      'admin_manual_booking_validation_customer':
          'เลือกสมาชิกหรือกรอกชื่อและเบอร์โทรของผู้ใช้ทั่วไป',
      'admin_manual_booking_preview_notice':
          'ตัวอย่างหน้าคนขับ: จะแสดงข้อความยืนยันบัญชีธนาคาร',
      'driver_call_badge_admin_manual': 'คอลที่แอดมินลงทะเบียน',
      'driver_call_badge_no_commission': 'ไม่มีค่าคอมมิชชัน',
      'driver_call_bank_account_confirm_notice':
          'จะโอนเข้าบัญชีที่ลงทะเบียนตอนสมัครสมาชิก กรุณาตรวจสอบว่าบัญชีถูกต้องอีกครั้ง',
      'admin_home_banner_title': 'จัดการแบนเนอร์หน้าแรก',
      'admin_home_banner_order_label': 'ลำดับการแสดง',
      'admin_home_banner_order_hint': 'เว้นว่างเพื่อเพิ่มท้ายสุด',
      'admin_home_banner_upload_button': 'อัปโหลดแบนเนอร์',
      'admin_home_banner_create_success': 'อัปโหลดแบนเนอร์แล้ว',
      'admin_home_banner_create_failed': 'อัปโหลดแบนเนอร์ไม่สำเร็จ',
      'admin_home_banner_empty': 'ยังไม่มีแบนเนอร์',
      'admin_home_banner_active': 'เปิดใช้งาน',
      'admin_home_banner_inactive': 'ปิดใช้งาน',
      'admin_home_banner_delete': 'ลบ',
      'admin_home_banner_delete_success': 'ลบแบนเนอร์แล้ว',
      'admin_home_banner_move_up': 'เลื่อนขึ้น',
      'admin_home_banner_move_down': 'เลื่อนลง',
      'account_feature_coming_soon': 'เร็วๆ นี้',
      'account_recent_bookings_title': 'การใช้งานล่าสุด',
      'account_recent_bookings_empty': 'ยังไม่มีการใช้งานล่าสุด',
      'account_recent_bookings_load_failed': 'โหลดการใช้งานล่าสุดไม่สำเร็จ',
      'account_my_reviews_menu': 'รีวิวของฉัน',
      'account_customer_support_menu': 'ศูนย์บริการลูกค้า',
      'account_mileage_history_title': 'ประวัติไมล์',
      'account_mileage_history_empty': 'ยังไม่มีประวัติไมล์',
      'account_mileage_history_load_failed': 'โหลดประวัติไมล์ไม่สำเร็จ',
      'account_my_reviews_title': 'รีวิวของฉัน',
      'account_my_reviews_empty': 'ยังไม่มีรีวิวที่เขียน',
      'account_my_reviews_load_failed': 'โหลดรีวิวไม่สำเร็จ',
      'account_mileage_type_accrue': 'สะสม',
      'account_mileage_type_reversal': 'ยกเลิก',
      'landing_privacy_policy_link': 'นโยบายความเป็นส่วนตัว',
      'privacy_policy_title': 'นโยบายความเป็นส่วนตัว',
      'privacy_policy_load_failed':
          'ไม่สามารถโหลดนโยบายความเป็นส่วนตัวได้ กรุณาลองใหม่อีกครั้ง',
      'analytics_consent_title': 'คุกกี้วิเคราะห์',
      'analytics_consent_body':
          'เราใช้ Google Analytics เพื่อวัดการเข้าชมและการแปลงการจอง ไม่ส่งชื่อ โทรศัพท์ หรืออีเมล คุณสามารถเปลี่ยนได้ในการตั้งค่าความเป็นส่วนตัว',
      'analytics_consent_allow': 'อนุญาตการวิเคราะห์',
      'analytics_consent_deny': 'ปฏิเสธ',
      'analytics_consent_privacy': 'นโยบายความเป็นส่วนตัว',
      'analytics_consent_settings': 'การตั้งค่าการวิเคราะห์',
      'analytics_consent_settings_title': 'การยินยอมการวิเคราะห์',
      'landing_language_label': 'ภาษา',
      'support_title': 'ศูนย์บริการลูกค้า',
      'support_contact_channels_title': 'ติดต่อศูนย์บริการลูกค้า',
      'support_contact_channels_hint':
          'แตะไอคอนแอปแชทที่ต้องการเพื่อเริ่มสนทนากับเราได้ทันที',
      'support_wechat_qr_dialog_title': 'QR Code WeChat',
      'support_wechat_qr_dialog_hint':
          'สแกน QR code นี้ในแอป WeChat เพื่อเพิ่มเราเป็นเพื่อน',
      'support_page_intro':
          'หากจองยากหรือต้องการสอบถาม โปรดติดต่อศูนย์บริการลูกค้า สามารถแนบรูปตั๋วเครื่องบิน เวาเชอร์โรงแรม หรือภาพหน้าจอการจองได้',
      'support_inquiry_button': 'สอบถาม',
      'support_dialog_title': 'สอบถามศูนย์บริการลูกค้า',
      'support_close_button': 'ปิด',
      'support_inquiry_title': 'จองและสอบถามผ่านศูนย์บริการลูกค้า',
      'support_inquiry_subtitle':
          'ฝากคำถามการจอง ข้อมูลเที่ยวบิน จุดรับ และปลายทางไว้ได้เลย',
      'support_default_guide':
          'สวัสดีค่ะ/ครับ นี่คือศูนย์บริการลูกค้า T-Ride กรุณาฝากคำถามการจอง ข้อมูลเที่ยวบิน จุดรับ และปลายทางไว้ แล้วเราจะตรวจสอบและแนะนำให้',
      'support_input_label': 'ข้อความสอบถาม',
      'support_input_hint': 'กรอกคำถามเกี่ยวกับการจองหรือการเดินทาง',
      'support_attach_button': 'แนบรูปภาพ',
      'support_send_button': 'ส่ง',
      'support_sending': 'กำลังส่ง...',
      'support_receipt_number': 'หมายเลขรับเรื่อง',
      'support_submit_failed':
          'ส่งคำถามไม่สำเร็จ กรุณาลองใหม่อีกครั้งในภายหลัง',
      'support_attachment_help':
          'สามารถแนบรูปตั๋วเครื่องบิน เวาเชอร์โรงแรม หรือภาพหน้าจอการจองได้',
      'support_no_attachments': 'ยังไม่ได้เลือกไฟล์แนบ',
      'support_auto_receipt':
          'ข้อความตอบรับอัตโนมัติ: ได้รับคำถามของคุณแล้ว ศูนย์บริการลูกค้าจะตรวจสอบและแนะนำให้',
      'support_faq_title': 'คำถามที่พบบ่อย',
      'support_faq_placeholder':
          'คำถามที่พบบ่อยจะอัปเดตตามลำดับความสำคัญเมื่อมีข้อมูลคำถามจากลูกค้าเพิ่มขึ้น',
      'trust_flight_delay': 'ติดตามความล่าช้าของเที่ยวบิน',
      'trust_flight_delay_desc': 'อัปเดตเวลารับอัตโนมัติเมื่อเที่ยวบินเปลี่ยน',
      'trust_fixed_pricing': 'ราคาคงที่',
      'trust_fixed_pricing_desc': 'ราคาที่จองคือราคาที่จ่ายจริง',
      'trust_professional_drivers': 'คนขับมืออาชีพ',
      'trust_professional_drivers_desc':
          'คนขับมืออาชีพที่ผ่านการตรวจสอบ ปลอดภัยและสบาย',
      'trust_multilingual': 'รองรับหลายภาษา',
      'trust_multilingual_desc': 'จองได้ด้วยอังกฤษ เกาหลี จีน ญี่ปุ่น และไทย',
      'payment_method': 'วิธีชำระเงิน',
      'pay_driver_at_destination': 'ชำระให้คนขับที่ปลายทาง',
      'customer_payment_method': 'วิธีชำระเงิน',
      'customer_payment_pay_driver': 'ชำระเงินให้คนขับเมื่อถึงจุดหมาย',
      'customer_payment_cash': 'ชำระด้วยเงินสด',
      'customer_payment_qr': 'ชำระผ่าน QR',
      'customer_payment_card': 'ชำระด้วยบัตร',
      'customer_payment_bank_transfer': 'โอนผ่านบัญชีธนาคาร',
      'customer_payment_unknown': 'ไม่พบข้อมูลวิธีชำระเงิน',
      'customer_price_conditions':
          'ค่าทางด่วน ค่าจอดรถสนามบิน และค่ารอเพิ่มเติมอาจแตกต่างกันตามเงื่อนไขการจอง',
      'customer_luggage_guidance':
          'กรุณาระบุจำนวนกระเป๋าใบใหญ่และถุงกอล์ฟให้ถูกต้อง เพื่อให้เราแนะนำรถที่เหมาะสม ความจุจริงขึ้นอยู่กับรถที่ได้รับมอบหมาย',
      'customer_vehicle_guidance':
          'หากมีผู้โดยสารหรือสัมภาระจำนวนมาก รถที่เล็กกว่าคำแนะนำอาจไม่สามารถเลือกได้',
      'customer_vehicle_not_suitable':
          'ไม่เหมาะกับจำนวนผู้โดยสารหรือสัมภาระที่เลือก',
      'customer_confirmation_title': 'ตรวจสอบการจองขั้นสุดท้าย',
      'customer_confirmation_subtitle':
          'กรุณาตรวจสอบรายละเอียดก่อนยืนยันการจอง',
      'customer_confirmation_edit': 'แก้ไข',
      'customer_confirmation_trip': 'รายละเอียดการเดินทาง',
      'customer_confirmation_passengers': 'ผู้โดยสารและสัมภาระ',
      'customer_confirmation_vehicle_payment': 'รถและการชำระเงิน',
      'customer_confirmation_customer': 'ข้อมูลลูกค้า',
      'customer_confirmation_required':
          'กรุณาตรวจสอบรายละเอียดก่อนยืนยันการจอง',
      'customer_next_action': 'ขั้นตอนถัดไป',
      'customer_driver_assignment': 'การมอบหมายคนขับ',
      'customer_driver_pending': 'กำลังมอบหมายคนขับ',
      'customer_status_unknown_guidance': 'กรุณาตรวจสอบสถานะการจองล่าสุด',
      'customer_rate_driver': 'ให้คะแนนคนขับ',
      'customer_review_thanks': 'ขอบคุณสำหรับการให้คะแนน',
      'messenger_type': 'ประเภทแอปแชท',
      'messenger_id': 'ID แอปแชท',
      'messenger_type_hint': 'LINE, WhatsApp ฯลฯ',
      'customer_contact_sns_notice':
          'หลังจองเสร็จ เราจะติดต่อคุณล่วงหน้าผ่าน SNS เพื่อแจ้งข้อมูลรถและคนขับ หากติดต่อไม่ได้ การจองอาจถูกยกเลิก กรุณาระบุเบอร์ติดต่อหรือบัญชี SNS ที่สามารถเพิ่มเพื่อนได้',
      'contact_connect_title': 'เชื่อมต่อ Messenger',
      'contact_connect_booking_number': 'หมายเลขจอง',
      'contact_connect_intro':
          'การจอง T-Rider ของคุณถูกสร้างแล้ว กรุณาเชื่อมต่อผ่าน Messenger เพื่อรับข้อมูลคนขับและรถ',
      'contact_connect_choose_channel': 'เลือก Messenger',
      'contact_connect_email_soon': 'อีเมล (เร็วๆ นี้)',
      'contact_connect_after_launch':
          'หลังส่งข้อความพร้อมหมายเลขอ้างอิงการจอง ให้แตะปุ่มด้านล่าง',
      'contact_connect_confirm_sent': 'ฉันส่งข้อความแล้ว',
      'contact_connect_waiting': 'T-Rider กำลังยืนยันข้อความของคุณ',
      'contact_connect_waiting_hint':
          'โดยปกติใช้เวลาไม่กี่นาที กรุณาเปิดหน้านี้ไว้',
      'contact_connect_step_created': 'สร้างการจองแล้ว',
      'contact_connect_step_connecting': 'กำลังเชื่อมต่อ',
      'contact_connect_step_complete': 'เสร็จสิ้น',
      'contact_connect_ref_copied': 'คัดลอกหมายเลขอ้างอิงแล้ว',
      'contact_connect_launch_failed': 'ไม่สามารถเปิดแอป Messenger ได้',
      'contact_connect_unsafe_url': 'ลิงก์ Messenger นี้ไม่ได้รับอนุญาต',
      'contact_connect_load_failed': 'ไม่สามารถโหลดสถานะการเชื่อมต่อได้',
      'contact_connect_wechat_title': 'WeChat',
      'contact_connect_wechat_body':
          'สแกน QR code หรือคัดลอก WeChat ID แล้วส่งหมายเลขอ้างอิงการจอง',
      'contact_connect_wechat_id_copied': 'คัดลอก WeChat ID แล้ว',
      'contact_connect_invalid_link': 'ลิงก์เชื่อมต่อไม่ถูกต้อง',
      'additional_requests': 'คำขอเพิ่มเติม',
      'country_code_hint': 'TH, US, KR…',
      'country_search_hint': 'ค้นหาประเทศหรือกรอกเอง',
      'boarding_qr_title': 'QR สำหรับขึ้นรถ',
      'boarding_qr_hint':
          'แสดง QR นี้ให้คนขับสแกนตอนขึ้นรถ เมื่อคนขับสแกนแล้ว สถานะจะเปลี่ยนเป็นขึ้นรถเรียบร้อย',
      'booking_trust_message':
          'กรุณาเก็บหมายเลขการจองไว้ คุณสามารถตรวจสอบการจัดสรรคนขับและสถานะการเดินทางได้ที่หน้าค้นหาการจอง',
      'airport_meeting_semantics': 'คำแนะนำจุดนัดพบสนามบิน BKK ประตู {gate}',
      'airport_meeting_gate_3_title': 'คำแนะนำป้ายชื่อ · ประตู 3',
      'airport_meeting_gate_7_title': 'คำแนะนำจุดรับรถ · ประตู 7',
      'airport_meeting_gate_inside': 'ด้านในประตู',
      'airport_meeting_gate_outside': 'ด้านนอกประตู',
      'airport_meeting_no_picket_highlight':
          'หลังรับกระเป๋าแล้ว กรุณาแจ้งพร้อมรับรถก่อนออกจากสนามบิน',
      'airport_meeting_picket_highlight':
          'กรุณาอย่าออกนอกสนามบิน ให้รอด้านในประตู 3 และมองหาป้ายชื่อ',
      'airport_meeting_no_picket_body_1':
          'คนขับต้องใช้เวลาเคลื่อนรถไปยังจุดรับรถด้านนอกประตู 7',
      'airport_meeting_no_picket_body_2':
          'กรุณาตรวจสอบทะเบียนรถและคนขับก่อนขึ้นรถ',
      'airport_meeting_picket_body_1':
          'หลังรับกระเป๋าแล้ว ไปที่ด้านในประตู 3 และมองหาป้ายชื่อของคุณ',
      'airport_meeting_picket_body_2':
          'เจ้าหน้าที่จะประสานให้คนขับและรถมาที่หน้าประตู 3',
      'airport_meeting_step_luggage': 'รับกระเป๋าทั้งหมดให้เรียบร้อย',
      'airport_meeting_step_notify_driver': 'แจ้งคนขับว่าคุณพร้อมรับรถแล้ว',
      'airport_meeting_step_vehicle_number': 'ตรวจสอบหมายเลขทะเบียนรถที่ได้รับ',
      'airport_meeting_step_gate_7': 'ไปด้านนอกประตู 7 ชั้นผู้โดยสารขาเข้า',
      'airport_meeting_step_board': 'ยืนยันรถและคนขับ แล้วขึ้นรถ',
      'airport_meeting_step_gate_3': 'ไปด้านในประตู 3 ชั้นผู้โดยสารขาเข้า',
      'airport_meeting_step_find_picket': 'มองหาป้ายชื่อที่มีชื่อของคุณ',
      'airport_meeting_step_wait_staff': 'รอสักครู่ตามคำแนะนำของเจ้าหน้าที่',
      'airport_meeting_step_board_after_staff':
          'เมื่อได้รับแจ้งว่ารถมาถึงแล้ว ค่อยออกไปขึ้นรถ',
      'airport_meeting_do_not_exit': 'อย่าออกนอกสนามบินก่อนเจ้าหน้าที่แนะนำ',
      'airport_meeting_vehicle_pending':
          'ข้อมูลคนขับและรถยังรอการมอบหมาย จะแสดงที่นี่หลังมอบหมายแล้ว',
      'airport_meeting_vehicle_info': 'ข้อมูลคนขับและรถ',
      'airport_meeting_driver': 'คนขับ',
      'airport_meeting_driver_phone': 'เบอร์คนขับ',
      'airport_meeting_vehicle_type': 'ประเภทรถ',
      'airport_meeting_vehicle_color': 'สีรถ',
      'airport_meeting_vehicle_plate': 'ทะเบียนรถ',
      'ui_load_failed': 'โหลดข้อมูลไม่ได้ กรุณาลองอีกครั้ง',
      'ui_action_failed': 'ดำเนินการไม่สำเร็จ กรุณาลองอีกครั้ง',
      'ui_generic_error': 'เกิดข้อผิดพลาด กรุณาลองอีกครั้ง',
      'ui_retry': 'ลองอีกครั้ง',
      'loading_recommendation': 'กำลังโหลดคำแนะนำ...',
      'calculating_price': 'กำลังคำนวณราคา...',
      'guest_lookup_title': 'ค้นหาการจอง',
      'guest_lookup_loading': 'กำลังโหลดการจองที่บันทึกไว้...',
      'guest_lookup_subtitle': 'กรอกหมายเลขจองและเบอร์โทรเพื่อดูรายละเอียดทริป',
      'guest_lookup_booking_number': 'หมายเลขจอง',
      'guest_lookup_phone': 'เบอร์โทรศัพท์',
      'guest_lookup_invalid_number': 'กรอกหมายเลขจองที่ถูกต้อง',
      'guest_lookup_invalid_phone': 'กรอกเบอร์โทรที่ใช้จอง',
      'guest_lookup_find': 'ค้นหาการจอง',
      'guest_lookup_not_found': 'ไม่พบการจอง กรุณาตรวจสอบหมายเลขจองและเบอร์โทร',
      'guest_lookup_load_error': 'โหลดการจองไม่ได้ กรุณาลองอีกครั้ง',
      'guest_lookup_trip_details': 'รายละเอียดทริป',
      'guest_lookup_pickup': 'รับ',
      'guest_lookup_service': 'บริการ',
      'guest_lookup_from': 'จาก',
      'guest_lookup_to': 'ถึง',
      'guest_lookup_driver': 'คนขับ',
      'guest_lookup_driver_phone': 'เบอร์คนขับ',
      'guest_lookup_driver_status': 'สถานะคนขับ',
      'guest_lookup_vehicle': 'รถ',
      'guest_lookup_vehicle_photo_load_failed': 'ไม่สามารถโหลดรูปรถได้',
      'guest_lookup_payment': 'การชำระเงิน',
      'guest_lookup_total': 'รวม',
      'guest_lookup_another': 'ค้นหาการจองอื่น',
      'guest_lookup_refresh': 'รีเฟรช',
      'guest_lookup_refresh_needs_phone':
          'กรุณากรอกเบอร์โทรในแบบฟอร์มค้นหาเพื่อรีเฟรชการจองนี้',
      'booking_complete_track_cta': 'ติดตามการจองของฉัน',
      'auth_social_login_title':
          'Create an account to make your next booking easier',
      'auth_social_login_description':
          'View booking history and auto-fill your details on future trips.',
      'auth_google_continue': 'Continue with Google',
      'auth_kakao_continue': 'ดำเนินการต่อด้วย Kakao',
      'auth_kakao_callback_error':
          'เข้าสู่ระบบ Kakao ไม่สำเร็จ โปรดลองอีกครั้ง',
      'auth_kakao_callback_already_processed':
          'คำขอเข้าสู่ระบบนี้ถูกดำเนินการแล้ว โปรดลองอีกครั้งหากจำเป็น',
      'auth_kakao_callback_slow_loading': 'ใช้เวลานานกว่าปกติ โปรดรีเฟรชหน้า',
      'auth_kakao_callback_refresh': 'รีเฟรชหน้า',
      'profile_completion_title': 'กรอกข้อมูลโปรไฟล์',
      'profile_completion_body':
          'กรุณากรอกชื่อและหมายเลขโทรศัพท์เพื่อใช้งาน T-Rider ต่อ',
      'profile_completion_submit': 'บันทึกและดำเนินการต่อ',
      'profile_completion_name_required': 'กรุณากรอกชื่อ',
      'profile_completion_phone_required':
          'กรุณากรอกหมายเลขโทรศัพท์ที่ถูกต้อง (อย่างน้อย 5 ตัวอักษร)',
      'profile_completion_phone_duplicate':
          'หมายเลขโทรศัพท์นี้ถูกใช้งานโดยบัญชีอื่นแล้ว',
      'auth_line_continue': 'ดำเนินการต่อด้วย LINE',
      'auth_line_callback_error': 'เข้าสู่ระบบ LINE ไม่สำเร็จ โปรดลองอีกครั้ง',
      'auth_line_callback_state_mismatch':
          'ไม่สามารถยืนยันการเข้าสู่ระบบ LINE ได้ โปรดลองอีกครั้ง',
      'auth_line_callback_already_processed':
          'คำขอเข้าสู่ระบบนี้ถูกดำเนินการแล้ว โปรดลองอีกครั้งหากจำเป็น',
      'auth_line_callback_slow_loading': 'ใช้เวลานานกว่าปกติ โปรดรีเฟรชหน้า',
      'auth_line_callback_refresh': 'รีเฟรชหน้า',
      'auth_social_login_later': 'Maybe later',
      'auth_social_login_connected': '{name}, your account is connected.',
      'my_bookings_title': 'การจองของฉัน',
      'my_bookings_empty':
          'ยังไม่มีการจองที่เชื่อมต่อ จองและเข้าสู่ระบบเพื่อดูรายการที่นี่',
      'my_bookings_view_button': 'ดูการจองของฉัน',
      'my_bookings_driver_summary': 'คนขับ: {name}',
      'my_bookings_load_error': 'ไม่สามารถโหลดรายการจองได้ โปรดลองอีกครั้ง',
      'booking_cancel_section': 'ยกเลิกการจอง',
      'booking_cancel_action': 'ยกเลิกการจอง',
      'booking_cancel_policy_hint':
          'คุณสามารถยกเลิกได้จนถึง 2 ชั่วโมงก่อนเวลารับ',
      'booking_cancel_confirm_title': 'ต้องการยกเลิกการจองนี้หรือไม่?',
      'booking_cancel_confirm_action': 'ยืนยันการยกเลิก',
      'booking_cancel_confirm_irreversible':
          'เมื่อยกเลิกแล้วจะไม่สามารถย้อนกลับได้',
      'booking_cancel_success': 'ยกเลิกการจองของคุณแล้ว',
      'booking_cancel_failed': 'ไม่สามารถยกเลิกการจองนี้ได้ กรุณาลองอีกครั้ง',
      'booking_cancel_blocked_within_two_hours':
          'ไม่สามารถยกเลิกการจองภายใน 2 ชั่วโมงก่อนเวลารับที่กำหนด',
      'booking_cancel_blocked_trip_started':
          'ไม่สามารถยกเลิกการจองที่เริ่มการเดินทางแล้ว',
      'booking_cancel_blocked_already_cancelled': 'การจองนี้ถูกยกเลิกแล้ว',
      'booking_cancel_blocked_completed':
          'ไม่สามารถยกเลิกการจองที่เสร็จสิ้นแล้ว',
      'booking_cancel_blocked_no_show':
          'ไม่สามารถยกเลิกการจองที่ถูกทำเครื่องหมายว่าไม่มา',
      'booking_cancel_blocked_invalid_pickup':
          'ไม่สามารถยกเลิกการจองนี้ได้เนื่องจากเวลารับไม่ถูกต้อง',
      'booking_cancel_blocked_locked': 'การจองนี้ถูกล็อกและไม่สามารถยกเลิกได้',
      'booking_cancel_driver_assigned': 'มอบหมายคนขับแล้ว: {name}',
      'booking_cancel_driver_assigned_generic': 'มอบหมายคนขับแล้ว',
      'guest_status_guidance_pending':
          'ได้รับการจองของคุณแล้ว เราจะยืนยันในเร็วๆ นี้',
      'guest_status_guidance_confirmed':
          'การจองได้รับการยืนยันแล้ว จะมีการมอบหมายคนขับก่อนเวลารับ',
      'guest_status_guidance_reassignment':
          'คนขับเดิมไม่สามารถรับงานได้ กำลังจัดหาคนขับใหม่ ตารางเวลาและค่าโดยสารไม่เปลี่ยนแปลง',
      'status_reassignment_in_progress': 'กำลังจัดหาคนขับใหม่',
      'admin_ops_severity_critical': 'เร่งด่วนมาก',
      'admin_ops_reason_critical_reassignment': 'จัดคนขับใหม่เร่งด่วนมาก',
      'admin_ops_reason_urgent_reassignment': 'จัดคนขับใหม่เร่งด่วน',
      'admin_ops_reason_driver_released_reassignment':
          'คนขับคืนงาน · จัดคนขับใหม่',
      'admin_ops_reason_critical_unassigned': 'เร่งด่วนมาก · ยังไม่มอบหมาย',
      'admin_ops_reason_urgent_unassigned': 'เร่งด่วน · ยังไม่มอบหมาย',
      'driver_release_assignment': 'คืนงาน',
      'driver_release_assignment_title':
          'ต้องการคืนงานที่ได้รับมอบหมายนี้หรือไม่?',
      'driver_release_assignment_message':
          'การจองของลูกค้ายังคงอยู่ และจะมอบหมายคนขับคนอื่น',
      'driver_release_assignment_cancel': 'เก็บงานไว้',
      'driver_release_assignment_confirm': 'คืนงาน',
      'driver_release_assignment_success': 'ยกเลิกการมอบหมายงานแล้ว',
      'driver_release_assignment_failed':
          'คืนงานไม่สำเร็จ กรุณาลองใหม่อีกครั้ง',
      'driver_release_assignment_blocked': 'สถานะปัจจุบันไม่สามารถคืนงานได้',
      'driver_release_assignment_emergency_hint':
          'ภายใน 2 ชั่วโมงก่อนเวลารับ การคืนงานปกติถูกจำกัด คืนงานได้เฉพาะกรณีฉุกเฉินเท่านั้น',
      'driver_release_assignment_irreversible':
          'เมื่อคืนงานแล้วจะย้อนกลับไม่ได้',
      'driver_release_reason_label': 'เหตุผลที่คืนงาน',
      'driver_release_reason_detail_label': 'รายละเอียด',
      'driver_release_reason_detail_required': 'กรุณาระบุเหตุผลอื่น',
      'driver_release_reason_VEHICLE_BREAKDOWN': 'รถเสีย',
      'driver_release_reason_ACCIDENT': 'อุบัติเหตุ',
      'driver_release_reason_DRIVER_ILLNESS': 'เจ็บป่วย',
      'driver_release_reason_FAMILY_EMERGENCY': 'เหตุฉุกเฉินครอบครัว',
      'driver_release_reason_SCHEDULE_CONFLICT': 'ตารางชนกัน',
      'driver_release_reason_LOCATION_TOO_FAR': 'ระยะทางไกลเกินไป',
      'driver_release_reason_OTHER': 'อื่นๆ',
      'driver_assignment_ended_confirm': 'ตกลง',
      'driver_booking_not_found': 'ไม่พบข้อมูลการจอง',
      'driver_assignment_ended_generic_title': 'ยกเลิกการมอบหมายงานแล้ว',
      'driver_assignment_ended_generic_message':
          'ไม่สามารถดำเนินการจองนี้ต่อได้',
      'driver_assignment_ended_generic_snackbar':
          'ยกเลิกการมอบหมายงาน {bookingNumber} แล้ว',
      'driver_assignment_ended_customer_title': 'ลูกค้ายกเลิกการจองแล้ว',
      'driver_assignment_ended_customer_message':
          'งานนี้ถูกยกเลิกการมอบหมายโดยอัตโนมัติเนื่องจากลูกค้ายกเลิกการจอง',
      'driver_assignment_ended_customer_snackbar':
          'ลูกค้ายกเลิก {bookingNumber} งานถูกยกเลิกการมอบหมายแล้ว',
      'driver_assignment_ended_admin_title': 'แอดมินยกเลิกการจองแล้ว',
      'driver_assignment_ended_admin_message':
          'แอดมินยกเลิกการจองแล้ว งานของคุณถูกยกเลิกการมอบหมาย',
      'driver_assignment_ended_admin_snackbar':
          'แอดมินยกเลิก {bookingNumber} งานถูกยกเลิกการมอบหมายแล้ว',
      'driver_assignment_ended_released_title': 'คืนงานเรียบร้อยแล้ว',
      'driver_assignment_ended_released_message':
          'คืนงานเรียบร้อยแล้ว การจองนี้จะเปิดให้คนขับคนอื่นอีกครั้ง',
      'driver_assignment_ended_released_snackbar':
          'คืนงาน {bookingNumber} เรียบร้อยแล้ว',
      'driver_assignment_ended_reassigned_title': 'มอบหมายให้คนขับคนอื่นแล้ว',
      'driver_assignment_ended_reassigned_message':
          'การจองนี้ถูกมอบหมายให้คนขับคนอื่นแล้ว',
      'driver_assignment_ended_reassigned_snackbar':
          '{bookingNumber} ถูกมอบหมายให้คนขับคนอื่นแล้ว',
      'driver_assignment_ended_completed_title': 'การเดินทางเสร็จสิ้นแล้ว',
      'driver_assignment_ended_completed_message': 'การจองนี้เสร็จสิ้นแล้ว',
      'guest_status_guidance_driver_assigned':
          'มอบหมายคนขับแล้ว และจะออกเดินทางไปจุดรับเร็วๆ นี้',
      'guest_status_guidance_on_route': 'คนขับกำลังเดินทางไปจุดรับ',
      'guest_status_guidance_driver_arrived': 'คนขับถึงจุดรับแล้ว',
      'guest_status_guidance_picked_up': 'กำลังเดินทางไปยังจุดหมาย',
      'guest_status_guidance_settlement_pending':
          'การเดินทางสิ้นสุดแล้ว ขอบคุณที่ใช้บริการ',
      'guest_status_guidance_completed':
          'การเดินทางเสร็จสิ้นแล้ว กรุณาให้คะแนนคนขับ',
      'guest_status_guidance_cancelled':
          'การจองนี้ถูกยกเลิกแล้ว กรุณาติดต่อศูนย์บริการลูกค้าหากต้องการข้อมูลเพิ่มเติม',
      'guest_status_guidance_no_show':
          'การจองนี้ถูกทำเครื่องหมายว่าไม่มาตามนัด',
      'guest_lookup_issue_dropoff_qr': 'ออก QR ลงรถ',
      'guest_lookup_trip_completed_qr_hidden':
          'ทริปเสร็จสิ้น QR ที่ใช้งานอยู่จะถูกซ่อน',
      'guest_lookup_dropoff_qr_token': 'โทเค็น QR ลงรถ: {token}',
      'guest_lookup_boarding_qr_security':
          'QR ขึ้นรถออกเมื่อสร้างการจอง ไม่สามารถกู้คืนจากหน้าค้นหาได้',
      'guest_lookup_booking_number_hint': 'TX202607010001',
      'booking_trip_completed': 'ทริปเสร็จสิ้น',
      'booking_dropoff_qr_title': 'QR สำหรับจบการเดินทาง',
      'booking_dropoff_qr_hint':
          'ตอนลงรถ กรุณาแสดง QR นี้ให้คนขับสแกนอีกครั้ง เมื่อสแกนแล้ว ระบบจะเปลี่ยนสถานะเป็นสิ้นสุดการเดินทาง',
      'booking_dropoff_qr_unavailable':
          'QR ลงรถใช้ได้หลังขึ้นรถและก่อนทริปเสร็จ',
      'booking_qr_load_failed':
          'ไม่สามารถโหลด QR ได้ กรุณาตรวจสอบสถานะการจองหรือติดต่อแอดมิน',
      'booking_refresh_dropoff_qr': 'รีเฟรช QR ลงรถ',
      'booking_issue_new_dropoff_qr': 'ออก QR ลงรถใหม่',
      'booking_review_title': 'ให้คะแนนทริป',
      'booking_review_thanks': 'ขอบคุณสำหรับรีวิว',
      'booking_review_rating': 'คะแนน: {rating} / 5',
      'booking_review_comment': 'ความคิดเห็น (ไม่บังคับ)',
      'booking_review_submitting': 'กำลังส่ง...',
      'booking_review_submit': 'ส่งรีวิว',
      'booking_review_load_error': 'โหลดรีวิวไม่ได้',
      'review_card_title': 'การเดินทางครั้งนี้เป็นอย่างไรบ้าง?',
      'review_card_subtitle': 'กรุณาเลือกคะแนน',
      'review_star_label': '{value} ดาว',
      'review_rating_desc_1': 'ไม่พอใจมาก',
      'review_rating_desc_2': 'ไม่พอใจ',
      'review_rating_desc_3': 'ปานกลาง',
      'review_rating_desc_4': 'พอใจ',
      'review_rating_desc_5': 'พอใจมาก',
      'review_comment_label': 'แสดงความคิดเห็นเพิ่มเติม (ไม่บังคับ)',
      'review_comment_hint': 'กรุณาอย่าใส่ข้อมูลส่วนตัว',
      'review_submit_button': 'ส่งคะแนน',
      'review_success_title': 'ขอบคุณสำหรับความคิดเห็นของคุณ',
      'review_success_body':
          'ความคิดเห็นของคุณช่วยให้เราพัฒนาบริการให้ดียิ่งขึ้น',
      'review_tag_FRIENDLY': 'เป็นมิตร',
      'review_tag_SAFE_DRIVING': 'ขับรถปลอดภัย',
      'review_tag_CLEAN_VEHICLE': 'รถสะอาด',
      'review_tag_ON_TIME': 'ตรงเวลา',
      'review_tag_GOOD_COMMUNICATION': 'สื่อสารดี',
      'review_tag_UNSAFE_DRIVING': 'ขับรถไม่ปลอดภัย',
      'review_tag_LATE_ARRIVAL': 'มาสาย',
      'review_tag_VEHICLE_NOT_CLEAN': 'รถไม่สะอาด',
      'review_tag_UNFRIENDLY_SERVICE': 'บริการไม่สุภาพ',
      'review_tag_ROUTE_ISSUE': 'มีปัญหาเรื่องเส้นทาง',
      'review_tag_OTHER_ISSUE': 'ปัญหาอื่น',
      'admin_booking_review_section': 'รีวิวจากลูกค้า',
      'admin_detail_more_reasons': 'เหตุผลเพิ่มเติม {count} รายการ',
      'admin_detail_customer': 'ข้อมูลลูกค้า',
      'admin_detail_trip': 'ข้อมูลการเดินทาง',
      'admin_detail_driver_vehicle': 'คนขับและรถ',
      'admin_detail_pricing_settlement': 'ค่าโดยสารและการชำระบัญชี',
      'admin_detail_available': 'ใช้งานได้',
      'admin_detail_status_history': 'ประวัติการเปลี่ยนสถานะ',
      'admin_detail_technical': 'ข้อมูลทางเทคนิค',
      'admin_detail_booking_id': 'รหัสการจองภายใน',
      'admin_detail_raw_status': 'สถานะการจองดิบ',
      'admin_detail_service_code': 'รหัสประเภทบริการ',
      'admin_detail_updated': 'เวลาแก้ไข',
      'admin_detail_created': 'เวลาสร้างการจอง',
      'admin_pickup_requested_at': 'เวลาที่ขอรับ',
      'admin_detail_country': 'ประเทศ',
      'admin_detail_passengers': 'ผู้โดยสาร (ผู้ใหญ่ / เด็ก / ทารก)',
      'admin_detail_luggage': 'สัมภาระ',
      'admin_detail_special_requests': 'คำขอพิเศษ',
      'admin_customer_preference_summary_prefix': 'ความต้องการของลูกค้า:',
      'admin_customer_preference_badge': 'ความต้องการ',
      'admin_detail_flight': 'เที่ยวบิน',
      'admin_detail_recommend_drivers': 'แนะนำคนขับ',
      'admin_notes_title': 'บันทึกภายใน',
      'admin_notes_admin_only': 'เฉพาะผู้ดูแลระบบเท่านั้นที่สามารถดูได้',
      'admin_notes_hint': 'เพิ่มบันทึกการดำเนินงาน',
      'admin_notes_add': 'เพิ่มบันทึก',
      'admin_notes_empty': 'ยังไม่มีบันทึกภายใน',
      'admin_notes_append_only':
          'ไม่สามารถแก้ไขบันทึกเดิมได้ กรุณาเพิ่มบันทึกใหม่เพื่อแก้ไขข้อมูล',
      'admin_notes_remaining': 'เหลือ {count} ตัวอักษร',
      'admin_notes_load_failed': 'ไม่สามารถโหลดบันทึกภายในได้',
      'admin_notes_add_failed': 'ไม่สามารถเพิ่มบันทึกภายในได้',
      'admin_booking_low_rating_badge': 'ต้องตรวจสอบคะแนนต่ำ',
      'admin_booking_review_rating': 'คะแนน',
      'admin_booking_review_tags': 'แท็ก',
      'admin_booking_review_comment': 'ความคิดเห็น',
      'booking_notification_updates': 'อัปเดต',
      'booking_notification_enabling': 'กำลังเปิดใช้...',
      'booking_notification_enable': 'เปิดการแจ้งเตือน',
      'booking_notification_empty': 'ยังไม่มีอัปเดต',
      'booking_notification_default_title': 'การแจ้งเตือน',
      'booking_notification_load_error': 'โหลดการแจ้งเตือนไม่ได้',
      'booking_notification_enabled': 'เปิดการแจ้งเตือนแล้ว',
      'booking_notification_permission_denied': 'สิทธิ์การแจ้งเตือนถูกปฏิเสธ',
      'booking_notification_unsupported':
          'เบราว์เซอร์นี้ไม่รองรับ push notification',
      'booking_notification_config_missing':
          'ไม่ได้ตั้งค่า push notification ในสภาพแวดล้อมนี้',
      'booking_notification_register_failed': 'ลงทะเบียนการแจ้งเตือนไม่สำเร็จ',
      'track_driver_title': 'ติดตามคนขับ',
      'track_driver_unavailable': 'ยังไม่มีตำแหน่งคนขับ',
      'track_driver_stale': 'ตำแหน่งอาจไม่เป็นปัจจุบัน',
      'track_driver_live': 'ตำแหน่งสด',
      'track_driver_updated': 'อัปเดต {time}',
      'track_driver_load_error': 'โหลดตำแหน่งคนขับไม่ได้',
      'customer_driver_location_title': 'ตำแหน่งคนขับ',
      'customer_driver_location_waiting':
          'มีการจัดคนขับแล้ว คุณจะเห็นตำแหน่งสดเมื่อคนขับเริ่มเดินทางไปยังจุดรับ',
      'customer_driver_location_live': 'กำลังแชร์ตำแหน่งคนขับ',
      'customer_driver_location_updated_label': 'อัปเดตล่าสุด',
      'customer_driver_location_updated_now': 'อัปเดตเมื่อสักครู่',
      'customer_driver_location_updated_minutes':
          'อัปเดตเมื่อ {minutes} นาทีที่แล้ว',
      'customer_driver_location_temporarily_unavailable':
          'ไม่สามารถตรวจสอบตำแหน่งคนขับได้ชั่วคราว กรุณาลองใหม่อีกครั้ง',
      'customer_driver_location_stale': 'ตำแหน่งคนขับไม่ได้อัปเดตล่าสุด',
      'customer_driver_location_ended': 'การแชร์ตำแหน่งสดสิ้นสุดแล้ว',
      'customer_driver_location_open_map': 'เปิดในแผนที่',
      'customer_driver_location_recenter': 'ไปยังตำแหน่งคนขับ',
      'customer_driver_on_route': 'คนขับกำลังเดินทางไปยังจุดรับ',
      'customer_driver_arrived':
          'คนขับมาถึงจุดรับแล้ว กรุณาตรวจสอบข้อมูลคนขับและรถ',
      'customer_trip_in_progress': 'กำลังเดินทาง',
      'customer_driver_location_load_failed':
          'ไม่สามารถตรวจสอบตำแหน่งคนขับได้ กรุณาลองใหม่อีกครั้ง',
      'customer_driver_location_retry': 'ลองอีกครั้ง',
      'wizard_step_progress': 'ขั้นตอน {current} จาก {total}',
      'pickup_date_past': 'วันรับไม่สามารถเป็นอดีตได้',
      'pickup_time_minimum': 'เวลารับต้องอยู่หลังเวลาปัจจุบัน',
      'pickup_time_am': 'AM',
      'pickup_time_pm': 'PM',
      'pickup_time_select': 'เลือกเวลารับ',
      'pickup_time_enter_manually': 'กรอกเวลาเอง',
      'pickup_time_invalid': 'กรุณากรอกเวลาที่ถูกต้อง เช่น 09:30 AM',
      'pickup_time_apply_manual': 'ใช้',
      'field_required': '(จำเป็น)',
      'name_sign_info_before_price':
          'หากเลือกบริการป้ายชื่อ จะมีค่าบริการเพิ่ม ',
      'name_sign_info_after_price': ' และจุดรับจะเปลี่ยนเป็น ',
      'name_sign_info_after_gate': '。',
      'name_sign_service_price_highlight': '100 บาท',
      'name_sign_service_gate_highlight': 'ประตู 3',
      'name_sign_text_label': 'ชื่อที่จะแสดงบนป้าย',
      'name_sign_text_hint': 'เช่น SOMCHAI JAIDEE',
      'wizard_required_name_sign_text': 'กรุณากรอกชื่อที่จะแสดงบนป้าย',
      'wizard_customer_email_invalid': 'กรุณากรอกอีเมลที่ถูกต้อง',
      'pickup_datetime_required': 'กรุณากรอกวันและเวลารับ',
      'booking_location_reselect_required':
          'ไม่สามารถระบุพิกัดสถานที่ได้ กรุณาเลือกจุดรับ/จุดหมายอีกครั้ง',
      'flight_number_hint': 'TG401, 7C2203',
      'flight_number_invalid':
          'กรุณาตรวจสอบหมายเลขเที่ยวบิน เช่น TG401 หรือ 7C2203',
      'flight_lookup_search': 'ค้นหา',
      'flight_lookup_not_found':
          'ไม่พบเที่ยวบินสำหรับหมายเลขนี้ กรุณาตรวจสอบหมายเลขเที่ยวบินอีกครั้ง',
      'flight_lookup_invalid_date':
          'ไม่สามารถค้นหาเที่ยวบินด้วยวันรับได้ คุณยังสามารถจองต่อโดยใช้หมายเลขเที่ยวบินที่กรอกไว้ได้',
      'flight_lookup_provider_unavailable':
          'บริการค้นหาเที่ยวบินไม่พร้อมใช้งานชั่วคราว คุณยังสามารถจองต่อโดยใช้หมายเลขเที่ยวบินที่กรอกไว้ได้',
      'flight_lookup_departure': 'ออกเดินทาง',
      'flight_lookup_arrival': 'ถึงที่หมาย',
      'flight_lookup_estimated_arrival': 'เวลาถึงโดยประมาณ',
      'flight_lookup_confirm': 'นี่คือเที่ยวบินของฉัน',
      'booking_number_copied': 'คัดลอกหมายเลขจองแล้ว',
      'booking_number_copy_failed': 'ไม่สามารถคัดลอกหมายเลขจองได้',
      'booking_number_copy': 'คัดลอกหมายเลขจอง',
      'booking_number_save_notice':
          'กรุณาเก็บหมายเลขจองไว้ คุณจะต้องใช้ในการตรวจสอบการจอง',
      'wizard_edit_section': 'แก้ไข',
      'wizard_section_incomplete': 'กรุณาทำขั้นตอนนี้ให้เสร็จก่อนดำเนินการต่อ',
      'wizard_completion_progress': 'กรอกข้อมูลที่จำเป็น {completed}/{total}',
      'wizard_required_service': 'กรุณาเลือกประเภทบริการ',
      'wizard_step_route_title': 'เส้นทาง',
      'wizard_step_schedule_title': 'วันที่และเวลา',
      'wizard_step_vehicle_title': 'เลือกรถ',
      'wizard_step_customer_title': 'ข้อมูลผู้โดยสาร',
      'wizard_step_review_title': 'ตรวจสอบการจอง',
      'wizard_step_indicator': '{current}/{total} {title}',
      'wizard_cta_route': 'เลือกวันที่และเวลา',
      'wizard_cta_schedule': 'เลือกรถ',
      'wizard_cta_vehicle_unselected': 'กรุณาเลือกรถ',
      'wizard_cta_vehicle_selected': 'กรอกข้อมูลผู้โดยสาร · {price}',
      'wizard_cta_customer': 'ตรวจสอบการจอง · {price}',
      'wizard_cta_confirm': 'ยืนยันการจอง · {price}',
      'wizard_same_place_error': 'จุดต้นทางและปลายทางต้องไม่เหมือนกัน',
      'wizard_swap_route': 'สลับต้นทาง/ปลายทาง',
      'wizard_trust_toll_included': 'รวมค่าทางด่วนแล้ว',
      'wizard_trust_no_airport_parking': 'ไม่มีค่าจอดรถสนามบิน',
      'wizard_trust_no_night_surcharge': 'ไม่มีค่าบริการเพิ่มช่วงดึก/เช้ามืด',
      'wizard_trust_flight_delay_wait': 'รอฟรีหากเที่ยวบินล่าช้า',
      'wizard_places_searching': 'กำลังค้นหาสถานที่…',
      'wizard_places_no_results_hint':
          'ไม่พบสถานที่ ลองคำค้นหาอื่นหรือเลือกจากแผนที่',
      'wizard_summary_expand': 'แสดงสรุปการจอง',
      'wizard_summary_collapse': 'ซ่อนสรุปการจอง',
      'wizard_required_origin': 'กรุณาเลือกจุดต้นทาง',
      'wizard_required_destination': 'กรุณาเลือกจุดปลายทาง',
      'wizard_required_passengers': 'กรุณากรอกข้อมูลผู้โดยสาร',
      'wizard_required_vehicle': 'กรุณาเลือกรถ',
      'wizard_required_customer': 'กรุณากรอกข้อมูลของคุณ',
      'wizard_required_customer_name': 'กรุณากรอกชื่อของคุณ',
      'wizard_required_customer_phone': 'กรุณากรอกหมายเลขโทรศัพท์',
      'select_on_map': 'เลือกจากแผนที่',
      'map_picker_title': 'เลือกตำแหน่งบนแผนที่',
      'map_picker_hint': 'แตะแผนที่เพื่อปักหมุดตำแหน่ง',
      'map_picker_confirm': 'ใช้ตำแหน่งนี้',
      'current_location': 'ตำแหน่งของฉัน',
      'locating_current_position': 'กำลังตรวจสอบตำแหน่งปัจจุบัน',
      'location_permission_required':
          'ต้องอนุญาตการเข้าถึงตำแหน่งเพื่อใช้ตำแหน่งปัจจุบัน',
      'location_permission_denied': 'ไม่ได้รับอนุญาตให้เข้าถึงตำแหน่ง',
      'location_permission_permanently_denied':
          'โปรดอนุญาตการเข้าถึงตำแหน่งในการตั้งค่าเบราว์เซอร์หรืออุปกรณ์',
      'location_service_disabled': 'บริการตำแหน่งของอุปกรณ์ปิดอยู่',
      'location_unavailable': 'ไม่สามารถตรวจสอบตำแหน่งปัจจุบันได้',
      'location_timeout': 'หมดเวลาตรวจสอบตำแหน่ง โปรดลองอีกครั้ง',
      'location_requires_https':
          'ไม่สามารถใช้ตำแหน่งในสภาพแวดล้อมนี้ได้ โปรดลองอีกครั้งผ่านที่อยู่ HTTPS',
      'low_location_accuracy':
          'ความแม่นยำของตำแหน่งต่ำ โปรดปรับตำแหน่งบนแผนที่',
      'wizard_vehicle_prerequisites':
          'กรุณากรอกบริการ ต้นทาง ปลายทาง เวลารับ และผู้โดยสารก่อน',
      'wizard_pricing_after_vehicle': 'ราคาจะคำนวณหลังจากเลือกรถ',
      'admin_dispatch_retry': 'ลองอีกครั้ง',
      'admin_ops_search': 'ค้นหาเลขจอง ลูกค้า เบอร์โทร คนขับ หรือทะเบียนรถ',
      'admin_ops_filters': 'ตัวกรอง',
      'admin_ops_filter_apply': 'ใช้',
      'admin_ops_filter_reset': 'รีเซ็ต',
      'admin_ops_filter_date_from': 'วันรับจาก (YYYY-MM-DD)',
      'admin_ops_filter_date_to': 'วันรับถึง (YYYY-MM-DD)',
      'admin_ops_filter_date_from_invalid':
          'กรุณาเลือกวันที่เริ่มต้นที่ถูกต้อง',
      'admin_ops_filter_date_to_invalid': 'กรุณาเลือกวันที่สิ้นสุดที่ถูกต้อง',
      'admin_ops_filter_date_range_invalid':
          'วันที่สิ้นสุดต้องไม่อยู่ก่อนวันที่เริ่มต้น',
      'admin_ops_filter_service_type': 'ประเภทบริการ',
      'admin_ops_filter_origin': 'พื้นที่ต้นทาง',
      'admin_ops_filter_destination': 'พื้นที่ปลายทาง',
      'admin_ops_filter_settlement': 'สถานะชำระเงิน',
      'admin_ops_filter_low_rating': 'คะแนนต่ำเท่านั้น',
      'admin_ops_filter_has_inquiry': 'มีข้อความสอบถาม',
      'admin_ops_view_needs_action': 'ต้องดำเนินการ',
      'admin_ops_view_today': 'วันนี้',
      'admin_ops_view_upcoming': 'กำลังจะมาถึง',
      'admin_ops_view_in_progress': 'กำลังเดินทาง',
      'admin_ops_view_settlement': 'รอชำระเงิน',
      'admin_ops_view_completed': 'เสร็จสิ้น',
      'admin_ops_view_cancelled': 'ยกเลิก',
      'admin_ops_view_all': 'ทั้งหมด',
      'admin_ops_summary_needs_action': 'ต้องดำเนินการ',
      'admin_ops_summary_unassigned': 'ยังไม่มอบหมาย',
      'admin_ops_summary_today': 'วันนี้',
      'admin_ops_summary_in_progress': 'กำลังเดินทาง',
      'admin_ops_summary_settlement': 'รอชำระ',
      'admin_ops_summary_issues': 'ปัญหา',
      'admin_ops_result_total': 'ทั้งหมด {total}',
      'admin_ops_unassigned_result_total': 'ยังไม่มอบหมายทั้งหมด {total}',
      'admin_ops_result_showing': 'แสดง {count}',
      'admin_ops_result_filtered': 'ตามตัวกรองปัจจุบัน',
      'admin_ops_severity_urgent': 'ด่วน',
      'admin_ops_severity_soon': 'ต้องทำเร็ว',
      'admin_ops_severity_review': 'ตรวจสอบ',
      'admin_ops_reason_low_rating': 'ต้องตรวจสอบคะแนนต่ำ',
      'admin_ops_reason_receipt_rejected': 'สลิปถูกปฏิเสธ',
      'admin_ops_reason_receipt_review': 'ต้องยืนยันสลิป',
      'admin_ops_reason_receipt_missing': 'ยังไม่ส่งสลิป',
      'admin_ops_reason_pickup_overdue_unassigned':
          'เลยเวลารับ · ยังไม่มอบหมาย',
      'admin_ops_reason_pickup_overdue_stalled': 'เลยเวลารับ · ค้างสถานะ',
      'admin_ops_reason_pickup_soon_unassigned': 'ใกล้เวลารับ · ยังไม่มอบหมาย',
      'admin_ops_reason_customer_inquiry': 'มีข้อความลูกค้า',
      'admin_ops_reason_boarding_delay': 'ขึ้นรถล่าช้า',
      'admin_ops_reason_long_trip': 'เดินทางนานผิดปกติ',
      'admin_ops_reason_status_stale': 'ไม่มีการเปลี่ยนสถานะ',
      'admin_ops_next_low_rating': 'ตรวจสอบคะแนนต่ำจากลูกค้า',
      'admin_ops_next_receipt_review': 'มีสลิปแล้ว กรุณายืนยันการชำระ',
      'admin_ops_next_receipt_missing': 'แจ้งคนขับให้ส่งสลิป',
      'admin_ops_next_assign_driver': 'มอบหมายคนขับก่อนเวลารับ',
      'admin_ops_next_customer_inquiry': 'ตอบข้อความลูกค้า',
      'admin_ops_next_boarding_delay': 'ตรวจสอบการขึ้นรถที่ล่าช้า',
      'admin_ops_next_long_trip': 'ตรวจสอบการเดินทางที่ยาวนาน',
      'admin_ops_next_confirm_settlement': 'ยืนยันค่าคอมมิชชัน 200 บาท',
      'admin_ops_cta_settlement_detail': 'รายละเอียดชำระ',
      'admin_ops_cta_review_rating': 'ตรวจคะแนน',
      'admin_ops_cta_check_status': 'ตรวจสถานะ',
      'admin_ops_cta_view_booking': 'ดูการจอง',
      'admin_ops_cta_review_act': 'ตรวจสอบและดำเนินการ',
      'admin_ops_more_reasons': 'เหตุผลเพิ่มเติม {count} รายการ',
      'admin_ops_select_booking': 'เลือกการจองเพื่อดูตัวอย่าง',
      'admin_ops_settlement_rejected': 'สลิปถูกปฏิเสธ',
      'admin_ops_settlement_submitted': 'ส่งสลิปแล้ว',
      'admin_ops_settlement_missing': 'ยังไม่ส่งสลิป',
      'admin_ops_next_action_label': 'งานถัดไป',
      'admin_ops_archive_show': 'แสดงการจองที่ซ่อน',
      'admin_ops_archive_hide': 'ซ่อนรายการจองที่ซ่อนอยู่',
      'admin_ops_archive_bulk': 'ซ่อนการจองทดสอบ ({count})',
      'admin_ops_archive_confirm_title':
          'ซ่อนการจองที่เลือกเป็นข้อมูลทดสอบหรือไม่?',
      'admin_ops_archive_warning':
          'การจองบางรายการมีประวัติการมอบหมาย งานเดินทาง หรือการชำระเงินแล้ว ข้อมูลการจองและข้อมูลที่เกี่ยวข้องจะไม่ถูกลบ แต่จะถูกซ่อนจากรายการหลักของผู้ดูแลเท่านั้น',
      'admin_ops_archive_info':
          'ข้อมูลการจองและข้อมูลที่เกี่ยวข้องจะไม่ถูกลบ แต่จะถูกซ่อนจากรายการหลักของผู้ดูแลเท่านั้น',
      'admin_ops_archive_hidden_notice':
          'การจองที่ซ่อนจะไม่แสดงในหน้าปฏิบัติการหลัก หน้าคนขับ/ลูกค้า และคิวตรวจชำระ',
      'admin_ops_archive_success': 'ซ่อนการจองที่เลือกแล้ว',
      'admin_ops_archive_restore_success': 'กู้คืนการจองที่ซ่อนแล้ว',
      'admin_ops_archive_failed': 'ซ่อนการจองไม่สำเร็จ',
      'admin_ops_archive_restore_failed': 'กู้คืนการจองไม่สำเร็จ',
      'admin_ops_archive_restore': 'กู้คืน',
      'admin_ops_archive_badge': 'ซ่อน/Test',
      'admin_ops_route_airport_pickup': 'สนามบิน → จุดหมาย',
      'admin_ops_route_airport_dropoff': 'จุดต้นทาง → สนามบิน',
      'admin_ops_route_city_transfer': 'จุดต้นทาง → จุดหมาย',
      'admin_dashboard': 'แดชบอร์ด',
      'admin_reservations': 'การจอง',
      'admin_drivers': 'คนขับ',
      'admin_pricing': 'ราคารถ',
      'admin_golf': 'สนามกอล์ฟ',
      'admin_airports': 'สนามบิน',
      'admin_notifications': 'การแจ้งเตือน',
      'admin_translations': 'แปลภาษา',
      'admin_settings': 'ตั้งค่า',
      'admin_settlement_section': 'ยืนยันการชำระบัญชี',
      'admin_settlement_status': 'สถานะการชำระบัญชี',
      'admin_settlement_transfer_slip': 'สลิปโอนเงิน',
      'admin_settlement_slip_submitted': 'อัปโหลดสลิปโอนเงินแล้ว',
      'admin_settlement_slip_not_submitted': 'ยังไม่ได้อัปโหลด',
      'admin_settlement_waiting_slip':
          'สามารถยืนยันการรับเงินได้หลังจากคนขับอัปโหลดสลิปโอนเงินแล้ว',
      'admin_settlement_view_slip': 'ดูสลิปโอนเงิน',
      'admin_settlement_slip_load_failed': 'ไม่สามารถโหลดสลิปโอนเงินได้',
      'admin_detail_operations_summary': 'สรุปสถานะปฏิบัติการ',
      'admin_detail_current_status': 'สถานะปัจจุบัน',
      'admin_detail_last_status_change': 'เปลี่ยนสถานะล่าสุด',
      'admin_detail_status_duration': 'ระยะเวลาของสถานะปัจจุบัน',
      'admin_detail_driver_assignment_status': 'คนขับ/การมอบหมาย',
      'admin_detail_previous_driver': 'คนขับก่อนหน้า',
      'admin_detail_driver_status': 'สถานะคนขับ',
      'admin_detail_settlement_status': 'สถานะการชำระบัญชี',
      'admin_detail_receipt_status': 'สถานะสลิปโอนเงิน',
      'admin_detail_recommended_action': 'งานที่แนะนำ',
      'admin_detail_settlement_required': 'ต้องชำระบัญชี',
      'admin_detail_settlement_completed': 'ชำระบัญชีแล้ว',
      'admin_detail_receipt_missing': 'ยังไม่มีสลิปโอนเงิน',
      'admin_detail_receipt_submitted': 'ส่งสลิปโอนเงินแล้ว',
      'admin_detail_receipt_rejected': 'สลิปโอนเงินถูกปฏิเสธ',
      'admin_detail_settlement_actionable': 'สถานะการอนุมัติ',
      'admin_detail_settlement_not_actionable':
          'ไม่สามารถอนุมัติการชำระบัญชีในสถานะการจองปัจจุบันได้',
      'admin_detail_settlement_state_mismatch':
          'การเดินทางเสร็จสิ้นแล้ว แต่รายการจองยังไม่ได้เปลี่ยนเป็นสถานะรอตรวจสอบการชำระเงิน โปรดตรวจสอบสถานะการจองและข้อมูลการชำระเงิน',
      'admin_detail_rejection_reason': 'เหตุผลการปฏิเสธ',
      'admin_detail_rejection_reason_missing': 'ไม่ได้บันทึกเหตุผลการปฏิเสธ',
      'admin_detail_rejected_at': 'เวลาปฏิเสธ',
      'admin_detail_receipt_submitted_at': 'เวลาส่งสลิป',
      'admin_detail_receipt_uploaded_at': 'เวลาอัปโหลดสลิป',
      'admin_detail_receipt_file': 'ไฟล์สลิปโอนเงิน',
      'admin_detail_receipt_content_type': 'ชนิดไฟล์',
      'admin_detail_receipt_file_size': 'ขนาดไฟล์',
      'admin_detail_settlement_approved_at': 'เวลาอนุมัติการชำระบัญชี',
      'admin_detail_settlement_approval_mode': 'วิธีอนุมัติ',
      'admin_detail_settlement_manual_approval': 'อนุมัติด้วยตนเอง',
      'admin_detail_additional_memo': 'บันทึกเพิ่มเติม',
      'admin_detail_actor_system': 'ระบบ',
      'admin_detail_actor_admin': 'ผู้ดูแล',
      'admin_detail_actor_driver': 'คนขับ',
      'admin_detail_actor_customer': 'ลูกค้า',
      'admin_detail_actor_unknown': 'ไม่ทราบ',
      'admin_detail_settlement_load_failed':
          'ไม่สามารถโหลดข้อมูลการชำระบัญชีได้',
      'admin_detail_retry_settlement': 'โหลดข้อมูลชำระบัญชีอีกครั้ง',
      'admin_detail_diag_pickup_overdue_stalled':
          'เลยเวลารับแล้ว แต่สถานะการเดินทางยังไม่คืบหน้า โปรดตรวจสอบการมอบหมายคนขับและสถานะคนขับปัจจุบัน',
      'admin_detail_diag_boarding_delay':
          'ลูกค้าขึ้นรถล่าช้าหลังจากคนขับถึงจุดรับ โปรดตรวจสอบสถานะกับคนขับหรือลูกค้า',
      'admin_detail_diag_long_trip':
          'การเดินทางใช้เวลานานกว่าที่คาด โปรดตรวจสอบตำแหน่งคนขับและสถานะการเดินทาง',
      'admin_detail_diag_status_stale':
          'สถานะการจองไม่มีการเปลี่ยนแปลงเป็นเวลานาน โปรดตรวจสอบคนขับและสถานะการเดินทางปัจจุบัน',
      'driver_settlement_receipt_upload': 'อัปโหลดสลิปโอนเงิน',
      'today_bookings': 'จองวันนี้',
      'today_revenue': 'รายได้วันนี้',
      'pending_count': 'รอดำเนินการ',
      'awaiting_driver': 'รอคนขับ',
      'vehicle': 'รถ',
      'amount': 'จำนวน',
      'send': 'ส่ง',
      'type_message': 'พิมพ์ข้อความ...',
      'other_airport': 'สนามบินอื่นในไทย',
      'booking_summary': 'สรุปการจอง',
      'service_type': 'ประเภทบริการ',
      'thb': 'THB',
      'driver_pwa_install_title': 'ติดตั้งแอป T-Ride Driver',
      'driver_pwa_install_body':
          'เพื่อรับงานและใช้งานได้สะดวกยิ่งขึ้น กรุณาติดตั้ง T-Ride ไว้บนหน้าจอหลักของโทรศัพท์',
      'driver_pwa_install_now': 'ติดตั้งตอนนี้',
      'driver_pwa_install_preparing': 'กำลังเตรียมติดตั้ง...',
      'driver_pwa_install_later': 'ไว้ภายหลัง',
      'driver_pwa_install_manual':
          'หากหน้าต่างติดตั้งไม่แสดง ให้แตะเมนู ⋮ ของ Chrome แล้วเลือก “ติดตั้งแอป” หรือ “เพิ่มลงในหน้าจอหลัก”',
      'driver_pwa_install_ios_steps':
          'บน iPhone หรือ iPad ให้เปิด Safari แตะปุ่มแชร์ เลือก “เพิ่มไปยังหน้าจอโฮม” แล้วแตะ “เพิ่ม”',
      'driver_pwa_install_open_in_chrome':
          'เบราว์เซอร์นี้ไม่สามารถติดตั้งแอปอัตโนมัติได้ กรุณาเปิดหน้านี้ใน Chrome แล้วติดตั้ง T-Ride จากเมนู Chrome',
      'driver_pwa_install_success': 'ติดตั้ง T-Ride สำเร็จแล้ว',
      'driver_pwa_install_dismissed':
          'ยกเลิกการติดตั้งแล้ว สามารถติดตั้งใหม่ได้ในครั้งถัดไป',
      'driver_pwa_install_unavailable':
          'ตอนนี้ยังเริ่มติดตั้งอัตโนมัติไม่ได้ กรุณาใช้เมนูของเบราว์เซอร์เพื่อติดตั้งแอป',
      'driver_pwa_install_error':
          'ไม่สามารถเริ่มการติดตั้งได้ กรุณาใช้เมนูของเบราว์เซอร์เพื่อติดตั้งแอป',
    },
  };

  static const Map<String, Map<String, String>>
  _driverApplicationTranslations = {
    'en': {
      'driver_application_cta': 'Apply as a driver',
      'driver_application_status_cta': 'Check application status',
      'driver_application_status_saved_cta': 'Check saved application status',
      'driver_apply_title': 'สมัครคนขับ (기사 등록 신청)',
      'driver_apply_resubmit_title': 'ส่งใบสมัครคนขับอีกครั้ง (기사 신청 재신청)',
      'driver_apply_section_account': 'ข้อมูลบัญชี (계정 정보)',
      'driver_apply_section_driver_info': 'ข้อมูลคนขับ (기사 정보)',
      'driver_apply_section_documents': 'เอกสารและ SNS (문서 및 SNS)',
      'driver_apply_full_name': 'ชื่อ (이름)',
      'driver_apply_phone': 'เบอร์โทรศัพท์ (전화번호)',
      'driver_apply_password': 'รหัสผ่าน (비밀번호)',
      'driver_apply_password_confirm': 'ยืนยันรหัสผ่าน (비밀번호 확인)',
      'driver_apply_phone_country_code': 'รหัสประเทศโทรศัพท์ (전화 국가번호)',
      'driver_apply_country': 'ประเทศ (국가)',
      'driver_apply_language': 'ภาษา (언어)',
      'driver_apply_license_number': 'เลขที่ใบขับขี่ (면허번호)',
      'driver_apply_license_country': 'ประเทศที่ออกใบขับขี่ (면허 발급 국가)',
      'driver_apply_license_expiry': 'วันหมดอายุใบขับขี่ (면허만료일)',
      'driver_apply_license_expiry_picker': 'เลือกวันที่ (날짜 선택)',
      'driver_apply_license_expiry_invalid':
          'กรุณากรอกวันที่ในรูปแบบ YYYY-MM-DD (날짜는 YYYY-MM-DD 형식으로 입력해 주세요.)',
      'driver_apply_license_expiry_past':
          'วันหมดอายุใบขับขี่ต้องไม่เป็นวันที่ผ่านมาแล้ว (면허만료일은 오늘 이전 날짜일 수 없습니다.)',
      'driver_apply_bank_name': 'ชื่อธนาคาร (은행명)',
      'driver_apply_bank_account_number': 'เลขที่บัญชี (계좌번호)',
      'driver_apply_bank_account_holder': 'ชื่อบัญชี (예금주)',
      'driver_apply_vehicle_type': 'ประเภทรถ (차량 유형)',
      'driver_apply_vehicle_make': 'ยี่ห้อรถ (제조사)',
      'driver_apply_vehicle_model': 'รุ่นรถ (모델)',
      'driver_apply_vehicle_year': 'ปีรถ (연식)',
      'driver_apply_vehicle_year_invalid':
          'กรุณากรอกปีรถเป็นตัวเลข 4 หลัก เช่น 2020',
      'driver_apply_vehicle_color': 'สีรถ (색상)',
      'driver_apply_vehicle_plate': 'ทะเบียนรถ (차량번호)',
      'driver_apply_service_areas': 'พื้นที่ที่ขับเป็นหลัก (주로 운행 하는 지역)',
      'driver_apply_service_areas_hint': 'กรุงเทพ, พัทยา (방콕, 파타야)',
      'driver_apply_line_id': 'LINE ID (라인 ID)',
      'driver_apply_line_qr': 'QR Code LINE (라인 QR코드)',
      'driver_apply_vehicle_photos': 'รูปรถ (차량 사진)',
      'driver_apply_insurance_certificate': 'หลักฐานประกันภัย (보험가입증명서)',
      'driver_apply_vehicle_registration': 'เล่มทะเบียนรถ (차량등록증)',
      'driver_apply_tax_certificate': 'หลักฐานการชำระภาษี (세금납부증명서)',
      'driver_apply_signup_submit': 'ส่งคำขอสมัคร (가입 요청)',
      'driver_apply_resubmit': 'ส่งคำขออีกครั้ง (다시 신청)',
      'driver_apply_no_file': 'ยังไม่ได้แนบไฟล์ (첨부된 파일 없음)',
      'driver_apply_upload_required': 'จำเป็น (필수)',
      'driver_apply_upload_empty': 'ยังไม่ได้เลือกไฟล์ (파일 선택 전)',
      'driver_apply_upload_selected': 'เลือกไฟล์แล้ว (파일 선택 완료)',
      'driver_apply_upload_error': 'ต้องตรวจสอบไฟล์นี้ (확인이 필요합니다)',
      'driver_apply_upload_processing': 'กำลังเตรียมไฟล์... (파일 준비 중...)',
      'driver_apply_upload_optimized_size':
          '{original} → {upload} optimized (최적화됨)',
      'driver_apply_upload_privacy_processed_size':
          'Privacy protected, upload size {size} (개인정보 보호 처리 완료, 업로드 용량 {size})',
      'driver_apply_upload_ready_size': 'Upload size {size} (업로드 용량 {size})',
      'driver_apply_upload_select': 'เลือกไฟล์ (파일 선택)',
      'driver_apply_upload_add_photo': 'เพิ่มรูปภาพ (사진 추가)',
      'driver_apply_upload_change': 'เปลี่ยนไฟล์ (파일 교체)',
      'driver_apply_upload_remove': 'ลบไฟล์ (제거)',
      'driver_apply_upload_image_help':
          'แนบไฟล์รูปภาพ JPG หรือ PNG เท่านั้น (JPG, PNG 이미지만 첨부해 주세요.)',
      'driver_apply_vehicle_photo_help':
          'แนบรูปรถ 3-6 รูป เห็นรถและป้ายทะเบียนชัดเจน (차량과 번호판이 보이는 사진 3-6장을 첨부해 주세요.)',
      'driver_apply_upload_document_help':
          'แนบไฟล์ JPG, PNG หรือ PDF (JPG, PNG 또는 PDF 파일을 첨부해 주세요.)',
      'driver_apply_file_pick_failed':
          'ไม่สามารถเลือกไฟล์ได้ กรุณาลองอีกครั้ง (파일을 선택하지 못했습니다. 다시 시도해 주세요.)',
      'driver_apply_image_processing_failed':
          'Could not process this image. Please choose a JPG or PNG file again. (이미지를 처리하지 못했습니다. JPG 또는 PNG 파일을 다시 선택해 주세요.)',
      'driver_apply_file_too_large_after_compression':
          'This file is still larger than 10MB after optimization. Please choose a smaller image. (최적화 후에도 10MB를 초과합니다. 더 작은 파일을 선택해 주세요.)',
      'driver_apply_total_upload_size':
          'Total upload size: {size} / {limit} (총 업로드 용량: {size} / {limit})',
      'driver_apply_total_upload_warning':
          'The upload is large. It may take longer on mobile networks. (업로드 용량이 큽니다. 모바일 네트워크에서는 시간이 더 걸릴 수 있습니다.)',
      'driver_apply_total_upload_too_large':
          'The selected files are too large to submit. Please remove some photos or choose smaller files. (선택한 파일이 너무 큽니다. 일부 사진을 삭제하거나 더 작은 파일을 선택해 주세요.)',
      'driver_apply_vehicle_photo_count': 'รูปรถ {count}/6 (차량 사진 {count}/6)',
      'driver_apply_vehicle_photo_limit_reached':
          'แนบรูปรถครบ 6 รูปแล้ว (차량 사진을 최대 6장까지 등록했습니다.)',
      'driver_apply_duplicate_file':
          'ไฟล์นี้ถูกเพิ่มแล้ว จึงไม่เพิ่มซ้ำ (이미 추가한 파일은 중복 등록하지 않았습니다.)',
      'driver_apply_vehicle_photo_count_error':
          'แนบรูปรถอย่างน้อย 3 รูป และไม่เกิน 6 รูป (차량 사진은 최소 3장, 최대 6장 첨부해야 합니다.)',
      'driver_apply_file_required': 'กรุณาแนบไฟล์นี้ (필수 파일을 첨부해 주세요.)',
      'driver_apply_invalid_file_type':
          'ประเภทไฟล์ไม่ถูกต้อง กรุณาแนบไฟล์ JPG, PNG หรือ PDF ตามช่องที่กำหนด (파일 형식이 올바르지 않습니다. 항목에 맞는 JPG, PNG 또는 PDF 파일을 첨부해 주세요.)',
      'driver_apply_personal_consent':
          'ยินยอมให้เก็บและตรวจสอบข้อมูลส่วนบุคคล (개인정보 수집 및 검토에 동의합니다.)',
      'driver_apply_terms_consent':
          'ยินยอมตามกฎการตรวจสอบและการให้บริการคนขับ (기사 심사 및 운영 규정에 동의합니다.)',
      'driver_apply_false_info_notice':
          'หากส่งข้อมูลเท็จ การอนุมัติอาจถูกยกเลิก (허위 정보 제출 시 승인이 취소될 수 있습니다.)',
      'driver_apply_submit_failed':
          'ไม่สามารถส่งคำขอสมัครได้ (신청을 제출하지 못했습니다.)',
      'driver_apply_upload_too_large':
          'The attached files are too large. Please reduce the photo size or choose smaller files.',
      'driver_apply_server_unavailable':
          'The server connection is not available. Please try again in a moment.',
      'driver_apply_request_failed':
          'Could not process the application request. Please try again in a moment.',
      'driver_apply_vehicle_load_error':
          'ไม่สามารถโหลดประเภทรถได้ (차량 유형을 불러오지 못했습니다.)',
      'driver_apply_vehicle_required': 'กรุณาเลือกประเภทรถ (차량 유형을 선택해 주세요.)',
      'driver_apply_consent_required':
          'กรุณายอมรับข้อกำหนดที่จำเป็น (필수 동의가 필요합니다.)',
      'driver_apply_service_area_required':
          'กรุณาระบุพื้นที่ที่ขับเป็นหลัก (주로 운행 하는 지역을 입력해 주세요.)',
      'driver_apply_validation_required': 'กรุณากรอกข้อมูลนี้ (필수 입력입니다.)',
      'driver_apply_password_min':
          'รหัสผ่านต้องมีอย่างน้อย 6 ตัวอักษร (비밀번호는 6자리 이상 입력해야 합니다.)',
      'driver_apply_password_mismatch': 'รหัสผ่านไม่ตรงกัน (비밀번호가 일치하지 않습니다.)',
      'driver_apply_submitted_message':
          'ส่งคำขอให้ผู้ดูแลอนุมัติแล้ว (관리자에게 승인 요청 했습니다.)',
      'driver_apply_simple_submitted_message':
          'ส่งคำขอให้ผู้ดูแลอนุมัติแล้ว (관리자에게 승인 요청 했습니다.)',
      'driver_apply_line_group_instruction':
          'สแกน QR Code ด้านล่างเพื่อเข้ากลุ่ม LINE คนขับ และแจ้งว่าได้ส่งคำขอสมัครแล้ว (아래 QR 코드를 스캔하여 드라이버 LINE 단체방에 입장하고 가입 신청했다고 알려주세요.)',
      'driver_apply_line_qr_unavailable':
          'QR Code LINE จะปรากฏที่นี่เมื่อพร้อมใช้งาน (LINE 단체방 QR 코드가 준비되면 여기에 표시됩니다.)',
      'driver_apply_open_line_group': 'เปิดกลุ่ม LINE (LINE 단체방 열기)',
      'driver_application_title': 'Driver application',
      'driver_application_resubmit_title': 'Resubmit driver application',
      'driver_application_section_account': 'Account information',
      'driver_application_section_driver_info': 'Driver information',
      'driver_application_section_documents': 'Documents and SNS',
      'driver_application_section_basic': 'Basic information',
      'driver_application_section_license': 'Driving license',
      'driver_application_section_vehicle': 'Vehicle information',
      'driver_application_section_service': 'Service availability',
      'driver_application_section_consent': 'Consent',
      'driver_application_full_name': 'Full name',
      'driver_application_password_confirm': 'Confirm password',
      'driver_application_phone_country_code': 'Phone country code',
      'driver_application_license_number': 'License number',
      'driver_application_license_country': 'License country',
      'driver_application_license_expiry': 'License expiry date',
      'driver_application_experience': 'Driving experience (years)',
      'driver_application_vehicle_ownership': 'Vehicle ownership',
      'driver_application_vehicle_type': 'Vehicle type',
      'driver_application_vehicle_make': 'Make',
      'driver_application_vehicle_model': 'Model',
      'driver_application_vehicle_year': 'Year',
      'driver_application_vehicle_color': 'Color',
      'driver_application_vehicle_plate': 'Plate number',
      'driver_application_service_areas': 'Service areas',
      'driver_application_service_areas_hint': 'Bangkok, Pattaya',
      'driver_application_languages': 'Languages',
      'driver_application_notes': 'Introduction / notes',
      'driver_application_bank_name': 'Bank name',
      'driver_application_bank_account_number': 'Bank account number',
      'driver_application_bank_account_holder': 'Account holder',
      'driver_application_line_id': 'LINE ID',
      'driver_application_line_qr': 'LINE QR image',
      'driver_application_vehicle_photos': 'Vehicle photos',
      'driver_application_insurance_certificate': 'Insurance certificate',
      'driver_application_vehicle_registration': 'Vehicle registration',
      'driver_application_tax_certificate': 'Tax certificate',
      'driver_application_signup_submit': 'Request signup',
      'driver_application_vehicle_photo_count_error':
          'Attach 3 to 6 vehicle photos.',
      'driver_application_file_required': 'Attach all required documents.',
      'ui_select': 'Select',
      'driver_application_personal_consent':
          'I agree to personal data collection and review.',
      'driver_application_terms_consent':
          'I agree to driver screening and operating rules.',
      'driver_application_false_info_notice':
          'False information may cancel approval.',
      'driver_application_submit': 'Submit application',
      'driver_application_resubmit': 'Resubmit application',
      'driver_application_resubmit_cta': 'Edit and resubmit',
      'driver_application_submit_failed': 'Could not submit application.',
      'driver_application_vehicle_load_error': 'Could not load vehicle types.',
      'driver_application_vehicle_required': 'Select a vehicle type.',
      'driver_application_consent_required': 'Required consent is missing.',
      'driver_application_service_area_required':
          'Enter at least one service area.',
      'driver_application_validation_required': 'Required',
      'driver_application_password_min':
          'Password must be at least 6 characters.',
      'driver_application_password_mismatch': 'Passwords do not match.',
      'driver_application_submitted_message':
          'Your application has been received.',
      'driver_application_number': 'Application number',
      'driver_application_submitted_at': 'Submitted at',
      'driver_application_reviewed_at': 'Reviewed at',
      'driver_application_status_title': 'Application status',
      'driver_application_status_loading': 'Checking application status...',
      'driver_application_status_failed': 'Could not check application status.',
      'driver_application_status_manual': 'Enter application number and token',
      'driver_application_status_token': 'Status token',
      'driver_application_status_lookup': 'Check status',
      'driver_application_status_pending': 'Under review',
      'driver_application_status_approved': 'Approved',
      'driver_application_status_rejected': 'Rejected',
      'driver_application_pending_title': 'Application under review',
      'driver_application_approved_title': 'Approved',
      'driver_application_rejected_title': 'Rejected',
      'driver_application_copy_number': 'Copy application number',
      'driver_application_number_copied': 'Application number copied',
      'driver_application_login_cta': 'Go to driver login',
      'driver_application_clear_saved': 'Clear saved status token',
      'admin_driver_application_menu': 'Driver Applications',
      'admin_driver_application_title': 'Driver application management',
      'admin_driver_application_subtitle':
          'Review new driver registration requests.',
      'admin_driver_application_search':
          'Search number, name, email, phone, plate',
      'admin_driver_application_all_statuses': 'All statuses',
      'admin_driver_application_empty': 'No driver applications found',
      'admin_driver_application_detail': 'Driver application detail',
      'admin_driver_application_approve': 'Approve',
      'admin_driver_application_reject': 'Reject',
      'admin_driver_application_approve_title': 'Approve application',
      'admin_driver_application_approve_help':
          'A DRIVER account, driver profile, and primary vehicle will be created.',
      'admin_driver_application_reject_title': 'Reject application',
      'admin_driver_application_reject_help':
          'Rejection reason is shown to the applicant.',
      'admin_driver_application_rejection_reason': 'Rejection reason',
      'admin_driver_application_admin_note': 'Internal admin note',
      'admin_driver_application_admin_note_private':
          'This note is not shown to the applicant.',
      'admin_driver_application_approved': 'Application approved',
      'admin_driver_application_rejected': 'Application rejected',
      'admin_driver_application_processed':
          'This application has already been processed.',
      'admin_driver_application_review_info': 'Review result',
      'admin_driver_application_view_needs_action': 'Needs action',
      'admin_driver_application_view_approved': 'Approved',
      'admin_driver_application_view_closed': 'Closed',
      'admin_driver_application_view_all': 'All',
      'admin_driver_vehicle_menu': 'Vehicle Approvals',
      'admin_driver_vehicle_title': 'Vehicle approval',
      'admin_driver_vehicle_subtitle':
          'Review additional vehicles submitted by drivers.',
      'admin_driver_vehicle_search': 'Search driver name, plate, vehicle type',
      'admin_driver_vehicle_empty': 'No vehicles found',
      'admin_driver_vehicle_approve': 'Approve',
      'admin_driver_vehicle_reject': 'Reject',
      'admin_driver_vehicle_approve_title': 'Approve vehicle',
      'admin_driver_vehicle_approve_help':
          'The vehicle becomes active and eligible for call matching.',
      'admin_driver_vehicle_reject_title': 'Reject vehicle',
      'admin_driver_vehicle_reject_help':
          'The rejection reason is shown on the driver vehicle list.',
      'admin_driver_vehicle_rejection_reason': 'Rejection reason',
      'admin_driver_vehicle_approved': 'Vehicle approved',
      'admin_driver_vehicle_rejected': 'Vehicle rejected',
    },
    'ko': {
      'driver_application_cta': '기사 등록 신청',
      'driver_application_status_cta': '신청 상태 확인',
      'driver_application_status_saved_cta': '저장된 신청 상태 확인',
      'driver_application_title': '기사 등록 신청',
      'driver_application_resubmit_title': '기사 신청 재신청',
      'driver_application_section_basic': '기본 정보',
      'driver_application_section_license': '운전면허',
      'driver_application_section_vehicle': '차량 정보',
      'driver_application_section_service': '운행 가능 정보',
      'driver_application_section_consent': '동의',
      'driver_application_full_name': '이름',
      'driver_application_password_confirm': '비밀번호 확인',
      'driver_application_phone_country_code': '전화 국가번호',
      'driver_application_license_number': '면허번호',
      'driver_application_license_country': '면허 발급 국가',
      'driver_application_license_expiry': '면허 만료일',
      'driver_application_experience': '운전 경력(년)',
      'driver_application_vehicle_ownership': '차량 소유 형태',
      'driver_application_vehicle_type': '차량 유형',
      'driver_application_vehicle_make': '제조사',
      'driver_application_vehicle_model': '모델',
      'driver_application_vehicle_year': '연식',
      'driver_application_vehicle_color': '색상',
      'driver_application_vehicle_plate': '차량번호',
      'driver_application_service_areas': '운행 가능 지역',
      'driver_application_service_areas_hint': '방콕, 파타야',
      'driver_application_languages': '사용 언어',
      'driver_application_notes': '자기소개/추가 메모',
      'driver_application_personal_consent': '개인정보 수집 및 검토에 동의합니다.',
      'driver_application_terms_consent': '기사 심사 및 운영 규정에 동의합니다.',
      'driver_application_false_info_notice': '허위 정보 제출 시 승인이 취소될 수 있습니다.',
      'driver_application_submit': '신청 제출',
      'driver_application_resubmit': '다시 신청',
      'driver_application_resubmit_cta': '정보 수정 후 다시 신청',
      'driver_application_submit_failed': '신청을 제출하지 못했습니다.',
      'driver_apply_upload_too_large':
          '첨부 파일 용량이 너무 큽니다. 사진 크기를 줄이거나 다시 선택해 주세요.',
      'driver_apply_server_unavailable': '서버 연결이 원활하지 않습니다. 잠시 후 다시 시도해 주세요.',
      'driver_apply_request_failed': '가입 요청을 처리하지 못했습니다. 잠시 후 다시 시도해 주세요.',
      'driver_application_vehicle_load_error': '차량 유형을 불러오지 못했습니다.',
      'driver_application_vehicle_required': '차량 유형을 선택해 주세요.',
      'driver_application_consent_required': '필수 동의가 필요합니다.',
      'driver_application_service_area_required': '운행 가능 지역을 하나 이상 입력해 주세요.',
      'driver_application_validation_required': '필수 입력입니다',
      'driver_application_password_min': '비밀번호는 8자 이상이어야 합니다.',
      'driver_application_password_mismatch': '비밀번호가 일치하지 않습니다.',
      'driver_application_submitted_message': '신청이 접수되었습니다.',
      'driver_application_number': '신청번호',
      'driver_application_submitted_at': '접수 시간',
      'driver_application_reviewed_at': '검토 시간',
      'driver_application_status_title': '신청 상태',
      'driver_application_status_loading': '신청 상태를 확인하는 중...',
      'driver_application_status_failed': '신청 상태를 확인하지 못했습니다.',
      'driver_application_status_manual': '신청번호와 상태 토큰 입력',
      'driver_application_status_token': '상태 토큰',
      'driver_application_status_lookup': '상태 확인',
      'driver_application_status_pending': '검토 중',
      'driver_application_status_approved': '승인 완료',
      'driver_application_status_rejected': '반려',
      'driver_application_pending_title': '신청 검토 중',
      'driver_application_approved_title': '승인 완료',
      'driver_application_rejected_title': '반려되었습니다',
      'driver_application_copy_number': '신청번호 복사',
      'driver_application_number_copied': '신청번호를 복사했습니다',
      'driver_application_login_cta': '기사 로그인으로 이동',
      'driver_application_clear_saved': '저장된 상태 토큰 삭제',
      'admin_driver_application_menu': '기사 신청 관리',
      'admin_driver_application_title': '기사 신청 관리',
      'admin_driver_application_subtitle': '신규 기사 등록 신청을 검토합니다.',
      'admin_driver_application_search': '신청번호, 이름, 이메일, 전화, 차량번호 검색',
      'admin_driver_application_all_statuses': '전체 상태',
      'admin_driver_application_empty': '기사 신청이 없습니다',
      'admin_driver_application_detail': '기사 신청 상세',
      'admin_driver_application_approve': '승인',
      'admin_driver_application_reject': '반려',
      'admin_driver_application_approve_title': '신청 승인',
      'admin_driver_application_approve_help':
          'DRIVER 계정, 기사 프로필, 기본 차량이 생성됩니다.',
      'admin_driver_application_reject_title': '신청 반려',
      'admin_driver_application_reject_help': '반려 사유는 신청자에게 표시됩니다.',
      'admin_driver_application_rejection_reason': '반려 사유',
      'admin_driver_application_admin_note': '내부 관리자 메모',
      'admin_driver_application_admin_note_private': '이 메모는 신청자에게 표시되지 않습니다.',
      'admin_driver_application_approved': '신청을 승인했습니다',
      'admin_driver_application_rejected': '신청을 반려했습니다',
      'admin_driver_application_processed': '이미 처리된 신청입니다.',
      'admin_driver_application_review_info': '검토 결과',
      'admin_driver_application_view_needs_action': '처리 필요',
      'admin_driver_application_view_approved': '승인 완료',
      'admin_driver_application_view_closed': '종료',
      'admin_driver_application_view_all': '전체',
      'admin_driver_vehicle_menu': '차량 승인',
      'admin_driver_vehicle_title': '차량 승인',
      'admin_driver_vehicle_subtitle': '기사가 추가 등록한 차량을 검토합니다.',
      'admin_driver_vehicle_search': '기사명, 번호판, 차종 검색',
      'admin_driver_vehicle_empty': '차량이 없습니다',
      'admin_driver_vehicle_approve': '승인',
      'admin_driver_vehicle_reject': '거절',
      'admin_driver_vehicle_approve_title': '차량 승인',
      'admin_driver_vehicle_approve_help': '차량이 활성화되어 콜 매칭 대상이 됩니다.',
      'admin_driver_vehicle_reject_title': '차량 거절',
      'admin_driver_vehicle_reject_help': '거절 사유는 기사 차량 목록에 표시됩니다.',
      'admin_driver_vehicle_rejection_reason': '거절 사유',
      'admin_driver_vehicle_approved': '차량을 승인했습니다',
      'admin_driver_vehicle_rejected': '차량을 거절했습니다',
    },
    'th': {
      'driver_application_cta': 'สมัครเป็นคนขับ',
      'driver_application_status_cta': 'ตรวจสอบสถานะใบสมัคร',
      'driver_application_status_saved_cta': 'ตรวจสอบใบสมัครที่บันทึกไว้',
      'driver_application_title': 'สมัครคนขับ',
      'driver_application_resubmit_title': 'ส่งใบสมัครคนขับอีกครั้ง',
      'driver_application_section_basic': 'ข้อมูลพื้นฐาน',
      'driver_application_section_license': 'ใบขับขี่',
      'driver_application_section_vehicle': 'ข้อมูลรถ',
      'driver_application_section_service': 'พื้นที่ให้บริการ',
      'driver_application_section_consent': 'ความยินยอม',
      'driver_application_full_name': 'ชื่อเต็ม',
      'driver_application_password_confirm': 'ยืนยันรหัสผ่าน',
      'driver_application_phone_country_code': 'รหัสประเทศโทรศัพท์',
      'driver_application_license_number': 'เลขใบขับขี่',
      'driver_application_license_country': 'ประเทศที่ออกใบขับขี่',
      'driver_application_license_expiry': 'วันหมดอายุใบขับขี่',
      'driver_application_experience': 'ประสบการณ์ขับรถ (ปี)',
      'driver_application_vehicle_ownership': 'การครอบครองรถ',
      'driver_application_vehicle_type': 'ประเภทรถ',
      'driver_application_vehicle_make': 'ยี่ห้อ',
      'driver_application_vehicle_model': 'รุ่น',
      'driver_application_vehicle_year': 'ปีรถ',
      'driver_application_vehicle_color': 'สี',
      'driver_application_vehicle_plate': 'ทะเบียนรถ',
      'driver_application_service_areas': 'พื้นที่ให้บริการ',
      'driver_application_service_areas_hint': 'กรุงเทพ, พัทยา',
      'driver_application_languages': 'ภาษา',
      'driver_application_notes': 'แนะนำตัว / หมายเหตุ',
      'driver_application_personal_consent':
          'ฉันยินยอมให้เก็บและตรวจสอบข้อมูลส่วนบุคคล',
      'driver_application_terms_consent':
          'ฉันยอมรับการคัดกรองคนขับและกฎการให้บริการ',
      'driver_application_false_info_notice':
          'ข้อมูลเท็จอาจทำให้การอนุมัติถูกยกเลิก',
      'driver_application_submit': 'ส่งใบสมัคร',
      'driver_application_resubmit': 'ส่งอีกครั้ง',
      'driver_application_resubmit_cta': 'แก้ไขและส่งอีกครั้ง',
      'driver_application_submit_failed': 'ไม่สามารถส่งใบสมัครได้',
      'driver_apply_upload_too_large':
          'ไฟล์แนบมีขนาดใหญ่เกินไป กรุณาลดขนาดรูปภาพหรือเลือกไฟล์ใหม่',
      'driver_apply_server_unavailable':
          'ไม่สามารถเชื่อมต่อเซิร์ฟเวอร์ได้ กรุณาลองใหม่อีกครั้งในภายหลัง',
      'driver_apply_request_failed':
          'ไม่สามารถดำเนินการสมัครได้ กรุณาลองใหม่อีกครั้งในภายหลัง',
      'driver_application_vehicle_load_error': 'ไม่สามารถโหลดประเภทรถได้',
      'driver_application_vehicle_required': 'กรุณาเลือกประเภทรถ',
      'driver_application_consent_required': 'ต้องยินยอมรายการที่จำเป็น',
      'driver_application_service_area_required':
          'กรุณาใส่พื้นที่ให้บริการอย่างน้อยหนึ่งรายการ',
      'driver_application_validation_required': 'จำเป็นต้องกรอก',
      'driver_application_password_min': 'รหัสผ่านต้องมีอย่างน้อย 8 ตัวอักษร',
      'driver_application_password_mismatch': 'รหัสผ่านไม่ตรงกัน',
      'driver_application_submitted_message': 'ได้รับใบสมัครแล้ว',
      'driver_application_number': 'เลขที่ใบสมัคร',
      'driver_application_submitted_at': 'เวลาส่ง',
      'driver_application_reviewed_at': 'เวลาตรวจสอบ',
      'driver_application_status_title': 'สถานะใบสมัคร',
      'driver_application_status_loading': 'กำลังตรวจสอบสถานะ...',
      'driver_application_status_failed': 'ไม่สามารถตรวจสอบสถานะได้',
      'driver_application_status_manual': 'กรอกเลขใบสมัครและโทเคนสถานะ',
      'driver_application_status_token': 'โทเคนสถานะ',
      'driver_application_status_lookup': 'ตรวจสอบสถานะ',
      'driver_application_status_pending': 'กำลังตรวจสอบ',
      'driver_application_status_approved': 'อนุมัติแล้ว',
      'driver_application_status_rejected': 'ถูกปฏิเสธ',
      'driver_application_pending_title': 'ใบสมัครกำลังตรวจสอบ',
      'driver_application_approved_title': 'อนุมัติแล้ว',
      'driver_application_rejected_title': 'ใบสมัครถูกปฏิเสธ',
      'driver_application_copy_number': 'คัดลอกเลขใบสมัคร',
      'driver_application_number_copied': 'คัดลอกเลขใบสมัครแล้ว',
      'driver_application_login_cta': 'ไปที่เข้าสู่ระบบคนขับ',
      'driver_application_clear_saved': 'ลบโทเคนสถานะที่บันทึกไว้',
      'admin_driver_application_menu': 'จัดการใบสมัครคนขับ',
      'admin_driver_application_title': 'จัดการใบสมัครคนขับ',
      'admin_driver_application_subtitle': 'ตรวจสอบคำขอลงทะเบียนคนขับใหม่',
      'admin_driver_application_search': 'ค้นหาเลข ชื่อ อีเมล โทรศัพท์ ทะเบียน',
      'admin_driver_application_all_statuses': 'ทุกสถานะ',
      'admin_driver_application_empty': 'ไม่พบใบสมัครคนขับ',
      'admin_driver_application_detail': 'รายละเอียดใบสมัครคนขับ',
      'admin_driver_application_approve': 'อนุมัติ',
      'admin_driver_application_reject': 'ปฏิเสธ',
      'admin_driver_application_approve_title': 'อนุมัติใบสมัคร',
      'admin_driver_application_approve_help':
          'ระบบจะสร้างบัญชี DRIVER โปรไฟล์คนขับ และรถหลัก',
      'admin_driver_application_reject_title': 'ปฏิเสธใบสมัคร',
      'admin_driver_application_reject_help':
          'เหตุผลการปฏิเสธจะแสดงให้ผู้สมัครเห็น',
      'admin_driver_application_rejection_reason': 'เหตุผลการปฏิเสธ',
      'admin_driver_application_admin_note': 'บันทึกภายในผู้ดูแล',
      'admin_driver_application_admin_note_private':
          'บันทึกนี้ไม่แสดงให้ผู้สมัครเห็น',
      'admin_driver_application_approved': 'อนุมัติใบสมัครแล้ว',
      'admin_driver_application_rejected': 'ปฏิเสธใบสมัครแล้ว',
      'admin_driver_application_processed': 'ใบสมัครนี้ถูกดำเนินการแล้ว',
      'admin_driver_application_review_info': 'ผลการตรวจสอบ',
      'admin_driver_application_view_needs_action': 'ต้องดำเนินการ',
      'admin_driver_application_view_approved': 'อนุมัติแล้ว',
      'admin_driver_application_view_closed': 'ปิดแล้ว',
      'admin_driver_application_view_all': 'ทั้งหมด',
      'admin_driver_vehicle_menu': 'อนุมัติรถ',
      'admin_driver_vehicle_title': 'อนุมัติรถ',
      'admin_driver_vehicle_subtitle': 'ตรวจสอบรถเพิ่มเติมที่คนขับส่งมา',
      'admin_driver_vehicle_search': 'ค้นหาชื่อคนขับ ทะเบียน ประเภทรถ',
      'admin_driver_vehicle_empty': 'ไม่พบรถ',
      'admin_driver_vehicle_approve': 'อนุมัติ',
      'admin_driver_vehicle_reject': 'ปฏิเสธ',
      'admin_driver_vehicle_approve_title': 'อนุมัติรถ',
      'admin_driver_vehicle_approve_help':
          'รถจะเปิดใช้งานและเข้าสู่การจับคู่งานได้',
      'admin_driver_vehicle_reject_title': 'ปฏิเสธรถ',
      'admin_driver_vehicle_reject_help':
          'เหตุผลการปฏิเสธจะแสดงในรายการรถของคนขับ',
      'admin_driver_vehicle_rejection_reason': 'เหตุผลการปฏิเสธ',
      'admin_driver_vehicle_approved': 'อนุมัติรถแล้ว',
      'admin_driver_vehicle_rejected': 'ปฏิเสธรถแล้ว',
    },
    'ja': {
      'driver_application_cta': 'ドライバー登録申請',
      'driver_application_status_cta': '申請状況を確認',
      'driver_application_status_saved_cta': '保存済み申請状況を確認',
      'driver_application_title': 'ドライバー登録申請',
      'driver_application_resubmit_title': 'ドライバー申請の再提出',
      'driver_application_section_basic': '基本情報',
      'driver_application_section_license': '運転免許',
      'driver_application_section_vehicle': '車両情報',
      'driver_application_section_service': '対応エリア',
      'driver_application_section_consent': '同意',
      'driver_application_full_name': '氏名',
      'driver_application_password_confirm': 'パスワード確認',
      'driver_application_phone_country_code': '電話国番号',
      'driver_application_license_number': '免許番号',
      'driver_application_license_country': '免許発行国',
      'driver_application_license_expiry': '免許有効期限',
      'driver_application_experience': '運転経験（年）',
      'driver_application_vehicle_ownership': '車両所有形態',
      'driver_application_vehicle_type': '車両タイプ',
      'driver_application_vehicle_make': 'メーカー',
      'driver_application_vehicle_model': 'モデル',
      'driver_application_vehicle_year': '年式',
      'driver_application_vehicle_color': '色',
      'driver_application_vehicle_plate': '車両番号',
      'driver_application_service_areas': '対応エリア',
      'driver_application_service_areas_hint': 'バンコク, パタヤ',
      'driver_application_languages': '対応言語',
      'driver_application_notes': '自己紹介 / メモ',
      'driver_application_personal_consent': '個人情報の収集と審査に同意します。',
      'driver_application_terms_consent': 'ドライバー審査と運用規定に同意します。',
      'driver_application_false_info_notice': '虚偽情報がある場合、承認が取り消されることがあります。',
      'driver_application_submit': '申請を送信',
      'driver_application_resubmit': '再申請',
      'driver_application_resubmit_cta': '修正して再申請',
      'driver_application_submit_failed': '申請を送信できませんでした。',
      'driver_apply_upload_too_large':
          '添付ファイルの容量が大きすぎます。写真サイズを小さくするか、別のファイルを選択してください。',
      'driver_apply_server_unavailable': 'サーバーに接続できません。しばらくしてからもう一度お試しください。',
      'driver_apply_request_failed': '申請を処理できませんでした。しばらくしてからもう一度お試しください。',
      'driver_application_vehicle_load_error': '車両タイプを読み込めませんでした。',
      'driver_application_vehicle_required': '車両タイプを選択してください。',
      'driver_application_consent_required': '必須同意が必要です。',
      'driver_application_service_area_required': '対応エリアを1つ以上入力してください。',
      'driver_application_validation_required': '必須項目です',
      'driver_application_password_min': 'パスワードは8文字以上です。',
      'driver_application_password_mismatch': 'パスワードが一致しません。',
      'driver_application_submitted_message': '申請を受け付けました。',
      'driver_application_number': '申請番号',
      'driver_application_submitted_at': '受付時間',
      'driver_application_reviewed_at': '審査時間',
      'driver_application_status_title': '申請状況',
      'driver_application_status_loading': '申請状況を確認中...',
      'driver_application_status_failed': '申請状況を確認できませんでした。',
      'driver_application_status_manual': '申請番号とステータストークンを入力',
      'driver_application_status_token': 'ステータストークン',
      'driver_application_status_lookup': '状況確認',
      'driver_application_status_pending': '審査中',
      'driver_application_status_approved': '承認済み',
      'driver_application_status_rejected': '却下',
      'driver_application_pending_title': '申請審査中',
      'driver_application_approved_title': '承認済み',
      'driver_application_rejected_title': '却下されました',
      'driver_application_copy_number': '申請番号をコピー',
      'driver_application_number_copied': '申請番号をコピーしました',
      'driver_application_login_cta': 'ドライバーログインへ',
      'driver_application_clear_saved': '保存済みトークンを削除',
      'admin_driver_application_menu': 'ドライバー申請管理',
      'admin_driver_application_title': 'ドライバー申請管理',
      'admin_driver_application_subtitle': '新規ドライバー登録申請を審査します。',
      'admin_driver_application_search': '番号、名前、メール、電話、車両番号を検索',
      'admin_driver_application_all_statuses': 'すべての状態',
      'admin_driver_application_empty': 'ドライバー申請はありません',
      'admin_driver_application_detail': 'ドライバー申請詳細',
      'admin_driver_application_approve': '承認',
      'admin_driver_application_reject': '却下',
      'admin_driver_application_approve_title': '申請を承認',
      'admin_driver_application_approve_help':
          'DRIVERアカウント、ドライバープロフィール、主車両が作成されます。',
      'admin_driver_application_reject_title': '申請を却下',
      'admin_driver_application_reject_help': '却下理由は申請者に表示されます。',
      'admin_driver_application_rejection_reason': '却下理由',
      'admin_driver_application_admin_note': '内部管理メモ',
      'admin_driver_application_admin_note_private': 'このメモは申請者には表示されません。',
      'admin_driver_application_approved': '申請を承認しました',
      'admin_driver_application_rejected': '申請を却下しました',
      'admin_driver_application_processed': 'この申請は処理済みです。',
      'admin_driver_application_review_info': '審査結果',
      'admin_driver_application_view_needs_action': '対応が必要',
      'admin_driver_application_view_approved': '承認済み',
      'admin_driver_application_view_closed': '終了',
      'admin_driver_application_view_all': 'すべて',
      'admin_driver_vehicle_menu': '車両承認',
      'admin_driver_vehicle_title': '車両承認',
      'admin_driver_vehicle_subtitle': 'ドライバーが追加登録した車両を審査します。',
      'admin_driver_vehicle_search': 'ドライバー名、車両番号、車種を検索',
      'admin_driver_vehicle_empty': '車両はありません',
      'admin_driver_vehicle_approve': '承認',
      'admin_driver_vehicle_reject': '却下',
      'admin_driver_vehicle_approve_title': '車両を承認',
      'admin_driver_vehicle_approve_help': '車両が有効になり、コールマッチング対象になります。',
      'admin_driver_vehicle_reject_title': '車両を却下',
      'admin_driver_vehicle_reject_help': '却下理由はドライバーの車両一覧に表示されます。',
      'admin_driver_vehicle_rejection_reason': '却下理由',
      'admin_driver_vehicle_approved': '車両を承認しました',
      'admin_driver_vehicle_rejected': '車両を却下しました',
    },
    'zh': {
      'driver_application_cta': '申请成为司机',
      'driver_application_status_cta': '查看申请状态',
      'driver_application_status_saved_cta': '查看已保存的申请状态',
      'driver_application_title': '司机注册申请',
      'driver_application_resubmit_title': '重新提交司机申请',
      'driver_application_section_basic': '基本信息',
      'driver_application_section_license': '驾驶执照',
      'driver_application_section_vehicle': '车辆信息',
      'driver_application_section_service': '可服务信息',
      'driver_application_section_consent': '同意事项',
      'driver_application_full_name': '姓名',
      'driver_application_password_confirm': '确认密码',
      'driver_application_phone_country_code': '电话国家代码',
      'driver_application_license_number': '驾照号码',
      'driver_application_license_country': '驾照签发国家',
      'driver_application_license_expiry': '驾照到期日',
      'driver_application_experience': '驾驶经验（年）',
      'driver_application_vehicle_ownership': '车辆所有形式',
      'driver_application_vehicle_type': '车辆类型',
      'driver_application_vehicle_make': '品牌',
      'driver_application_vehicle_model': '型号',
      'driver_application_vehicle_year': '年份',
      'driver_application_vehicle_color': '颜色',
      'driver_application_vehicle_plate': '车牌号',
      'driver_application_service_areas': '可服务区域',
      'driver_application_service_areas_hint': '曼谷, 芭提雅',
      'driver_application_languages': '可用语言',
      'driver_application_notes': '自我介绍 / 备注',
      'driver_application_personal_consent': '我同意收集并审核个人信息。',
      'driver_application_terms_consent': '我同意司机审核和运营规则。',
      'driver_application_false_info_notice': '提交虚假信息可能导致批准被取消。',
      'driver_application_submit': '提交申请',
      'driver_application_resubmit': '重新申请',
      'driver_application_resubmit_cta': '修改后重新申请',
      'driver_application_submit_failed': '无法提交申请。',
      'driver_apply_upload_too_large': '附件文件过大。请缩小照片尺寸或选择较小的文件。',
      'driver_apply_server_unavailable': '无法连接服务器。请稍后再试。',
      'driver_apply_request_failed': '无法处理申请请求。请稍后再试。',
      'driver_application_vehicle_load_error': '无法加载车辆类型。',
      'driver_application_vehicle_required': '请选择车辆类型。',
      'driver_application_consent_required': '请同意必填项目。',
      'driver_application_service_area_required': '请至少输入一个可服务区域。',
      'driver_application_validation_required': '必填项',
      'driver_application_password_min': '密码至少需要8个字符。',
      'driver_application_password_mismatch': '两次密码不一致。',
      'driver_application_submitted_message': '申请已收到。',
      'driver_application_number': '申请编号',
      'driver_application_submitted_at': '提交时间',
      'driver_application_reviewed_at': '审核时间',
      'driver_application_status_title': '申请状态',
      'driver_application_status_loading': '正在查看申请状态...',
      'driver_application_status_failed': '无法查看申请状态。',
      'driver_application_status_manual': '输入申请编号和状态令牌',
      'driver_application_status_token': '状态令牌',
      'driver_application_status_lookup': '查看状态',
      'driver_application_status_pending': '审核中',
      'driver_application_status_approved': '已批准',
      'driver_application_status_rejected': '已驳回',
      'driver_application_pending_title': '申请审核中',
      'driver_application_approved_title': '已批准',
      'driver_application_rejected_title': '申请已驳回',
      'driver_application_copy_number': '复制申请编号',
      'driver_application_number_copied': '申请编号已复制',
      'driver_application_login_cta': '前往司机登录',
      'driver_application_clear_saved': '删除已保存的状态令牌',
      'admin_driver_application_menu': '司机申请管理',
      'admin_driver_application_title': '司机申请管理',
      'admin_driver_application_subtitle': '审核新的司机注册申请。',
      'admin_driver_application_search': '搜索编号、姓名、邮箱、电话、车牌',
      'admin_driver_application_all_statuses': '全部状态',
      'admin_driver_application_empty': '没有司机申请',
      'admin_driver_application_detail': '司机申请详情',
      'admin_driver_application_approve': '批准',
      'admin_driver_application_reject': '驳回',
      'admin_driver_application_approve_title': '批准申请',
      'admin_driver_application_approve_help': '将创建 DRIVER 账号、司机资料和主车辆。',
      'admin_driver_application_reject_title': '驳回申请',
      'admin_driver_application_reject_help': '驳回原因会显示给申请人。',
      'admin_driver_application_rejection_reason': '驳回原因',
      'admin_driver_application_admin_note': '内部管理员备注',
      'admin_driver_application_admin_note_private': '此备注不会显示给申请人。',
      'admin_driver_application_approved': '申请已批准',
      'admin_driver_application_rejected': '申请已驳回',
      'admin_driver_application_processed': '此申请已处理。',
      'admin_driver_application_review_info': '审核结果',
      'admin_driver_application_view_needs_action': '需处理',
      'admin_driver_application_view_approved': '已批准',
      'admin_driver_application_view_closed': '已结束',
      'admin_driver_application_view_all': '全部',
      'admin_driver_vehicle_menu': '车辆审批',
      'admin_driver_vehicle_title': '车辆审批',
      'admin_driver_vehicle_subtitle': '审核司机追加登记的车辆。',
      'admin_driver_vehicle_search': '搜索司机名、车牌、车型',
      'admin_driver_vehicle_empty': '没有车辆',
      'admin_driver_vehicle_approve': '批准',
      'admin_driver_vehicle_reject': '驳回',
      'admin_driver_vehicle_approve_title': '批准车辆',
      'admin_driver_vehicle_approve_help': '车辆将激活并可参与叫车匹配。',
      'admin_driver_vehicle_reject_title': '驳回车辆',
      'admin_driver_vehicle_reject_help': '驳回原因会显示在司机车辆列表中。',
      'admin_driver_vehicle_rejection_reason': '驳回原因',
      'admin_driver_vehicle_approved': '车辆已批准',
      'admin_driver_vehicle_rejected': '车辆已驳回',
    },
  };

  static const Map<String, String> _driverUiTranslations = {
    'driver_support_title': '고객센터\n(ศูนย์บริการลูกค้า)',
    'driver_support_resources': '자료실\n(เอกสาร)',
    'driver_support_faq': '자주 묻는 질문\n(คำถามที่พบบ่อย)',
    'driver_support_inquiry': '1:1 문의\n(สอบถาม 1:1)',
    'driver_support_empty': '등록된 내용이 없습니다.\n(ยังไม่มีข้อมูล)',
    'driver_support_line_help':
        'LINE으로 관리자에게 1:1 문의할 수 있습니다.\n(ติดต่อผู้ดูแลโดยตรงผ่าน LINE)',
    'driver_settlement_payment_account': '정산 계좌\n(บัญชีสำหรับชำระเงิน)',
    'driver_settlement_next_job_notice':
        '송금증을 업로드한 뒤 관리자가 입금을 확인하면 다음 운행을 배정받을 수 있습니다.\n(อัปโหลดสลิปแล้วรอผู้ดูแลยืนยัน ก่อนรับงานถัดไป)',
    'driver_login_title': '기사 로그인\n(เข้าสู่ระบบคนขับ)',
    'driver_brand': 'T-Ride 기사\n(T-Ride คนขับ)',
    'driver_phone': '전화번호\n(เบอร์โทรศัพท์)',
    'driver_password': '비밀번호\n(รหัสผ่าน)',
    'driver_login': '로그인 / เข้าสู่ระบบ',
    'driver_application_cta': '기사 등록 신청 / สมัครคนขับ',
    'driver_load_failed': '정보를 불러올 수 없습니다\n(ไม่สามารถโหลดข้อมูลได้)',
    'driver_action_failed': '처리 중 오류가 발생했습니다\n(เกิดข้อผิดพลาดระหว่างดำเนินการ)',
    'driver_nav_jobs': '업무\n(งาน)',
    'driver_nav_home': '홈\n(หน้าหลัก)',
    'driver_nav_today': '오늘\n(วันนี้)',
    'driver_nav_history': '운행 기록\n(ประวัติการเดินทาง)',
    'driver_nav_account': '내 정보\n(บัญชีของฉัน)',
    'driver_qr_scan_menu': 'QR 스캔\n(สแกน QR)',
    'driver_qr_scan_instruction':
        '고객의 QR 코드를 카메라에 비춰 주세요.\n(เล็งกล้องไปที่ QR ของลูกค้า)',
    'driver_qr_scan_no_active_job':
        '스캔할 운행 중 예약이 없습니다.\n(ไม่มีงานที่กำลังดำเนินการสำหรับสแกน QR)',
    'driver_qr_scan_invalid':
        '유효하지 않거나 사용할 수 없는 QR 코드입니다.\n(QR นี้ไม่ถูกต้องหรือไม่สามารถใช้งานได้แล้ว)',
    'driver_qr_scan_success_boarding':
        '고객 탑승이 확인되었습니다.\n(ยืนยันการขึ้นรถของลูกค้าแล้ว)',
    'driver_qr_scan_success_dropoff':
        '운행 종료가 확인되었습니다.\n(ยืนยันการสิ้นสุดการเดินทางแล้ว)',
    'driver_nav_notifications': '알림\n(การแจ้งเตือน)',
    'driver_nav_settlement': '정산\n(การเงิน)',
    'driver_nav_profile': '내 정보\n(บัญชี)',
    'driver_refresh': '새로고침 / รีเฟรช',
    'driver_retry': '다시 시도 / ลองอีกครั้ง',
    'driver_jobs_error_title':
        '예약 정보를 불러올 수 없습니다\n(ไม่สามารถโหลดข้อมูลการจองได้)',
    'driver_jobs_empty_title':
        '오늘 배정된 예약이 없습니다\n(วันนี้ยังไม่มีงานที่ได้รับมอบหมาย)',
    'driver_jobs_empty_message':
        '새 예약이 배정되면 여기에 표시됩니다.\n(งานใหม่จะแสดงที่นี่เมื่อได้รับมอบหมาย)',
    'driver_jobs_today_date': '오늘 배정된 예약 - {date}\n(งานที่ได้รับมอบหมายวันนี้)',
    'driver_jobs_group_active': '진행 중\n(งานปัจจุบัน)',
    'driver_jobs_group_upcoming': '예정\n(งานที่กำลังจะมาถึง)',
    'driver_jobs_group_completed': '완료\n(เสร็จสิ้น)',
    'driver_jobs_tab_new': '새 콜\n(งานใหม่)',
    'driver_jobs_tab_mine': '내 업무\n(งานของฉัน)',
    'driver_jobs_tab_past': '지난 업무\n(งานที่ผ่านมา)',
    'driver_jobs_open_history': '전체 운행 기록 보기\n(ดูประวัติการเดินทางทั้งหมด)',
    'driver_claim_call': '이 콜 수락 / รับงานนี้',
    'driver_claim_confirm_title': '이 콜을 수락할까요?\n(ต้องการรับงานนี้หรือไม่?)',
    'driver_claim_confirm_message':
        '확인 전 시간과 장소를 다시 확인해 주세요.\n(กรุณาตรวจสอบเวลาและสถานที่ก่อนยืนยัน)',
    'driver_claim_vehicle_pick_title':
        '어느 차량으로 가시겠습니까?\n(จะใช้รถคันไหนไปรับงาน?)',
    'driver_claim_vehicle_pick_help':
        '이 콜에 사용 가능한 승인 차량을 선택해 주세요.\n(เลือกรถที่อนุมัติแล้วซึ่งใช้รับงานนี้ได้)',
    'driver_claim_vehicle_exact_badge': '동일 차종 / ประเภทตรงกัน',
    'driver_claim_vehicle_cancel': '취소 / ยกเลิก',
    'driver_claim_success': '콜 수락 완료 / รับงานแล้ว',
    'driver_claim_already_assigned':
        '다른 기사가 먼저 수락했습니다 / งานนี้มีคนขับรับไปแล้ว',
    'driver_open_calls_online_required':
        '온라인 상태에서만 콜을 받을 수 있습니다.\n(รับงานได้เฉพาะเมื่อออนไลน์เท่านั้น)',
    'driver_open_calls_empty': '배차 가능한 콜이 없습니다.\n(ไม่มีงานที่พร้อมให้รับ)',
    'driver_urgent_badge': '긴급\n(ด่วน)',
    'driver_urgent_section_title': '긴급 요청 콜\n(งานเร่งด่วน)',
    'driver_urgent_call_new_arrived': '긴급 요청 콜이 도착했습니다.\n(มีงานเร่งด่วนเข้ามา)',
    'driver_urgent_accept': '수락 / รับงาน',
    'driver_urgent_already_locked':
        '다른 기사가 이미 수락한 콜입니다.\n(คนขับคนอื่นรับงานนี้แล้ว)',
    'driver_urgent_eta_dialog_title': '도착 예상 시간 입력\n(กรอกเวลาที่จะถึง)',
    'driver_urgent_eta_dialog_message':
        '도착 예상 시간을 입력하시면 고객이 수락을 한 이후 예약이 확정됩니다. 도착 예상시간을 입력해주세요\n(กรอกเวลาที่จะถึง ลูกค้ายืนยันแล้วจึงจะยืนยันการจอง)',
    'driver_urgent_eta_minutes_label': '도착 예상 시간 (분)\n(นาที)',
    'driver_urgent_eta_submit': '제출 / ส่ง',
    'driver_urgent_eta_countdown': '남은 시간: {time}\n(เวลาที่เหลือ: {time})',
    'driver_urgent_eta_invalid':
        '유효한 분 단위 숫자를 입력해 주세요.\n(กรุณากรอกจำนวนนาทีที่ถูกต้อง)',
    'driver_urgent_eta_success':
        '도착 예상 시간이 제출되었습니다. 고객 확인을 기다려 주세요.\n(ส่งเวลาแล้ว รอลูกค้ายืนยัน)',
    'driver_urgent_confirmed':
        '고객이 수락했습니다. 예약이 배정되었습니다.\n(ลูกค้ายืนยันแล้ว ได้รับงานแล้ว)',
    'driver_urgent_min_eta_hint':
        '이전 거절로 {minutes}분 미만 ETA가 필요합니다.\n(ต้องน้อยกว่า {minutes} นาที)',
    'driver_urgent_awaiting_customer':
        '고객 확인 대기 중 (남은 시간 {time})\n(รอลูกค้ายืนยัน เหลือ {time})',
    'driver_urgent_eta_lock_expired':
        '도착 예상 시간 입력 시간이 만료되었습니다.\n(หมดเวลากรอกเวลาที่จะถึงแล้ว)',
    'driver_urgent_eta_not_fast_enough':
        '이전 거절보다 더 빠른 도착 예상 시간을 입력해 주세요.\n(กรุณากรอกเวลาที่จะถึงให้เร็วกว่าครั้งก่อน)',
    'driver_urgent_round_ended':
        '고객이 거절했거나 협상 라운드가 종료되었습니다.\n(ลูกค้าปฏิเสธหรือรอบการเจรจาจบแล้ว)',
    'driver_urgent_cancelled': '긴급 요청 콜이 취소되었습니다.\n(งานเร่งด่วนถูกยกเลิกแล้ว)',
    'driver_settlement_block_title':
        '아직 신규 업무를 받을 수 없습니다\n(ยังไม่สามารถรับงานใหม่ได้)',
    'driver_settlement_block_message':
        '커미션을 송금하고 슬립을 업로드한 뒤 관리자 확인을 기다려 주세요.\n(กรุณาชำระค่าคอมมิชชั่นและอัปโหลดสลิป หลังจากแอดมินตรวจสอบแล้วจึงจะรับงานใหม่ได้)',
    'driver_settlement_block_cta': '정산 화면으로 / ไปที่หน้าชำระเงิน',
    'driver_call_badge_admin_manual': '관리자 등록 콜 / งานที่แอดมินลงทะเบียน',
    'driver_call_badge_no_commission': '커미션 없음 / ไม่มีค่าคอมมิชชัน',
    'driver_call_bank_account_confirm_notice':
        '회원가입시 등록된 계좌로 지급될 예정이오니 계좌가 맞는지 한번 더 확인해주세요\n(เงินจะโอนเข้าบัญชีที่ลงทะเบียนตอนสมัคร กรุณาตรวจสอบบัญชีอีกครั้ง)',
    'driver_home_new_calls_title': '새 콜이 있습니다\n(มีงานใหม่)',
    'driver_home_new_calls_message':
        '새 콜 {count}건이 대기 중입니다.\n(มีงานใหม่ {count} งาน)',
    'driver_home_new_calls_cta': '새 콜 보기 / ดูงานใหม่',
    'driver_home_urgent_calls_title': '긴급 요청 콜\n(งานเร่งด่วน)',
    'driver_home_urgent_calls_message':
        '긴급 요청 콜 {count}건이 대기 중입니다.\n(มีงานเร่งด่วน {count} งาน)',
    'driver_home_urgent_calls_cta': '긴급 콜 보기 / ดูงานเร่งด่วน',
    'driver_call_customer': '고객에게 전화 / โทรหาลูกค้า',
    'driver_account_notifications': '알림\n(การแจ้งเตือน)',
    'driver_step_progress':
        '단계 {step}/{total}\n(ขั้นตอนที่ {step} จาก {total})',
    'driver_step_assigned': '배정 확인됨 / งานได้รับการยืนยัน',
    'driver_step_assigned_hint':
        'Stand by 전 예약 내용을 확인하세요.\n(ตรวจสอบรายละเอียดก่อนยืนยัน Stand by)',
    'driver_step_standby': 'Stand by 확인 / ยืนยัน Stand by',
    'driver_step_standby_hint':
        '픽업 1시간 전부터 확인 가능합니다.\n(ยืนยันได้ก่อนเวลานัดหมาย 1 ชั่วโมง)',
    'driver_step_depart_pickup': '픽업지로 출발 / เดินทางไปรับลูกค้า',
    'driver_step_depart_pickup_hint':
        '지도를 열고 픽업 장소로 이동하세요.\n(เปิดแผนที่แล้วไปยังจุดรับ)',
    'driver_step_arrived_pickup': '픽업지 도착 / ถึงจุดรับแล้ว',
    'driver_step_arrived_pickup_hint':
        '픽업 지점에 도착하면 확인하세요.\n(ยืนยันหลังถึงจุดรับ)',
    'driver_step_customer_boarded': '고객 탑승 / ลูกค้าขึ้นรถแล้ว',
    'driver_step_customer_boarded_hint':
        '고객이 탑승하면 확인하세요.\n(ยืนยันหลังลูกค้าขึ้นรถ)',
    'driver_step_in_transit': '운행 중 / กำลังเดินทาง',
    'driver_step_finish_trip': '운행 종료 / ยืนยันจบงาน',
    'driver_step_finish_trip_hint':
        '종료 후 커미션 송금과 슬립 업로드가 필요합니다.\n(หลังจบงานต้องชำระค่าคอมมิชชั่นและอัปโหลดสลิป)',
    'driver_step_settlement': '정산 필요 / ชำระค่าคอมมิชชั่น',
    'driver_step_settlement_hint':
        '커미션 송금과 슬립 업로드를 완료하세요.\n(ชำระค่าคอมมิชชั่นและอัปโหลดสลิป)',
    'driver_today_greeting_generic': '안녕하세요\n(สวัสดี)',
    'driver_today_greeting_named': '안녕하세요, {name} 기사님\n(สวัสดี คุณ{name})',
    'driver_today_trip_count':
        '미운행 예약이 {count}건 있습니다\n(มีงานที่ยังไม่ดำเนินการ {count} งาน)',
    'driver_today_current_trip': '지금 할 일\n(งานปัจจุบัน)',
    'driver_today_cta_continue': '운행 계속하기 / ดำเนินงานต่อ',
    'driver_today_cta_settlement': '정산 확인 / ตรวจสอบการชำระเงิน',
    'driver_today_cta_settlement_submit': '정산 및 송금증 제출 / ชำระเงินและส่งหลักฐาน',
    'driver_today_cta_settlement_waiting': '입금 확인 대기 중 / รอตรวจสอบการชำระเงิน',
    'driver_today_remaining_title':
        '오늘 남은 예약 {count}건\n(งานที่เหลือวันนี้ {count} งาน)',
    'driver_today_completed_title': '완료된 운행 ({count})\n(งานที่เสร็จแล้ว)',
    'driver_today_empty_title': '예정된 운행이 없습니다.\n(ไม่มีงานที่กำหนดไว้)',
    'driver_today_empty_message':
        '새 예약이 배정되면 알림으로 알려드립니다.\n(ระบบจะแจ้งเตือนเมื่อมีงานใหม่)',
    'driver_quick_navigate': '길찾기 / นำทาง',
    'driver_quick_call': '전화 / โทร',
    'driver_status_guidance_assigned':
        '고객의 픽업요청 시간과 출발지를 확인하시고, 시간에 맞춰 이동하시기 바랍니다.\n(กรุณาตรวจสอบเวลาที่ลูกค้าขอรับและจุดต้นทาง แล้วออกเดินทางให้ตรงเวลา)',
    'driver_status_guidance_on_route':
        '픽업 장소로 이동 중입니다. 도착하면 도착 버튼을 눌러 주세요.\n(กำลังเดินทางไปจุดรับ เมื่อถึงแล้วให้กดปุ่มถึงจุดรับ)',
    'driver_status_guidance_arrived':
        '픽업 장소에 도착했습니다. 고객이 탑승한 후 탑승 확인 버튼을 눌러 주세요.\n(ถึงจุดรับแล้ว เมื่อลูกค้าขึ้นรถแล้วให้กดยืนยัน)',
    'driver_status_guidance_picked_up':
        '고객을 목적지로 이동 중입니다. 목적지 도착 후 운행을 종료하세요.\n(กำลังพาลูกค้าไปยังจุดหมาย เมื่อถึงแล้วให้จบงาน)',
    'driver_status_guidance_settlement':
        '운행이 종료되었습니다. 정산 처리가 완료될 때까지 신규 배차를 받을 수 없습니다.\n(จบงานแล้ว ยังรับงานใหม่ไม่ได้จนกว่าการชำระบัญชีจะเสร็จสิ้น)',
    'driver_history_range_7d': '최근 7일 / 7 วันล่าสุด',
    'driver_history_range_30d': '최근 30일 / 30 วันล่าสุด',
    'driver_history_range_all': '전체 / ทั้งหมด',
    'driver_history_empty':
        '표시할 완료 운행 또는 정산 내역이 없습니다.\n(ไม่มีรายการเดินทางที่เสร็จสิ้นหรือรายการชำระเงิน)',
    'driver_history_subtitle':
        '완료된 운행과 정산 내역을 확인할 수 있습니다.\n(ดูประวัติการเดินทางที่เสร็จสิ้นและรายการชำระเงิน)',
    'driver_history_pending_settlement':
        '정산 필요 {count}건\n(ต้องดำเนินการชำระเงิน {count} รายการ)',
    'driver_account_settlement': '정산\n(การชำระบัญชี)',
    'driver_account_support': '고객센터\n(ศูนย์บริการลูกค้า)',
    'driver_account_profile': '내 프로필\n(โปรไฟล์ของฉัน)',
    'driver_vehicles_menu': '차량 관리\n(จัดการรถ)',
    'driver_vehicles_title': '차량 관리\n(จัดการรถ)',
    'driver_vehicles_empty': '등록된 차량이 없습니다.\n(ยังไม่มีรถที่ลงทะเบียน)',
    'driver_vehicle_add': '차량 추가 / เพิ่มรถ',
    'driver_vehicle_add_title': '차량 추가\n(เพิ่มรถ)',
    'driver_vehicle_submit': '승인 요청 / ส่งคำขออนุมัติ',
    'driver_vehicle_add_success':
        '차량이 승인 대기로 등록되었습니다.\n(ลงทะเบียนรถแล้ว รอการอนุมัติ)',
    'driver_vehicle_add_failed': '차량을 등록하지 못했습니다.\n(ไม่สามารถลงทะเบียนรถได้)',
    'driver_vehicle_type_label': '차종 / ประเภทรถ',
    'driver_vehicle_plate_label': '번호판 / ทะเบียนรถ',
    'driver_vehicle_photos_title': '차량 사진\n(รูปรถ)',
    'driver_vehicle_photos_hint':
        '차량 사진 3~6장을 업로드해 주세요.\n(อัปโหลดรูปรถ 3–6 รูป)',
    'driver_vehicle_photos_required':
        '차량 사진 3장 이상이 필요합니다.\n(ต้องมีรูปรถอย่างน้อย 3 รูป)',
    'driver_vehicle_insurance_title': '보험증\n(กรมธรรม์ประกัน)',
    'driver_vehicle_registration_title': '차량등록증\n(ทะเบียนรถ)',
    'driver_vehicle_doc_hint':
        '서류 사진 또는 PDF를 업로드해 주세요.\n(อัปโหลดรูปเอกสารหรือ PDF)',
    'driver_vehicle_doc_required':
        '필수 서류를 업로드해 주세요.\n(กรุณาอัปโหลดเอกสารที่จำเป็น)',
    'driver_vehicle_status_pending': '승인 대기 / รออนุมัติ',
    'driver_vehicle_status_approved': '승인됨 / อนุมัติแล้ว',
    'driver_vehicle_status_rejected': '거절됨 / ถูกปฏิเสธ',
    'driver_vehicle_rejection_reason_label': '거절 사유 / เหตุผลที่ปฏิเสธ',
    'driver_vehicle_primary_badge': '대표 차량\n(รถหลัก)',
    'driver_account_vehicle_title': '내 차량\n(รถของฉัน)',
    'driver_account_vehicle_empty':
        '등록된 차량 정보가 없습니다.\n(ยังไม่มีข้อมูลรถที่ลงทะเบียนไว้)',
    'driver_account_vehicle_color': '차량 색상 / สีรถ',
    'driver_account_vehicle_year': '차량 연식 / ปีรถ',
    'driver_account_vehicle_model': '차량 모델 / รุ่นรถ',
    'driver_profile_edit_title': '프로필 수정 / แก้ไขโปรไฟล์',
    'driver_profile_save': '저장 / บันทึก',
    'driver_profile_save_success': '저장되었습니다.\n(บันทึกเรียบร้อยแล้ว)',
    'driver_profile_save_failed':
        '저장하지 못했습니다. 다시 시도해 주세요.\n(บันทึกไม่สำเร็จ กรุณาลองอีกครั้ง)',
    'driver_profile_readonly_email': '이메일 (변경 불가) / อีเมล (แก้ไขไม่ได้)',
    'driver_profile_upload_avatar': '프로필 사진 변경 / เปลี่ยนรูปโปรไฟล์',
    'driver_profile_upload_vehicle_photo': '차량 사진 변경 / เปลี่ยนรูปรถ',
    'driver_account_language': '언어\n(ภาษา)',
    'driver_account_notification_settings': '알림 설정\n(การตั้งค่าการแจ้งเตือน)',
    'driver_account_terms': '이용약관\n(ข้อกำหนดการใช้งาน)',
    'driver_account_privacy': '개인정보처리방침\n(นโยบายความเป็นส่วนตัว)',
    'driver_account_terms_placeholder': '이용약관 페이지 준비 중입니다',
    'driver_account_privacy_placeholder': '개인정보처리방침 페이지 준비 중입니다',
    'driver_logout_confirm_title': '로그아웃할까요?\n(ต้องการออกจากระบบหรือไม่)',
    'driver_logout_confirm_message':
        '다시 로그인해야 합니다.\n(ต้องเข้าสู่ระบบใหม่อีกครั้ง)',
    'driver_action_start_on_route': '픽업지로 출발 / ออกเดินทางไปรับลูกค้า',
    'driver_action_accept_booking': '운행 준비 확인 / ยืนยันว่าพร้อมปฏิบัติงาน',
    'driver_view_booking_detail': '예약 상세 보기 / ดูรายละเอียดการจอง',
    'driver_action_mark_arrived': '픽업지 도착 / ถึงจุดรับแล้ว',
    'driver_action_mark_picked_up': '고객 탑승 확인 / รับลูกค้าขึ้นรถแล้ว',
    'driver_action_end_trip': '운행 종료 / ยืนยันจบงาน',
    'driver_action_complete_trip': '운행 종료 / ยืนยันจบงาน',
    'driver_confirm_no': '아니요 / ไม่',
    'driver_confirm_yes': '예 / ใช่',
    'driver_confirm_start_on_route_title': '픽업 장소로 이동을 시작할까요?',
    'driver_confirm_start_on_route_message': '지금 픽업 장소로 이동을 시작합니다.',
    'driver_confirm_start_on_route_yes': '이동 시작 / เริ่มเดินทาง',
    'driver_confirm_accept_booking_title': '운행을 확정할까요?',
    'driver_confirm_accept_booking_message':
        '픽업 기준 1시간 전부터 운행 확정이 가능합니다. 확정 후 준비되면 픽업 장소로 이동하세요.',
    'driver_confirm_accept_booking_yes': '운행 확정 / ยืนยัน',
    'driver_accept_booking_processing': '운행을 확정하고 있습니다\n(กำลังยืนยันงาน)',
    'driver_success_accept_booking': '운행이 확정되었습니다\n(ยืนยันงานแล้ว)',
    'driver_release_assignment': '배정 반납 / คืนงานที่ได้รับมอบหมาย',
    'driver_release_assignment_title':
        '이 예약의 배정을 반납하시겠습니까?\n(ต้องการคืนงานที่ได้รับมอบหมายสำหรับการจองนี้หรือไม่)',
    'driver_release_assignment_message':
        '고객 예약은 유지되며 다른 기사를 다시 배정합니다.\n(การจองของลูกค้ายังคงอยู่ และจะมอบหมายให้คนขับท่านอื่น)',
    'driver_release_assignment_cancel': '유지 / คงไว้',
    'driver_release_assignment_confirm': '배정 반납 / คืนงาน',
    'driver_release_assignment_success': '배정이 해제되었습니다.\n(คืนงานเรียบร้อยแล้ว)',
    'driver_release_assignment_failed':
        '배정 반납에 실패했습니다. 다시 시도해 주세요.\n(ไม่สามารถคืนงานได้ กรุณาลองอีกครั้ง)',
    'driver_release_assignment_blocked':
        '현재 상태에서는 배정을 반납할 수 없습니다.\n(ไม่สามารถคืนงานได้ในสถานะปัจจุบัน)',
    'driver_release_assignment_emergency_hint':
        '픽업까지 2시간 이내에는 일반 배정 반납이 제한됩니다. 긴급 상황인 경우에만 배정을 반납할 수 있습니다.\n(ภายใน 2 ชั่วโมงก่อนรับลูกค้า จะคืนงานได้เฉพาะกรณีฉุกเฉินเท่านั้น)',
    'driver_release_assignment_irreversible':
        '반납 후에는 되돌릴 수 없습니다.\n(หลังคืนงานแล้วจะไม่สามารถย้อนกลับได้)',
    'driver_release_reason_label': '반납 사유 / เหตุผลในการคืนงาน',
    'driver_release_reason_detail_label': '상세 설명 / รายละเอียดเพิ่มเติม',
    'driver_release_reason_detail_required':
        '기타 사유를 입력하세요.\n(กรุณาระบุเหตุผลอื่น)',
    'driver_release_reason_VEHICLE_BREAKDOWN': '차량 고장 / รถเสีย',
    'driver_release_reason_ACCIDENT': '사고 / อุบัติเหตุ',
    'driver_release_reason_DRIVER_ILLNESS': '질병 / คนขับป่วย',
    'driver_release_reason_FAMILY_EMERGENCY': '가족 긴급 상황 / เหตุฉุกเฉินครอบครัว',
    'driver_release_reason_SCHEDULE_CONFLICT': '일정 충돌 / ตารางงานซ้ำซ้อน',
    'driver_release_reason_LOCATION_TOO_FAR': '거리 문제 / จุดรับอยู่ไกลเกินไป',
    'driver_release_reason_OTHER': '기타 / อื่นๆ',
    'driver_confirm_mark_arrived_title': '픽업 장소에 도착하셨습니까?',
    'driver_confirm_mark_arrived_message': '픽업 장소에 도착했습니까?',
    'driver_confirm_mark_arrived_yes': '도착 / ถึงแล้ว',
    'driver_confirm_mark_picked_up_title': '고객이 차량에 탑승했습니까?',
    'driver_confirm_mark_picked_up_message': '탑승 확인 후 운행이 시작됩니다.',
    'driver_confirm_mark_picked_up_yes': '탑승 확인 / ยืนยันขึ้นรถ',
    'driver_confirm_end_trip_title': '고객을 목적지에 내려드렸습니까?',
    'driver_confirm_end_trip_message': '운행 종료 후 정산 단계로 이동합니다.',
    'driver_confirm_end_trip_yes': '운행 종료 / จบการเดินทาง',
    'driver_confirm_customer_boarded_processing':
        '탑승을 확인하고 있습니다\n(กำลังยืนยันการขึ้นรถ)',
    'driver_end_trip_processing': '운행을 종료하고 있습니다\n(กำลังจบงาน)',
    'driver_trip_action_processing': '처리 중입니다\n(กำลังดำเนินการ)',
    'driver_customer_payment_amount': '고객에게 받을 금액\n(ยอดที่ต้องรับจากลูกค้า)',
    'driver_payment_method': '결제 방법\n(วิธีชำระเงิน)',
    'driver_payment_method_pay_driver': '기사에게 현장 결제\n(ชำระกับคนขับที่จุดหมาย)',
    'driver_payment_method_bank_transfer': '계좌이체\n(โอนเงินผ่านธนาคาร)',
    'driver_payment_method_card': '카드 결제\n(บัตร)',
    'driver_payment_method_unknown': '결제 방식 확인 필요\n(ต้องตรวจสอบวิธีชำระเงิน)',
    'driver_payment_summary_title': '결제 및 수입\n(การชำระเงินและรายได้)',
    'driver_customer_total_amount': '고객 결제 총액\n(ยอดชำระรวมของลูกค้า)',
    'driver_company_commission':
        '회사에 납부할 수수료\n(ค่าคอมมิชชันที่ต้องชำระให้บริษัท)',
    'driver_expected_income': '기사 예상 수입\n(รายได้โดยประมาณของคนขับ)',
    'driver_income_unavailable':
        '수입 정보를 확인할 수 없습니다. 관리자에게 문의해 주세요.\n(ไม่สามารถยืนยันข้อมูลรายได้ได้ กรุณาติดต่อผู้ดูแล)',
    'driver_commission_unavailable':
        '수수료 정보 확인 필요\n(ต้องตรวจสอบข้อมูลค่าคอมมิชชัน)',
    'driver_receipt_status': '영수증 상태\n(สถานะสลิป)',
    'driver_settlement_status_not_due_yet': '아직 납부 대상 아님\n(ยังไม่ถึงกำหนดชำระ)',
    'driver_settlement_status_due': '납부 필요\n(ต้องชำระ)',
    'driver_settlement_status_overdue': '납부 지연\n(เกินกำหนดชำระ)',
    'driver_settlement_status_receipt_submitted':
        '영수증 검토 중\n(กำลังตรวจสอบสลิป)',
    'driver_settlement_status_rejected': '영수증 반려\n(สลิปถูกปฏิเสธ)',
    'driver_settlement_status_completed': '정산 완료\n(ชำระเสร็จแล้ว)',
    'driver_settlement_status_waived': '수수료 면제\n(ยกเว้นค่าคอมมิชชัน)',
    'driver_settlement_status_unknown': '상태 확인 필요\n(ต้องตรวจสอบสถานะ)',
    'driver_receipt_status_none': '영수증 없음\n(ยังไม่มีสลิป)',
    'driver_receipt_status_submitted': '영수증 제출됨\n(ส่งสลิปแล้ว)',
    'driver_receipt_status_rejected': '영수증 반려\n(สลิปถูกปฏิเสธ)',
    'driver_receipt_status_approved': '영수증 승인됨\n(อนุมัติสลิปแล้ว)',
    'driver_receipt_status_unknown': '영수증 상태 확인 필요\n(ต้องตรวจสอบสถานะสลิป)',
    'driver_settlement_not_due_yet_message':
        '아직 회사 수수료 납부 대상이 아닙니다. 신규 배차를 받을 수 있습니다.\n(ยังไม่ถึงกำหนดชำระค่าคอมมิชชัน สามารถรับงานใหม่ได้)',
    'driver_settlement_due_message':
        '회사 수수료를 납부하고 영수증을 제출해 주세요.\n(กรุณาชำระค่าคอมมิชชันและอัปโหลดสลิป)',
    'driver_settlement_due_blocked_message':
        '회사 수수료를 납부하고 영수증을 제출해 주세요. 정산 완료 전까지 신규 배차를 받을 수 없습니다.\n(กรุณาชำระค่าคอมมิชชันและอัปโหลดสลิป ก่อนชำระเสร็จจะยังรับงานใหม่ไม่ได้)',
    'driver_settlement_overdue_message':
        '수수료 납부 기한이 지났습니다. 수수료 납부와 영수증 제출이 필요합니다.\n(เลยกำหนดชำระค่าคอมมิชชันแล้ว กรุณาชำระและอัปโหลดสลิป)',
    'driver_settlement_receipt_review_message':
        '영수증 검토 중입니다. 관리자 승인 후 신규 배차를 받을 수 있습니다.\n(กำลังตรวจสอบสลิป รอผู้ดูแลอนุมัติก่อนรับงานใหม่)',
    'driver_settlement_rejected_message':
        '영수증이 반려되었습니다. 사유를 확인하고 다시 제출해 주세요.\n(สลิปถูกปฏิเสธ กรุณาตรวจสอบเหตุผลและส่งใหม่)',
    'driver_settlement_completed_message':
        '정산이 완료되었습니다.\n(ชำระค่าคอมมิชชันเสร็จแล้ว)',
    'driver_settlement_waived_message':
        '이번 운행의 회사 수수료가 면제되었습니다. 신규 배차를 받을 수 있습니다.\n(ยกเว้นค่าคอมมิชชันสำหรับเที่ยวนี้ สามารถรับงานใหม่ได้)',
    'driver_new_calls_blocked_by_settlement':
        '운행은 종료됐지만 정산이 남아 있습니다. 정산 완료 후 신규 배차를 받을 수 있습니다.\n(จบทริปแล้วแต่ยังมีงานชำระค้างอยู่ กรุณาชำระให้เสร็จก่อนรับงานใหม่)',
    'driver_success_picked_up': '고객 탑승이 확인되었습니다',
    'driver_success_end_trip': '운행이 종료되었습니다. 정산을 확인해 주세요.',
    'driver_trip_settlement_pending_info':
        '운행이 종료되었습니다. 정산 처리가 완료될 때까지 신규 배차를 받을 수 없습니다.',
    'driver_trip_completed_info': '정산이 확인되어 운행이 완료되었습니다.',
    'driver_standby_status': '운행 확정\n(ยืนยันงาน)',
    'driver_standby_confirmed': '확정됨\n(ยืนยันแล้ว)',
    'driver_standby_airport_reference':
        '항공편 도착 기준 시각\n(เวลาอ้างอิงเที่ยวบินถึง)',
    'driver_standby_departure_reference':
        '차량 출발 기준 시각\n(เวลาอ้างอิงออกเดินทาง)',
    'driver_standby_available_from': '운행 확정 가능 시간\n(ยืนยันได้ตั้งแต่)',
    'driver_standby_too_early':
        '아직 Stand by를 확정할 수 없습니다. 예약 기준 1시간 전부터 가능합니다.\n(ยังไม่ถึงเวลายืนยัน Stand by สามารถยืนยันได้ก่อนเวลานัดหมาย 1 ชั่วโมง)',
    'driver_standby_reference_missing':
        '예약 기준 시간을 찾을 수 없습니다. 관리자에게 문의해 주세요.\n(ไม่พบเวลานัดหมายของงานนี้ กรุณาติดต่อแอดมิน)',
    'driver_standby_status_not_allowed':
        '이 예약은 현재 운행 확정을 할 수 없습니다. 관리자에게 상태 확인을 요청해 주세요.\n(ไม่สามารถยืนยันงานนี้ได้ กรุณาติดต่อแอดมินเพื่อตรวจสอบสถานะการจอง)',
    'driver_open_google_maps': 'Google Maps에서 보기\n(เปิดใน Google Maps)',
    'driver_google_maps_failed':
        'Google Maps를 열 수 없습니다.\n(ไม่สามารถเปิด Google Maps ได้)',
    'driver_location_unavailable': '위치 정보 없음\n(ไม่มีข้อมูลตำแหน่ง)',
    'driver_name_sign_required':
        '네임보드 서비스가 신청되었습니다. 고객 미팅 전 피켓을 준비하세요.\n(ลูกค้าเลือกบริการป้ายชื่อ กรุณาเตรียมป้ายก่อนพบลูกค้า)',
    'driver_settlement_popup_title': '정산이 필요합니다',
    'driver_settlement_popup_message':
        '운행이 종료되었습니다. 회사 수수료를 송금한 뒤 정산 상세에서 송금증을 업로드하세요.',
    'driver_action_scan_boarding': '탑승 QR 스캔 / สแกน QR ขึ้นรถ',
    'driver_action_scan_dropoff': '하차 QR 스캔 / สแกน QR ลงรถ',
    'driver_job_assigned_hint':
        '준비되면 픽업 장소로 이동하세요\n(พร้อมแล้วให้เดินทางไปยังจุดรับ)',
    'driver_status_on_route': '이동 중\n(กำลังเดินทางไปรับ)',
    'driver_status_arrived': '픽업 장소 도착\n(ถึงจุดรับแล้ว)',
    'driver_status_picked_up': '고객 탑승\n(รับผู้โดยสารแล้ว)',
    'driver_status_no_show': '고객 미탑승\n(ผู้โดยสารไม่มา)',
    'driver_success_boarding': '고객 탑승 처리가 완료되었습니다\n(ยืนยันการขึ้นรถแล้ว)',
    'driver_success_completed': '운행이 완료되었습니다\n(จบงานเรียบร้อยแล้ว)',
    'driver_success_on_route': '운행을 시작했습니다\n(เริ่มเดินทางแล้ว)',
    'driver_success_arrived': '기사 도착으로 변경했습니다\n(เปลี่ยนสถานะเป็นถึงจุดรับแล้ว)',
    'driver_call_failed': '전화를 연결할 수 없습니다\n(ไม่สามารถโทรออกได้)',
    'driver_detail_error': '예약 정보를 불러올 수 없습니다\n(ไม่สามารถโหลดข้อมูลการจองได้)',
    'driver_section_trip': '예약 상세\n(รายละเอียดการจอง)',
    'driver_detail_status': '상태\n(สถานะ)',
    'driver_detail_origin': '픽업 장소\n(จุดรับ)',
    'driver_detail_destination': '목적지\n(จุดหมายปลายทาง)',
    'driver_detail_vehicle': '차량 종류\n(ประเภทรถ)',
    'driver_detail_passengers': '인원\n(จำนวนผู้โดยสาร)',
    'driver_detail_luggage': '짐\n(สัมภาระ)',
    'driver_detail_flight_number': '항공편 번호\n(หมายเลขเที่ยวบิน)',
    'driver_detail_special_requests': '요청사항\n(คำขอเพิ่มเติม)',
    'driver_detail_customer_info': '고객 정보\n(ข้อมูลลูกค้า)',
    'driver_detail_customer_name': '고객 이름\n(ชื่อลูกค้า)',
    'driver_pickup_time': '픽업 시간\n(เวลารับ)',
    'driver_message_customer': '고객에게 메시지 보내기 / ส่งข้อความหาลูกค้า',
    'driver_estimated_arrival': '예상 도착 시간\n(เวลาถึงโดยประมาณ)',
    'driver_next_action': '다음 작업\n(ขั้นตอนถัดไป)',
    'driver_section_settlement': '정산\n(การชำระบัญชี)',
    'driver_settlement_loading_failed':
        '정산 정보를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.\n(ไม่สามารถโหลดข้อมูลการชำระเงินได้ กรุณาลองอีกครั้ง)',
    'driver_rejection_reason': '반려 사유\n(เหตุผลที่ไม่อนุมัติ)',
    'driver_view_settlement': '정산 상세 / ดูรายละเอียด',
    'driver_qr_boarding_title': '탑승 QR\n(QR ขึ้นรถ)',
    'driver_qr_dropoff_title': '하차 QR\n(QR ลงรถ)',
    'driver_qr_boarding_help':
        '고객 앱의 탑승 QR을 스캔하세요.\n(สแกน QR ขึ้นรถจากแอปลูกค้า)',
    'driver_qr_dropoff_help': '고객 앱의 하차 QR을 스캔하세요.\n(สแกน QR ลงรถจากแอปลูกค้า)',
    'driver_qr_boarding_manual_label': '탑승 QR 코드\n(รหัส QR ขึ้นรถ)',
    'driver_qr_dropoff_manual_label': '하차 QR 코드\n(รหัส QR ลงรถ)',
    'driver_qr_manual_hint':
        '카메라 없이 코드를 직접 입력할 수 있습니다.\n(กรอกรหัสเองได้โดยไม่ใช้กล้อง)',
    'driver_qr_manual_entry': '코드 직접 입력\n(กรอกรหัสด้วยตนเอง)',
    'driver_qr_use_manual': '직접 입력 / กรอกรหัส',
    'driver_qr_use_camera': '카메라 / กล้อง',
    'driver_qr_submit': '확인 / ยืนยัน',
    'driver_notification_no_target':
        '연결된 예약이 없는 알림입니다\n(การแจ้งเตือนนี้ไม่มีงานที่เชื่อมโยง)',
    'driver_mark_all_read': '모두 읽음 / อ่านทั้งหมด',
    'driver_notifications_empty': '알림이 없습니다\n(ยังไม่มีการแจ้งเตือน)',
    'driver_call_status_ready': '콜 수신 가능 / พร้อมรับงาน',
    'driver_call_status_offline': '콜 수신 일시 중지 / หยุดรับงานชั่วคราว',
    'driver_call_status_active_trip': '운행 중 / กำลังให้บริการ',
    'driver_call_status_active_trip_helper': '새 콜 수신 불가 / ยังรับงานใหม่ไม่ได้',
    'driver_call_status_settlement_required': '정산 확인 필요 / ต้องตรวจสอบยอดชำระ',
    'driver_call_status_customer_issue_review':
        '고객 이슈 확인 중 / กำลังตรวจสอบปัญหาลูกค้า',
    'driver_call_status_account_under_review': '계정 검토 중 / กำลังตรวจสอบบัญชี',
    'driver_call_status_account_restricted': '계정 제한됨 / บัญชีถูกจำกัด',
    'driver_call_status_driver_approval_pending': '기사 승인 대기 / รออนุมัติคนขับ',
    'driver_call_status_vehicle_review_required': '차량 확인 필요 / ต้องตรวจสอบรถ',
    'driver_call_status_unknown': '상태 확인 필요 / ต้องตรวจสอบสถานะ',
    'driver_call_status_detail_title': '콜 수신 상태 / สถานะการรับงาน',
    'driver_call_status_detail_ready':
        '새 예약 콜을 받을 수 있습니다. / สามารถรับงานใหม่ได้',
    'driver_call_status_detail_offline':
        '콜을 받으려면 온라인으로 전환해 주세요. / กรุณาออนไลน์เมื่อต้องการรับงาน',
    'driver_call_status_detail_active_trip':
        '현재 운행을 마친 뒤 새 콜을 받을 수 있습니다. / รับงานใหม่ได้หลังจบทริปปัจจุบัน',
    'driver_call_status_detail_settlement_required':
        '이전 운행의 정산 확인이 완료되어야 새 콜을 받을 수 있습니다. / ต้องยืนยันยอดชำระของงานก่อนหน้าให้เรียบร้อยก่อนรับงานใหม่',
    'driver_call_status_detail_customer_issue_review':
        '운영팀이 고객 이슈를 확인하고 있습니다. / ทีมงานกำลังตรวจสอบปัญหาของลูกค้า',
    'driver_call_status_detail_account_under_review':
        '운영팀이 기사 계정을 검토하고 있습니다. / ทีมงานกำลังตรวจสอบบัญชีคนขับ',
    'driver_call_status_detail_account_restricted':
        '기사 계정이 제한되어 있습니다. 운영팀에 문의해 주세요. / บัญชีคนขับถูกจำกัด กรุณาติดต่อทีมงาน',
    'driver_call_status_detail_driver_approval_pending':
        '기사 승인이 아직 완료되지 않았습니다. / การอนุมัติคนขับยังไม่เสร็จสิ้น',
    'driver_call_status_detail_vehicle_review_required':
        '콜을 받으려면 유효한 활성 차량이 필요합니다. / ต้องมีรถที่ใช้งานได้ก่อนรับงาน',
    'driver_call_status_detail_unknown':
        '계속 표시되면 새로고침하거나 운영팀에 문의해 주세요. / หากยังแสดงอยู่ กรุณารีเฟรชหรือติดต่อทีมงาน',
    'driver_notification_new': '새 알림 / ใหม่',
    'driver_notification_mark_read': '읽음 / อ่านแล้ว',
    'driver_notification_enable': '알림 켜기 / เปิดการแจ้งเตือน',
    'driver_notification_enabled': '알림이 켜졌습니다\n(เปิดการแจ้งเตือนแล้ว)',
    'driver_notification_denied':
        '알림 권한이 거부되었습니다\n(ไม่ได้รับอนุญาตให้แจ้งเตือน)',
    'driver_notification_unsupported':
        '이 브라우저는 푸시 알림을 지원하지 않습니다\n(เบราว์เซอร์นี้ไม่รองรับการแจ้งเตือน)',
    'driver_notification_unconfigured':
        '알림 서비스가 설정되지 않았습니다\n(ยังไม่ได้ตั้งค่าระบบแจ้งเตือน)',
    'driver_notification_failed':
        '알림 등록에 실패했습니다\n(ลงทะเบียนการแจ้งเตือนไม่สำเร็จ)',
    'driver_notification_default': '알림\n(การแจ้งเตือน)',
    'driver_rating_loading': '평점을 불러오는 중입니다\n(กำลังโหลดคะแนน)',
    'driver_rating_error': '평점을 불러올 수 없습니다\n(ไม่สามารถโหลดคะแนนได้)',
    'driver_no_ratings': '아직 평점이 없습니다\n(ยังไม่มีคะแนน)',
    'driver_rating_average': '평균 / คะแนนเฉลี่ย',
    'driver_rating_count': '개 리뷰 / รีวิว',
    'driver_logout': '로그아웃 / ออกจากระบบ',
    'driver_online': '온라인\n(พร้อมรับงาน)',
    'driver_offline': '오프라인\n(ออฟไลน์)',
    'driver_session_ready': '배차 대기 중\n(พร้อมรับงาน)',
    'driver_session_offline': '오프라인 상태입니다\n(ขณะนี้ออฟไลน์)',
    'driver_status_online_ready': '온라인 · 배차 대기중 / ออนไลน์ · พร้อมรับงาน',
    'driver_status_offline_unavailable':
        '오프라인 · 배차를 받을 수 없음 / ออฟไลน์ · ไม่สามารถรับงานได้',
    'driver_status_active_busy':
        '운행 중 · 오프라인 전환 불가 / กำลังให้บริการ · ไม่สามารถออฟไลน์ได้',
    'driver_active_job_in_progress': '진행 중인 운행이 있습니다\n(มีงานที่กำลังดำเนินการ)',
    'driver_active_job_stay_online':
        '운행 완료 전까지 온라인을 유지하세요.\n(กรุณาออนไลน์ไว้จนกว่าจะจบงาน)',
    'driver_profile_status_loading': '기사 상태를 불러오는 중입니다\n(กำลังโหลดสถานะคนขับ)',
    'driver_profile_status_error':
        '기사 상태를 불러올 수 없습니다\n(ไม่สามารถโหลดสถานะคนขับได้)',
    'driver_profile_go_online': '온라인 전환 / พร้อมรับงาน',
    'driver_profile_go_offline': '오프라인 전환 / ออฟไลน์',
    'driver_profile_offline_confirm': '오프라인으로 전환할까요?\n(ต้องการออฟไลน์หรือไม่)',
    'driver_profile_offline_blocked':
        '진행 중인 운행이 있어 완료 전에는 오프라인 전환이 제한됩니다.\n(มีงานที่กำลังดำเนินการ จึงยังไม่สามารถออฟไลน์ได้)',
    'driver_cancel': '취소 / ยกเลิก',
    'driver_try_anyway': '계속 시도 / ลองต่อ',
    'driver_last_seen': '마지막 접속: {date}\n(ออนไลน์ล่าสุด)',
    'driver_passengers': '명\n(ผู้โดยสาร)',
    'driver_live_location_share': '실시간 위치 공유\n(แชร์ตำแหน่งปัจจุบัน)',
    'driver_live_location_online_required':
        '온라인 전환 후 위치를 공유하세요.\n(กรุณาออนไลน์ก่อนแชร์ตำแหน่ง)',
    'driver_live_location_active_job_only':
        '진행 중인 운행에서만 사용할 수 있습니다\n(ใช้ได้เฉพาะระหว่างมีงาน)',
    'driver_live_location_last_sent': '마지막 전송 {time}\n(ส่งล่าสุด)',
    'driver_location_status_title': '위치 공유\n(แชร์ตำแหน่ง)',
    'driver_location_sharing': '위치 공유 중\n(กำลังแชร์ตำแหน่ง)',
    'driver_location_starting': '위치 공유를 시작하고 있습니다\n(กำลังเริ่มแชร์ตำแหน่ง)',
    'driver_location_stopped': '위치 공유가 종료되었습니다\n(หยุดแชร์ตำแหน่งแล้ว)',
    'driver_location_temporarily_unavailable':
        '위치 공유가 일시적으로 중단되었습니다. 자동으로 다시 시도합니다.\n(การแชร์ตำแหน่งหยุดชั่วคราว ระบบจะลองใหม่อัตโนมัติ)',
    'driver_location_permission_required': '위치 권한이 필요합니다\n(ต้องอนุญาตตำแหน่ง)',
    'driver_location_permission_explanation':
        '현재 운행 중에만 위치를 공유합니다.\n(แชร์ตำแหน่งเฉพาะระหว่างเที่ยวนี้)',
    'driver_location_permission_denied':
        '위치 권한을 허용하지 않았습니다. 운행은 계속할 수 있지만 고객에게 기사 위치가 표시되지 않을 수 있습니다.\n(ไม่ได้อนุญาตตำแหน่ง ยังขับงานต่อได้ แต่ลูกค้าอาจไม่เห็นตำแหน่งคนขับ)',
    'driver_location_service_disabled':
        '휴대폰 위치 서비스가 꺼져 있습니다.\n(บริการตำแหน่งของอุปกรณ์ปิดอยู่)',
    'driver_location_retry': '위치 공유 다시 시도\n(ลองแชร์ตำแหน่งอีกครั้ง)',
    'driver_location_open_settings': '설정 열기\n(เปิดการตั้งค่า)',
    'driver_location_auto_start_guidance':
        '픽업 장소로 이동을 시작하면 위치 공유가 자동으로 시작됩니다.\n(เมื่อเริ่มไปยังจุดรับ ระบบจะแชร์ตำแหน่งอัตโนมัติ)',
    'driver_location_not_shared_warning':
        '위치가 공유되지 않고 있습니다. 운행은 계속할 수 있습니다.\n(ยังไม่ได้แชร์ตำแหน่ง แต่ยังทำงานต่อได้)',
    'driver_location_error':
        '위치 공유에 실패했습니다. 다시 시도해 주세요.\n(แชร์ตำแหน่งไม่สำเร็จ กรุณาลองใหม่)',
    'driver_settlement_title': '수수료 정산\n(การชำระค่าคอมมิชชัน)',
    'driver_settlement_empty': '정산 내역이 없습니다\n(ยังไม่มีรายการชำระบัญชี)',
    'driver_settlement_status': '상태\n(สถานะ)',
    'driver_settlement_due_label': '납부 기한\n(กำหนดชำระ)',
    'driver_settlement_rejection': '반려 사유\n(เหตุผลที่ไม่อนุมัติ)',
    'driver_settlement_select_receipt': '송금증 선택 / เลือกสลิปโอนเงิน',
    'driver_settlement_replace_receipt': '파일 변경 / เปลี่ยนไฟล์',
    'driver_settlement_selected': '선택한 파일: {name}\n(ไฟล์ที่เลือก)',
    'driver_settlement_uploading': '업로드 중\n(กำลังอัปโหลด)',
    'driver_settlement_upload': '송금증 업로드 / อัปโหลดสลิปโอนเงิน',
    'driver_settlement_retry_upload': '업로드 재시도 / ลองอัปโหลดอีกครั้ง',
    'driver_settlement_invalid_file':
        '지원 파일: JPG, JPEG, PNG, PDF\n(รองรับไฟล์ JPG, JPEG, PNG, PDF)',
    'driver_settlement_file_too_large':
        '파일은 10MB 이하여야 합니다\n(ไฟล์ต้องมีขนาดไม่เกิน 10 MB)',
  };

  static const Map<String, Map<String, String>>
  _airportMeetingActionTranslations = {
    'en': {
      'airport_meeting_step_notify_driver':
          'After arrival and collecting your luggage, notify your driver.',
      'airport_meeting_notify_button': 'Tell driver I’m ready',
      'airport_meeting_message_driver': 'Message driver',
      'airport_meeting_notify_sent': 'Pickup notification sent',
      'airport_meeting_notify_failed':
          'Could not send the pickup notification. Please try again.',
      'airport_meeting_notify_waiting_driver':
          'Pickup notification becomes available after a driver is assigned.',
      'airport_meeting_notify_luggage_first':
          'Press only after you have collected all luggage.',
      'airport_meeting_notify_confirm_title': 'Send pickup notification',
      'airport_meeting_notify_confirm_body':
          'Have you collected all luggage? Sending this alert tells your driver that you are ready for pickup.',
      'airport_meeting_notify_cancel': 'Cancel',
      'airport_meeting_notify_confirm_send': 'Send',
    },
    'ko': {
      'airport_meeting_step_notify_driver':
          '기사에게 픽업 알림을 보냅니다. 꼭 수하물을 찾으신 후에 알림을 보내주시기 바랍니다',
      'airport_meeting_notify_button': '짐 찾았어요, 기사님에게 알리기',
      'airport_meeting_message_driver': '기사에게 메시지 보내기',
      'airport_meeting_notify_sent': '픽업 알림을 보냈습니다',
      'airport_meeting_notify_failed': '픽업 알림을 보내지 못했습니다. 다시 시도해 주세요.',
      'airport_meeting_notify_waiting_driver': '기사 배정 후 픽업 알림을 보낼 수 있습니다.',
      'airport_meeting_notify_luggage_first': '수하물을 찾으신 후에만 눌러주세요.',
      'airport_meeting_notify_confirm_title': '픽업 알림 보내기',
      'airport_meeting_notify_confirm_body':
          '수하물을 모두 찾으셨나요? 알림을 보내면 기사에게 고객님이 픽업 준비가 되었다는 메시지가 전달됩니다.',
      'airport_meeting_notify_cancel': '취소',
      'airport_meeting_notify_confirm_send': '보내기',
    },
    'zh': {
      'airport_meeting_step_notify_driver': '抵达并取到行李后，请通知司机。',
      'airport_meeting_notify_button': '告诉司机我已准备好',
      'airport_meeting_message_driver': '联系司机',
      'airport_meeting_notify_sent': '接机通知已发送',
      'airport_meeting_notify_failed': '无法发送接机通知，请重试。',
      'airport_meeting_notify_waiting_driver': '司机分配后即可发送接机通知。',
      'airport_meeting_notify_luggage_first': '请仅在取齐所有行李后点击。',
      'airport_meeting_notify_confirm_title': '发送接机通知',
      'airport_meeting_notify_confirm_body': '您已取齐所有行李了吗？发送后，司机将收到您已准备好上车的消息。',
      'airport_meeting_notify_cancel': '取消',
      'airport_meeting_notify_confirm_send': '发送',
    },
    'ja': {
      'airport_meeting_step_notify_driver': '到着して荷物を受け取ったら、ドライバーへお知らせください。',
      'airport_meeting_notify_button': '準備完了をドライバーに知らせる',
      'airport_meeting_message_driver': 'ドライバーにメッセージ',
      'airport_meeting_notify_sent': 'ピックアップ通知を送信しました',
      'airport_meeting_notify_failed': '通知を送信できませんでした。もう一度お試しください。',
      'airport_meeting_notify_waiting_driver': 'ドライバーが割り当てられると通知を送信できます。',
      'airport_meeting_notify_luggage_first': '荷物をすべて受け取ってから押してください。',
      'airport_meeting_notify_confirm_title': 'ピックアップ通知を送信',
      'airport_meeting_notify_confirm_body':
          '荷物をすべて受け取りましたか？送信すると、乗車準備ができたことをドライバーへ通知します。',
      'airport_meeting_notify_cancel': 'キャンセル',
      'airport_meeting_notify_confirm_send': '送信',
    },
    'th': {
      'airport_meeting_step_notify_driver':
          'เมื่อมาถึงและรับสัมภาระแล้ว ให้แจ้งคนขับว่าพร้อมขึ้นรถ',
      'airport_meeting_notify_button': 'แจ้งคนขับว่าพร้อมขึ้นรถแล้ว',
      'airport_meeting_message_driver': 'ส่งข้อความหาคนขับ',
      'airport_meeting_notify_sent': 'ส่งข้อความแจ้งคนขับแล้ว',
      'airport_meeting_notify_failed':
          'ส่งข้อความแจ้งคนขับไม่สำเร็จ กรุณาลองอีกครั้ง',
      'airport_meeting_notify_waiting_driver':
          'ส่งข้อความแจ้งรับรถได้หลังจากจัดสรรคนขับแล้ว',
      'airport_meeting_notify_luggage_first':
          'กรุณากดหลังจากรับสัมภาระครบแล้วเท่านั้น',
      'airport_meeting_notify_confirm_title': 'ส่งข้อความแจ้งรับรถ',
      'airport_meeting_notify_confirm_body':
          'รับสัมภาระครบแล้วใช่ไหม เมื่อส่ง คนขับจะได้รับข้อความว่าคุณพร้อมขึ้นรถแล้ว',
      'airport_meeting_notify_cancel': 'ยกเลิก',
      'airport_meeting_notify_confirm_send': 'ส่ง',
    },
  };

  String t(String key) {
    final airportMeetingText =
        _airportMeetingActionTranslations[languageCode]?[key] ??
        _airportMeetingActionTranslations['en']?[key];
    if (airportMeetingText != null) return airportMeetingText;
    // Driver bilingual map wins for shared driver chrome keys. Release /
    // reassignment keys intentionally live only in locale maps so en/ko/zh/ja/th
    // each return a real translation (no key fallback, no forced bilingual).
    final driverText = _driverUiTranslations[key];
    if (driverText != null) return driverText;
    final lang = _translations[languageCode] ?? _translations['en']!;
    final featureLang = _driverApplicationTranslations[languageCode];
    final featureEn = _driverApplicationTranslations['en']!;
    return lang[key] ??
        featureLang?[key] ??
        _translations['en']![key] ??
        featureEn[key] ??
        key;
  }
}

class AppLocalizationsDelegate extends LocalizationsDelegate<AppLocalizations> {
  final String languageCode;

  AppLocalizationsDelegate(this.languageCode);

  @override
  bool isSupported(Locale locale) =>
      AppLocalizations.supportedLanguages.contains(languageCode);

  @override
  Future<AppLocalizations> load(Locale locale) async =>
      AppLocalizations(languageCode);

  @override
  bool shouldReload(covariant AppLocalizationsDelegate old) =>
      old.languageCode != languageCode;
}

extension AppLocalizationsContext on BuildContext {
  AppLocalizations get l10n {
    final locale = Localizations.localeOf(this);
    return AppLocalizations(locale.languageCode);
  }
}
