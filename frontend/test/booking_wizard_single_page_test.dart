...
void main() {
  ...
  group('booking complete copy', () {
    testWidgets('copy button copies reservation number and shows snackbar', (tester) async {
  ...
  group('single-page booking wizard', () {
    ...
    testWidgets('shows 7 expanded sections without booking confirmation step', (tester) async {
    ...
    testWidgets('allows customer input before pickup step is complete', (tester) async {
      ...
      expect(find.text('Complete service, origin, destination, pick-up time, and passengers first.'),
          findsWidgets);
    });

    testWidgets('confirm button disabled until all required fields complete', (tester) async {
      await pumpWizard(tester);
    ...
    testWidgets('shows validation hints for incomplete sections', (tester) async {
      await pumpWizard(tester);
    ...
    testWidgets('shows flight number prefix icon for airport pickup', (tester) async {
    ...
    testWidgets('restores saved wizard state and clamps old step index', (tester) async {
      final saved = BookingWizardState(
        step: 7,
        serviceType: BookingServiceType.airportPickup,
  ...
  group('booking complete review section', () {
    testWidgets('shows review details without duplicating route summary fields', (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          home: BookingCompletePage(
            result: const BookingCreateResult(
    ...
    testWidgets('review section has no horizontal overflow at 360px', (tester) async {
      tester.view.physicalSize = const Size(360, 900);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      addTearDown(tester.view.resetDevicePixelRatio);
...
class _CapturingBookingApi implements BookingApiService {
  ...
  Future<PricingResult> calculatePricing({
    ...
    return const PricingResult(currency: 'THB', chargeItems: [], totalAmount: 1500);
  }
