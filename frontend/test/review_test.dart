...
class _FakeReviewApi extends BookingReviewApi {
  _FakeReviewApi({this.getState, this.submitResult, this.submitError});
...
void main() {
  ...
  test('zh review strings resolve without English fallback', () {
    ...
    expect(
      zh.t('guest_status_guidance_settlement_pending'),
      contains('感谢您的乘坐'),
    );
  ...
  test('ja review strings resolve without English fallback', () {
    ...
    expect(
      ja.t('guest_status_guidance_settlement_pending'),
      contains('ご利用ありがとうございました'),
    );
  ...
  testWidgets('submit failure keeps form state for retry', (tester) async {
    final api = _FakeReviewApi(
      submitError: const BookingReviewApiException('Network error'),
    );
    await tester.pumpWidget(
      _reviewFormScaffold(
        BookingReviewForm(
  ...
  testWidgets('admin hide flow', (tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(home: AdminReviewDetailPage(reviewId: 1, api: api)),
    );
    await tester.pumpAndSettle();
