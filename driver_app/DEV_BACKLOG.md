...
## 17. [해결 추정] 예약취소 시 '픽업 시각이 올바르지 않습니다' 오류 (2026-07-29 1회 발생)

### 배경

예약 TX202607290003 취소 시도 시 INVALID_PICKUP_TIME 오류 발생. DB 데이터 확인 결과 scheduled_pickup_at 값과 parseServiceDateTimeToMs() 파싱 로직 모두 정상 작동 확인됨 (null 아님, 정상 파싱 성공). 재시도 시 정상적으로 취소됨 — 재현 안 됨.

### 상태

재현 불가로 원인 미상. 일시적 현상(배포 타이밍 등)으로 추정. 재발 시 발생 즉시 서버 로그(`docker compose logs -f tride-backend`)를 실시간으로 확인해서 정확한 원인 파악 필요.
