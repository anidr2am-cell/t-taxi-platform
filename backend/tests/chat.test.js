...
function buildHarness(overrides = {}) {
  ...
  const chatRepository = {
    ...
    async findParticipant(_c, roomId, role, userId) {
      return state.participants.find((p) =>
        ...
        && (userId == null ? p.user_id == null : p.user_id === userId)) ?? null;
    },
    ...
    async findMessageById(_c, messageId, roomId = null) {
      return state.messages.find((m) =>
        m.id === messageId && (roomId == null || m.chat_room_id === roomId)) ?? null;
    },
    ...
    async countUnreadForParticipant(_c, roomId, participant) {
      return state.messages.filter((m) =>
        ...
        && (!participant.last_read_at || m.created_at > participant.last_read_at)).length;
    },
  ...
  const bookingRepository = {
    ...
    async findActiveDriverBookingByNumberForUpdate(_c, driverUserId, bookingNumber) {
      if (!state.driverActive || bookingNumber !== 'TX202607010001') return null;
  ...
  const service = new ChatService(
    ...
    chatRepository,
    bookingRepository,
    { async findByUserId() { return { name: 'Driver A' }; } },
    { async findById() { return { name: 'Admin' }; } },
    outboxRepository,
    outboxProcessor,
  );
...
test('one room per booking via ensureRoom idempotency', async () => {
  ...
  const room = await service.getRoom('TX202607010001', { id: 8, role: ROLES.CUSTOMER }, null);
...
test('valid customer access', async () => {
  const { service, state } = buildHarness();
  state.rooms.push({ id: 1, booking_id: 10, room_code: 'CHAT-TX202607010001', is_active: 1, created_at: new Date() });
  state.participants.push({ id: 1, chat_room_id: 1, user_id: 8, participant_role: 'CUSTOMER', display_name: 'Kim', last_read_at: null });
  const room = await service.getRoom('TX202607010001', { id: 8, role: ROLES.CUSTOMER }, null);
...
test('assigned driver access', async () => {
  const { service, state } = buildHarness();
  state.rooms.push({ id: 1, booking_id: 10, room_code: 'CHAT-TX202607010001', is_active: 1, created_at: new Date() });
  const room = await service.getRoom('TX202607010001', { id: 44, role: ROLES.DRIVER }, null);
  assert.equal(room.sendingAllowed, true);
...
test('old reassigned driver cannot send', async () => {
  ...
  state.rooms.push({ id: 1, booking_id: 10, room_code: 'CHAT-TX202607010001', is_active: 1, created_at: new Date() });
  await assert.rejects(
    () => service.sendMessage('TX202607010001', { id: 44, role: ROLES.DRIVER }, null, {
      text: 'hello',
...
test('admin access', async () => {
  const { service, state } = buildHarness();
  state.rooms.push({ id: 1, booking_id: 10, room_code: 'CHAT-TX202607010001', is_active: 1, created_at: new Date() });
  const room = await service.getRoom('TX202607010001', { id: 1, role: ROLES.ADMIN }, null);
  assert.equal(room.bookingNumber, 'TX202607010001');
...
test('message trim and validation', async () => {
  const { service } = buildHarness();
  await assert.rejects(
    () => service.sendMessage('TX202607010001', { id: 8, role: ROLES.CUSTOMER }, null, {
      text: '   ',
      clientMessageId: '11111111-1111-4111-8111-111111111111',
    }),
...
test('duplicate clientMessageId returns one message', async () => {
  ...
  state.rooms.push({ id: 1, booking_id: 10, room_code: 'CHAT-TX202607010001', is_active: 1, created_at: new Date() });
  state.participants.push({ id: 1, chat_room_id: 1, user_id: 8, participant_role: 'CUSTOMER', display_name: 'Kim', last_read_at: null });
  state.participants.push({ id: 2, chat_room_id: 1, user_id: 44, participant_role: 'DRIVER', display_name: 'Driver', last_read_at: null });
  const payload = {
    text: 'Hello',
    clientMessageId: '22222222-2222-4222-8222-222222222222',
  };
  await service.sendMessage('TX202607010001', { id: 8, role: ROLES.CUSTOMER }, null, payload);
  await service.sendMessage('TX202607010001', { id: 8, role: ROLES.CUSTOMER }, null, payload);
  assert.equal(state.messages.length, 1);
});

test('terminal booking read-only rule', async () => {
  const { service, state } = buildHarness({ booking: booking({ status: BOOKING_STATUS.COMPLETED }) });
  state.rooms.push({ id: 1, booking_id: 10, room_code: 'CHAT-TX202607010001', is_active: 1, created_at: new Date() });
  state.participants.push({ id: 1, chat_room_id: 1, user_id: 8, participant_role: 'CUSTOMER', display_name: 'Kim', last_read_at: null });
  const room = await service.getRoom('TX202607010001', { id: 8, role: ROLES.CUSTOMER }, null);
...
test('chat notification outbox created once per recipient', async () => {
  ...
  state.participants.push({ id: 1, chat_room_id: 1, user_id: 8, participant_role: 'CUSTOMER', display_name: 'Kim', last_read_at: null });
  state.participants.push({ id: 2, chat_room_id: 1, user_id: 44, participant_role: 'DRIVER', display_name: 'Driver', last_read_at: null });
  await service.sendMessage('TX202607010001', { id: 8, role: ROLES.CUSTOMER }, null, {
    text: 'Ping driver',
...
test('unread excludes own messages', async () => {
  ...
  const driverRoom = await service.getRoom('TX202607010001', { id: 44, role: ROLES.DRIVER }, null);
...
test('read state decreases unread count', async () => {
  ...
  state.participants.push({ id: 1, chat_room_id: 1, user_id: 8, participant_role: 'CUSTOMER', display_name: 'Kim', last_read_at: null });
  state.participants.push({ id: 2, chat_room_id: 1, user_id: 44, participant_role: 'DRIVER', display_name: 'Driver', last_read_at: null });
  const sent = await service.sendMessage('TX202607010001', { id: 8, role: ROLES.CUSTOMER }, null, {
    text: 'hello driver',
    clientMessageId: '66666666-6666-4666-8666-666666666666',
  });
  const read = await service.markRead('TX202607010001', { id: 44, role: ROLES.DRIVER }, null, {
