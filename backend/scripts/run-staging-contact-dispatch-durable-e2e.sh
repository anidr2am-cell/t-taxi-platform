...
docker cp /opt/t-ride/backend/scripts/staging-contact-dispatch-durable-e2e.js tride-backend:/srv/tride/backend/scripts/staging-contact-dispatch-durable-e2e.js
...
docker exec \
  ...
  tride-backend node scripts/staging-contact-dispatch-durable-e2e.js | tee /tmp/contact-dispatch-durable-e2e.log

BOOKING="$(grep -E '^CONTACT_DISPATCH_E2E_BOOKING=' /tmp/contact-dispatch-durable-e2e.log | tail -n 1 | cut -d= -f2-)"
if [ -n "${BOOKING}" ]; then
  echo "DB_CHECK booking=${BOOKING}"
  bash /opt/t-ride/backend/scripts/run-staging-contact-dispatch-durable-db-check.sh "${BOOKING}"
fi
