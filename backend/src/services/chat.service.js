...
class ChatService {
  ...
  mapParticipantRole(userRole) {
    ...
    if (userRole === ROLES.ADMIN || userRole === ROLES.SUPER_ADMIN) return 'ADMIN';
    return 'CUSTOMER';
  }
  ...
  async ensureRoom(conn, booking) {
    let room = await this.chatRepository.findRoomByBookingIdForUpdate(conn, booking.id);
    if (room) return room;
    const roomCode = `CHAT-${booking.booking_number}`;
    try {
      const roomId = await this.chatRepository.insertRoom(conn, booking.id, roomCode);
      room = {
        id: roomId,
  ...
  async ensureCustomerParticipant(conn, room, booking, authUser, guestAccessToken) {
    if (authUser?.role === ROLES.CUSTOMER && booking.customer_user_id === authUser.id) {
      ...
      if (!participant) {
        const participantId = await this.chatRepository.insertParticipant(conn, room.id, {
          userId: authUser.id,
          participantRole: 'CUSTOMER',
          displayName: booking.customer_name ?? 'Customer',
        });
        participant = await this.chatRepository.findParticipantById(conn, participantId);
      }
    ...
    let participant = await this.chatRepository.findGuestParticipant(conn, room.id);
    if (!participant) {
      const participantId = await this.chatRepository.insertParticipant(conn, room.id, {
        userId: null,
        participantRole: 'CUSTOMER',
  ...
  async ensureDriverParticipant(conn, room, booking, driverUserId) {
    const active = await this.bookingRepository.findActiveDriverBookingByNumberForUpdate(
      conn,
      driverUserId,
  ...
  async resolveAccess(conn, bookingNumber, authUser, guestAccessToken) {
    ...
    if (authUser?.role === ROLES.DRIVER) {
      const access = await this.ensureDriverParticipant(conn, room, booking, authUser.id);
      return { booking, room, ...access, actorRole: ROLES.DRIVER };
    }
    if (authUser?.role === ROLES.ADMIN || authUser?.role === ROLES.SUPER_ADMIN) {
      const access = await this.ensureAdminParticipant(conn, room, booking, authUser);
  ...
  async buildRoomResponse(conn, context) {
    const participants = await this.chatRepository.listParticipants(conn, context.room.id);
    const unreadCount = await this.chatRepository.countUnreadForParticipant(
      conn,
      context.room.id,
      context.participant,
    );
    const lastMessage = await this.chatRepository.findLastMessage(conn, context.room.id);
    return {
  ...
  async getRoom(bookingNumber, authUser, guestAccessToken) {
    ...
    try {
      ...
      const context = await this.resolveAccess(conn, bookingNumber, authUser, guestAccessToken);
      await conn.commit();
  ...
  async listMessages(bookingNumber, authUser, guestAccessToken, query = {}) {
    ...
    try {
      await conn.beginTransaction();
      const context = await this.resolveAccess(conn, bookingNumber, authUser, guestAccessToken);
      const rows = await this.chatRepository.listMessages(conn, context.room.id, {
      ...
      const items = rows.map((row) => {
        const readByCurrentUser = context.participant.last_read_at
          ? new Date(row.created_at) <= new Date(context.participant.last_read_at)
          : false;
  ...
  async queueChatNotifications(conn, {
    booking,
    messageId,
    clientMessageId,
    senderParticipantId,
  ...
  async sendMessage(bookingNumber, authUser, guestAccessToken, input) {
    ...
    try {
      ...
      const saved = await this.chatRepository.findMessageById(conn, messageId, context.room.id);
      await conn.commit();
  ...
  async markRead(bookingNumber, authUser, guestAccessToken, input = {}) {
    ...
    try {
      ...
      await this.chatRepository.updateParticipantLastRead(conn, context.participant.id, readAt);
      await this.chatRepository.insertMessageRead(conn, target.id, context.participant.id);
      const unreadCount = await this.chatRepository.countUnreadForParticipant(
        conn,
        context.room.id,
  ...
  async listAdminChats(authUser, query = {}) {
    ...
    const conn = await this.pool.getConnection();
    try {
      const rows = await this.chatRepository.listAdminChatSummaries(conn, authUser.id, {
        search: query.search ?? query.q ?? null,
        unreadOnly: query.unreadOnly === true || query.unread_only === 'true',
        limit,
        offset,
      });
      const total = await this.chatRepository.countAdminChatSummaries(conn, authUser.id, {
        search: query.search ?? query.q ?? null,
        unreadOnly: query.unreadOnly === true || query.unread_only === 'true',
      });
      const items = [];
      for (const row of rows) {
        ...
        if (row.admin_participant_id) {
          unreadCount = await this.chatRepository.countUnreadForParticipant(conn, row.room_id, {
        ...
        items.push({
          ...
          lastMessageText: row.last_message_text ? this.previewText(row.last_message_text) : null,
